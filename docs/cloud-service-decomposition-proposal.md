...
- 第 12 项内容排期调度器从自动化在线注册表取账号（`onlineAccounts`），意味着「自动化停 ⇒ 零创作被触发」。**在线闸保留**（用户 2026-08-03 裁定），拆分后改为经跨进程窄口向自动化服务问同一份清单；MUST NOT 因此改成「只看排期表、不看在线」。
  该口问不到时 MUST 整轮跳过并具名留痕，MUST NOT 回空清单——空清单与「真的没人在线」在外部完全同形，会让全员静默停摆且看着正常。
  **本条 2026-08-03 修订**（change `wire-content-scheduler-into-api-process`）：原文写的是「MUST 改为按排期表扫描，MUST NOT 以 Edge 在线为创作触发的前置条件」，与该次裁定相左，已按裁定改写，不留两份互斥的权威。
  裁定同时**去掉的是浏览器环境绑定**，不是在线闸：账号连着引擎才执行排期，但连的是哪个环境不再参与判定（dev/ol 部署目标隔离是另一回事，属红线，原样保留）。
...
**kernel 花名册增量（change `deploy-derived-services-to-dev`，实测于 `aidcp-cloud@e6a3143`）**：新增 `src/kernel/persona-soul-parse.ts` —— 从 `src/agents/persona-soul-codec.ts`（content）**析出的纯解析那一段**：人设闭子集（identity / interests / writing_language / behavior_guidelines）的结构校验，外加同步读 `account_persona` 流的取值口（解析 + JSON 归一 + 失败回 null 三件事收在一处）。**它必须进 kernel 而不是留在任一属主**：这段解析被两个组装根共用（单体与派生接口服务），而同步读消费方按「同游标必同载荷」判定 —— 两处各留一份实现时，同一份人设文本解出两种结构、同一个游标发出两种载荷摘要，消费方整条拒收，**且两侧各自的行为测试全绿**（2026-08-04 dev 切流演练实测，游标 902）。序列化那一半留在 content（PersonaGenerator 专用形态），残壳按引用取用本文件；**通用装载器 `src/soul/loader.ts` 仍进不了 kernel**（模块级 `new Set` 单例 + 引 api 段写作语言判定），本文件也不覆盖它那三个 api 段自管字段（engagement_rules / mandatory_interactions / browse_patterns）。只 import kernel 的 like-affinity / soul-types / sync-read-snapshot（仅类型）/ yaml；四条准入正则实跑零命中。**分层计数 content −0 / kernel +1**（析出新文件，非搬迁）。两个仓各钉一条同值载荷摘要用例（`test/acceptance/persona-soul-parse-single-source.test.ts`），任一侧漂移一侧当场红。
