...
> ④边缘端 publish flow（`flows/publish-post.ts`）已实现，发布审批链路打通；⑤协议已升级到 **v2（40 个消息类型）**，
...
| 浏览执行层 `browse` | aidcp-edge | 已实现；命令分发 + 结构化上报 + 卡片过滤 | `aidcp-edge/src/browse/`（browse-session/feed-scroller/modal-controller/note-extractor/search-handler/card-filter） |
...
| 协议层 `protocol` | aidcp-cloud | 已实现 **v2（40 个消息类型）**；`docs/protocol.md` 已同步 | `aidcp-cloud/src/comm/protocol.ts`（边侧 `aidcp-edge/src/comm/protocol.ts` 为投影） |
...
1. **协议文档落后** → **已修复**。`docs/protocol.md` 已从 v1 重写为 v2，补齐浏览编排、角色驱动指令、结构化上报、风控预算、发布审批共 40 个消息类型。
