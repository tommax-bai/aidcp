...
## 簇 127 — Native 产物门禁：打包态与 CI 编译器版本对账

> 来源 change：`enforce-native-engine-artifact-gates` / `harden-native-engine-runtime-contracts`
> （均**活跃未归档**）。
> **2026-07-31 用户裁定**：真机项从各 change 的 `tasks.md` 移除、统一收拢到本文件。
>
> **前置环境**：本机打一次安装包（按 CLAUDE.md §6，打包只在用户明确要求时执行）+ 一次 CI 运行日志。
> **本簇与其他簇不共享环境**，可单独在一次发版窗口里顺带验完。

- [ ] 127.1 **（打包态产物校验与冒烟）** [原 11.6] 打一次 macOS 安装包并在真机启动，确认改造后的产物校验、
  打包资源解析与冒烟全过；代码级验证 MUST NOT 声称覆盖这一项。
- [ ] 127.2 **（跨平台打包的报错是否指向真因）** [原 11.7] 在 macOS 主机上执行一次 Windows 打包，
  确认跨平台资源解析的报错确实指向真因；CI 上两个作业各自宿主等于目标、不触发此路径，故只能靠人工跨平台跑一次。
  **注**：该报错改造本身已于 2026-07-31 显式弃守（产物门禁 9.1–9.3），本条留作弃守决定的复核依据——
  若真机发现它**不是**响亮失败而是静默发错包，弃守结论须推翻。
- [ ] 127.3 **（CI 上实际生效的 Rust 编译器版本）** [原 11.8 / 9.7.9] 确认本机之外的 CI 运行器上实际生效的
  Rust 编译器版本与工具链声明钉死的版本一致；当前无 CI 运行日志、产物里也不记录编译器版本，事后无法对账，
  需拿到新流水线的首次运行日志才能定论。

> **一条不属于真机、但随本次移出一并登记的资料缺口**（原 `harden-native-engine-runtime-contracts` 9.7.11）：
> 早期简报 C 段提到的七个簇里「维持原判」条目只给了编号（F-IPC / INJ / TXT / PACE / GEST / TIME / RETRY /
> PLAT-OBS / BUILD / DRIFT 系列）、没有正文与原始状态。该 change 未据其编号做任何断言。
> 这是**引用悬空**而非待验行为——若今后需要并案，须先补齐正文；补不齐则视为该简报作废。
