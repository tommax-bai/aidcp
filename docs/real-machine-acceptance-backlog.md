...
- [ ] 86.1 **精确 `/publish <昵称>` 直接排队** — 管理群发 `/publish 工程师大白`：**不再**弹「请确认用户委托任务」确认卡，直接回「委托任务已直接排队」进度卡（状态 queued）；随后下游**逐篇内容人审卡照常弹出**，通过后才真发（AC-PUB 不破）。
- [ ] 86.2 **精确 `/comment <昵称>` 直接排队** — 同上，`/comment` 直接排队、评论人审在发前仍触发。
