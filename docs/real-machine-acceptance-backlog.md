...
> **2026-08-05 补 · 簇 60 增补（能力名修复的行为验证 + OL 放行位撤回）**
>
> 两条都是「代码已上、行为未验」，**MUST NOT 因为部署成功就读成已验**。
>
> - [ ] **派生自动化按协议常量识别边缘能力（`_v1` 后缀）真的生效了。**
>   判据两条都要：① 有账号**带着未用满的浏览配额**真跑到 Reel 上之后，日志里不再出现
>   `facebook_reel_follow_edge_capability_missing`；② 至少观察到一次 **Reel 关注真被下发**。
>   只有 ① 不算 —— 2026-08-05 部署后复查的那 5 分钟里评估器一次都没跑（17 个账号配额已用满在睡），
>   「零告警」的分母是 0。缺陷形态见 `deploy-derived-services-to-dev` tasks 6.2b，承接任务为其 7.5。
> - [ ] **OL 接口服务的临时 schema 放行位已撤回，且撤回后重启仍能过契约门。**
>   `AIDCP_ALLOW_SCHEMA_AHEAD=0110_facebook_global_policy_single_scope` 是 2026-08-05
>   为救活「一重启即死」临时加的，撤回时机是 `unify-facebook-global-policy-across-targets` 上线之后。
>   承接任务为该 change 的 7.7 / 8.8；现场记录见 `restore-panel-capability-wiring` tasks 9.4b。
>   **放行位留着 = 这台机器的契约门对这一段版本永久失效**，而它只在启动时跑、平时零告警。

> **2026-08-05 补 · 簇 60 结账（`deploy-derived-services-to-dev` task 8.2）**
>
> 对 08-04 那批「未验」逐条给结论。**划掉的都附证据；没划掉的补上为什么没覆盖到。**
>
> - ~~边缘真连上来跑一趟浏览~~ **已验**：OL 上边缘持续在线，同一 6 小时窗口风控账本记了
>   **1703 次真实浏览**、193 点赞、14 关注、15 加群，换页节拍 p50 26.5s / p90 36.4s。
>   （08-04 那次「没观察到」的原因是整个上午 8787 上一条边缘都没有，与切流无关。）
> - ~~按小时开火的周期任务~~ **部分已验**：观察期已收官（tasks 7.1），15 项基线里
>   **11 项落点确认在跑**。⚠️ 余下 4 项**不是「验过是好的」而是「至今没观察到它醒」**，
>   其中委托任务泵**起了但一句话不说**（「起了」与「没起」在日志上同形，当时是靠查表反推的），
>   已单列为待修，别把它读进「已验」。
> - ~~跨进程路由对面真的注册了~~ **已验**：13 条点名的路由在 dev 上真打过（tasks 6.2），
>   写口用不合法参数探、零副作用，且有一条编造路由做负对照（404）。
>   另有全部 71 个注册器的静态普查、缺口 0。
> - [ ] **`runAutomationMain` 与飞书 `/delegate` 仍未真跑**（既有项，本批仍未覆盖）。
>   没覆盖的原因：两者都要一次真实运营触发，本批的验证位都在只读/探针层。
> - [ ] **契约门「拒启 ⇒ 被进程管理器记成失败」的整链没在同一台机器上从头跑通。**
>   两段分别实测：拒启退非 0（OL 真事故的 journal）、退非 0 进 failed（dev 一次性探针，
>   用的是修好后的配置）。**没有为了凑这条链去把哪台真服务弄崩** —— 真造一次账本超前
>   要么改共库、要么停一个在跑的接口面。拼接处是机械的（重启限速不看退出原因），
>   但这仍是「两段 + 一个推理」，不是一次端到端观测，引用时 MUST 按这个口径。

> **2026-08-04 补 · 簇 60 增补（派生三服务切流当天的实测结论与遗留）**
>
> 切流已完成（12:07，单体停、三个派生服务接管，详见
> `docs/handoff-2026-08-04-derived-services-cutover.md` §6–§11）。
> **已验（附证据）**：面板登录 / 桌面客户端 name+key 登录 / 面板本地属主池读 /
> 跨进程读自动化侧 11 条与内容侧 1 条 / 注定缺席的 7 条答具名 503 /
> 飞书长连接 1 条落在接口进程 / 自动化写者锁落在自动化进程 / 边-云口 8787 起来 /
> 内容进程取图片模型从「保守默认」变「真值」。
>
> **未验（附为什么没观察到）**：
> - [ ] **边缘真连上来跑一趟浏览** —— 切流前后 8787 上都是 **0 条**边缘连接，整个上午没有边缘在线，
>   所以「边缘断连重连」与「浏览闭环在派生形态下真跑」两项**一次都没观察到**，与切流无关。
> - [ ] **按小时开火的周期任务** —— 排期心跳 / 委托泵 / 恢复扫描在业务入口放行后才起，
>   放行时间 12:07，当天未到点，**没有观察到任何一次真开火**。
> - [ ] `runAutomationMain` 与飞书 `/delegate` 仍未真跑（既有项，本批未覆盖）。
>
> **本批新登记的待修（详见 change tasks 8.1 逐条）**：
> - [ ] **面板「环境」页在派生形态下答 500**（`client_env_automation_read_port_not_configured`）。
>   console 主力页面，优先级最高。
> - [ ] **跨进程失败在面板层全兜成 500 `internal_error`**：从 console 侧分不出
>   「对面没起 / 路由没注册 / 自己炸了」。
> - [ ] **配置镜像失效信号只有落地端**：自动化侧中继与四个限频配置存储的版本推进器未接线 ⇒
>   后台改配额 / 节奏 / 单场上限 / 续场后，接口进程侧镜像版本不前进。
> - [ ] **同步读就绪度仍会周期性抖**（安静时段没有重发）。业务入口是一次性闩、今天不影响；
>   但按当下新鲜度判断的消费方仍会间歇 fail-closed。
> - [ ] **验证码云端协助在派生形态下整体不可用，已显式关掉**（自动化侧缺 token secret、
>   面板侧路由未注入）。**这是如实关掉、不是修好**，恢复需两侧重新接线。
> - [ ] **这类跨进程错误没有告警、没有巡检**：单体自 08-02 起每天在报同游标载荷漂移，
>   无人处置，直到 08-04 把整台 dev 拖到起不来。**这是根债。**
> - [ ] **收口动作**：停用为验证客户鉴权口而建的账号 `cutover-verify-0804`，
>   并删掉 ECS 上的 `/root/.cutover-verify-key`。
>
...
>   下次开后台时顺手确认一眼即可闭合。
> - [ ] **互动运行控制行的播种守卫走一遍真账号**（同批修的第二处同形缺陷）。
>   守卫从内联 `SELECT 1 FROM accounts` 改成经账号平台读端口问。
>   **注意 dev 当前互动域是关的**（日志 `interaction_schema_missing_run_0042`），
>   所以这条修改在 dev 上**根本没被执行过**——单测覆盖了三种分支，但真机零跑。
>   开互动域的环境上必须验：正常账号能拿到运行控制、非互动平台账号仍是 404、不产生孤儿控制行。
- [ ] **撤权 hold 改走 4a 窄端口后，运行控制快照仍算得出来**（change
  `split-cloud-automation-production-runtime` 批 H 第 4 片；cloud `c394f36`）。
  运行控制投影里的「该账号还挂着未物化的清理准入吗」原先直调接口属主存储，现在经
  `offboard-admission-ledger` 的新读面问。**与上一条同一个环境前置**：dev 互动域是关的
  （`interaction_schema_missing_run_0042`），所以这处消费方在 dev 上**根本不执行**，
  契约测试与往返用例覆盖了形状、真机零跑。
  开互动域的环境上要验两件：① 有 hold 的账号读写位仍被拦住；
  ② **端口问不到时是响亮失败，不是放行** —— 这个布尔是放行条件，`false` 意味着「没有 hold、
  可以放行」，任何把读失败降级成 false 的行为都会给正在被撤权的环境重新放开互动写。
- [ ] **自动化进程的真装配 `runAutomationMain()` 一次都没跑过**（change
  `split-cloud-automation-production-runtime` 批 H 第 5 片；automation `0044881`）。
  它把十二个工厂接进组装根并写出 `main()`，**typecheck 干净、全量 2148 pass、结构断言
  钉住了关停语义 / 门的顺序 / 单份判定，但这套装配一次都没有被真的执行过** ——
  它要一个真的 automation 属主库（风控写者锁在**构造期**就抢），本地桩验不了。
  这条**不是「顺手确认一眼」**，是三进程真跑（批次 5）的第一步，要单独排期。
  真机上要按顺序验的四件：
  ① **schema 契约门先跑、且拦得住**：账本落后时进程以非 0 退出，而不是起来了再说；
  ② **写者锁的独占是真的**：同一 target 上起第二个进程必须拒绝启动
     （部署形态因此 MUST 是 stop→start，滚动 / 蓝绿会让后起的那个抢不到锁）；
  ③ **就绪闸真的挡得住**：同步读没就绪时端口在听、探活如实报「未放行」，业务入口不放行；
  ④ **关停不打死进程**：`close()` 走完之后属主池才关，且那一族「裸 `pool.end()`」的存储
     一个都没被调到（结构断言只能证明代码里没写，证明不了运行期没有别的路径调到）。
- [ ] **四类限频配置在三进程形态下会不会热加载**（同片实读发现，tasks 3.5h；**未修**）。
  配额 / 节奏 / 单场 / 续场四张表**属主就是自动化进程**，但让内存副本保持新鲜的那个轮询刷新器
  归接口域、不会被派生进自动化仓 ⇒ 该进程**启动读一次之后再不刷新**。
  真机判据很直接：**后台改一次单场时长或配额档，看自动化进程是否在下一个会话就按新值跑。**
  预期今天是「不生效且不报错」—— 这条要在三进程真跑时确认，别等运营先撞上。
- [ ] **排期发帖 / 排期评论在三进程形态下没有任何进程在跑**（change
  `split-cloud-automation-production-runtime` tasks 4.1g；**不是缺陷、是没接**）。
  排期 tick（`ContentScheduler`）在归属表里是 **api**，类也确实在 `aidcp-api` 仓里，
  **但 api 的手写 `main()` 没有构造它** —— 单体下它照常跑，所以这条只在三进程形态下成立。
  **不阻塞门清零**（门问的是「本包缺不缺跨属主依赖」），但三进程真跑前必须有结论：
  要么把排期器接进接口进程，要么显式裁定它归谁。
  现象会是「到点什么都不发生，且没有任何一行日志说为什么」。
- [ ] **接口进程不服务 `schedule-feedback`，而自动化已经在调它**（同上 change，tasks 3.5j；只记不修）。
  **不是漏一行注册**：单体注册的是一个转发器，真落点是内容排期器，而排期器在三进程下没人构造
  （即上一条）。⇒ **这两条是同一件事的两半，要一起解**。
  当前后果已如实降级（自动化侧把回程失败吞成一行 warn，名额不归还、不拖住调度链），
  但账面上仍不诚实：自动化装配把这条能力标成 `wired`，对面根本没这条路由。
  接的时候 MUST 照单体形态：**缺排期器时响亮抛具名错误，绝不回 false** ——
...
>
> - [ ] **排期真到点触发一次**（change `wire-content-scheduler-into-api-process`）。接口进程现在**真的构造并启动**
>   内容排期调度器了——在此之前它没有任何进程在跑，而「到点什么都没发生」与「队列里暂时没活」在外部完全同形。
>   要在真机上确认：命中错峰分钟后确有一次触发、小时格被占住、同格不重触发。
>   **单测证不了这条**：本地全程是桩，心跳、错峰分钟、数据库原子占位三件事只有真跑一小时才同时成立。
> - [ ] **换环境后稿件真被投出**（同 change，用户 2026-08-03 裁定的行为变更）。自动发帖不再绑浏览器环境：
>   账号在 A 环境触发、后来只在 B 环境在线，稿件**应当照常从 B 发出**（日志留一行 info 记录环境不同）。
>   改前它会永远卡着。要真机确认「换环境后不再卡稿」，且 dev/ol 部署目标隔离**没被顺带削弱**（另一台机器的稿子仍不下发）。
> - [ ] **回程真归还小时格**（同 change）。评论链在自动化进程跑，接管边端失败时经跨进程回程把这一小时的名额还给
>   接口进程的调度器。**契约层已证形状往返、无调度器时具名失败**，但「还回去之后本小时剩余分钟真的再试」
>   要真有一次边端接管失败才走得到。
> - [ ] **两个进程一起跑，确认这族路由真的通**（同 change）。本 change **不声称三进程真跑通**——六仓测试各自全绿
>   恰恰是这类事故的标准现场。要点名验的是新加的 12 条 `content-scheduling` 路由 + `availableCount` 那一条：
>   客户端建得出来、调用编译得过、两仓测试全绿，**只有真把接口与自动化两个进程一起跑起来才知道有没有 404**。
>   已加机械闸（注册集合逐字等于路由表 + 只许下降的路由清单闸，两个方向都做过变异实测），但闸只管本仓装配、
>   管不了对面进程有没有起。属拆仓批次 5 的范围。
> - [ ] **FB 排期发帖的素材可用数真跨得过去**（同 change）。`availableCount` 是本批新开的 api→content 读口；
>   不接这一跳的表现**不是报错**，而是每格按 0 处置、日志说「素材不足」而事实是根本没问。
>   要在真机上确认 FB 账号有素材时排期发帖真能起，而不是永远停在「素材不足」。
>
> - [ ] **飞书 `/delegate` 端到端跑一次，看幂等台账真的落行、重投拿回同一结果**
>   （change split-cloud-automation-production-runtime，2026-07-30 随 `aidcp-cloud@319b0af` 部署 dev）。
>   自由文本委托现在经**接收方 + 幂等台账**（表 `operator_command_receipt`，属主 automation）。
>   启动期已自证台账可用（日志「运营指令幂等台账已就绪」）、单体全量零回归，
>   **但这条链一次都没真跑过**——要真发一条飞书消息才触发。要确认三件：
>   ① 一条 `/delegate` 后 `operator_command_receipt` 里落下一行、`state='applied'`；
>   ② **重发同一条消息**（同一个飞书 messageId）拿回**首次那份回执**、且**没有**建出第二条委托任务；
>   ③ 确认卡 / 直接排队 / 控制类三种渲染与改动前**逐字一致**。
>   **为什么单看编译与单测发现不了**：接收方的用例全用内存假台账，PG 那份 SQL 的每个分支只在真库上验过一次
>   （手工脚本，非自动化）；而「渲染逐字一致」这件事没有任何机械手段守着——
>   改的是组装根里的回执分流，单测覆盖不到那一层。
>
...
- [ ] **defer-transient-publish-predispatch-failures — 可恢复档重投真能走完** —— 桩只能证明「调度了一次重投」，
      证明不了第二次真能从头跑完整条序列。真机验：人为在提交前制造一次抖动（断网 / 图源 503），确认草稿保持待审、
      授权未烧、熔断计数不动，且**下一次重投真的发出去了**。
- [ ] **defer-transient-publish-predispatch-failures — 素材归还窗口是否被抢** —— 可恢复档把图集置回可用，
      重投前这段时间**另一条草稿可能把它取走**。该窗口原本就存在于被抢占那条路（同样归还 + 重投），
      但本次把它从 1 次拉长到最多 3 次。真机验：同账号多稿并发时，重投后仍取到同一组图。
- [ ] **defer-transient-publish-predispatch-failures — 重投耗尽的飞书文案** —— 「重试 N 次未成」那条通知的
      渲染与路由在组装根里，无单测覆盖（要起整个服务才跑得到）。真机验一次文案与去向。
...
- [ ] 70.6 小红书零回归 — 该项不再宣称「逐字输入辅助未受影响」；Native 小红书发布的真实 CDP 键入、段落 / 光标 / 相似度与失败清场，统一按**簇 125**的写动作与发布真机矩阵验收。簇 70 只负责 Facebook 长正文预算，不重复定义小红书判据。
...
- [ ] 87.16 **一次“审核并发送”真机闭环（`wechat-reply-safety-simplification`）** — 用包含 Edge `2fc75c3` 的源码客户端（本 change 未打安装包），对用户批准的可删除评论修改文字后只点一次主按钮：必须按保存→批准→发送串行，发送入队只显示“等待平台确认”；平台 `confirmed` 后才显示成功。再制造一次发送门禁拒绝，页面必须停在“已批准，待发送”并保留直接重试入口，不能回滚批准或染绿。
- [ ] 87.17 **人工/自动冷却与数量门禁分工真机复核（`wechat-reply-safety-simplification`）** — 对同一命名测试账号分别验人工已批准和 low-risk auto 候选：人工路径不因新登录冷却或通用 `quota:*` 单独拒绝，但仍受 active auth、identity、capability、专属 minute/hour/day/thread 限速和熔断约束；自动候选继续等待新登录冷却。需要真实平台副作用的部分沿用 87.3/87.7 的授权前置，未获批准不得放量。

<!-- wechat-reply-safety-simplification dev baseline (2026-07-20): Cloud 54b6f8e and Console 16ad709 deployed from clean master after backup cloud.bak.20260720-112236.tar.gz, console.bak.20260720-112236.tar.gz and target-local .env backup. Cloud active since 11:25:25 CST; 8787/8090/8088/5432 listened, panel health returned {"ok":true}, PostgreSQL SELECT 1 passed, Feishu WSClient onReady appeared, deployed Cloud/Console artifact hashes matched local, and isales services remained active. Runtime truth at verification: interaction schema=legacy_read_only, deploy env=dev, configured/effective global write=true, quantity gate logged as interaction_windows; runtime controls aggregate was 3 accounts / comment-read 2 / comment-reply 2 / DM-read 2 / DM-send 2 / unpaused 2 / circuit-closed 3, auth states active=1, authenticating=1, disabled=1. No real reply was sent and no Edge installer was built. -->
...
- [ ] 90.5 **加群成功闸与云端一致** — 加群条目的成功判据镜像云端证据闸（`ok && clicked`）。核：客户端说「加入了小组」时，云端 `interaction.occurred{join_group}` 同时发生；~~客户端说「申请加入…等待管理员通过」时云端**不**记 join_group~~。二者 MUST NOT 打架。
  > **【判据已变，2026-07-17 · change `fb-join-quota-counts-attempts`，cloud master `293c4e5` 已部署 dev】** 划掉的那半**已被推翻**：待审批的加群，云端**现在会**发 `interaction.occurred{join_group}` 并计入风控配额——因为它真的点了、FB 真的收到了申请，配额是风控预算、计的是「平台观察到多少动静」。**照旧判据验收会把正确行为当 bug 报回来。**
  > **新判据**：客户端说「申请加入…等待管理员通过」时 → 云端 **MUST** 记一笔 join_group 配额；**但** MUST NOT 记成成功加群（不进展示账本 `interaction_feed`、成员账本仍是 `status='pending'`、后台 FB 群页仍显示「已加入 N / 待审批 M」中的待审批那一栏）。客户端文案**不变**（仍说「等待管理员通过」，不得改口说已加入）。**「计了配额」与「加成功了」是两个问题、两个答案，验收要分别核。**
- [ ] 90.6 **搜索条目与在场感** — 定向搜索出「在「<真实群名>」搜「<词>」，找到 N 条」/「没有匹配的帖子」（二者可区分）；浏览侧搜索出「搜索「<词>」，找到 N 条」，且在场感为「正在看「<词>」的搜索结果…」而**非**「正在浏览推荐流…」（后者在搜索结果页上是假话）。

### change `platform-honest-usage-caps` FB 收藏 / 关注格不再撒谎（cloud `6122083` 已 land + **已部署 dev**；**不需要出安装包**；登记于 2026-07-17）

**背景**：配额表全局（PK `(tier, action)`、无 platform 维度），云端组装用量载荷时完全平台盲 ⇒ FB 账号收到「收藏 25/天」「关注 15/天」，而 registry 早已声明 FB 无收藏概念、无关注执行器。客户端据此画出「0/25」+ `has-limit` 卡片 + **永远 0% 的进度条** = 在展示一个不存在的计划。已改为按 registry 声明摘掉这两项上限（计数照发，只是没了分母）。

**本条不需要新客户端**：云端摘 `quotas` 键，现有客户端的清洗规则（`cleanOptionalCounts`）本就保留缺席 ⇒ **任何已在运营机上的客户端立刻生效**，无需等出包（这正是本 change 相对簇 90 其余项的差别）。

**已在 dev 用真实数据验过的部分**（不必再核）：dev 库 7 个 FB 账号 platform 列真是 `facebook`；真实 day 档 = `{view:150, like:50, collect:25, comment:8, follow:15, publish:1}`；跑部署上去的那份代码 → FB 摘掉 `[collect, follow]` 其余逐位不变 / XHS 摘掉 `[]` / 未知平台照发六项。**下面要核的是「屏幕上真的变了」这一段**，即云端投影 → ui.snapshot → 客户端渲染的那一跳。

**前置环境**：同簇 90 其余项（tom 分组 FB 测试号，headful 客户端连 dev `ws://121.89.85.150:8787`），建议同一次真机 session 合验；**客户端版本不限**（不需要含 `40aa902` 的新包）。

- [ ] 90.7 **FB 收藏格 / 关注格不再显示 `/N` 与进度条，且完成态第一次可触发** — 核 ① FB 环境「今日进展」的收藏格与关注格显示「收藏 0」「关注 0」**无 `/25`、无 `/15`、无进度条、无 `has-limit` 边框**（此前必有）；② **同屏一致性**：正下方「今日节奏」窗口条里收藏 / 关注也不再出现 `0/N`，**尤其是第一条「本轮计划」session 窗口**（它的上限来自另一套全局配置 `session_config_global`、`collects:5` / `follows:3`，是本 change 实装期才发现的第二个上限面——若它仍显示「收藏 0/5」而 KPI 格已诚实，说明 session 侧过滤没生效）；③ **「今天先到这里，明天继续」完成态在 FB 上第一次可触发**（此前被永远完不成的「收藏 25」堵死） —— **项数判据已变（2026-07-17，change `platform-honest-usage-metrics`）**：FB 的带上限指标现为 **5 项**（浏览 / 点赞 / 评论 / 发帖 / **加群**），不再是本条原写的 4 项、更不是 6 项。按原文验收会把正确行为当 bug 报回来；④ **勾一次慢启动开关后收藏格 MUST NOT 闪回 `0/25`**（回执 `dayQuotas` 会直接覆盖 KPI 上限，是第 4 个上限面；此项需客户端含慢启动开关的包，无包则跳过并注明）；⑤ **不回归小红书**：XHS 环境六格上限、进度条、完成态**逐位如常**（这是首要回归判据，不是善意期待）。

### change `platform-honest-usage-metrics` FB 收藏 / 关注整格消失 + 加群格放出（cloud `f127435` / edge `8976637` 已 land，cloud **已部署 dev**；**客户端那一半要出包才可见**；登记于 2026-07-17）

**背景**：上一条（`platform-honest-usage-caps`）只做了半步——摘掉 FB 的收藏 / 关注**上限**，却刻意保留了**计数**，理由是「计数是事实、只有承诺会撒谎」。那个理由在结构不支持的动作上不成立：FB 的「收藏 0」不是观测，是云端对一个不存在的动作物化出来的常量；它读作「今天还没收藏」、暗示明天会有数字，而真相是「这个平台没有收藏」。用户 2026-07-17 裁定：**不支持就不要展示**。同批把 FB 真做、真烧日配额（均衡档 3/天）、后台用量表早在显示的**加群**放到客户端。

**验收分两组，取决于运营机上的客户端版本**（本 change 的云端半已在 dev、客户端半**默认不出包**）：

**A 组 · 无新包也能验（云端半，任何版本客户端立刻生效）**

- [ ] 90.14 **FB 展开的「今日节奏」里，收藏 / 关注两行整行消失** — 展开今日进展卡，核四个窗口（本轮计划 / 当前节奏 / 阶段节奏 / 今日计划）里**都不再出现收藏行与关注行**（此前是「收藏 0/-」这种带计数无上限的半行）。**「本轮计划」那条尤其要看**：它的预算来自另一套零平台维度的全局配置，是最容易漏投影的一面。
- [ ] 90.15 **FB 窗口明细里出现「加群」行** — 同上展开区应出现加群行，且「今日计划」窗口显示 `加群 N/3`（均衡档）。N 的口径 = **今天发出去几次加群申请（点了就算、含待审批）**，与后台仪表盘用量表「加群」那格的 `用了` **数字必须一致**——两块屏幕读的是同一个计数器，不一致即说明有一侧读错了源。
- [ ] 90.16 **不回归小红书（首要判据，不是善意期待）** — XHS 环境的 KPI 六格、四个窗口的六行、上限、进度条、完成态**逐位如常**，且**任何地方都不得冒出「加群」**（小红书没有群）。dev 纯函数层与部署后实测均已坐实，这里核真实账号载荷。

**B 组 · 需要含 `8976637` 的新客户端包**

- [ ] 90.17 **FB 顶部 KPI 格：收藏 / 关注整格不见，加群格出现** — FB 环境的今日进展应是 **5 格**：浏览 / 点赞 / 评论 / 发帖 / **加群**。核：收藏格与关注格**整格不存在**（不是显示 0、不是灰掉、不是空位）；加群格显示 `N/3` 且有进度条。
- [ ] 90.18 **格子隐藏后分隔线与折行不塌** — 核 FB 5 格 / XHS 6 格 / 视频号 1 格三种形状下：分隔线对齐、**无空格位、无错位竖线**；把窗口拖窄到折行后同样不塌。（原实现靠 `:nth-child` 画边框，而 `:nth-child` 数的是 DOM 位置、不管 `display:none` ⇒ 隐藏两格必然错位；已改为间隙透底。）
- [ ] 90.19 **收藏格 MUST NOT 闪回（本 change 最容易静默回归的一条）** — FB 环境跑起来、发生一次真实点赞后，**盯住收藏格**：它 MUST NOT 短暂出现「收藏 0」再消失。（客户端本地乐观累加曾会把云端摘掉的键物化回来，格子当场重现、≤60s 后下一份云端快照才又消失。已加守卫 + 单测，这里核真机。）
- [ ] 90.20 **视频号只剩「发帖」一格 —— 这是预期，不是 bug** — 视频号环境的今日进展只应有发帖一格（其注册表声明浏览 / 点赞 / 收藏 / 评论 / 关注全不支持，只做入站互动）。**看到只剩一格不要报 bug**；它此前那六个 0 与 FB 的「收藏 0」是同一个谎。

**本批不治、只登记**

- [ ] 90.21 **【登记，预先存在】慢启动 day1-2 会让加群显示「0/0 今日计划已完成」** — FB 冷启动曲线 day1-2 的 `join_group` 上限是 `[0,0]` ⇒ 勾了慢启动的 FB 新号在头两天会看到「加群 0/0 · 今日计划已完成」。**非本 change 引入**：同一条曲线的 `comment` / `publish` 今天就已经这么显示。要治得先想清楚「今天的计划就是 0 次」该怎么说人话，与本 change 解耦。
- [ ] 90.22 **【登记，承 90.8】平台错标现在多一个后果** — 错标成小红书的 FB 环境（`ON CONFLICT DO NOTHING RETURNING platform` 既有行永不覆盖 + 平台缓存无 delete + 缺失值静默回落小红书）不但拿不到收藏 / 关注的摘除，**加群格也不会出现**，而症状与病灶依旧一模一样、无信号可辨。最小治法仍是 90.8 建议的「载荷回带云端按哪个平台投影的」作诊断字段。

- [ ] 90.8 **【登记，非本 change 治】平台错标不可纠错——建议另立 change** — `account-store.ts:238` `ON CONFLICT DO NOTHING RETURNING platform` 使既有行 platform **永不被覆盖**；平台缓存全仓无 `delete`/`clear`；`normalizePlatformId` 对**缺失**值静默回落小红书。合起来：AdsPower remark 缺 `plat` 的 FB 环境**首次登记即永久是小红书** ⇒ **拿不到本 change 的摘除，且症状（收藏 0/25）与病灶一模一样、无信号可辨**（见 memory `adspower-env-platform-label`）。dev 现有 7 个 FB 账号未中此坑（platform 列都对），但这不代表运营机不会中。**最小治法**（建议）：载荷回带「云端按哪个平台做的投影」作**诊断字段、不作决策依据**，客户端发现与本地环境平台不符即旁白告警——把静默错标变成可见事故。可参照 `account-level-slow-start` 的 fail-honest 形态。

- [ ] 90.9 **【登记，独立 nit，非本 change 治】`edge-companion-ui` spec:934 的理由是 falsifiable-by-work** — 「加群与搜索 MUST NOT 贡献任何计数」所附理由是「（二者在云端权威计数投影中均不存在对应字段）」——这是一句关于**今日实现**的事实、不是原则：字段哪天被加上，该要求的自陈理由就蒸发了。其**真正依据**是上方的四档模型（待第三方批准 MUST 自成一档、MUST NOT 贡献计数）——一个标量计数器没有第四档，表达不了「申请 3 个、批了 0 个」。理由应改为引用该模型。**本 change 放弃修**：该句被包在 `:921` 起的大要求块内，MODIFIED 须整块复制（含全部 scenario），为一句括号措辞与已排队的 4 个 change 抢同一 spec 文件不划算，且与云端上限投影无因果关系。
...
> **补登 `fb-join-quota-counts-attempts`（2026-07-17，cloud master `293c4e5`，已部署 dev，backup `cloud.bak.20260717-154048`，healthcheck 绿）**：加群的分/时/日配额是**风控预算**——它约束的是「平台观察到这个号做了多少动静」，此前却读「我们办成了几件事」。边缘对「点了加入、FB 收到申请、管理员还没批」**诚实回 `ok:false, clicked:true`**，而记账闸要求 `ok` ⇒ **我们点了、FB 记了一笔申请、我们的风控记 0**。于是「日限 3」约束的是「今天批准了几个」，「今天发出去几个申请」不受任何约束、也不被任何计数器度量。改动只有一处：把 join_group 从 `ok` 的合取下解出来（`clicked` 判据 2026-07-09 就在闸里、只是被 `ok` 短路压着）。**边缘零改动、协议零改动、DB 零改动、无需出安装包。**
>
> **本批最重要的预期管理**：日限**第一次真正生效**，加群次数会下降——**这是之前一直在超发被纠正，不是回归**。若真的不够用，正确动作是评估日限**数值**，不是把分子改回去。
>
> **桩层**：`test/handler-join-quota.test.ts` 12 条；acceptance 55/55、全量 2401 pass / 0 fail、typecheck 干净。**桩层测不到的正是本批的要害**——记账闸只决定发不发事件，真正的 +1 在引导期接线的订阅者里，单测无缝可进；故下面几条**必须真机**。
>
> **前置环境**：tom 分组测试号（工程师大白 / Tmax）+ 越南招工类 FB 群（需要**未加入**的群若干，其中最好有需管理员审批的）；`AIDCP_FB_GROUP_JOIN_AUTO=true` 起自动加群；客户端连 dev（`ws://121.89.85.150:8787`）。**共享环境同簇 82 / 88 / 90 / 91（FB 真机批）**，建议同一次真机 session 合验。

- [ ] 90.8 **待审批的加群真的记进配额（本批的正面验收 · 最高价值）** — 让账号点进一个需审批的群、停在「待审批」。核：云端该账号当日 `join_group` 风控计数 **+1**（后台仪表盘用量表「加群」那格 `用了` 数 +1）。**此前这里恒为 0**——这一跳（emit → record → 计数器）全在引导期接线里，桩测一行都覆盖不到，**只能真机坐实**。
- [ ] 90.9 **额度耗尽后调度闸真的停手** — 连续发若干入群申请（含待审批）直到当日 `join_group` 计数撞上限（normal 档 = 3）。核：自动加群**不再起**；`quota_denied` 审计有记录。**这是「日限第一次真正生效」的直接验收**——此前待审批不计数，配额永远撞不到顶。
- [ ] 90.10 **计了配额 MUST NOT 变成「加成功了」（诚实闸 · 两个问题两个答案）** — 承 90.8 同一次待审批：核 ① 后台 FB 群页仍显示该群「待审批」、**不在「已加入」里**；② 展示账本（互动历史）**没有**这条加群；③ 客户端活动流文案仍是「申请加入…等待管理员通过」、**不得改口说已加入**。**与 90.5 是同一件事的两半，必须一起核。**
- [ ] 90.11 **点击「前」就发现已在待审批 ⇒ 不计（本批最容易搞反的一对）** — 对一个**已经处于待审批**的群再触发一次加群：边缘点击**前**就观测到「之前就已待审批」并回 `clicked:false`，reason 与 90.8 **逐字相同**（都是 `pending`）。核：配额计数 **不变**。**判据是「这次有没有真的点」，不是 reason 字符串**——按 reason 判会把这种情况也计进去 ⇒ 无故限死账号。
- [ ] 90.12 **慢渲染重试会真的多烧配额（预期管理，不是 bug）** — FB 群页渲染慢时，边缘回 `post_not_confirmed_slow`（**已经真点过**），云端短退避**重新下发、重新真点**。核：一个群可能消耗 **N 格**配额。这在「计尝试」这条轴上是**对的**（真点了 N 次 = FB 真收到 N 次），但设计里没建模；若真机发现 N 偏大，那是**重试策略**该调，不是配额分子该改回去。
- [ ] 90.13 **手动 `/comment --join` 与自动加群的预算相互作用（已知的松处，核一下真实影响面）** — 手动加群**按裁决刻意绕过配额闸**，但其回执照走记账闸；而记账函数内部会再过一次判定、撞小时窗（加群小时配额算出来 **= 1**）就静默丢数。核：运营一小时内手动加 2–3 个群后，后台「加群」用量格的数**明显少于真实点击次数**。**这不是本批引入的**（本批已刻意不动它），但它是「统一分子」那个后继 change 的前置——真机数据能坐实它有多严重。
...
- [ ] 93.10 **客户端审批恢复直连真机闭环** — 安装含 `restore-client-direct-publish-approval` 的新包后，分别在审核页点「发布 / 取消」：按钮 MUST 等 Cloud 权威受理结果，成功才收页并改变状态，断网 / 版本过期 / 飞书已先决时 MUST 留页并显示真实拒因；MUST 不再出现「委托任务已排队但审批未生效」。另在请求在途切到另一账号，旧应答 MUST NOT 改写新账号稿件状态。
...
**前置环境**：Cloud `d231be1` 已部署 dev，双重开关已启用；Edge `22c5be8` 已合入 master，需重启
`electron:dev` 或后续明确出包后才会加载新 capability 和界面。登记于 2026-07-17
（`wechat-channels-test-data-reset`）。本次交付只做了无损鉴权/健康检查，**没有拿当前真实账号执行重置**。

> **边界**：这里只清当前 dev 环境、当前账号、所选通道在 Cloud 的互动读取副本，并让 Edge 从空游标重读；
> 不删除视频号平台数据、不发送回复、不改授权 / 风控 / 互动配置。前置条件不满足必须拒绝，不能先删后报错。

- [ ] 95.1 **入口与保护条件真态** — 重启新 Edge 后，仅在「开发者详情」且 Cloud 返回
      `testTools.dataResetEnabled=true` 时显示「重置评论 / 重置私信」；旧 Edge、边缘离线、未暂停写入时 MUST 不可执行。
- [ ] 95.2 **单条评论可反复重读** — 先记下当前仅有的一条评论，暂停写入并确认该通道从未尝试发送，输入
      `重置评论` 后 Cloud 该通道副本归零，随后同步把同一条平台评论重新读回；私信、配置、授权和审计保持不变。
- [ ] 95.3 **单条私信可反复重读** — 与 95.2 对称，输入 `重置私信` 后只重置私信通道，平台上的原私信仍在，
      评论通道与其他状态不变。
- [ ] 95.4 **拒绝态不产生删除** — 分别造旧 Edge capability、写入未暂停、已有发送尝试、确认文字不匹配；
      每种情况 MUST 在删除前被拒绝，刷新后原 Cloud 记录仍在。Cloud 已提交但重读下发失败时 MUST 明示「部分完成」，
      不得假报完整成功。
...
**前置环境**：Edge `3a70981` 已合入并推送 `master`，但本 change **没有构建或发布安装包**，
也没有对真实视频号人为触发限流、断网或修改账号内容。登记于 2026-07-17
（`wechat-edge-auth-recoverable`）。

> **已验边界**：自动化已证明限流/上游故障的原因码、有上限持续退避、无浏览器自愈、停用取消、
> 零发帖 fail-closed 以及熔断 TTL/探针复位；acceptance 23/23、全量 1664/1664、typecheck 通过。
> 下列项均仍未做真机验证，不得写成已恢复或已发布。

- [ ] 96.1 **真机限流/断网自愈** — 使用用户明确命名的 dev 测试账号，在不发送评论/私信的前提下分别复现
      限流和短时断网；核对原因码为真实类别、退避按平台 `retryAfterMs`/上限执行，会话保持原账号，收件箱恢复后
      无需重扫二维码，且全程没有被自动反复打开浏览器。
- [ ] 96.2 **零发帖新号诚实接入** — 使用尚无任何视频号帖子的测试号完成授权；授权态 MUST 保持正常，
      MUST NOT 显示「接口改版」；评论读取在没有 `commentList` 证据前仍保持关闭，发出第一篇测试帖并取得读证据后才允许自动放行。
- [ ] 96.3 **`probeDm` 零会话 fail-closed 评估** — 当真实私信会话列表为空时，核定「`dmHistory` 请求成功即认为 `dmRead`
      probe passed」是否足够证明非空会话解析；若不足，单独提出 change 改为 gated，**不在本已收口 change 内追加顺手修复**。

---

## 簇 97 — 冷却改判为兜底：主闸是否真的重新掌权

**前置环境**：dev 云端已带 `cooldown-as-backstop-not-quota`（`b995207` + `b36acca`，2026-07-17 部署 dev）；
需 **dev 上有真实浏览车队跑满至少一整天**（跨上海自然日边界，因日窗是自然日硬清零、不是滑动 24h）。
登记于 2026-07-17。

> **改了什么（一句话）**：冷却从「实际的数量闸」降回「只防意外爆发的兜底」——四动作 120/300/600/1800s
> 统一为 15s，重启静默期 180s → 15s。**配额一个数字都没动**，但实际用量会**第一次真的逼近**面板上配的日额。
>
> **⚠️ 上线前须运营确认面板上的数就是他们要的**（点赞 50/日、收藏 25/日）。涨幅**无法预估**：
> 决定性参数（一场里冒出几个互动意图）从未测量过。**这是本 change 最大的单点风险，且它不是冷却的错**——
> 是「面板配的数从来没被真正逼近过」这件事终于暴露出来。
>
> **回滚成本 ≈ 一次云端 restart**：冷却是进程内内存态、零残留状态；静默期可用 `AIDCP_RESTART_QUIET_MS=180000`
> 秒级回滚、不改代码不发版。

- [ ] 97.1 **`cooldown` 跳过应趋零** — `journalctl -u aidcp-cloud.service | grep 'skip reason=cooldown'`
      对四动作（like / collect / follow / comment）应**趋零**。**仍有命中 ⇒ 15s 竟还在 binding ⇒ 配置或算术有出入，
      必须回头查，别当没看见**（这是本 change 唯一能证伪自己的信号）。
- [ ] 97.2 **日总量向上限靠、但不越过** — 每日实收 点赞 / 收藏 / 评论 / 关注 总量 vs 面板日额，
      预期「**向上限靠、但不越过**」。越过 ⇒ 主闸有洞（与冷却无关，另起 change）。
- [ ] 97.3 **`pacing_saturation` 告警频次应上升** — 撞分钟 / 小时窗发 P2。**这是主闸重新掌权的正信号，
      不是故障，别误当回归**。
- [ ] 97.4 **别把两个已知的良性现象误判成「冷却没生效」**：
      (a) **日初点赞比例闸**（日窗、views ≥ 10 才检、`(likes+1)/views ≤ 0.35`）——冷却不再吃意图后，
      日初 views=10 / likes=3 时第 4 赞会被 `ratio:like_view` 拒。长期无碍（50/150=0.33）。
      (b) **收藏/点赞比例向上漂** —— 收藏与点赞永远同数组下发，旧的长短不一冷却把「both」决策拆成「只点赞不收藏」；
      统一 15s 后赞藏恢复**同刻共发**。这是**可观测的行为口径阶跃**（更像人，但平台侧若对 save/like 有画像需盯），
      且系统内**无任何闸**在管这个比（见 5.4）。
- [ ] 97.5 **点赞最坏形状复核**（低优先）— 最坏序列从 0/120/240s 变成 0/15/30s。但分钟配额 3 **今天就允许**
      3 个赞瞬时连发（滑动窗只数量、不管间距）⇒ 15s 让最坏形状比「只有配额」**更好、不更差**。
      须在 dev 上核实**是否真有账号配了强制点赞规则**（mandatory 旁路今天就绕冷却 ⇒ 0 秒连发今天已上膛），别当假设。
- [ ] 97.6 **运营核数（非工程项）**：① 确认点赞 50/日、收藏 25/日就是他们要的；
      ② **激进档浏览 `per_hour=24` 疑似把 240 打错**（按公式本该 60，且比正常档的 80 还慢 3.3 倍；
      dev 库 2026-07-17 实测坐实）——与本 change 正交，但同属「面板数字没人对过账」。


## 簇 98 — 视频号收件箱两级导航：覆盖层是否真的遮挡、又真的让得开

**前置环境**：需 **本机跑 edge 桌面客户端**（`electron:dev` 即可，不必出安装包）+ 一个**已登录且有历史互动的视频号环境**
（评论与私信各至少 2 条，才验得到「逐条点得开」与「关闭后滚动位置保留」）。
代码：`aidcp-edge` `5b0380c`（change `wechat-inbox-im-drilldown`，2026-07-17 合入 master）。登记于 2026-07-17。

> **改了什么（一句话）**：收件箱从左右两栏改成手机 IM 式两级——列表只留昵称/时间/来源/状态，点开后详情
> **以覆盖层盖住列表**，关闭/返回/Esc 退回；两级切换时把分类标签排锚到视口顶部。
>
> **⚠️ 为什么必须真机**：本 change 的红线现象**单测原理上抓不到**。jsdom 不做样式层叠计算，`hidden` 在它眼里
> 只是个 DOM 属性——覆盖层**到底有没有真的挡住列表**，只有一条 CSS 文本断言看得住（已做变异验证：拆掉规则
> 恰好 1 个用例转红），**行为用例完全看不见**。而这个 bug 的形态正是「不报错、不白屏、覆盖层永久糊在列表上」——
> 样张阶段已实证中招一次。**下面 98.1 是本簇唯一能证伪自己的信号，别跳过**。
>
> **回滚成本 ≈ 回退一个提交**：纯渲染层，无协议、无云端、无数据迁移。

- [ ] 98.1 **列表级下每一条都点得开** — 进入视频号工作区，**逐条**点击列表里的每一条互动（至少 2 条），
      每条都应弹出详情。**任何一条点不动 ⇒ 覆盖层隐藏失效、正糊在列表上 ⇒ 红线复发，立即回退**。
      不要只点第一条就算过——覆盖层是整片盖住的，第一条能开不代表规则生效（它可能只是恰好在覆盖层的可点区）。
- [ ] 98.2 **详情确实遮挡昵称列表** — 点开后列表的头像/昵称条应被完整盖住、不露出半截。
      露出 ⇒ 覆盖层定位没铺满（查 `.iw-inbox` 的 `position: relative` 是否被后来的规则改掉）。
- [ ] 98.3 **关闭后列表滚动位置保留** — 列表滚到靠后的某条、点开、再关闭，应仍停在原处附近。
      跳回顶部 ⇒ 列表被卸载重绘了（design 决策 2 的前提破了）。
- [ ] 98.4 **锚定真的让消息区一次露全** — 在**窗口高度不足、收件箱被概览卡与设置卡挤到折线以下**的状态下点开一条：
      页面应自动滚到标签排贴顶、消息区完整可见、**无需再手动滚**。这是客户提出本 change 的原始痛点，
      也是 jsdom 里被守卫直接跳过的一段（`scrollIntoView` 在 jsdom 无实现）。
- [ ] 98.5 **三条退回路径都通** — 关闭图标 / 左上角返回箭头 / `Esc` 均应回到列表级且不残留选中。
- [ ] 98.6 **不默认选中** — 进入工作区、切换分类标签后，都应停在列表级、不自动展开第一条。
- [ ] 98.7 **客户观感定两个 open question**（design 里悬着的）：(a) 返回箭头与关闭叉是否都保留、还是砍掉一个；
      (b) 覆盖层打开期间后台拉到新数据会重排底下的列表，关掉后顺序可能变——是否需要「打开期间暂停重绘」。
      **两条都不是 bug，是待客户拍板的取舍**，别自行改掉。

## 簇 99 — 视频号生命周期控件：从「做了却报失败」回到诚实

**前置环境**：需 **本机跑 edge 桌面客户端**（`electron:dev` 即可，不必出安装包）+ 一个 **AdsPower 提供的视频号环境**
（必须是 AdsPower 分身，不能用「本机 Chrome」那个 self 兜底环境 —— 见下）。
代码：`aidcp-edge` `d24bb89`（change `wechat-lifecycle-status-honesty`，2026-07-17 合入 master）。登记于 2026-07-17。

> **改了什么（一句话）**：视频号工作台的 启动 / 恢复 / 暂停 / 关闭 四个按钮，此前**每次都报失败**
> （「启动失败：这条互动已不可用，或不属于当前环境。」），而动作其实**已经真的执行了**；现已改为按本机口径
> 校验回包，动作成功即如实呈现成功并立刻应用主进程回传的真实状态。
>
> **根因**：工作台里并存两个标识 —— 云端环境键（裸分身 ID）与本机运行时 ID（`ads-<分身ID>`）。守卫建立时两者
> 尚未分家；后来一次拆分只改了**发送侧**、漏了紧邻 3 行的**比对侧**，从此本机口径的回包永远比不过云端口径的键。
>
> **⚠️ 必须用 AdsPower 环境验**：self 兜底环境下两个标识凑巧同值，守卫**反而不误伤** —— 拿 self 验会假绿。
>
> **回滚成本 ≈ 回退一个提交**：纯渲染层单函数，无协议、无云端、无数据迁移。

- [ ] 99.1 **四个动作都不再谎报失败** — 对 AdsPower 视频号环境**逐个**点 启动 / 暂停 / 恢复 / 关闭：
      每次都应显示「已请求…当前环境。」这类成功文案。**任何一个仍报「…失败：这条互动已不可用，或不属于当前环境。」
      ⇒ 回归复发，立即回退**。不要只点一个就算过 —— 四个动作同源，但走的是不同的 action 分支。
- [ ] 99.2 **状态即时刷新、不等心跳** — 点「暂停」后按钮应**当场**变成「恢复」，不需要等下一次状态心跳追上来。
      仍要等几秒才变 ⇒ 回包真态没被应用（守卫仍在 `onLifecycleStatus` 之前抛）。
- [ ] 99.3 **动作确实作用于目标环境** — 多环境并存时暂停其中一个：只有该环境停，其余环境的运营与浏览器不受影响。
      这条是拆掉「标识缺失回落」那枚雷的验收面 —— 回落曾会让主进程静默改对**当前选中的另一个**环境下手。
- [ ] 99.4 **失败仍要能报失败** — 制造一次真失败（如动作发出前把该环境的核心杀掉），确认界面**如实报失败**。
      本次修的是「假失败」，不能矫枉过正修成「假成功」—— 那才是仓库红线的正面。

## 簇 100 — 记账只记既成事实：账本会不会终于说真话

### change `risk-record-actuated-facts` 真机验收（cloud master `7f5f4a3` 已 land + **已部署 dev**（backup `cloud.bak.20260717-193900`，healthcheck 绿）；登记于 2026-07-17）

**背景**：风控的记账函数在写下「这件事发生了」之前，先问一次「这个号现在还允许做这件事吗」——不允许就什么都不写。而它的调用点**全是事后回执**（边缘回报「我已在真实页面上做完了」之后才驱动），所以这道判断只可能在**既成事实**上开火，而它做的事是**抹掉证据**。拒绝记录一次真实点击，并不能把它变回没点过。

**它的来历直接把它自己钉死**：这句 `if (!canDo) return false` 出生时**就是「撞自己配额 → 自升威胁态」那个自残动作的触发点**；后来有个 change 专门来杀那个自残，**删掉了自残、把这句空壳留在了原地**。16 天里全仓没有一行设计说过「计数器不许写」。

**严重度不是「统计偏保守」，是预算真的会被打穿**：加群小时窗 = 1 ⇒ 运营一小时内手动加 3 个群（全真进），第 2、3 次撞小时窗被记账函数自己丢掉 ⇒ **日计数停在 1** ⇒ 日闸（=3）以为还有 2 格 ⇒ 自动加群再打 2 次 ⇒ **当天真实 5 次 / 预算 3**。**紧窗口的拒绝把松窗口的账本毒了。**

**用户裁决（2026-07-17）**：「**手动动作，评论和发布都应该记账，只是跳过配额限制**」⇒ **豁免的是闸，不是账。** 据此三处一并治：加群（送进记账却被丢）、评论（手动的根本不送）、发布（**谁都不送**——`record('publish')` 曾全仓零调用点 ⇒ 发布计数器恒 0）。

**⚠️ 桩层对本批的核心回归完全无感 —— 真机是唯一的真检验**：改动前全仓 2440 条测试**没有一条**能区分「写了」和「没写」（实测：打上补丁后零改测试全绿）。对抗性复核更用**注入实验**证明：把本批最微妙的那个 bug 原样种回去，全量 2443 **全绿**。已补 11 条新测试（含突变/注入验证），但下列判据桩测替代不了。

**桩层**：acceptance 55/55（**AC-RISK 零改测试全过**——这正是设计立的验收条件）、全量 2447 pass / 0 fail、typecheck 干净。

**前置环境**：tom 分组测试号（工程师大白 / Tmax）+ 越南招工类 FB 群；客户端连 dev（`ws://121.89.85.150:8787`）。**共享环境同簇 82 / 88 / 90 / 91（FB 真机批）**，建议同一次真机 session 合验；`view`/浏览节奏项可搭 XHS 号。

> **必须先跟运营讲明的三件（tasks 4.1，人来做，OL 上线前必须完成）**：
> ① **手动评论从此吃自动评论预算**（正常档日 8）——运营手动评满 8 条，机器当天不再自动评论。**这是裁决的直接意图，且推翻了一条既有明文用户决策**（`comment-search-command` 原写「命令评论 MUST NOT 计入风控配额」，还配了一条专门禁止回退的红线反例）。
> ② **手动加群从此吃自动加群预算**（正常档日 3）。
> ③ 各闸整体**更早拒绝**（计数器变准 = 变大）。**这是目的，不是回归**；若真的过紧，正确动作是评估**配额数值**，绝不是把丢数改回去。

- [ ] 100.1 **手动加群三次，日计数必须是 3（本批的正面验收 · 最高价值）** — 运营在**一小时内**手动 `/comment <昵称> --join` 加 3 个群（三次都真的点出去）。核：后台该账号当日「加群」用量 = **3**（改动前 = 1）。**这条直接证伪「小时窗毒日账本」。**
- [ ] 100.2 **额度耗尽后自动加群真的停手** — 承 100.1：日计数到 3（=正常档上限）后，**自动加群不再起**。改动前它会以为还有 2 格、再打 2 次 ⇒ 当天真实 5 次。
- [ ] 100.3 **手动评论吃掉自动评论预算** — 运营手动评满正常档日限（8 条）。核：当天**自治评论不再自动发**。**这是本批最可能被当 bug 报回来的一项**，且是①的正面验收——先讲明再验收。
- [ ] 100.4 **手动仍然不被拦（闸的豁免必须完好）** — 承 100.3：配额已耗尽时**再发一条手动 `/comment`**，MUST 仍能正常发出（人工授权、操作员全权）。**这一对（100.3 + 100.4）必须一起核**：只验一半会把「不拦」和「不记」又缠回去——那正是本批要拆的混淆。
- [ ] 100.5 **发布对账（白送的判据）** — dev 上真发若干条。核：后台发布数（来自 `publish_log` 覆盖）与风控发布计数**对得上**。**对不上就是本批出了问题**——判据取自同一口径（`status IN ('submitted','published')`）。注：`record('publish')` 此前零调用点 ⇒ 发布计数器恒 0，后台早已绕开它改读 `publish_log`（`panel/types.ts:381` 注释自陈）——**有人把显示修好了，把计数器留在死地**，这次它终于有数了。
- [ ] 100.6 **发布量不被误伤（数字一个没动的验收）** — 发布日配额**刻意未动**（仍 1/1/2）：`docs/risk-control.md` 特意选了它并写明「发布是最高风险动作；同一天多篇极易触发营销号判定」。核：dev 上发布节奏**与改动前一致**（排期实际 1、风控 1、扳机间隔 24h、文档 1 —— 四个数本就全是 1）。**若发现发布被卡**，说明「与排期一致」的判断错了，当场上报。
- [ ] 100.7 **`view` / 浏览节奏是否更早撞窗（影响面最大、最可能被当回归报）** — 手动 `/comment` 路径的 view 回执此前被自己丢掉（**该处丢弃方向是反的：少记 view 会跌破比例规则阈值 ⇒ 整条点赞比例规则不开火 ⇒ 更宽松**）。核：诚实记 view 后，浏览是否更早撞分钟窗（保守档 view 分钟窗 = 4）、点赞比例规则是否开始真正生效。**不在本批调数值**；若过紧，评估的是配额数字。
- [ ] 100.8 **微信入站已确认回复被记下、且不重复计数** — 视频号收件箱真发一条回复到 `confirmed`。核：① 风控 `dm_reply` 计数 +1（改动前**每一条**都被丢——`dm_reply` 三档配额皆 0 ⇒ 判定恒 false ⇒ 恒丢）；② 指标从 `denied` 变 `recorded_over_policy`（**含义变了**：不是「没记下」，是「记下了、但超策略」）；③ **重放同一条不产生第二笔**（占位只在真抛错时释放）。**这是本批最微妙的一处**：判据从「策略允不允许」改成「有没有写成」，只在重放时才显形。

## 簇 101 — 视频号读取开关：环境没启动时也该能改，改完要说实话

### change `wechat-read-controls-offline-toggle` 真机验收（edge master `13b56f4` 已 land + 推 origin/master；edge-only 无 ECS 部署、**未出安装包**；登记于 2026-07-17）

**前置环境**：需 **本机跑 edge 桌面客户端**（`electron:dev` 即可、不必出安装包——纯渲染层改动，主 checkout 已在含本修复的 master 上）+ 一个 **视频号（wechat-channels）环境**，且必须能造出「**核心子进程已停止 / 从未启动**」这一态（下有致命陷阱）。**共享环境同簇 98 / 99（同为视频号工作区、同一桌面客户端），建议同一次真机 session 合验。**

> **改了什么（一句话）**：视频号工作区顶部「收取互动 / 评论收取 / 私信收取」三个开关，此前在**该环境没启动时被灰掉、客户改不了**；现已放开——这条写入本就经主进程直发云端、不碰该环境的核心子进程，云端离线时按 CAS 正常落库并回「待生效」，该环境下次连上自动收敛。同时把云端回的投递状态**持久显示**出来（「已保存，待该环境下次连接后生效」），避免把一个假阻断换成一个假成功。
>
> **根因**：那道闸问错了问题——它拿「该环境核心子进程和云端的链路通不通」去授权一条**根本不经过该子进程**的写入。是拿 A 的在线状态去授权 B 的能力。
>
> **⚠️⚠️ 复现的致命陷阱（务必读）**：**只能用「已停止 / 从未启动」的环境复现**。**冷待机（点客户端里「关闭浏览器 / 转入后台」——浏览器关了但核心还活着）今天是好的**：那种态云端链路仍是 connected、开关本就可编辑。**若拿应用内「关闭浏览器」按钮去试，会看到开关一切正常，从而错误地得出「无法复现 / 已经好了」**。必须选一个**核心进程根本不在**的环境（从环境栏点选一个没启动过、或已 stop 掉的视频号号）。
>
> **回滚成本 ≈ 回退一个提交**：纯渲染层两个函数（`renderReadSettings` / `updateReadControls`）+ 一个 state 字段，无协议、无云端、无 IPC、无数据迁移。

- [ ] 101.1 **（A1 · 本 change 唯一未经真机观测的推断，先证它再往下）** 修复前，选中一个**已停止 / 从未启动**的视频号环境，三个开关应是**灰的（disabled）**。这一步同时证明推断成立：停止态环境的云端链路确实不是 connected。**推断依据**：客户端里「从未启动过的环境」默认就是 disconnected（`main.cjs:1030`）、核心退出后也置 disconnected（`main.cjs:2794`）、渲染层据此推出 connectivity（`renderer.js:231`），代码上闭合但**未在真机上眼见过**。**若这一步开关竟是亮的 ⇒ 根因另有其人，本修法仍对（那道闸本就问错了问题）但症状不会消失，立即停手、回头重新定位、别继续往下验。**
- [ ] 101.2 **（A2）停止态可编辑且真的写进去了** — 同一停止态环境，修复后三个开关**可点**；点一下后云端该账号的读取开关版本 +1、对应 boolean 真的变了（后台 / DB 可核）。证明本地不再拦一条云端完全能受理的写入。
- [ ] 101.3 **（A3）离线保存不冒充已生效** — 承 101.2：读取设置区应**持久**显示「Cloud 已保存 v{n}，待该环境下次连接后生效（需要启动该环境）」，**MUST NOT** 出现「已应用 / 已生效」。且该呈现**不因随后点别的**而消失（它落在持久位、非一次性通知位）。这是本 change 的主要风险面——放开闸却不如实说「还没生效」，就是把假阻断换成假成功。
- [ ] 101.4 **（A4）重连收敛，无需再点** — 启动该环境 → 客户端握手 → 欢迎信封带来最新读取开关快照 → 文案自动转成「本机已应用同一版本」。**全程不需要再点一次开关**。
- [ ] 101.5 **（A5）冷待机零回归（正是上面那个陷阱的正面验收）** — 一个**运行中**的视频号环境，点客户端里「转入后台 / 关闭浏览器」→ 三个开关**仍可编辑**，保存后读作「已保存并已下发本机」（enqueued，与 101.3 的 deferred 措辞可区分）。这条守着「别把浏览器现场和授权态压成一个布尔量」——把两者合并正是这个 bug 的形状。

> **已知、已分析、本 change 故意不改的残留（供后续 change 单独处置，勿在本簇当 bug 报）**：详情拉取（`loadDetail`）清 stale 是有条件的（只在 connected 时清），而列表拉取（`loadList`）无条件清——同为一次成功的云端拉取，对「数据新不新鲜」是同等证据，这个不对称没道理。**不在本 change 改**：`state.stale` 同时喂着回复草稿 / 发送那组写闸，动它会放开本 change 没分析过的写入。当前实际影响为零（列表拉取先跑并已清 stale，详情那条件是死条件）。

## 簇 102 — 会话启动现问配额：额度耗尽的账号是否终于整夜不再空转开浏览器

### change `session-start-quota-honest-sleep` 真机验收（cloud `cebd78e` 已 land + 推 origin/master + **已部署 dev**；cloud-only 无出包；登记于 2026-07-17）

**前置环境**：一个**当日 view 日额度已耗尽**的浏览账号连 dev（`ws://121.89.85.150:8787`）。最省事的造法＝勾一个**慢启动第 1 天**的账号（view 上限 20），让它自然跑满；或直接用触发本次报障的那个 FB 账号 `61591753702668`（edgeId `ads-k1ei3dbi`）。**共享 FB 真机环境同簇 82 / 88 / 90 / 91 / 100**，建议同一次 session 合验。**观测需跨越一次「已耗尽 → 次日零点」的夜窗**（或人为把某窗口配额调 0 造耗尽态，观测数十分钟即可）。

> **改了什么（一句话）**：额度耗尽的账号此前每隔约 4 分钟被云端叫醒一次去「开浏览器→翻 feed→跑两次模型→才发现没额度→再睡」，整夜约 67 轮空转。现在**会话一启动（含每次边缘重连重启）就先问一句额度，没有就当场进入浏览休眠**——浏览器保持停放，直到窗口释放（日窗＝次日零点）。
>
> **根因**：① 那个「知不知道没额度」的休眠标记挂在每条连接重造的调度组件上，重连即回出厂态（不是被谁取消，是从没被带过来）；② 会话启动闸只判调度开关 / 平台 / 人设，不问配额，要拖到「决定打开某篇笔记」前才发现——那时浏览器已开、feed 已扫、两次模型已跑。
>
> **回滚成本 ≈ 回退一个提交**：cloud 单文件两处（会话启动现问 + 休眠闸补日志）+ 一个测试文件，无协议、无边缘、无数据迁移；env 无新旋钮。回退即回到「每 4 分钟空转一次」的旧态。

- [ ] 102.1 **（本簇最高价值 · 正面验收）额度耗尽后浏览器整夜不再被叫醒** — 账号 view 日额度耗尽、边缘已让位（浏览器关、云端连接保留）。核：从耗尽时刻到次日零点，**浏览器不再被周期性唤醒**（边缘日志 `edge.log` 不再反复出现「冷待机期间收到唤醒触发 (cloud_command:page.scroll)」；云端 journal 该账号不再每 240s 一条 `sendCommand action=scroll sent=1`）。**改动前**：约每 4 分钟一轮、整夜约 67 轮。
- [ ] 102.2 **（云端侧同一事实的另一面）启动即休眠、且休眠期扣命令留了痕** — 云端 journal 该账号出现「view 配额暂不可用 → 休眠浏览 …s（reason=quota:day）」**紧跟在**「边缘 hello → 会话已重置并重启」之后（即**启动就休眠**，而非拖到 open_note 前）；且休眠期看门狗 nudge 被扣时留下 `command.suppressed reason=view_quota_sleep …`（节流：首条 + 每 50 条，不是逐条刷屏、也不是零条静默）。
- [ ] 102.3 **（反向不变量 · 别验歪）会话没有被结束、手动 / 定时仍畅通** — 承 102.1：额度耗尽期间该账号**会话仍在**（未 `session.end`）；此时**手动 `/comment`** 与**定时 / 手动发帖**仍能正常执行（走独立唤醒通道，结构性免疫本闸）。**这条守着「休眠 ≠ 停号、更 ≠ 拦死操作员」**，只验一半会把「不开浏览器空转」误读成「把账号冻住了」。
- [ ] 102.4 **（分钟窗不误伤 · 易被当回归报）临时窗满只短睡、随后自动重驱** — 找一个仅**分钟窗**临时打满 view 的时刻（日窗仍有余额）。核：会话照开、当场短睡（约 60s 量级），窗口一过**自动滚动重驱浏览**、继续正常开笔记——**不是睡到明天、也不是拒签会话**。若发现分钟窗满就把账号睡死一整天，即为误伤，当场上报。
- [ ] 102.5 **（验收未知项 · 观测性，非红线）1h 结束会话尾巴是否首次现形** — 设计已知：边缘约每 5 分钟重连一次会重置空闲时钟 ⇒ 看门狗「1h 判停滞结束会话」这条尾巴**今天可能从未触发过**。上线后观测一夜二选一：**(a)** 重连 churn 仍在 → 每次重连重新现问→再睡，行为收敛、1h 尾巴仍不现形；**(b)** churn 停了 → 1h 尾巴首次出现（→ endSession → 休息 → 续场闸六道判据无配额 → restartSession → **本 change 重新现问 → 再睡**）。**两条都安全**，本项只为确认落到哪条、以及顺带观察那个 5 分钟重连 churn 本身是否是独立缺口（**其根因本 change 未查**）。

> **已知、已分析、本 change 故意不改的残留（勿在本簇当 bug 报）**：
> - **`sliding-window-counter.ts:38` 的 `quota <= 0 → retryAfterMs undefined`**：某窗口配额被配成 0（如被 frozen / restricted 压到 0，或运营显式配 0）时，休眠时长回落 `VIEW_QUOTA_RECHECK_FALLBACK_MS`(60s) 重判，日志标签可能把风控终态贴成「view 配额暂不可用」——**标签不诚实**，但**行为安全**（刹车在，60s 重判也只是空转日志、不唤醒浏览器）。本 change 打不到（FB 慢启动第 1 天 view 上界 `[10,20]` 非 0），留后续 change 单独处置。
> - **frozen / restricted 账号被贴成「view 配额休眠」**：同上，是 `explain()` 对风控终态不返回 `retryAfterMs` 的既有行为，非本 change 引入。tasks 4.1 记了可选的日志分流修法（纯日志、不改判定），本次未做。

## 簇 103 — 环境→账号绑定：灵感库/委托从「恒空谎报」回到诚实

### change `curated-envkey-account-binding` 真机验收（cloud `94f9909` 已 land + 推 origin/master + **已部署 dev**；edge `0b7ec72` 已推 master、**未出安装包**；登记于 2026-07-17）

**前置环境**：大白 XHS 环境（env_key `k1e0ero8`，真实账号 `63e2ff0500000000260049ce`，归属客户 `076f320b-84f3-42b6-b190-4b219d214094`）连 dev（`ws://121.89.85.150:8787`）+ **该客户在客户端登录 client-auth**；另需一个**从未连过云端的环境**（造 binding_unknown 态）与一个**当前停机的已绑定环境**（造 create-post 离线拒绝态）。灵感库是 XHS 面，**可搭任意 XHS 号真机 session 合验**。edge 空态文案改动需**本机跑 edge 桌面客户端**（`electron:dev` 或含 `0b7ec72` 的安装包）才看得见；未出包前 binding_unknown 会落进通用错误卡（响亮可见、不够自解释，非红线违反）。

> **改了什么（一句话）**：客户端拿「环境编号」（8 字符分身 id）去问「这个账号收集了什么」，云端却把它原样当「登录账号 id」（24-hex）查库——两个键空间零交集，于是灵感库**恒空**、成稿汇总**恒 0**、委托任务列表**恒空**、对正当所有者还回 403「环境不归你」。两端都诚实、合起来把「失败」谎报成「你没有数据」。现在云端把环境编号先经绑定解析翻成真实账号再查；解析不出**诚实回 409/503**（未绑定/争用/离线/缺表各自可区分），**绝不 200-空**。绑定来自每次成功握手（自报账号），不追溯——上线当日约 16/18 环境未绑，连一次云端即自愈。
>
> **dev 已 read-only 坐实（部署当日）**：account_id 列已自建；大白 env 归属客户 076f320b、accounts 有 63e2ff05… 行、无跨客户争用；当前绑定 NULL（大白重启后尚未 hello）→ 解析器诚实回 binding_unknown；端到端模拟四判据全过 → curated_content 读出 **159 行**。**唯一缺的是大白真机 hello 一次把绑定写进库**（下面 103.1）。

- [ ] 103.1 **（本簇最高价值 · 自愈落库 → 灵感库出数）** 让大白环境连上 dev 完成一次握手 → 查 `client_environments.account_id WHERE env_key='k1e0ero8'` 应为 `63e2ff0500000000260049ce`（自动写入，无需人工）→ 该客户端灵感库读出 **≈159 条**（对照基线：`account_id='63e2ff05…'` 159 行、`='k1e0ero8'` 0 行）。**改动前**：同一环境读出 0 条、UI 画「精选池还是空的」。
- [ ] 103.2 **（未绑定环境诚实态，非空池）** 一个从未连过云端的环境（该客户已归属、但未 hello 过）在灵感库被选中 → 客户端呈现**自解释的「还不知道这个环境上是谁 · 连一次云端即可」**状态（含 0b7ec72 的文案；未出包时落通用错误卡）→ **MUST NOT** 呈现「精选池还是空的 / 系统发现适合当前账号的内容后会出现在这里」。云端该请求回 `409 binding_unknown`（非 200-空）。
- [ ] 103.3 **（不可逆写离线诚实拒绝 · D5 活体佐证）** 一个已绑定但**当前停机**的环境发起「由精选内容开帖」→ 云端回 `409 binding_unverified`、**零委托任务落库**（查 `delegated_tasks` 无新行）；**同一环境的读（列表/详情/成稿数/委托列表）仍正常返回**（离线不影响读——活体前置绝不外溢到读路由）。
- [ ] 103.4 **（委托 / 成稿不再恒空）** 大白绑定落库后，该客户端**委托任务列表**与**成稿汇总（referenceDraftCount）**返回真实账号名下的真实数量/任务行，不再因键空间不匹配恒为 0/空；对正当所有者的任务确认/暂停/恢复/取消**不再回 403「环境不归你」**。
- [ ] 103.5 **（D5 安全闸 · 观测性，难主动造）** 若曾出现同一账号被绑在归属不同客户的两个环境上（跨客户争用），写侧拒写绑定 + 发 P1 告警（`alerts` 表 `type='env_binding_conflict'`），读侧对两边都 fail-closed 回 `409 binding_conflict`。**难以主动构造**（需两客户 + 同账号），主要靠观测 `alerts` 表是否误报——上线后若无该 type 告警即符合预期（正常运营下不该出现）。

> **运营预期（勿当 bug 报）**：部署当日约 16/18 环境是未绑定态，会从「精选池还是空的」变成「还不知道这个环境上是谁」。**这是本 change 的目的、不是回归**——前者是谎、后者是真话；每个环境连上云端一次即自愈，无需任何人工操作、无回填脚本。

### change `slow-start-offline-toggle` 真机验收（cloud `368228a` 已 land + 推 origin/master + **已部署 dev**；edge `3b4025e` 已推 master、**未出安装包**；登记于 2026-07-18）

**改了什么（一句话）**：账号级慢启动开关原来必须「浏览器在线」才能改——云端靠**活会话**反查账号身份，环境没起就回 `edge_offline`。原作者把这当「不是缺陷」（那张卡的用量数据离线本来也不更新）；用户明确推翻。现在慢启动改用 `curated-envkey-account-binding` 建的**持久绑定**解析账号，离线也能改：写入存库、真态纯云端算、界面如实标「已保存」；解析不出**诚实回 409 binding_unknown / 503**，绝不假成功。同时**删除了靠活会话反查的 `resolveAccountIdForEdge`**（其最后一个生产调用点归零；保留反方向的 `resolveEdgeIdForAccount`，命令定向本质需要在线）。

**前置环境**：与簇 103 共享（大白 XHS 环境连 dev + 该客户 client-auth 登录；另需一个**已停止/从未启动**的环境造 binding_unknown 态）。慢启动开关的客户端可见性改动需**本机跑 edge 桌面客户端**（`electron:dev` 或含 `3b4025e` 的安装包）才看得见。**建议与簇 103 同一次真机 session 合验**——同一套绑定、同一批环境。

> **dev 已 read-only 坐实**：`resolveAccountIdForEdge` 实装已从部署树删除（grep=0）、slow-start 路由已上（7 处）；cloud 全量 2465 pass、慢启动读写 28 例定向测试全绿（含 binding_unknown 不编造 / 查询失败 503 不降级 / 非所有者 403 不泄露）。**桩验不了、必须真机的**：环境停机时开关真的可点且写入生效、未绑定环境显示 binding_unknown 专属文案。

- [ ] 103.6 **（本簇最高价值 · 离线可改开关）** 一个**已停止**（非冷待机——冷待机今天本就能改）的已绑定环境，在客户端切慢启动开关 → 写入成功、界面显示「已保存」（不是假装「已生效」）；重连该环境后真态一致。**改动前**：环境没起时开关回 `edge_offline`、点不动。
...
**改了什么（一句话）**：远程协助链原本只有鼠标——运营遇到「模糊数字图片类字符识别码」在协助页上无处键入、结构性无解。现在协助页多了一个答案输入框：点中截图上的输入框（1 个落点）→ 键入答案 → 回车提交，边缘用**真实键盘事件**逐字拟人输入。回执带诚实取证（`typeReport`），让运营 MUST 能区分「答案打错了」与「字根本没打进去」。顺带修了点击链路 4 个既存诚实缺陷、移除了背后无任何能力的「远程桌面」假入口。

**前置环境（关键：需 edge 出包 + 换装）**：一台运营机跑**含 `6a48d86` 的新 edge 安装包**（能力闸 fail-closed ⇒ **未换装的旧包对该功能一直是 409、暗的**，这本身是 104.6 要验的一项）+ 一个能真实弹出**模糊数字图片类字符识别码**的平台场景（XHS 触发验证码浮层最常见）；客户端连 dev（`ws://121.89.85.150:8787`，cloud master `04321be` 已部署、console 8088 已部署）。协助页从**飞书卡链接**点开（scoped token 授权，刻意在 console 登录门之外）。**这一批与既有验证码协助真机项（点击/轨迹/实时帧）共享同一环境**，建议同一次真机 session 合验。

> **本地/桩已坐实**：三仓单测全绿（edge 键盘原语 + handleClick 键入 7 例 / cloud 校验闸 + 版本偏斜 / console 页面 4 例），AC-PROTO-18 两端逐字段往返、真实键事件数==字符数且零 `Input.insertText`。**桩验不了、必须真机的是**：真实平台验证码上真的解开、焦点被 canvas/iframe 抢走时的表现、Enter 导航后不误报、以及能力闸 fail-closed 的换装门槛。

- [ ] 104.1 **（本簇最高价值 · 验证码真被解开）** 模糊数字图片类字符识别码 → 点中输入框 + 键入正确答案 + 回车 → `typeReport.focus='editable'` + `verified='match'` + `submitted=true`，settle 后 fresh 复检无遮罩 ⇒ `cleared` + 边缘发 `risk.captcha_cleared`、账号从 `restricted` 恢复。**验证码真被解开、账号真被放生**。
- [ ] 104.2 **（焦点不可读的诚实态）** 焦点被 canvas / iframe 抢走（厂商挂 tabindex 的容器）⇒ `typeReport.focus='opaque'` + `verified='unverifiable'`，协助页呈现「焦点在跨源/不可读元素内，无法证明字符已落入；请对照新画面确认」，**MUST NOT** 谎称成功或失败。观测 `focusTag` 是否足以事后判别打进了哪个元素。
- [ ] 104.3 **（点空了 ≠ 坐标越界）** 点在输入框之外（焦点未落定）⇒ `no_target` / `focus_not_landed`、`typed=0`、**绝不提交**；协助页显示「那一点没点到输入框」（不是坐标越界那套词）。
- [ ] 104.4 **（打字期换图）** 键入过程中挑战自刷新换新题（实时帧推进）⇒ 协助页仍冻结在被 pin 的旧帧、出「画面已更新」提示；提交用的是被 pin 帧的 snapshotId（边缘按该帧 crop 落点）。
- [ ] 104.5 **（Enter 提交导航后不被误报 failed）** 文本验证码成功的正常形态是 POST/导航 ⇒ Enter 后 `probeBlockingKind` 可能连抛，边缘有界重试（4 次/500ms）仍取不到判据时回 `verdict_unavailable_after_submit`（**不是** `click_failed`）+ `submitted:true` + 尽力回带新帧。**这是 task 0 修的既存缺陷在键入窗口下的常发形态**——真机必须确认解对的验证码不再被报成失败。
- [ ] 104.6 **（能力闸 fail-closed 的换装门槛）** 一台**未换装新包**的运营机上触发同类验证码 → 运营在协助页键入答案提交 ⇒ 云端 `409 edge_lacks_text_capability`、协助页显示人话「该机器客户端版本过旧，不支持远程输入」（**不裸打英文 reason**）、**命令未下发**（边缘无任何键入动作）。换装含 `6a48d86` 的新包后同一操作放行。
- [ ] 104.7 **（授权面不变 · 飞书卡直接键入）** 从**飞书卡链接**直接点开协助页（**未登录控制台**）即可键入并解开验证码——凭 URL scoped token 授权，无需先登录 console、无需挂 Bearer。这条链是唯一的远程处置路径（远程桌面已确认不存在），掐它没有退路。
- [ ] 104.8 **（无远程桌面入口 · 移除核对）** 确认协助页与飞书卡**已无任何「远程桌面 / 远程地址」入口**（console 按钮删、飞书卡行删、hello `remoteAddr` 字段删）；不可远程协助时飞书卡只发 notify-only 提示、不暗示存在另一条通道。
- [ ] 104.9 **（合成击键节奏残余风险 · 观测性）** GeeTest 等厂商自述持续采集 keystroke timings；lognormal + 每机偏置 + RTT 补偿做不到 OS 级不可分辨。**明知的残余风险**，seam 已在协议里钉好「采集运营真实击键时序」形状，作为后续分期。真机观测是否有键入类验证码因节奏被判异常的迹象。

## 簇 105 — 客户端浏览器停机时仍可发起洗稿

### change `client-rewrite-offline-start` 真机验收（cloud `5c17e3e` 已 land + 推 origin/master；登记于 2026-07-18）

**改了什么（一句话）**：精选内容洗稿入口不再把“浏览器在线”当作创建前置；云端改用已持久化的环境账号绑定创建固定 `review` 任务，先生成并落库候审稿。真正的平台发布仍需审批与活边缘定向，离线可洗稿不等于离线可发布。

> **本地已坐实**：client-auth 聚焦测试 28/28、acceptance 56/56、cloud 全量 2473 pass / 8 gated skip / 0 fail、typecheck 0；回归同时证明通用发布类委托在浏览器离线时仍回 `binding_unverified` 且不新增任务。**桩验不了、必须真机的是**：实际桌面客户端在所选环境浏览器未启动时能收到入队与候审稿真态，以及批准后离线发布闸仍不误报成功。

- [ ] 105.1 **（本簇最高价值 · 浏览器停机仍能洗稿）** 选择一个已至少 hello 过一次、持久绑定有效但当前浏览器已停止的 XHS 环境，在客户端精选内容页发起洗稿 → 请求不再显示「暂时无法确认这个环境当前登录的账号」；云端为该绑定账号创建且仅创建一条 `review` 任务，浏览器不被自动启动。
- [ ] 105.2 **（候审真态，不冒充发布）** 上述离线发起的任务完成云端生成后，客户端收到洗稿成品并进入 `pending_approval` 审核页；标题、正文、话题与配图可见，但界面 MUST NOT 显示平台已发布。
- [ ] 105.3 **（最终下发仍需在线）** 保持浏览器离线时批准该候审稿 → 云端诚实等待或失败，平台零副作用、不得广播到其它边缘、不得把审批成功表述为发布成功；随后启动正确环境并按既有重试/下发机制验证可继续兑现。
- [ ] 105.4 **（绑定失败仍 fail-closed）** 对从未 hello 的 `binding_unknown` 环境和跨客户争用的 `binding_conflict` 环境分别尝试洗稿 → 两者按各自原因拒绝、零任务落库，MUST NOT 从客户端参数或其它环境猜账号。
...
> **本地已坐实**：Edge acceptance 25/25、全量 1855/1855、typecheck 通过；Cloud 全量 2541 通过（8 个 gated skip）、typecheck 通过；无槽位启动、严格 welcome、可信 env→account 引导、browser-absent page-command 失败回执、第五项失败后立即放行下一 FIFO 项均有自动化回归。**当前 `/Applications/AIDCP.app` 仍是旧包**，所以本次真机观察到的“只有 4 个核心/假绿连接”不能写成已由新代码复验通过。

- [ ] 106.1 **（本簇最高价值 · 16 环境控制面在线）** 安装包含本 change 的新 Edge 包后，在客户端选中 16 个已归属且已绑定环境执行“全部启动”；即使浏览器并发设为 5，槽位外环境也应各有有效非空 `sessionId`，Cloud runtime 可查到对应 edge/account，客户端 Cloud 状态为已连接而浏览器状态独立显示排队/待机。
- [ ] 106.2 **（槽位外人设真态可见）** 对槽位外的已绑定人设账号确认 `personaBound=true` 可到达客户端并显示“已设置”；对权威未绑定账号显示“未设置”，握手/引导尚未成立时必须保持未知，不能因本地超时误弹生成人设。
- [ ] 106.3 **（5 槽位与启动失败继续调度）** 固定浏览器上限 5，令第 5 个 AdsPower 分身被另一窗口/设备占用：该环境明确显示占用失败且不重启轰炸，FIFO 下一环境立即取得空槽启动；最终运行/排队/失败数与 16 个启动请求可核对，不再停在“只有 4 个且其余无回复”。
- [ ] 106.4 **（任务唤醒与终局回执）** 向一个 Cloud 在线、浏览器缺席环境派页面任务：取得槽位时先启动 AdsPower、复核真实账号、必要时用真实身份重握手，再授予任务租约；死线内无槽位时回可恢复 `browser_wake_failed`，误达页面命令回 `browser_absent_wake_requested`，不得按 edge offline 或无回复处理。
- [ ] 106.5 **（错误握手不再假绿）** 在 dev 构造 hello `error` 和缺 `sessionId/serverVersion` 的畸形 welcome：客户端显示可诊断连接失败，Cloud/Edge 日志可按 edgeId/accountId 对上；不得再出现 `sessionId=?`、绿色“已连接云端”或已连接后无 runtime 的状态。
- [ ] 106.6 **（换号复核）** 让持久绑定仍指向账号 A，但槽位唤醒后的浏览器实际登录账号 B：任何页面读写前必须关闭 A 会话、以 B 获得有效 welcome；若身份不可读或重握手失败，浏览器归还槽位并保持控制面可诊断，不得在 A 会话下操作 B 页面。

## 簇 106b — 让位真的发生，且每一次拒绝都留痕 —— **已清账（2026-08-06）**

> **清账依据（用户 2026-08-06 确认）**：客户端已用**含本修复的源码重新编译**并装机，用真账号跑过，未见异常。
> 注意两条口径，别被版本号误导：
> - **版本号未抬**——重编译时没跑 `npm version patch`，装机客户端仍标 `0.3.26`，
>   但它已不是之前 tag 出去的那个 `0.3.26`。**光看版本号认不出装的是哪份代码。**
> - **106b.3 / 106b.4 是刻意构造的失败路径**（任务租约在跑时被要求让位 / 已 detach 但杀浏览器未确认），
>   本次未逐条构造，随「真机运行未见异常」一并清账，**不等于已正面复现**。

### change `admit-browser-standby-on-live-facts` 真机验收（Edge `00fda89`；登记于 2026-08-05，清账于 2026-08-06）

**改了什么（一句话）**：冷待机准入不再看两个由核心中文日志推断、且在 Native 引擎平台上已无写入方的生命周期标签，改看当场可验证的活事实；每一次拒绝让位都记具名原因与连续次数；进入待机被拒不再复用「关闭失败」回执，因而不会再把一个健康环境打成永久占槽的暂停态。

> **本地已坐实**：Edge acceptance 39/39、全量 3167（3166 pass / 1 gated skip）、typecheck 通过；含一条定向回归（两个推断轴均为陈旧值时仍让位）与一次变异自查（把该判据加回去，恰好由该定向回归 + 结构断言两条抓住）。**触发本次登记的真机现象**：2026-08-05 dev 车队，账号 Lê Thảo Hân（`ads-k1fd38qw`）小时浏览配额跑满后连续 32 分钟拒绝让位、锁死两个槽位之一，界面与日志零痕迹。**登记当时 `/Applications/AIDCP.app` 仍是不含本修复的 0.3.26 旧包**；2026-08-06 已换成含本修复的重新编译版本（版本号仍标 0.3.26，见本簇顶部清账依据）。

- [x] 106b.1 **（本簇最高价值 · 配额跑满后真的让位）** 让一个 Facebook 账号跑到小时浏览配额上限、云端待机提示转为 `eligible=true` 且剩余等待远超 5 分钟门槛：该环境应在 **5 分钟内**关闭浏览器进入冷待机，槽位被等槽位队列中的账号取走。这正是本次故障的正面复现。
- [x] 106b.2 **（拒绝可见）** 在开发者详情里确认「浏览器让位」与「让位诊断」两行：能读到当前模式、具名拒绝原因、连续拒绝次数与已持续时长；单次拒绝与连续拒绝在界面上可区分。
- [x] 106b.3 **（任务在跑时拒绝让位 ≠ 变砖）** 让某环境正在执行发布或评论任务（持有任务租约）时收到待机提示：核心应回 `task_lease_active`，该环境**保持运行态、浏览器保持开启、自动化意图不变**，不得出现暂停/停止；下一条待机提示到达时能重新判定并正常让位。
- [x] 106b.4 **（真失败仍如实呈现）** 制造一次「已 detach 但杀浏览器未确认」（`browser_close_unconfirmed`）：环境应显示浏览器真态未确认并挂持久失败卡片，但**同样不得**改运营的自动化意图。
- [x] 106b.5 **（日志可回溯）** 在原始日志里确认 `[cold-standby] 拒绝让出浏览器槽位 reason=… streak=… for=…s` 按首次 + 每 5 次的节流出现；一个卡住半小时的环境必须能只凭日志被认出来。

## 簇 107 — 发布任务等待本地浏览器槽位

### change `publish-waits-for-browser-slot` 真机验收（源码验证完成，Edge **未出安装包**；登记于 2026-07-19）

**改了什么（一句话）**：已批准发布到达一台浏览器并发已满的机器时，不再作废授权要求人工重批；草稿保持待发布，由现有扫描自动重试。本地任务全部收敛后，Edge 只请求 Electron 按现有安全闸重新评估待机，绝不直接关闭浏览器。

> **本地已坐实**：Cloud 聚焦发布测试 24/24、acceptance 59/59、全量测试退出码 0、typecheck 通过；Edge 聚焦调度/生命周期测试 36/36、acceptance 25/25、全量 1865/1865、typecheck 通过。**尚未做真实平台发布**，以下 6 环境 / 5 槽位场景必须在含本 change 的 Edge 构建上验收。

- [ ] 107.1 **（满并发时发布入队）** 同机启动 6 个 Facebook 环境、浏览器上限 5；向第 6 个已批准草稿下发发布。Cloud 应保留 `pending_approval` 与授权，只发一次“等待本机槽位”通知，不下发任何发布页面命令、不计熔断、不要求重新批准。
- [ ] 107.2 **（槽位释放后自动兑现）** 让任一在跑环境结束并按既有待机安全闸归还浏览器槽位；第 6 个环境按本机既有 FIFO 获得槽位并执行发布，且最多只有一个发布序列实际提交。
- [ ] 107.3 **（任务结束后的安全待机）** 发布或其他独占任务结束时若仍有最新待机提示，Electron 重新走最短驻留、验证码、认证、新任务和槽位等既有门禁；不满足时不得关浏览器，满足时可回收并放行下一环境。
- [ ] 107.4 **（负向语义不混淆）** 分别制造 edge 离线、CDP 不健康、普通 acquire 超时和已开始后的发布失败，确认它们仍走原有作废授权/失败/熔断语义，不能被误归类为“浏览器槽位等待”。

## 簇 108 — 视频号鉴权：可复用会话不启浏览器，profile 占用不再卡“鉴权中”

### change `wechat-browser-profile-occupied-status` 真机验收（源码已实现并自动化验证；Edge 未出安装包，登记于 2026-07-19）

**前置环境**：本机运行包含本 change 的 Edge 源码（可用 `electron:dev`，无需先出包）+ 一个已有加密视频号会话的 AdsPower profile + 可在另一 AdsPower 客户端/设备中制造同 profile 占用。Cloud 必须先部署支持 `INTERACTION_BROWSER_PROFILE_IN_USE` 的版本，避免旧 Cloud 严格枚举拒绝新状态。

> **本地已坐实**：provider 窄匹配与脱敏、有效加密会话零浏览器启动、占用后 `reauth_required + unavailable`、历史保留/写入暂停、显式重试与 accepted 不冒充 active 均有自动化覆盖；合同 schema/fixture 也已验证。**尚未拿真实 AdsPower 安全锁响应在新代码上复验，不能写成真机已恢复。**

- [ ] 108.1 **（API-only 快路径）** profile 未被占用且本地加密会话仍有效时启动环境：身份校验和已启用读取探针通过后直接显示“鉴权通过”，AdsPower 浏览器不应被打开，日志包含 `stored_session=valid browser=skipped mode=api_only`。
- [ ] 108.2 **（占用终态与隐私）** 先让已保存会话失效，再由另一设备/账号占用同一 profile 后启动：客户端必须从“鉴权中”收敛为“视频号：浏览器被占用”，浏览器态为不可用，历史互动仍可查看、写操作不可执行；Cloud/API/界面与普通日志均不得出现原始占用邮箱，本机诊断日志最多出现掩码。
- [ ] 108.3 **（解除占用后显式恢复）** 释放 profile 后点击“重试打开浏览器”：按钮受理时仍不得显示鉴权成功；只有浏览器登录/身份/读取探针全部通过、后续状态读回 `active` 后才显示“鉴权通过”，随后回到 `closed + API-only`。
- [ ] 108.4 **（无自动抢占与轰炸）** 保持 profile 被占用观察至少 5 分钟：Edge 不调用强制关闭/删除、不高频循环 `browser-profile/start`，槽位调度继续处理其他环境。

## 簇 109 — 客户端核心与浏览器执行器彻底解耦

### change `separate-client-core-browser-executor` 真机验收（Cloud `f57bfb7` 已 land + 推 origin/master + **已部署 dev**；Edge `857d332` 已 land + 推 origin/master、**未出安装包**；登记于 2026-07-20）

**改了什么（一句话）**：客户登录后，每个可信环境先建立只负责身份、Cloud 控制和 API-only 业务的常驻客户端核心；浏览器只在登录恢复或页面自动化时按槽位取得。人设、稿件删图、批准/驳回、互动工作区、换 Cloud 和视频号离场清理不再以浏览器、CDP、槽位或普通 Edge WS 在线为前置。

> **本地与 dev 已坐实**：Edge 全量 1961/1961、Cloud 全量 2650 pass / 8 gated skip、两端 acceptance 与 typecheck 全绿。macOS arm64 上 12 个浏览器缺席核心全部完成 hello，浏览器进程启动数 0，RSS 平均 141.5 MiB / 最大 150.9 MiB / 总计 1698.3 MiB，均低于 160 / 180 / 1920 MiB 阈值。Cloud 已在 dev 应用加法 migration `0049`，四个 `cleanup_grant_*` 列、约束和索引就位；服务、8787/8090/8091、两条 health、PostgreSQL、飞书长连接均正常，`isales-api` / `isales-scheduler` 未受影响；备份为 `/opt/aidcp/cloud.bak.20260720-052714Z.tar.gz` 与对应 `.env.bak`。**当前未构建或换装 Edge 客户端**，所以下列真实 AdsPower、页面动作和离场清理场景只登记，不冒充已跑。

- [ ] 109.1 **（本簇最高价值 · 登录后全核心在线且浏览器全关）** 在含 Edge `857d332` 的 `electron:dev` 或后续安装包中登录客户，选择支持上限内的全部已归属可信环境：无需点“开始自动化”，每行 `coreState=online`、`cloudState=connected`，浏览器保持 `closed`，AdsPower 服务未被核心 bootstrap 隐式启动，浏览器槽位占用仍为 0。
- [ ] 109.2 **（浏览器关闭仍可办 Cloud 业务）** 保持目标环境浏览器关闭，依次读取/生成/确认人设、删除候审稿一张配图、批准一稿、驳回一稿并读取互动工作区：全部只走客户鉴权窄接口，renderer 不获得 token/accountId；批准只显示“已受理/待执行”，平台零页面副作用且浏览器不被唤醒。
- [ ] 109.3 **（页面任务才申请执行器）** 向同一关闭浏览器的环境派页面自动化：先显示页面任务排队，再依次经过槽位/内存、provider、CDP、真实页面账号复核和页面租约；身份不匹配或 CDP 附着失败时回具名执行器失败，核心与 Cloud 仍在线，local/cloud/platform_api 操作仍可用。
...
## 簇 113 — 物理拆库写侧解耦：离场、授权、配置、账号守卫这四条链路还成不成立

**共享环境**：dev（`121.89.85.150`）+ 一个可解绑/改派的客户环境 + 一个 Facebook 环境（加群守卫）。
cloud master `7b316ce` 已部署 dev。**建议与簇 112 同一次真机 session 合验**——都在客户环境/离场这套数据上。

**改了什么（一句话）**：以前「解绑一个环境」这件事，是后台在**一笔数据库事务**里既改自己的账，又顺手去改
互动那边的账；现在改成后台只写自己的准入记录，互动那边由它自己收到通知后再落自己的账。数据同一份，
但两件事不再同一瞬间完成——中间有个「已受理、还没落地」的短暂状态，界面上应当**如实显示**，绝不能显示成
「查无此事」。

> **已坐实（不必重验）**：dev 真实数据上**存量认领**按设计生效——4 条终态离场台账全部在 api 域补出了准入行
> 并正确标为已物化。全量测试 3312·0·10 skip；两道机械门禁（跨属主行锁 / 跨属主写）均读到 0。

- [ ] 113.1 **（本簇最高价值 · 中间态是诚实的）** 解绑一个环境后**立刻**用客户 API 轮询 `GET /offboarding/:id`：
  MUST 返回 `state: 'accepted'`（已受理、尚未物化）且 `accountId` / `purgeDueAt` 为 **null**，
  **MUST NOT 返回 404**。等一次中继（秒级）后 MUST 变成真实台账视图。
  ⚠️ 这是本批**唯一新增的对外状态词**，客户端若把非枚举值当异常处理会在这里暴露。
- [ ] 113.2 **（准入闸真的挡住改派）** 一个正在清理的环境，管理员改派归属 MUST 被拒，且原因码要能区分
  「已受理未物化」与「已物化未清除」两种（由 `materialized_at` 决定）。
- [ ] 113.3 **（丢投兜底）** 人为让属主侧不可达（或直接不让中继跑），确认：准入行**不消失**，
  60s 对账循环重放后仍能物化；期间改派仍被拒（**只多拒不误放**）。
- [ ] 113.4 **（授权首写闸没被改松）** 边缘上报一次环境登录态：正常情况写入成功；
  把该环境置为「归属已撤销」后再报 → MUST 被拒且错误码/文案与改造前逐字一致。
  再**人为让 api 网关不可达** → MUST 响亮失败，**MUST NOT 因问不到就放行**。
- [ ] 113.5 **（回执有效期这条新拒绝）** 网关回执 10s 过期是新增的拒绝态（`auth_receipt_expired`，409，可重试）。
  正常路径下不该出现；若真机上频繁出现，说明 `connect()+BEGIN` 慢于 10s，那是**连接池压力**的信号，要查而不是调大。
- [ ] 113.6 **（配置改完另一进程多久看见）** 改一次限频/节奏/会话/恢复配置 → 面板与运行时读到新值的延迟
  MUST 在 **8 秒**内（中继兜底 2s + 刷新轮询 5s）。改完立刻看 = 可能还是旧值，**这是预期**，不是 bug。
- [ ] 113.7 **（账号守卫投影：陈旧就拒，不放行）** 停掉投影刷新（或把刷新周期调到很大）等它过期 →
  Facebook 加群与委托任务认领 MUST **停摆并响亮报错**（`account_projection_stale` 之类具名原因），
  **MUST NOT** 因为投影没跟上就当成「账号不存在」静默跳过，也 MUST NOT 放行。
  恢复刷新后 MUST 自动恢复。
- [ ] 113.8 **（分组标签滞后窗口）** 改一个账号的分组标签后立刻加群：写路径会**强制刷一次投影**再判，
  所以 MUST 当场生效；而三处纯展示面（facets / 进度 / 计数）允许滞后一个刷新周期（默认 30s）。
- [ ] 113.9 **（互动配置审计不再丢）** 触发几次会写互动审计的操作，确认审计条目最终都落库
  （现在是经 outbox 异步落，≤2s），且**时间戳是业务发生时间**、不是中继落库时间。
- [ ] 113.10 **（离场清理盲删的收窄方向）** 造一个「账号已改派到别的环境」的离场记录，等它到期清理 →
  MUST **只删环境维**的数据，账号级的运行控制与回复配置 **MUST 保留**，且审计里要有一条记明
  「本次按 env_only 收窄执行」。⚠️ 这条是 2026-08-14 那个硬期限的正面验收。
- [ ] 113.11 **（两台机器同扫一批行不出事）** dev 与 ol 的定时器扫同一批离场行。人为让两端同时到期，
  确认只有一台真正推进（计数 / 审计 / 绑定行删除都只发生一次），另一台**什么都不做也不报错**。

## 簇 114 — Facebook 规则模式两级节奏：真机跑满一个完整周期

> change `facebook-rule-mode-two-tier-cadence`（2026-07-28 已部署 dev：cloud `11e53ef` / console `0fae387`）。
> 节奏由「10 条浏览 → 1 批次含点赞+加群联系评论」改为「5 条浏览 → 1 轮次点赞；每 2 轮 → 1 次加群联系评论」。
> 加群频率不变（仍是每 10 条浏览一次），点赞翻倍。桩层已覆盖状态机与终态形状，**下面这些只有真账号真浏览才能验**。
> 前置：某个已开启规则模式的 Facebook 账号（dev 上有 21 个）在其活跃时段联机浏览。

- [ ] 114.1 **（一级节奏真的是 5 条）** 观察一个账号：第 5 条确认浏览当刻开出轮次并下发点赞。
  第 1–4 条 MUST NOT 触发任何动作，后台进度条走 `浏览 1/5 … 4/5`。
- [ ] 114.2 **（第 1 轮只点赞、且真的收尾）** 第 1 轮点赞回执落定后，MUST NOT 出现任何加群或评论命令；
  后台该轮显示「加群：本轮不适用 / 联系评论：本轮不适用」（中性色，**不是**待处理/未启动/失败）。
  紧接着账号 MUST 继续浏览并重新开始计数——**这条是活锁的正面验收**：若浏览停住不动，就是轮次没终结。
- [ ] 114.3 **（第 2 轮才加群）** 再攒满 5 条 → 第 2 轮点赞终态后 MUST 触发一次加群 + 联系评论，
  与改造前的行为逐字相同（选群、正文来源、审批、联系方式注入都不变）。
- [ ] 114.4 **（点赞被压制不连带停掉加群）** 人为让点赞日配额耗尽，再让账号跑满两轮：
  点赞如实记 `risk_suppressed`，但第 2 轮 MUST 照常加群——二级节奏按轮次序号推进、不按成功点赞数。
  这条若不成立，配额一满就会静默停掉全部加群与联系评论。
- [ ] 114.5 **（点赞原因不被终结抹掉）** 上一条的第 1 轮（只点赞、且被压制）终结后，
  后台 MUST 仍能读到 `daily_like_quota` 这类具体原因，**MUST NOT** 变成空白或被换成节奏相关文案。
- [ ] 114.6 **（存量账号无缝续跑）** 21 个存量账号的开关在迁移后仍为开启，进度从 `0/5` 重新收集
  （旧定义下的 11 行进度、418 条去重事实、38 个批次作为历史留存、新定义查不到）。
  后台 MUST NOT 出现「规则定义不一致」红标——出现即说明回填没覆盖到该账号。
- [ ] 114.7 **（点赞密度确实翻倍但没触顶）** 跑满数轮后核对当日点赞数：约为改造前的 2 倍，
  且未触发点赞比例闸（0.35）或日配额；加群次数 MUST 与改造前持平（每 10 条浏览一次）。
- [ ] 114.8 **（OL 侧不受影响）** OL 未部署本次改动且规则模式从未在 OL 跑过（实测运行时行 ol 侧 0 行）。
  确认 OL 行为无变化。⚠️ 注意 `facebook_rule_mode_config` **没有 execution_target 且两侧共库**，
  回填已把共享的 22 行改成新定义；OL 上的旧代码不读库中定义值，故不受影响，但 **OL 下次部署会自动切到新节奏**。

## 簇 115 — Facebook 规则模式缺联系方式降级：真的入群、真的发出、且没有一处说谎

> change `facebook-rule-comment-plain-fallback`（2026-07-28 已部署 dev：cloud `bf7dd24` / console `ca5ca1f`）。
> **本次具名放弃了两条已上线的 fail-closed 保证**，范围仅限 Facebook 规则模式的加群联系评论一条链路。
> 桩层已覆盖状态机、审批车道、回执话术与范围锁；**下面这些只有真账号真发才能验**。
> 前置：dev 上开着规则模式的 21 个账号**全部**没配联系方式，所以随便哪个跑起来都会走降级。

- [ ] 115.1 **（真的入群了）** 让一个开着规则模式、没配联系方式的账号跑到含加群的那一轮：
  MUST 真实下发加群并入群。⚠️ 这是本次**新增**的平台写动作——此前这类账号连加群命令都没发过，
  闸在加群之前就把整条链停住了。
- [ ] 115.2 **（真的发出了普通评论）** 入群成功后 MUST 在该群发出一条**不带联系方式**的评论；
  正文来自账号模板或群区域模板。若该账号两级模板都取不到，MUST 以具名原因诚实停止——
  **停止是合格结果**，不是失败；不合格的是「凭空编一段」或「回落生成式」。
- [ ] 115.3 **（三张卡不许自相矛盾）** 同一次动作看三处：人审卡（正文不带码）、触发回执、合并结果卡。
  三处 MUST 一致地表明本次**不带**联系方式，且结果卡要说清是降级。
  **MUST NOT** 出现「已发出一条带联系方式的评论（服务器已确认）」——那正是本次要修掉的那句谎。
- [ ] 115.4 **（授权没有外溢）** 把某账号设成「联系评论免审 + 普通评论需人审」再跑一轮：
  降级产出 MUST 走人审。若它被免审直接发出，就是把「对带码模板评论的免审」外溢到了一条
  从未为该车道授权的正文。
- [ ] 115.5 **（后台不冒充联系评论已完成）** 降级发出后，规则轮次的评论段 MUST 显示
  「已发出（未带联系方式）」（青色），**MUST NOT** 显示成绿色的「已确认」。
- [ ] 115.6 **（开关侧提示是真态）** 后台规则列对这 21 个账号 MUST 显示「未配联系方式：加群评论将降级为普通评论」；
  给其中一个补上联系方式后，该提示 MUST 消失且下一轮恢复为真·联系评论。
- [ ] 115.7 **（其余五个入口一个都没松）** 对同一个没配联系方式的账号发飞书 `/comment <昵称> --contact`：
  MUST 仍然 fail-closed、不加群不评论。内容排期 / 精选定向 / 引流热帖 / 委托任务同理。
  这条是本次红线放弃的**范围边界**，比降级本身更该盯。
- [ ] 115.8 **（加群配额的真实消耗）** 记录降级跑起来后 `join_group` 当日配额的实际消耗曲线。
  注意记账判据是「点到了」而非「整体成功」——评论最终失败也会占掉额度。
- [ ] 115.9 **（原生引擎路径同样正确）** 若该账号走原生页面引擎路径，同样核一遍 115.2/115.3。
  代码核验已确认两条路径对「没有联系方式」的处理结构一致（云端此时根本不下发该字段，
  走的就是普通评论那条老路），但仍 MUST NOT 以 JS 路径通过代表原生路径通过。

## 簇 116 — Facebook 首帖评论确认：真机上第一次报出「已评论」

> change `facebook-first-post-comment-confirmation`（2026-07-28：cloud `5b61f81` 已部署 dev；
> edge `3207561` 已合主干、**未出安装包**）。**桩层已覆盖判据本身，但本簇的前置无法绕开：**
> edge 改动不重新打包就到不了运营机，届时报出的仍是修好之前的旧行为。
>
> 已在真机坐实的事实（2026-07-28，账号 `61591934100810` 三轮加群+评论）：加群正常，评论**真的上墙**
> （FB 活动日志 + 三条刷新后仍在的 `comment_id`），系统却因四条判据全落空恒报 `verification_ambiguous`，
> 并据此打去重、烧掉目标帖。以下是修复后要复核的那一面。
> 前置：**出一版含 edge `3207561` 的安装包并在运营机升级**；一个开着规则模式的 FB 账号联机跑到加群那一轮。
>
> **⚠️ 该 change 归档时另留 5 条「已定位、本次不修」的代码项**（绑定证据 5.774s 漂移 / 同群多个加群按钮 /
> 可信点击前未把目标滚进视口 / `verification_ambiguous` 一名两义 / 租约丢弃命令无回执）——
> 逐条取证与判据见 `openspec/changes/archive/2026-07-28-facebook-first-post-comment-confirmation/design.md` §5，
> tasks.md §5 留有清单。**它们不是真机项、是待起 change 的修复候选**，别在本簇里当验收条跑。

- [ ] 116.1 **（终于能报成功）** 规则模式自动加群 + 首帖评论跑一次：`facebook_comment_audit` MUST 出现
  非 `verification_ambiguous` 的确认态。对照 FB 活动日志确认确实上墙——**两边都要看**，
  这条修的就是「墙上有、系统说没有」。
- [ ] 116.2 **（确认时延够用）** 记录回车到确认的实际耗时。真机实测服务器 id 在 **4.29s** 出现，
  现行确认窗口 9s；若真机常态超过 6s，说明窗口需要重新定尺寸。
- [ ] 116.3 **（数字 id 与占位 id 都判对）** 抓一次 live DOM 佐证：确认命中的是纯数字 id
  （如 `1531497545657803`），而回车瞬间的 `client:<uuid>` 占位 **MUST NOT** 判成功。
- [ ] 116.4 **（本人身份认得出群内链接）** 确认判据命中的作者链接是 `/groups/<gid>/user/<uid>/` 形态。
  这条是本次补的最根本一环——不补则后面两条判据根本没机会跑。
- [ ] 116.5 **（在飞不冒充成功）** 若抓到过渡态「发布中 / đang đăng」，该刻 MUST NOT 判成功。
- [ ] 116.6 **（水合期不再误判到底）** 观察群页刚打开、feed 尚未水合那几秒：
  滚动回执 MUST NOT 恒报 `no_target`，也 MUST NOT 从第一次探测就判「feed 已到底」。
- [ ] 116.7 **（去重不再白烧目标帖）** 确认态出现后核对：目标帖按「真的评论过」记去重，
  当日配额与群覆盖冷却都如实计入——此前 ambiguous 会打去重却不计配额、不落冷却。

## 簇 117 — Facebook 评论模板分块：整段广告作为一条发出去

> change `facebook-comment-template-blocks`（2026-07-28 已部署 dev：cloud `14263aa` / console `a95dd82`）。
> 分隔符由换行改为单独一行 `------`；运营手写模板不再走内容政策校验（用户定案）。
> **前置数据动作未做**：dev 上 5 条区域配置（全国 / 北宁 / 北宁北江 / 北江 / 河南）仍是同一段广告的
> **13 段碎片**，不先合并就还会随机发单行——本簇第一条就是它。

- [ ] 117.1 **（先把碎片合并）** 在后台把整段广告重新粘一次保存（新分隔符下即为一条），
  或授权直接改库合并。保存后回填 MUST 仍是整段、不被拆行。
- [ ] 117.2 **（发出去的是整段）** 触发一次模板评论：`facebook_comment_audit` 的 `text_length`
  MUST 接近整段长度（约 431），而不是单行长度（此前是 26）。
- [ ] 117.3 **（换行真的进了评论）** 在 FB 上肉眼看那条评论：MUST 保留分行排版，
  MUST NOT 被回车提前提交成半条。真机已实证 431 字符 / 12 个换行可完整输入（约 17.6s），本条是生产复核。
- [ ] 117.4 **（带电话的模板不再被拒）** 自带 `📞 Liên hệ:` 的模板 MUST 正常发出，
  不再出现 `compose_skipped / contains_contact`。
- [ ] 117.5 **（生成式那条路没被带松）** 让一个 `generated` 模式账号跑一次，正文若含联系方式/链接
...
## 簇 119 — 加群后首帖评论：慢群页跑通，且没有账号被钉死在空首页

> 来源 change：`restore-facebook-post-join-comment-continuity`（2026-07-28）。
> 事故样本：dev 环境 `ads-k1f44fo4`（Lu Mu）2026-07-28 11:23–11:25 加群成功但首帖没评论，
> 随后零命令悬停 60s 直到冷待机重启；同机同分钟对照账号走通全程。
> **云端已部署 dev（816eb5e）；边缘（d6d60c7）须重打桌面客户端包才在运营机生效**，
> 本簇的边缘相关项在出包前无法验。

- [ ] 119.1 **（四轮下滚真的发生）** 取一个「文档本身不滚、滚动条在 feed 祖先容器」的越南语群页：
  加群后的首帖探测 MUST 真的往下滚，位移 MUST NOT 恒为 0、到底判据 MUST NOT 恒真。
  修复前的形状是「第一轮就判已到底 → 四轮预算只跑一轮 → 首帖找不到即放弃」。
- [ ] 119.2 **（放宽后的窗口够用）** 在慢群页（越南代理，就绪耗时 ≥10s 的那类）跑一次加群后首帖评论：
  身份回读 MUST 在 20s 内完成、评论框绑定 MUST 在 12s 内完成，且 MUST NOT 出现外层
  （边端 90s 原子上限 / 云端 105s 开帖步）先到点把具名失败改判成 timeout。
- [ ] 119.3 **（按 URL 开帖没有被顺带放宽）** 关键词搜索路径的开帖步 MUST 仍是 45s，
  MUST NOT 因本次首帖放宽而一起变慢。
- [ ] 119.4 **（空首页账号不再被钉死）** 取一个首页恒空、已被切到 Reels 的账号，人为把它送回首页：
  滚动报无目标后 MUST 能再次授权切 Reels（每场至多两次），
  MUST NOT 再出现「零命令悬停到冷待机」的序列。
- [ ] 119.5 **（重开有界、不来回弹）** 观察一场内 Reels 重开次数：MUST NOT 超过两次；
  用尽后 MUST 诚实结束本场（日志出现 `scroll_no_target_without_next_step`），
  MUST NOT 在空首页与空 Reels 之间反复切换。
- [ ] 119.6 **（租约撞车不再静默）** 构造一次「任务释放与浏览命令同毫秒」：云端 MUST 立刻收到
  `task_lease_suppressed` 回执，MUST NOT 再等满自己的步超时；该回执 MUST NOT 被计入任何已完成动作或配额。
- [ ] 119.7 **（越南语 Feed 恢复控件）** 遇到只剩 `Đi đến Bảng feed` 的空白/阻塞页：
  MUST 由 Native 发一组 CDP 指针事件（零 DOM click），控件消失且 home surface 确认后才继续；
  控件不唯一 / 离屏 / 点后无后置状态时 MUST 诚实报未开始或结果不明。
- [ ] 119.8 **（待坐实的线索）** 取一次首帖返回的目标身份原值，判定
  `restore-facebook-post-join-comment-continuity/design.md` §6 那条
  「就地绑定指纹刻意不含固链、而身份回读可能依赖固链」是否成立。
  若成立，页面**越正常水合越必然失败**，与 §3.2 实测的「水合越慢越容易失败」方向相反，须另起 change。

## 簇 120 — Facebook feed 就地读恢复展开：这次「读过」到底有没有发生

> 来源 change：`restore-native-facebook-inline-expand-read`（2026-07-29）。
> 背景：Native 迁移（edge `4f04e9c`，2026-07-23）把 feed 就地读只搬了输出结构、没搬行为——
> 不点「查看更多」、不校验正文增长、无诚实失败、无环境变化回落、读后无停留地板，`thinkMs` 收下即丢弃。
> 规则模式与人设模式共用这条路径，两边都受影响；规则模式尤甚，因为它的「已确认浏览」
> 是直接兑换点赞与加群评论的凭证。
> **本 change 改 Rust 引擎 + 页面规则脚本 + 客户端会话层，必须重打边缘客户端包才在运营机上生效**；
> 云端零改动，仅 push 不改变现场行为。
>
> **⚠ 顺带查出、本 change 明确不做的一条代码缺口（不是真机项，别在真机上找它）**：
> `command-pacing` 要求边缘对「操作类」命令（开帖 / 主页 / 互动 / 翻图 / 滚评论）采用**最小间隔**
> 语义——维护单一锚点记上次操作完成时刻，下条操作只补足差额。Native 路径**整条缺失**：
> `aidcp-edge/src/native-page-engine/browse-session.ts` 的 `applyPacingSnapshot` 此前是空实现，
> 握手下发的 `opFloorsMs` 至今无人消费（本 change 只接了 `tempo`，并在该处注释里具名标出未接线）。
> 影响面：动作与动作之间没有下限间隔，云端决策快时会连着打两个动作；`thinkMs` 补回后掩盖了一部分
> 症状，但两者是不同的两层（`thinkMs` 是云端给这一次动作的犹豫，最小间隔是边缘对任意两次动作的地板）。
> **须另起 change 处理**，别当成本簇的验收项。

- [ ] 120.1 **（展开真的发生了）** 取一条被折叠的长帖：MUST 观察到「查看更多」被点击、
  上报正文长度大于折叠态长度。MUST NOT 出现「点了但上报的还是那几行」。
- [ ] 120.2 **（两种折叠形态的实际分布）** 采样若干长帖，分别记录「全文已在 DOM 内仅被视觉裁短」
  与「点击才补正文」两种形态各占多少。这决定免点击捷径的命中率，也决定回归前到底丢了多少内容。
  若捷径判据（textContent/innerText 比例 1.2 + 绝对差 30 字符）在真机上误判，须回写 design.md D4。
- [ ] 120.3 **（多语言展开控件命中率）** 在中 / 英 / 越三种账号写作语言下各跑一轮：
  展开控件 MUST 被识别。漏认的表现是「无展开控件的短帖」路径（正常成功、正文偏短），
  不会报错——所以 MUST 用「上报正文长度分布」而不是失败率来判断有没有漏认。
- [ ] 120.4 **（展开无效是真诚实）** 构造或抓取一条点了展开正文不变长的帖：
  MUST 回 `expand_no_effect`、MUST NOT 产生帖子详情上报，云端 MUST NOT 记这次浏览、
  规则进度 MUST NOT +1。同时确认云端兜底滚动照常触发、循环不卡。
- [ ] 120.5 **（环境变化回落走通）** 制造一次展开过程中弹层出现 / 卡位移的情形：
  MUST 回落到详情页导航重读并以 detail 面上报；回落也读不到时 MUST 是
  `inline_fallback_detail_unconfirmed` 具名失败，MUST NOT 退回那份不可信的就地读数。
- [ ] 120.6 **（停留与犹豫真的在等）** 采样一场会话的时间轴：feed 面开帖前 MUST 有一段与云端
  `thinkMs` 同量级的等待；读完到下一条滚动之间 MUST 满足「就地读 read floor 与云端 dwell 取较大者」，
  MUST NOT 是两者之和、也 MUST NOT 是秒滚。
- [ ] 120.7 **（规则模式吞吐下降的实测量级）** 对比修复前后同一账号单位时间的已确认浏览数与
  规则批次触发频率。预期下降，量级须记录下来作为后续调参基线；若下降幅度超出可接受范围，
  回头调 read floor 常量而不是砍掉展开。
- [ ] 120.8 **（人设模式的判断质量）** 取若干长帖，比对修复前后「打开后那道内容质量粗筛」
  拿到的正文长度与判定结果。这是本次修复真正的收益面——粗筛此前吃的是折叠段。
- [ ] 120.9 **（动作名归一无回归）** 确认 feed 面开帖失败的回执动作名为 `open_note`；
  云端评论迁移标志与评论支线时钟 MUST 能被这条回执正常清理。

## 簇 119 — Facebook 找卡回落路径：首页读不出内容的真正入口

> change `restore-native-facebook-feed-card-discovery`（2026-07-29，edge `2a4752a` 已合 master）。
> 真机基线（Xu Nu 越南语首页）：`div[role="feed"]`=0、`[role="article"]`=2 且均为 286px 空壳 ⇒ Native 找卡恒零；
> 补回退役实现的回落路（正文标记 → 上溯含作者链接的祖先）后，冷启动零指针条件下发现 4–5 张真卡。
> **可上报卡仍为 0** —— 该版式时间戳 href 是诱饵、需可信悬停才换真链接（见 119.5）。
> **本 change 改的是编入 Rust 二进制的注入脚本，必须重打边缘客户端包才生效。**

- [ ] 119.1 **（语义化版式零回归·真机）** 找一个仍渲染 `div[role="feed"]` + 水合 `[role="article"]` 的账号或页面：
  合并后的卡集合 MUST 逐位等于只走语义路的结果，MUST NOT 因回落路多出或少掉任何一张。
- [ ] 119.2 **（跨版式不误抓）** 在群组页 / 搜索结果页 / Reels 上各跑一次：回落路 MUST NOT 把整页、`[role="main"]`、
  侧栏或推荐位当成卡。判据是每张卡都有自己的作者与正文，且边界高度与视觉上的一条帖子吻合。
- [ ] 119.3 **（慢网下不误判空壳）** 限速到弱网再进首页：水合过滤 MUST NOT 把「作者链接尚未渲染但正文已出」的真卡
  判成空壳（判据取「作者链接**或**正文标记」的或，本条是复核这个或真的兜住了）。
- [ ] 119.4 **（点赞定位唯一命中率）** 候选集变大后，按 permalink 精确匹配 MUST 仍恒得唯一命中；
  MUST NOT 出现同一帖既命中语义卡又命中回落卡导致 `ambiguous_target`。
- [ ] 119.5 **（诱饵链接：下一步的前置观测）** 记录首页上「已水合但无可接受 permalink」的卡占比。
  这个比例就是「必须靠可信悬停才能读到」的那部分工作量。实测两条已确认：可信悬停视口内 7/7 有效、视口外无效；
  页面内合成的指针事件一律无效。若后续版式出现「悬停也不换链接」的卡，MUST 当场记下来，别默认悬停万能。
- [ ] 119.6 **（发现修好后的实际闭环）** 打包后取一个账号观察：滚动 MUST 不再因「一张卡都没有」而空转；
  present_unreportable MUST 以真实水合卡数为依据（而不是此前的 2 个空壳）。

## 簇 120 — Facebook 帖子地址的交互式采集：真机未验证

> change `acquire-facebook-feed-post-identity-by-hover`（2026-07-29，edge `78f89cf` 已合 master）。
> Facebook 把帖子地址扣在 DOM 之外，只有可信指针落到时间戳上才换出。已实装有界采集回合
> （只移动不按下、跨轮共享 8s 预算、每卡 2 候选、每轮 3 卡、未命中无声降级）。
> **端到端收益未证实——合入理由是「不生效即等于没改」，不是「已验证可用」。**
> 改的是编入 Rust 二进制的引擎与注入脚本，**必须重打边缘客户端包才生效**。

- [ ] 120.1 **（核心未决）** 视口内的时间戳悬停后是否稳定换出地址。已知矛盾数据：Xu Nu 手工测 5/5，
  Vo Tu 实装路径 0/1。逐个排除：停留时长（当前 1.2s）、指针是「瞬移到坐标」还是「移动过去」、
  账号/版式差异。**在这条查清之前，不要宣称首页已恢复可读。**
- [ ] 120.2 **（吞吐天花板）** 实测 Vo Tu 一屏 10 张卡，时间戳 cy 从 -2417 到 +3678，
  **任一时刻只有约 1 张落在视口内**。若这是常态，则一轮滚动最多补 1 张地址。
  须评估这对每小时浏览量的实际影响，必要时调整采集在滚动节奏中的位置。
- [ ] 120.3 **（红线真机复核）** 采集过程中 MUST NOT 打开任何帖子、MUST NOT 触发任何点赞。
  单测已断言只下发移动，本条是生产复核。
- [ ] 120.4 **（归属正确性）** 每张新解析出的地址 MUST 属于该卡本人（作者与地址中的主页标识一致），
  MUST NOT 是邻卡的。
- [ ] 120.5 **（时长）** 单次滚动命令的实际耗时 MUST 显著小于 30s 的命令原子上限
  （注意：真正的紧约束是这个 30s，不是云端 240s 看门狗）。
- [ ] 120.6 **（跨版式）** 群组页 / 搜索结果页是否也存在诱饵地址，候选筛选是否同样适用。
- [ ] 120.7 **（阈值再标定）** 候选筛选当前只按高度（≤24px）+ 有文字 + 无图标 + 无标签。
  已知时间戳高度在两台机上恒为 15。若遇到高度不同的版式，MUST 扩阈值而非加新判据——
  本 change 已经因为多加了一条宽高比判据而在一台机上零命中。

## 簇 120 — Facebook 时间预算整体上调后的真机复核

> 来源 change：`restore-facebook-post-join-comment-continuity`（2026-07-29 第二批）。
> 诱因：长评论逐字输入没输完就撞命令 deadline（真机实测 277 字符 → 78.1s 到点，分毫不差）。
> 云端已部署 dev（`aidcp-cloud 9013a5f`）；边缘（`aidcp-edge 70f6947`）**须重打桌面客户端包**才生效。

- [ ] 120.1 **（长评论真的发得出去）** 取一条 400–600 字符的越南语招聘长文跑一次评论：
  MUST 完成逐字输入并提交，MUST NOT 再出现 `comment_deadline_exceeded`。
  记录实际耗时，与「27s + 330ms×字数」的预算对比，验证 165ms/字符这个实测均速。
- [ ] 120.2 **（上限装得下）** 记录一次接近上限（≥800 字符）的评论：预算被 180s 上限夹住时，
  MUST 仍能完成；若失败，说明 165ms/字符 的估算偏乐观，须回头调 per-char 或上限。
- [ ] 120.3 **（带联系方式的评论不再被云端先掐断）** 跑一条带联系方式的评论：
  云端算出的预算 MUST ≥ 边端，MUST NOT 再出现「边端还在输入、云端已判 timeout」。
  这条修复与 ×1.5 无关，是一直存在的口径不一致。
- [ ] 120.4 **（会话真的开得起来）** 观察 Facebook 环境启动：会话超时已抬到 180s，
  它必须同时穿过边缘准入与引擎协议准入。MUST NOT 出现 `session.open` 被拒（那是全平台级故障）。
- [ ] 120.5 **（加群没有变慢到影响节奏）** 加群预算随整体抬到 135s、内层就绪 45s / 确认 68s：
  观察实际加群耗时分布 MUST NOT 明显上升（放宽的是容错上界，不是行为）。
- [ ] 120.6 **（判稳未被放大是对的）** 判稳预算最终保持 6s / 3.5s 未动（与并行落地的帖子身份采集
  共用命令预算）。观察慢群页上是否因此仍有「判稳不够」的失败；若有，须连同身份采集的预算一起复算，
  MUST NOT 单独抬判稳。

## 簇 121 — Facebook 内容派生的会话内帖子身份：可上报卡数是否真的从 0 变正

> 来源 change：`generalize-facebook-content-derived-post-identity`（2026-07-29）。
> cloud `424834d` 已部署 dev；edge `b717433`（含前序 `1d377fc`）**须重打桌面客户端包**才生效——
> 改的是编入 Rust 二进制的引擎与注入脚本，仅 push 不生效。
> 前置事实（真机实测，勿重新论证）：群组帖零交互下拿不到任何可接受地址（Vo Tu 上 6/6 卡为 0），
> 浏览链路 100% 阻塞；现改为签发内容派生的会话内引用（作者 + 正文 + 稳定链接 + 媒体标识 → SHA-256）。
> **本簇最该盯的不是「有没有卡」，而是「引用会不会漂到别的帖子上」。**

- [ ] 121.1 **（核心收益）** Vo Tu（`k1f44fit`，群组帖为主）冷启动 + 纯程序化滚动：
  可上报卡数 MUST > 0（基线实测 0）。逐张核对作者与正文与页面上那条一致。
- [ ] 121.2 **（点赞真的落在那张卡上）** 对会话内引用的卡下发点赞：MUST 命中、MUST 由按钮状态
  由「赞」变「已赞」确认，且 MUST NOT 点到邻卡。这是本 change 收益的另一半，桩层已覆盖三种结局，
  但「按钮状态在真实版式上认得出来」只能在真机证。
- [ ] 121.3 **（虚拟化复用不漂）** 滚走再滚回同一条帖子：引用要么解析到原帖，要么诚实报
  `stale_target` / `ambiguous_target`，**MUST NOT 解析到现在占着那个 DOM 位置的另一条帖子**。
  这是全簇风险最高的一条——虚拟化复用是最可能的漂移源。
- [ ] 121.4 **（碰撞率）** 招聘 / 租房群是最坏场景（同模板刷屏帖）：采样一屏内的引用碰撞率。
  碰撞的**允许**后果只有「少读一条」（被误判成已读）；若观察到「操作打在另一条帖子上」，
  说明证据构成不足以抗碰撞，须回头加证据维度而非放宽校验。
- [ ] 121.5 **（云端从不下发导航类命令）** 全程 MUST NOT 出现对会话内引用的 `open_note{navigate}`
  或定向评论。云端日志里出现 `content_ref_not_addressable` 是**闸生效的正常记号**，不是故障；
  但它若高频出现，说明某个角色一直在对这类卡提无法满足的要求，值得单独看。
- [ ] 121.6 **（不落库、不进人工）** 会话内引用的浏览与点赞 MUST 照常计入风控计数，
  但 MUST NOT 出现在：跨会话去重表、点赞血缘、后台「按笔记互动」展示账本、精选库、引流待评线索卡。
  抽查后台各页 + 库内按 `aidcp:` 前缀查一次，命中即为回归。
- [ ] 121.7 **（浏览量口径已放宽，不是 bug）** 会话内引用不跨会话去重 ⇒ 同一条帖子在两次会话里
  各记一次浏览。这是**设计**（引用换会话即失效）。记录一次实际重复量级，供日后判断是否需要
  另加口径，**MUST NOT 当成计数缺陷反复排查**。
- [ ] 121.8 **（就地读的导航回落不再抛硬错）** 就地读遇「环境变化」时，会话内引用的卡应如实回
  `context_changed`，MUST NOT 出现 `native navigation target is not an allowlisted Facebook page`
  这类合成错误——那会把「换页了」抹成「命令非法」。

## 簇 122 — 小红书运行期身份与验证码键入取证：装齐了，真机一次没跑

> 来源 change：`restore-native-xiaohongshu-session-guards`（第一波 2026-07-29 / 第二波 2026-07-30，均**活跃未归档**）。
> edge master `c7c9ea8`；本流 12 个提交：`3236d23` `ac99d64` `95af401` `197edae` `d033504` `bfaa33d`
> `7433a24` `c01bdcb` `0966c43` `1295122` `01605c8` `bed0e83`。
> **零真机动作**：edge-only 无 ECS 面 ⇒ 无部署可做；**未打桌面客户端包**；未在真机上做过任何读写。
> 改的是**编入 Rust 二进制的引擎 + 客户端核心 + 外壳投影**，**仅 push 不生效，必须重打包**（同批还改了外壳侧
> 两个文件，见 122.17 的打包红线）。
> **前置环境**：一台装了新包的运营机 + 一个 `tom` 分组的小红书分身（见 memory `real-machine-test-accounts`）
> + 一次飞书 / 客户端左栏肉眼确认。本簇与簇 123 是**同一台机器、同一个分身**，应在一次真机 session 里连着验。
>
> **本簇为什么整簇都只能真机**：这一批行为的桩层覆盖是齐的（新增身份重立 36 条 / 验证码键入取证 9 条 /
> 点击结果 8 条 + 假 CDP 若干），但桩证的是**结构与控制流**：命令有没有被拒、状态机有没有走到那一格、
> 回执字段有没有按实测填。证不了的恰是本批的要害——**真实页面上焦点落在哪、探针多久回一次、
> 运营在左栏看到的是哪句话、云端那边等了多久**。
> **红线复述**：本批全部行为真机零跑，**不得当成已确认事实**；「读不到」与「没有」是两态，不得压成一态。

**§3 验证码键入取证侧**

- [ ] 122.1 **（六格取证逐格对账）** 真实小红书验证码浮层上跑一次带答案的远程协助，把回执里的六格取证
  （焦点分级 / 清空三态 / 实际派发字符数 / 回读三态 / 是否已提交 / 提交后复检）与页面实际状态**逐格**对上。
  重点两处：焦点标签在**内嵌页 / 画布**形态下的真实取值；不透明档下「已尝试清空」是否真清干净。
  **桩为什么验不了**：假 CDP 回的焦点与清空结果是夹具自己铺的，真实页面的焦点行为（iframe 边界、
  canvas 上根本没有可聚焦元素）造不出来。
- [ ] 122.2 **（回读三态里「读不到」那一格是否真的可达）** 判据是**读代码推出来的、从未观测过**：
  取当前焦点元素回 null 在真实验证码页是否真的出现；出现时是否应改判为焦点丢失。
  同批另一个新原因码（清空验证不可得）也只经假 CDP 验过 ⇒ 顺带看一眼**云端 / 控制台对未知原因串的展示
  是否退化成空白**（协议上该字段是自由串，静态检查不会拒 ⇒ 静态门禁永远发现不了这种展示层断裂）。
  **桩为什么验不了**：桩里没有真实浏览器的焦点模型，也没有控制台渲染层。
- [ ] 122.3 **（提交后复检的真实行为）** 回车触发导航时页面探测是否**稳定失败** ⇒ 据此决定是否需要恢复
  退役实现的 4×500ms 有界复检（现为单次沉降 + 单次探测，读不到即判定不可得）。
  **桩为什么验不了**：假 CDP 里「回车后页面正在导航」这个中间态是模拟的，真实导航时机与探测竞速的分布只有真机有。
- [ ] 122.4 **（期望值已翻转，别照旧结论看）** 键入模式的语义本波被订正成「回执带取证」而不是「确有字符派发」。
  须制造两次：① **点位没点中输入框（零派发）** ⇒ 控制台**不再**提示「客户端太旧」（这是订正后的期望行为）；
  ② **真正的老客户端忽略下发文本** ⇒ 该提示**仍会**出现。
  **桩为什么验不了**：这是云端版本偏斜探测的展示口径，两侧联跑才看得到；边缘桩只能证回执字段。
- [ ] 122.5 **（搜索框探针抛异常后云端不会卡住）** 该命令现在以「探测失败」收尾，而不是回一份「找不到搜索框」的
  回执。须核对云端对这条错误分支的处置**不会把调度卡住**（不改代码，只核对）。
  **桩为什么验不了**：边缘桩看不见云端调度器的等待与重排。
- [ ] 122.6 **（第一波项）远程协助键入一次真实验证码** — 确认回执里的实际派发字符数、回读三态、是否提交
  与现场一致，且**答案明文未出现在任何日志 / 落库 / URL 中**。
  **桩为什么验不了**：明文泄漏面是「日志文件 + 数据库 + 请求地址」三处的真实内容，桩里这三处都是假的。
- [ ] 122.7 **（第一波项）真触发一次验证码全链** — 边缘发出阻断信号、云端唤起远程协助、账号风控态迁移，
  清除后收到**配对的**解除信号。
  **桩为什么验不了**：跨边缘 / 云端 / 风控三方的状态迁移闭环。
- [ ] 122.8 **（第一波项）小红书阻断桶的误报率** — 连续观察若干场浏览，确认正常页面（含笔记详情弹层、
  看图态、AI 搜索结果页）**不被判成阻断**（已知根因：阻断信号计数与弹层计数都不排除笔记详情容器、
  且已退化成子串计数 — 见该 change 的 oracle.md）。
  **桩为什么验不了**：误报率是真实页面分布上的统计量，夹具页面数量有限、且正是按已知形态铺的。
- [ ] 122.9 **（第一波项）低置信「未识别」阻断桶的缺席影响面** — 观察是否存在「真阻断但既非验证码指纹
  也非登录墙」的实际形态，据此决定是否值得为它单起一个 change。**旧的未识别判据本身是误报源，勿照抄。**
  **桩为什么验不了**：要找的是「我们还没建模的那类页面」，定义上不可能在桩里存在。

**§5 运行期身份侧（本流全部身份行为真机零跑）**

- [ ] 122.10 **（核心收益：换号 / 掉登录终于测得出来）** ① 运行中在同一分身里退出登录 ⇒ 周期校验在
  **≤2 拍（默认 60s）**内判失效，重立链跑完并**停在无身份态、绝不回落默认账号（红线）**；
  ② 同一用户数据目录换登另一账号 ⇒ 判换号、按新 id 重连云端、云端拆掉旧会话，恢复后可正常浏览。
  **桩为什么验不了**：桩能证链条顺序，证不了「真实退出登录后页面变成什么、多久变」。
- [ ] 122.11 **（两个预算未在真机定档）** ① 触发失效时页面正停在**创作发布页** ⇒ 归位到消费端首页后
  能读出身份锚点（重立链的归位重读预算是按「整页重载」拍的）；② **正向登出探针的陈旧预算 =
  3 × 观测节拍 ≈ 6s** 是否合适——真实页面上探针的成功率与间隔分布**从未观测过**：太紧会把健康读数
  当陈旧（**漏判**真登出），太松会把陈旧当新鲜（**误判**登出）；③ 确认「连续 3 拍打一次存活告警」
  在真机上不会每天刷屏。
  **桩为什么验不了**：这两个数都是**时间分布**，桩里时间是夹具给的常量。
- [ ] 122.12 **（四态运行姿态与外壳呈现逐条对账；桩已覆盖八条路径，真机零跑）**
  ① **终局身份下点「恢复」** — 界面**一秒都不许闪现运行中**，且给出的处理办法可执行（停 → 重登 → 启）；
  ② 制造一次「重设基线之后收尾失败」（如唤醒观测异常） — 左栏出现「**自动化已停摆（身份正常）**」
  而非全绿，且点一次「恢复」真能把浏览与观测拉回来；
  ③ **终局下切云端环境** — 如实报重绑失败、云端侧**看不到**该账号上线、重启后能连到新选的云端；
  ④ **终局下云端仍派发布 / 评论** — 边缘回诚实拒绝回执、云端不挂起、**平台上无该账号的新痕迹**；
  ⑤ **正常换号触发一次重立** — 「正在重新确认账号身份」在几秒内自动收尾、不产生红角标
  （这是**本波新增的界面行为**：正常换号时运营会看到几秒该提示 + 云端显示断开，诚实但此前没有，
  须确认不会被误读成故障）；
  ⑥ **唤醒被拒**（已设账号覆盖值 + 唤醒读不出身份） — 留在待机态、可再次唤醒、浏览器被杀回槽位
  且**槽位没泄漏**。
  **桩为什么验不了**：这一条整条是「运营眼睛看到什么」，桩里没有界面。
- [ ] 122.13 **（把两个已知缺陷拍成事实，用作 P1 / P2 的代价基线）**
  ① 拔掉客户端核心↔外壳的进程间通道后触发终局与停摆 ⇒ 确认兜底日志行出现，且界面**只红一行**、
  随后被下一行普通日志写回运行中（**这正是待做项 P2 要修的现象，先把它拍成事实**）；
  ② 身份落到终局时云端下发任务认领 ⇒ 用云端日志确认它**空等满认领超时（200s）**才失败
  （**这正是待做项 P1 的代价基线，修完要能对比**）；
  ③ 终局 / 停摆各触发一次，确认持久红卡出现且**不被后续普通日志清掉**。
  **桩为什么验不了**：①③ 要的是「界面被下一行日志冲掉」这个时间序上的事实；② 要的是云端那侧的等待时长。
- [ ] 122.14 **（第一波项）提交窗口的实际效果** — 在评论提交进行中发起一次高档位抢占，确认协调器回
  「窗口占用中 + 剩余预算」而非**在提交中途接管**。
  **桩为什么验不了**：抢占与真实提交那一瞬间的竞速。
- [ ] 122.15 **（第一波项 · 推断未坐实）** 简报判定「提交窗口缺失目前只表现为接管失败、不撕裂写入」
  依赖「写命令不做飞行中取消」这一当前实现，**未真机复现**；须在真机上确认修复前后是否真出现过**重复提交**。
  **桩为什么验不了**：重复提交的证据在平台侧（评论区里有没有两条）。
- [ ] 122.16 **（第一波项 · 推断未坐实）** 小红书运行期换号 / 掉登录的**真实发生形态与频率**没有线上数据支撑，
  只有代码与旧注释对照；**周期与防抖阈值须按一次真机观察定档**（现值是拍的）。
  **桩为什么验不了**：要的是生产分布，不是单次行为。
- [ ] 122.17 **（发版前必做 · 打包红线）** 本波改了外壳侧两个文件（运行姿态投影与终态白名单）。
  按 CLAUDE.md §5 打包红线，**出包前须在本机跑一遍编译后的打包产物**（起一次核心、确认能走到
  云端连接 / 指纹浏览器调用），别把外壳类回归留给运营机。
  **桩为什么验不了**：打包态特有的路径问题（`app.asar` 当目录用那类）在本地开发态与单测里**结构上不可能出现**。

## 簇 123 — 小红书写动作拟人化与云端节奏消费：从「一次性赋值」回到真手势

> 来源 change：`restore-native-actuation-humanization-and-locating`（2026-07-30，**活跃未归档**）。
> edge master `c7c9ea8`；本流 10 个提交：`306410b` `e748a56` `1b890ea` `25fa1c3` `92f7882` `f652786`
> `17892c2` `044f103` `4593922` `c7c9ea8`。
> **零真机动作**、**未打桌面客户端包**：本流主体是**编进 Rust 二进制的输入 / 滚轮原语与引擎分发**，
> **仅 push 不生效**。前置环境与簇 122 完全相同 ⇒ **同一次真机 session 一起验**。
>
> **两条行为面新增，验收时优先盯**：
> ① **小红书从此会等**——翻页 / 关帖 / 返回的停留此前只在 Facebook 生效，本波泛化到小红书；
> ② **小红书写动作从此真打字、真滚轮**——评论与发布字段改成「真实指针轨迹落焦 + 逐字键入 +
> 换行拆成独立裸回车 + 有界归尾确认」，翻页改成惯性滚轮手势。
> **一处能力回退（唯一一处「以前能做现在不做」）**：富文本评论框 + 联系方式串码现在是**零派发结构性拒绝**，
> 解锁前置就是 123.5。
> **小红书真机验收自 Native 迁移（2026-07-22）以来一次都没做过** —— 本簇同时是那次迁移的第一次真机复核。

**A. 由云端节奏消费（§4）引入**

- [ ] 123.1 **（观测口径须改写，别照旧条目验）** 节奏等待落在客户端核心层、在**命令死线之外** ⇒
  **不会**拉长单命令墙钟，但**会**拉长命令间隔 ⇒ 影响的是**会话看门狗的 idle 判定**，不是单命令超时。
  观测对象应是 **idle 间隔分布**，不是单命令耗时分布。诊断依据已就位（节奏层新增两行有界日志）。
  **桩为什么验不了**：看门狗的 idle 判定是长跑会话上的时间行为。
- [ ] 123.2 **（小红书第一次开始「等」）** 泛化后小红书每次翻页 / 关帖 / 返回都会新增一段等待。
  **属行为面新增，须确认不触发看门狗**、也不把一场会话的产出压到不可接受。
  **桩为什么验不了**：桩里等待是被夹具跳过或压缩的，真实累计效应只有跑一整场才看得到。
- [ ] 123.3 **（关帖不再接受犹豫值字段 = 行为收紧）** 现网云端本就不发该字段；若某个未知路径发了，
  命令会变成**非法请求、根本不下发**。跑一轮浏览闭环确认无非法请求。
  **桩为什么验不了**：要证的是「现网云端真的不发」，那是云端侧的事实。
- [ ] 123.4 **（兜底档不叠出异常长停留）** 旧云端 / 断连时详情停留走本地采样兜底并乘风控档位，
  确认断连恢复后不会因此叠出异常长停留。
  **桩为什么验不了**：断连 / 恢复的真实时序。

**B. 由写动作输入半边（§8）引入**

- [ ] 123.5 **（能力回退的解锁前置）** 小红书评论框对 **Shift+Enter** 的绑定是**软换行**还是**提交**？
  确认为软换行后，才可恢复「富文本评论框 + 联系方式串码」这条能力（加一种软换行按键形态并按形态分派）。
  **桩为什么验不了**：这是真实编辑器的按键绑定。实装方明确拒绝在桩上赌——**赌错的后果是发出半条评论**。
- [ ] 123.6 **（评论框形态）** 评论路径当前**一律按普通字符写换行**（受控输入框口径）；页面分片已能区分
  受控框与富文本两态，但评论路径当前不据此分流。真机确认这个口径在现役评论框上正确。
  **桩为什么验不了**：真实评论框是哪一态、裸回车在它上面是不是提交。
- [ ] 123.7 **（单次往返成本的真实值）** 初始为估值、之后由实测**单调抬升**。确认长正文（≥3000 字）的
  实际写入次数与总耗时，据此复核往返次数上限与停顿上限。
  **桩为什么验不了**：往返成本 = 真实 CDP 网络时延。
- [ ] 123.8 **（逐字输入降级的真实频次）** 需真机跑长正文才知道降级多频繁、往返成本被单调抬到多高。
  **注意**：这条记账目前**到不了运营视野**（引擎的错误输出只在进程级失败时才挂出，而降级是**成功路径**
  ⇒ 记账行随进程退出丢弃，见该 change §9.3-2），**观测须直接看子进程输出**。
  **桩为什么验不了**：桩里没有真实文本长度分布，也没有真实往返成本这个放大器。
- [ ] 123.9 **（提交窗口抬到命令墙钟上限后的抢占影响）** 确认抢占方不会等太久 / 不触发重排风暴。
  **桩为什么验不了**：抢占方的等待感与重排风暴是多任务并发下的真实调度现象。
- [ ] 123.10 **（预算前置拒绝会不会误伤真稿）** 换行极多的正文会在**开工前零派发拒绝**，按预算推算
  约在 40 段以上触发。确认实际稿件不会撞上。
  **桩为什么验不了**：要的是真实稿件的段落数分布。
- [ ] 123.11 **（拟人点击真的落在编辑器上）** 评论与发布字段的落焦改成**真实指针轨迹点击**
  （此前是页面侧聚焦调用），确认落点几何（分片回的是元素矩形中心）在真实布局上确实命中编辑器、
  **不会点到遮挡层**。
  **桩为什么验不了**：真实布局里的遮挡关系。
- [ ] 123.12 **（定时设置：裁定已下，只剩真机标定）** 定时字段是**分段日期时间控件**，退役实现用的
  就是原生赋值；照任务字面逐字打**会是功能回归**。
  **2026-08-01 用户裁定：保持原生 value setter，把定时字段从 8.1 的「全线逐字输入」覆盖面里显式划出。**
  判据与 2.9 / 3.7 两条弃守同源 —— 该路径缺的是反检测质量，不产生任何假成功（写进去的值是真的、
  回读也是真的）。**所以「需裁定」这一半已结案，本条只剩真机标定那一半**：真要改成硬件级输入，
  正解是**键盘分段输入**，前置是标定分段控件的键位行为 —— 是否接受数字键、是否要补零、
  段间切换用 Tab 还是方向键。标定完成后与 2.9 / 3.7 一并进反检测专项。
  **桩为什么验不了**：分段控件的输入语义。
- [ ] 123.13 **（新增的「读不到」原因码在云端侧尚未登记消费）** 回执理由是裸串、两侧无机械对账；
  真机上先当排障 token 用，顺带确认控制台对未知理由串的展示**不退化成空白**。
  **桩为什么验不了**：展示层断裂只在云端 / 控制台上可见。

**C. 由滚动半边（§8）引入**

- [ ] 123.14 **（两条旧结论未复核，任务点名）** 宽 / 窄两套布局的可滚元素**是否真的不同**、
  页面内滚动在窄布局上**是否真是空操作**。`docs/xhs-layout-states.md` 是迁移前文档，未在当前布局复核过。
  实现对两态都作了处理（认内层容器 / 回落窗口），真机确认它在两套布局上各自解析到哪个元素。
  **红线**：**不得**把「小红书 feed 已因此永不推进」当既成事实写进代码注释或回执理由。
  **桩为什么验不了**：夹具页面就是按待验证的旧结论铺的，自证循环。
- [ ] 123.15 **（可滚区选择器命中面）** 确认命中的是**真正的滚动容器**，而不是某个内容溢出但溢出语义
  不是滚动的壳（代码已加语义判据，需验证判据够不够）。
  **桩为什么验不了**：真实页面的容器层级与样式计算。
- [ ] 123.16 **（半屏位移够不够 / 是否过密 = feed 单次位移口径）** 位移取「视口一半、裁进 360–700」。
  确认相邻两次扫描的可见卡片**确实有重叠**，且**没有密到把同一批卡反复评估**。
  建议做法：连续 5 次翻页，核对相邻两次上报卡片集的重合度。
  **桩为什么验不了**：重合度取决于真实卡片高度与懒渲染节奏（这正是历史「只刷不点」活锁的形态）。
- [ ] 123.17 **（落点几何是否命中可滚区）** 落点取「可滚元素矩形 ∩ 视口」的中心。确认它没落在
  悬浮头部 / 侧栏 / 广告位上（那会让滚轮作用在**错误的**滚动容器上）。
  **桩为什么验不了**：真实页面的浮动层位置。
- [ ] 123.18 **（翻页前关浮层的控件识别）** 确认小红书详情浮层的关闭控件真能命中；命中不到时当前行为
  是**照常去滚**（位移会诚实报，但 feed 不会推进）。
  **桩为什么验不了**：真实浮层的关闭控件形态。
- [ ] 123.19 **（懒渲染换掉滚动容器那一拍）** 是否**真会**让分片读不到可滚区（本波是按分片代码路径
  推的现场，**未在真页面上观测过**）。
  **桩为什么验不了**：懒渲染换容器是真实框架行为，桩里是用旋钮模拟出来的。
- [ ] 123.20 **（最小耐心轮次是否够）** 6 轮 × 50ms ≈ 300ms 是否覆盖真机小红书 feed **平滑滚动的
  起步延迟上限** —— 这是唯一能把这个经验值变成有依据的数字的办法。
  **桩为什么验不了**：起步延迟是真实平滑滚动的动画属性。
- [ ] 123.21 **（到底判定「读不到」的真实成因与频率）** 分片在解析成功时恒带该字段，**这一态在真机上
  是否可达未证**。
  **桩为什么验不了**：可达性问题——桩里能造出来，不代表真机产生得出来。
- [ ] 123.22 **（滚动「派发失败」的真实频次）** 收集诊断串出现频次，判断它在生产里是否常见到值得
  升级成**独立回执理由**（当前与「页面到底了」在回执字段上仍是一态，只在诊断日志分开）。
  **桩为什么验不了**：频次是生产统计量。
- [ ] 123.23 **（位置回退在真机上到底可不可达）** 本波裁定「位置回退**也算**动过」（判据用不等，不用大于），
  论据之一是「懒渲染换掉滚动容器 / feed 刷新回顶 ⇒ 位置从 p 跳回 0，页面确实动了」。
  这条**在真机上未观测过**。同时确认消费方**不会**因「到底了且没动」提前收工。
  **桩为什么验不了**：回退是真实框架行为；此处只需一次观测就能把裁定从推理升为事实。

> **2026-07-31 补登（用户裁定：真机项全部移出各 change 的 tasks.md）**：以下 123.24–123.28 来自同一 change
> **更早几波**的 7.2–7.7，此前只留在 `tasks.md` 里、未并簇。其中 123.25–123.27 需要 **Facebook 分身**，
> 与簇 126 共用环境，可一并验。原 7.7（锚点暂存与晋升的命中率提升）**不再登记**——
> 其所依赖的实装（锚点暂存区 / 反污染丢弃）已于 2026-07-30 显式弃守，立论不再成立。

- [ ] 123.24 **（节奏恢复后会不会撞看门狗）** [原 7.2] 在 dev 真机上确认：恢复时间指令消费后，
  一次浏览闭环不会因单命令墙钟变长触发会话看门狗；记录实际的单命令耗时分布与看门狗阈值余量。
  **与 123.1 / 123.2 是同一个风险的不同切面**（那两条看的是命令**间隔**，这条看的是**单命令墙钟**），
  应一起观察。
- [ ] 123.25 **（对齐滚动改有界步进后还够不够）** [原 7.3] 在 dev 真机上确认：改为有界步进后，
  首页点赞的对齐滚动仍能在现有轮次上限内把控件带进可视带；若不足，记录实测轮次并单独调整上限。
  **需 Facebook 分身。**
- [ ] 123.26 **（多帧指针轨迹会不会划走反应浮层）** [原 7.4] 在 dev 真机上确认：指针原语改为多帧轨迹后，
  Facebook 首页点赞、评论提交、加群按钮三条路径的成功率不低于改动前；两步点赞的第二步（反应浮层「赞」项）
  仍能提交。**重点看多帧轨迹是否会在移动途中划出浮层 hover 区致其收起——这是该批改动唯一有理由怀疑
  会新增失败的路径。** 需 Facebook 分身。
- [ ] 123.27 **（短视频兜底滚轮改共享手势后仍推进）** [原 7.5] 在 dev 真机上确认：短视频兜底滚轮改用
  共享手势后，推进仍可用且未出现「测不到移动却报推进」。**无参照物**——旧 Reels 也无位移实测，须新建基线。
  需 Facebook 分身。
- [ ] 123.28 **（节奏与手势特征是否真被平台判别 · 未坐实）** [原 7.6] 「当前节奏与手势特征已被平台判别」
  这一因果**无法在代码里坐实**，只能通过真机上的限流信号频次做前后对照观察。
  该 change 只按已归档的节奏与反检测规格恢复应有行为，**不主张判别因果**——本条是观察项，不是验收项。

> **2026-08-01 补登**：以下 123.29–123.31 来自同一 change 的第四波（`aidcp-edge d7dc24b`，H.1 与 8.1 裁定）。
> 同机同分身，与本簇其余条目一次跑完。

- [ ] 123.29 **（评论提交后平台清不清空评论框 · 本簇优先级最高）** [原 H.1] 评论到达确认现在要求
  **两条独立证据同时成立**：正文出现在评论区，且**编辑器已被平台清空**。后者是补回来的结构必要条件——
  没有它，判据就只剩一条宽松子串扫描，而我们自己刚写进去的正文就在页面上（富文本编辑器的 textContent
  是活的），一条根本没发出去的评论会被判确认。退役实现拿它当必要条件、有真机历史支撑，但 Native 迁移
  之后一次都没复核过。**若真机上并不清空，全部评论会从「已确认」退成「不确定」**——方向诚实，
  但日志上极显眼，且云端可能据此重投。**这条不确认，H.1 的收益就只剩「不再自证」，代价却是全量回执降级。**
- [ ] 123.30 **（有界轮询窗口够不够 / 新原因码的真实频次）** [原 H.1] 确认窗口从「固定睡 800ms 后单次
  采样」改成有界轮询（8 轮 × 250ms = 2000ms，按迭代次数限界），轮数是照退役量级取的、未实测。
  同时新增原因码 `submitted_editor_not_cleared`（正文看到了、编辑器没清空）——真机上它到底是
  「平台慢了一拍」还是「扫描仍读到了自己写的那份」，只有频次能分辨。
  **云端侧尚未登记消费该原因码**（回执理由是裸串、两侧无机械对账），先当排障 token 用。
> **8.1 的定时设置裁定不在这里另开条目** —— 它并进了上面的 **123.12**（那条本来就是为这件事留的）。

> **2026-08-01 补登（第五波，`aidcp-edge ab31c57`）**：以下 123.32–123.34 来自 8.1 的话题消费点。
> 同机同分身，与本簇其余条目一次跑完；**都需要真的发一条带 tag 的稿子**。

- [ ] 123.32 **（话题的建议下拉与真 token 选择器今天是否仍然成立）** 加话题整套判据
  （`.tippy-box[role="tooltip"]` / `#creator-editor-topic-container .item` /
  `a.tiptap-topic[data-topic]` / 隐藏后缀 `span.content-hide`）移植自退役实现的**实机校准**结论，
  但那次校准在 Native 迁移**之前**。选择器若已漂移，行为是**诚实地全部回未确认**（不会误贴一个无关话题），
  但话题会全线贴不上；而云端把这条指令算作尽力而为、稿子照发，**不会有任何报警**。
  **只能靠真机发一条带 tag 的稿子看**：话题贴上了没有、回执是不是 `publish_candidate_unconfirmed`。
  **桩为什么验不了**：桩里的下拉与 token 是我们自己造的，它只能证明代码按判据走，不能证明判据对得上今天的页面。
- [ ] 123.33 **（「认不出目标候选就不点」的真实触发频率）** 触发时正文里会留下一串裸 `#关键词`
  （与退役实现同行为）。确认这个残留可接受，以及触发是否频繁到值得改成「打字前先探一次下拉」。
  **桩为什么验不了**：频率是真实候选质量决定的。
- [ ] 123.34 **（`mention` / `location` / `collection` 三支的 token 判据标定）** 这三支**仍走注入路由，
  但 edge `ea2c4c9` 已先关闭自证成功出口**：候选已点击而缺独立结构证据时固定回
  `ambiguous / publish_candidate_unconfirmed`；`topic` 仍由 Rust 真 token 判据确认。
  **没有直接恢复正向确认的理由可核对**：退役实现对这三支用的也是通用锚点 + 页面子串，同样自证，
  **没有任何实机校准过的判据可移植**。要把它们也搬进引擎，前置是在真机上认出各自
  「真的选中了」的结构信号（提及 token / 地点入口回显 / 合集入口回显各是什么元素）。
  **桩为什么验不了**：这正是「在桩上现编一个选择器 = 拿运营机赌运气」那件事本身。
  - **2026-08-02 订正（edge `ea2c4c9`）**：后置校验盘点中的 `publish_add_with_candidate` 已从
    below-bar 改为达标：topic 有强证据，其余三支在标定前**没有 confirmed 出口**，也没有被洗成
    not-applicable。故本条现在负责恢复 positive confirmation，而不是阻断诚实性库存归零；标定完成后
    仍须把对应分支改为只认实测结构信号，并补 failure-first 真机结构夹具。

## 簇 124 — Native 引擎重连绑定：同机多环境并行下的端口回收行为

> 来源 change：`harden-native-engine-runtime-contracts`（**活跃未归档**）。
> **2026-07-31 用户裁定**：Native 迁移修复这批 change 的真机验收项从各自 `tasks.md` 移除、统一收拢到本文件。
> 原任务号见每条条目开头的方括号，回溯时按该编号在 change 的 git 历史里找原文。
>
> **前置环境**：一台装了新包的运营机 + **同机并行开两个以上指纹浏览器环境**（这是本簇的关键条件，
> 单环境跑不出来）。本簇与簇 125 用同一台机器，可连着验。
>
> **为什么整簇只能真机**：端口回收与再分配是操作系统 + 指纹浏览器进程的真实行为，桩层造不出来。

- [ ] 124.1 **（跨环境错投的真实发生概率）** [原 9.7.1] 同机多环境并行时，指纹浏览器释放的调试端口被
  另一环境复用的频率与分配策略，未实测；需在真机上观察端口回收行为后再评估「重连身份证据判据」是否足够。
  **这条是判断块①修复够不够的唯一判据**——判据本身可以先落地，但「够不够」只能靠这次观察定论。
- [ ] 124.2 **（引擎死后的恢复链）** [原 9.7.3] 「引擎死 → 结束会话失败 → 下次开始还是同一个死会话」
  这条链路是从代码路径推出的，未在真机日志里确认过实例；需真机杀一次引擎子进程验证恢复行为。
- [ ] 124.3 **（重连重试占死命令槽位）** [原 9.7.4] 重连后重试占死唯一命令槽位、宿主预算过后放弃等待
  且不发取消，导致后续命令被顶回——未在真机日志确认过实例；需真机制造一次 CDP 断连后观察。

## 簇 125 — 小红书 Native 切换：只读矩阵与写动作验收（自迁移以来真机零覆盖）

> 来源 change：`native-page-engine-production-cutover` / `restore-native-xiaohongshu-action-honesty` /
> `harden-native-engine-runtime-contracts` / `enforce-native-engine-artifact-gates`（均**活跃未归档**）。
> **2026-07-31 用户裁定**：真机项从各 change 的 `tasks.md` 移除、统一收拢到本文件。
>
> **前置环境**：一台装了新包的运营机 + 一个 `tom` 分组的小红书分身（见 memory `real-machine-test-accounts`）。
> **与簇 122 / 123 是同一台机器、同一个分身 ⇒ 三簇应在一次真机 session 里连着验。**
>
> **本簇的分量**：小红书侧自 Native 迁移以来**真机零覆盖**。簇 122 / 123 覆盖的是「会话守卫」与
> 「写动作拟人化」两条线，本簇补的是**页面矩阵本身**与**动作诚实性**——即「命令说成功时页面上到底发生了什么」。
> 这一层没验过，等于整个小红书面的 Native 行为都只有桩层证据。

- [ ] 125.1 **（只读矩阵八态）** [原 cutover 9.4] 在授权范围内跑一遍只读实况矩阵，覆盖首页 / 发现 /
  搜索 / 笔记 / 主页 / 通知 / 错误页 / 可观测登录态八个状态，逐态记录确切的后置条件。
- [ ] 125.2 **（写动作与发布歧义）** [原 cutover 9.5] 写 / 动作类验收只在单独的目标级授权下做，
  覆盖互动与发布歧义，且不得借此放宽已授权的动作范围。定时发布至少逐项核对：开关本来已开时不再点击；
  从 off 切 on 后平台状态真翻转；目标北京时间逐字保留到分钟；提交前仍为同一开关态与同一分钟；只命中
  「定时发布」而不是普通「发布」。同批记录 E7 身份面：立即发布的 fresh success 结果是否给出唯一笔记 id / 带
  `xsec_token` 的公开详情链；定时管理页的内部句柄、完整日期分钟、定时 / 已发布状态与公开链接分别落在哪个
  节点 / 属性；到期前仍定时必须保持 pending，到期后只有真实公开 id + tokenized URL 才确认。任一证据读不到须
  停在提交前或保持 submitted / scheduled pending，MUST NOT 为了验收而强行点普通发布、取列表第一条或拼裸链接。
- [ ] 125.3 **（开帖是否真的落到错误页）** [原 honesty 5.1 / 9.7.5] 在 dev、`tom` 分组账号上从首页封面
  开一篇笔记，记录开帖后地址是否带访问令牌、返回详情正文是否为空。若单页应用拦截了程序化点击、
  导航仍走内部路由，则本条降级为指纹问题；若确认落错误页，需追加实装任务：开帖改用可信指针输入触发就地弹层
  （参照 `docs/xhs-layout-states.md:58-62`；裸链 300031 结论出自 2026-06-27，Native 路径未复核）。
- [ ] 125.4 **（看图挂起是否复现）** [原 honesty 5.2 / 9.7.6] 真机跑一次详情深读，确认是否出现
  「看图这一步一直挂着直到会话看门狗杀场」的实例。复现则登记独立跟进项：云端深读等待表加有界超时；
  未复现则记录为未复现，不做云端改动。
- [ ] 125.5 **（点赞类翻转窗口）** [原 honesty 5.3] 真机测量点赞 / 收藏 / 关注的状态翻转耗时分布，
  确认有界窗口上限足够；记录实测值，不写进 spec（旧注释实测翻转 300–600ms、上限 1500ms，可作窗口起点）。
- [ ] 125.6 **（通知去重折叠的真实规模）** [原 honesty 5.4 / 9.7.10] 真机跑一次含同一发送者多条通知的巡视，
  确认修复前后处理条数差异；当前只有代码与旧注释对照，无线上数据。同批确认「行选择器退化」的后果规模。
- [ ] 125.7 **（通知行选择器）** [原 honesty 5.5] 在真机上 dump 一次通知页结构，确认采用的容器结构与当前平台一致；
  结构若已变，以本次 dump 为准并记录（`.tabs-content-container > .container` 与 `note-id` 属性出自
  2026-06-24 dump，须重测）。
- [ ] 125.8 **（评论合成文本的编辑器行为）** [原 honesty 5.6] 真机发一条含联系方式串码的评论，
  确认合成后的完整文本被平台原样接受（不被自动补全劫持、不被截断）。
- [ ] 125.9 **（未读角标判据是否仍成立）** [原 honesty 5.8] 在宽（左侧栏）与窄（底部图标栏）两种布局各测一次，
  确认取到的是可见入口、无未读时不误报、数字角标与无数字红点都认出。判据出自 2026-06-23 校准、
  窄布局曾真机实测漏报 10 条未读，距今一月余，**不得当成已验证事实**。必须用本次新打包并安装的客户端继续核对
  完整发送状态机：同一账号每次 `clear -> unread` 物理波次只发一个 `notification.detected`；未读数字变化不重发；
  Cloud 重连后同一物理波次以同一 epoch 向新 session 最多补发一次；账号切换后新账号的未读不被旧账号状态压住；
  验证码恢复帧必须先见配对的 `risk.captcha_cleared`、再见 `notification.detected`。源码 / jsdom / 假 Cloud 通过不得
  代替本项，也不得仅凭 Edge 有发送日志就宣称 Cloud 已接纳或平台巡视已完成。
- [ ] 125.10 **（清零是否真收敛）** [原 honesty 5.9] 在真机上制造多于一屏的未读，确认滚到底循环 +
  真实角标计数合起来能让该栏计数真归零、三栏清零循环能正常结束；桩验不了平台的真实加载与角标更新。
- [ ] 125.11 **（空根塌陷的真实结论）** [原 9.7.2] 空根塌陷需要导航瞬时窗口（`document.body` 为 `null`）
  才触发，**未复现**。该窗口现已可在 jsdom 里复现且有回归断言，但**真机仍需验证导航瞬间的实际结论**
  是「未开始」而非在页面内抛异常。
- [ ] 125.12 **（提交窗口缺失的真实表现）** [原 9.7.8] 「提交窗口缺失目前只表现为接管失败、不撕裂写入」
  这一结论依赖「写命令不做飞行中取消」的当前实现，未真机复现。

## 簇 126 — Facebook Native 残余对齐：反应计数、评论入口、列表面终局、同意条

> 来源 change：`restore-native-facebook-residual-parity` / `harden-native-engine-runtime-contracts`
> （均**活跃未归档**）。
> **2026-07-31 用户裁定**：真机项从各 change 的 `tasks.md` 移除、统一收拢到本文件。
>
> **前置环境**：dev + Facebook 分身。**多条需要跨界面语言采样**（中文 / 英文 / 越南语 / 西语），
> 采样时须能切换账号写作语言或界面语言。

- [ ] 126.1 **（反应汇总控件的跨语言形态）** [原 9.4 / 9.7.7] 采样四种界面语言下，首页卡与详情页的
  反应汇总控件是否存在、其 accessible label 与渲染文本的确切形态。判据：能否稳定抽到与页面显示一致的数字。
  **未核验前不得断言「所有版式都不存在带数字的中性按钮」**——若某些版式的中性控件文本含数字，
  读数会从恒零变成随机偏差，判据须相应收紧。
- [ ] 126.2 **（反应锚点实际选中了哪个控件·反向）** [原 9.4b] 同一批采样里还须记录反应控件锚点在各语言下
  **实际选中了哪个控件**。中文汇总标签因全角冒号被排除，但英文形态在词边界下**会**命中——若它先于中性控件出现，
  点赞执行器就会去按一个非切换控件。判据：逐语言记录选中控件是可切换的中性控件还是汇总控件。
  **这是既有状态、非迁移引入**；若采样证实命中，另起 change 修点赞定位。
- [ ] 126.3 **（运营机上的构建声明什么能力）** [原 9.5] 确认运营机上实际运行的桌面客户端构建是否声明就地定位能力
  （本机源码声明了它，但运行中的那个包声明什么是运行期事实）。这决定「切到详情面」的开帖指令在生产上
  是每次都发、还是结构不可达。
- [ ] 126.4 **（跳过原因分布）** [原 9.6] 在 dev 上核对 Facebook 自动评论链在修好读数之前的实际跳过原因分布，
  确认「整平台被判为不值得评」这一后果链在日志里真的成立（当前结论由代码路径推出，未在生产日志里实证）。
- [ ] 126.5 **（折叠态评论入口的展开时延）** [原 9.7] 观察折叠态评论入口在真实版式下点击一次后编辑框的出现时延，
  据此确认重探轮次预算足够，且一次点击不会把已展开的编辑框收起（旧预算基线：6 轮 × 700px、每轮 500ms）。
- [ ] 126.6 **（编辑器补入的不可见字符量）** [原 9.8] 采样编辑器在提交前实际会补入的不可见字符量
  （零宽字符、自动链接化、提及控件化），据此确认额外字数容差取值（旧值 4 的立论是「零宽/不间断空格无害、
  超出即 @提及污染」，采样应能判 4 与 10 哪个成立）。
- [ ] 126.7 **（非首页列表面到底的频率与恢复效果）** [原 9.9] 观察小组页 / 搜索结果页「本批看完」在生产上的
  实际发生频率与恢复效果，据此再定非首页面到底的恢复动作（回首页 / 换关键词 / 停）。
  同批须坐实：群 / 搜索面是否有对应「没有更多帖子」文案。
- [ ] 126.8 **（Unicode 兼容形 / 分解形·既有盲区）** [原 9.10] 反应与动作标签的 Unicode 兼容形 / 分解形
  不被识别。经复核这是**两代共有的盲区**，不是 Native 迁移引入的回归，只登记不修；真机判据是采样各语言下
  标签的实际 Unicode 形态并确认是否需要归一化。
- [ ] 126.9 **（同意条的真实形态）** [原 9.11] 采样 Facebook 同意条在各界面语言下的真实形态——它是不是
  对话框（还是非对话框的底部横幅）、接受按钮的可见文案与 aria-label、同文案接受按钮是否会并存多个、
  以及首页在同意条在场时是否同时挂着与同意无关的可见对话框。同批须核验一处**方向相反**的疑点：
  正文取样有 5000 字符截断，退役实现读的是完整正文，长页面上 cookie 文案若落在截断之外会判成「无同意条」
  并放行——这条**未经真机确认，MUST NOT 写成既成事实**。
- [ ] 126.10 **（滚动高度抗噪阈值）** [原 9.12] 采样 Facebook 三个列表面在「没有新内容」时滚动高度的实际抖动幅度
  与懒加载单批的真实增量，确认 100px 抗噪阈值在 Native 版式上仍是合适取值。判据：抖动幅度稳定低于阈值、
  懒加载单批增量稳定高于阈值；若两者重叠，须改用「连续 N 轮无增长」而非单纯抬阈值。

## 簇 127 — Native 产物门禁：打包态与 CI 编译器版本对账 —— **已废弃（2026-07-31 用户裁定）**

> **用户裁定：不打客户端安装包；需要「打一次客户端」才能验证的项目一律不做。**
> 本簇整簇正是那一类——它的三条（打 macOS 包验产物校验与冒烟 / 在 macOS 上打 Windows 包验报错指向真因 /
> 拿 CI 首次运行日志核编译器版本）**都以「打一次包」为前置**，前置不发生，验收无从谈起。
> 原条目全文存 git 历史（本文件 2026-07-31 提交），需要时按簇号回溯。
>
> **随之一并作废的立论**：原 127.2 曾被留作「跨平台打包弃守决定的复核依据」——
> 即「若真机发现它不是响亮失败而是静默发错包，弃守结论须推翻」。既然不打包、也不跨平台出包，
> **该复核不会发生，产物门禁 9.1–9.3 的弃守就此成为终局**，不再是「待复核的弃守」。
>
> **⚠️ 同日事实订正（别照上面那句字面读）**：废弃的理由**不是**「包永远不更新」——
> 用户澄清他**一直在手动打包测试其他功能，包在持续更新**。真正的理由是
> **不为这批工作产生任何「专门去打个包验一下」的待办**：他打包是为了测自己的东西，不是为了给这批做验收。
> 两个直接后果，别搞混：
> - **需要「装了新包的运营机」的那些簇（122 / 123 / 125 / 126）不是无限期挂起** —— 包在更新，
>   这批修复会随下一次出包自然进入运营机，那些真机项届时是可验的。
> - **本簇（127）仍然废弃** —— 它验的是「打包这件事本身」（产物校验 / 跨平台资源解析 / CI 编译器版本），
>   跟用户日常打包想测的东西无关。127.2 的 Windows 交叉打包与 127.3 的 CI 日志更是他根本不走的路径。
>
> **另有一条因此订正回待办、不在本簇**：产物门禁 5.5（打包态校验接受新字段）一度被同批判成弃守，
> 那是**误判**——它是打包态的代码路径、每次出包都跑，不是验收活；且 5.1 因顺序红线只加字段没抬版本号，
> 下一个动版本号的人会炸打包。详见该 change tasks.md §5 抬头。

> **保留一条给将来的人**：这三条不是「已验证无问题」，是「不打算验」。
> 哪天真要出安装包并想验打包链本身，**MUST 先回来看一眼本簇的 git 历史**——尤其 CLAUDE.md §5 记着的那条打包红线
> （子进程工作目录落进归档包会让核心进程根本起不来，只在打包态暴露、本地开发与单测结构上抓不到）。

## 簇 128 — 停手/续跑分界那一批：引擎诊断通路 + 群首帖恢复（登记于 2026-08-04）

**前置**：运营机装上**下一次出的包**（同 122 / 123 / 125 / 126 那条前置——用户日常打包会自然带上，
本簇不产生「专门去打个包」的待办）。另需一个可加入的 Facebook 群与一个在线边缘。

**为什么这一簇必须真机**：这两条 change 的全部改动都编进边缘引擎与注入脚本，**仅推送代码不生效**；
而且第一条要修的正是「诊断行到不了任何人眼前」，在装上新包之前，**第二条的任何行为调整都无法观测**。
⇒ **两条要在同一次真机 session 里一起验，顺序是先确认诊断行到得了日志，再看首帖行为。**

### 引擎诊断通路（`surface-native-engine-diagnostics`）

- [ ] 128.1 诊断行真的出现在运营机日志里——**尤其是成功路径上的那些**（命令正常返回、进程不退出）。
      这是本 change 存在的理由：此前那类行结构上不可达，「已经记账了」在生产上等价于「没记账」。
- [ ] 128.2 未识别类诊断行（panic / backtrace）的真实形态与量，据此复核每行与每命令两个上限是否设得对。
      上限本身是有界的且超限会响亮报出丢弃条数，但**具体数值是估的**，只有真机给分布。
- [ ] 128.3 指针拟人轨迹降级的真实发生频率。设计里「可达面很窄」是**推断**不是实测；
      若实际频率高于预期，该看的是预算分配本身，不是那条留痕。
- [ ] 128.4 通路打开后对**现存全部**引擎诊断输出做一次内容安全清查。
      起点：本批已修的群根那条（原本挟带两个原始 URL 路径）。**其余 `eprintln!` 载荷本批未逐条审。**
      判据是「只含引擎自生的、有界的、有限词表元素」；截断不算脱敏。
- [ ] 128.5 失败归因误报复核：宿主侧有一处按关键词匹配日志行来记「最后一条失败行」，
      转发进来的引擎诊断或 panic 若含那些词，会被当成失败原因显示给运营。
      今天无命中（诊断信封用的字段名不撞），但**每新增一条引擎诊断这个暴露面就大一点**。

### 群首帖恢复（`restore-facebook-first-post-recovery`）

- [ ] 128.6 **阻断态不被当成「没落地」重试**（本簇最重要一条）。加群后首帖路径上遇到验证码 / 登录墙时，
      必须按各自的终局停手并具名上报，**MUST NOT** 因为「地址不是群根」就归进姿态类去重试。
      实装中一度就是这样，作者自查发现并修掉——**它是本 change 要治的毛病的镜像，值得真机确认一次。**
- [ ] 128.7 落地等待真的挡住陈旧页面：上一段任务把浏览器留在同群某条帖详情页时，
      首帖路径不再在旧页面上选帖。
- [ ] 128.8 纠正预算复活后真的能救回来：一次姿态抖动后自救重跳一次并成功进入首帖。
      改造前这条分支静态不可达（准备阶段必然把标记置上）。
- [ ] 128.9 姿态类失败真的续跑而非终止：剩余下滚轮数被用掉，而不是第一次不顺就返回。
- [ ] 128.10 带跟踪参数的群链接不再无条件失败，且失败原因不再显示成「帖子身份无法确认」。

> **MUST NOT 把本簇任何一条读成「已验证」**——两条 change 都已 landed，但 landed 只代表代码写完。
> 本簇未勾的项就是至今无人在真机上看过。

## 簇 129 — 小红书会话真的开得起来，且开不起来时看得见（登记于 2026-08-04）

> 来源：change `restore-xiaohongshu-native-session-honesty`（edge `3b789d8` / `78b49fe` / `9a4093d` / `e6ea316`，已 landed）。
>
> **本簇整体的前提是重新打包的桌面客户端**：引擎是随包分发的 Rust 二进制，装机版 `0.3.25` 里的
> 那个二进制仍带旧准入。打包属用户显式触发（CLAUDE.md §6），故本 change 收尾不含出包——
> 也因此，**在出包之前本簇一条都验不了**。

### 引擎准入（A 段）

- [ ] 129.1 **小红书会话在运营机上真的开起来**：客户端起号后不再出现
      `NativePageEngineError: invalid session timeout`，并走到首扫上报。
      这是本 change 唯一一条「生产已停摆」的验收项。
- [ ] 129.2 **微信视频号的浏览器旁车同样恢复**。它走同一条非 Facebook 分支、同样被拒了六天，
      但它的失败此前更难看见（旁车是一次性登录用途）。
- [ ] 129.3 Facebook 侧无回归：会话超时与各命令预算不受本次改动影响（本地 cargo + 全量已绿，真机复核一次）。

### 启动失败的可见性（B 段）

- [ ] 129.4 **结构性失败真的翻红**：人为让引擎在门口拒收一次，确认外壳出现持久失败卡片、
      `edge` 轴离开正常，且**日志行清不掉它**。
- [ ] 129.5 **非结构性失败真的重试且有上限**：浏览器端点不可达时排队自愈，用尽后的措辞
      **仍不说「做不到」**，并且中途成功会把预算还回去。
- [ ] 129.6 **首扫失败后周期巡视仍在起拍**（改造前它排在首扫之后，一抛就整条不再武装）。
- [ ] 129.7 **云端侧目前收不到这条事实** —— 见下方「已知未闭合」。真机上确认一次：
      失败发生时云端**确实没有**任何角色/看门狗动作，以免日后误以为它已经接上了。

### 界面三态（C 段）

- [ ] 129.8 精选读失败 + 草稿为 0 时，首页标题**点名哪个分区读不到**，不再宣布「暂无待处理内容」，
      且与紧挨着的计数格子结论一致。
- [ ] 129.9 单边来源失败时，「最值得看的灵感」是**可重试失败**，不再显示讲赞藏准入条件的空态文案。
- [ ] 129.10 引擎会话没跑起来时，页面**不声称正在浏览**、脉动圆点关闭，且与诊断面板对同一状态的
      措辞一致（此前同一个 `ready`，诊断面板标「待任务」、内容页讲「正在浏览」）。
- [ ] 129.11 账号身份行显示为「小萝北（AI 助手） ｜ 账号 工程师大白（环境名）」这类形态，
      环境备注名不带 `@`、不冒充平台身份；待真昵称采到后应变为 `@<昵称>（平台昵称）`。
      **注意 129.11 的后半段依赖 129.1**：真昵称要靠云端派一次本人主页访问去采，
      而那要浏览闭环先跑起来——两者是同一条链。

### 已知未闭合（本 change 明确没做，登记备查）

- [ ] 129.12 **云端没有 `error` 这条边的消费者**：edge→cloud 的 `error` 落进云端 `handler.ts` 的
      `default:`、被回一条 `unsupported_type`。也就是说启动失败**确实离开了本进程、也到达了
      云端传输层，但云端没有任何角色或看门狗会因此动作**。要让云端据此升级需要一条新的
      edge→cloud 消息类型（协议四处同步 + 撞协议热点文件的单写者约束），另立 change。
- [ ] 129.13 `automationState` 仍会落在 `ready`：它在外壳 `deriveStates()` 里按「子进程在 + 云端已连」
      推导，运行姿态不参与。本 change 按「二选一」走了姿态通道（`edge` 轴 + 持久卡片确实会翻），
      没有改 `deriveStates`——那与 C 段 3.3 是同一条链，同时改必撞。
- [ ] 129.14 `automation_stalled` 在「重试中」窗口的措辞略强于事实（其定义写着「没有任何东西会再来
      重启它们」，而有界重试排着队时这句要打折）。当前把进度写进了原因串（`…:1/3`），
      严格贴合需要第五种运行姿态。
- [ ] 129.15 暂停不归还已消费的自愈预算（作废通道但不重置计数），预算只在一次真正成功的启动后归零。
      保守方向（防 pause/resume 循环变成无限重试）；若认为「操作员手动恢复 = 新的一趟」，口径需改。

> **MUST NOT 把本簇任何一条读成「已验证」**——change 已 landed，但 landed 只代表代码写完，
> 而且本簇在重新打包之前**物理上无法验证**。

---

## 簇 130 — 启动中途关闭：不再冒出没人认领的核心（登记于 2026-08-04）

change `cancel-in-flight-environment-launch`（edge master `b3d9978`）。**与簇 129 共享同一个前提：
必须重新打包桌面客户端才能验证**，在此之前本簇每一条都物理上无法确认。

真机形态（2026-08-04 20:10，58 环境车队，本机复现）：某环境启动流水线跑到一半时点关闭，客户端走了
「当前没有核心 → 无事可停」分支并宣布「浏览器已确认关闭」，9 秒后同一趟启动仍把核心拉了起来；
该核心连上云端跑了 13 分钟、原始日志零行、占住一个浏览器名额、界面永久停在「关闭中」。

> **2026-08-05 打包后冒烟**：用户重新打包并安装（`/Applications/AIDCP.app`，09:28；已核对包内确含
> `b3d9978` 与 `e903cf7` 两处标记、旧判据 `binding_untrusted` 已不在包中），观察结论是「看起来 ok」。
> 这是一次**整体冒烟**，不等于下面逐条已验——尤其 130.4 / 130.5 / 130.6 需要具体触发才看得到。

- [x] 130.1 多环境车队启动过程中，对某个**尚未拉起核心**的环境点关闭 → 此后不再出现属于它的核心进程；
      用 `ps` 对账（进程数 = 界面显示的运行中环境数），且该环境不停在「关闭中」。
      <!-- 2026-08-05 关闭：运营观察打包后未再复现滞留「关闭中」。注意日志里 `launch cancelled at
      commit point` 本轮 0 次 = 这一趟没有人在启动中途按过关闭，该取消路径**未被专门触发** -->
- [x] 130.2 对某个**已经拉起核心**的环境点关闭 → 走的是「有子进程」的关闭路径（发关闭指令），
      核心退出后名额归还、队列里下一个环境接着起。
      <!-- 2026-08-05 关闭：同上，运营观察未见异常；未逐次对账名额归还 -->
- [x] 130.3 关闭 → 立刻再点启动：新的一趟正常起来，没有两个核心并存、没有 edgeId 撞车。
      <!-- 2026-08-05 关闭：运营观察；未做「关闭后立刻启动」的定向操作 -->
- [x] 130.4 原始日志里能看到被取代代次的输出（`[superseded-generation]` 前缀）——这条是**可观测性**
      验收：本次事故 13 分钟零日志正是这一层缺失造成的。
      <!-- 2026-08-05 **有硬证据**：当日 edge.log 内 `[superseded-generation]` 46 行，此前同类窗口为 0 行 -->
- [x] 130.5 某环境超出 180s 就绪预算时，界面如实显示「已超预算 / 队列已放行下一个 / 仍在继续启动」，
      且**没有**被标成失败、错误或已放弃。
      <!-- 2026-08-05 按用户裁定关闭，**本轮未触发**：当日日志 `启动已超过` 0 次，即没有环境超出就绪预算 -->
- [x] 130.6 受限离场清理核心与无浏览器控制面 bootstrap 核心仍能正常起（准入闸没有误杀它们）。
      <!-- 2026-08-05 按用户裁定关闭，**本轮未触发**：当日无离场清理 / 控制面 bootstrap 核心启动；
      另 `launch admission denied` 0 次 = 兜底闸一次都没拒收过（符合预期，它本就该几乎不响） -->

### 已知未闭合（本 change 明确没做，登记备查）

- [ ] 130.7 **浏览器并发名额的统计口径没动**：一个卡住但确实持有浏览器的核心仍然永久占一格。
      放掉它的名额会导致超额开浏览器打爆内存，本 change 判定「占着是诚实的」。若日后出现
      「核心卡死但浏览器已不在」的真实样本，需要的是**证据驱动的名额回收**，不是超时即放。
- [ ] 130.8 `handleEdgeLogLine` 里的人设 / 发布审批 / 浏览器窗口回执与引擎命令诊断**仍受代次门约束**
      （本 change 只放开了原始日志留痕）。它们是按 id 关联的语义，改动会超出本 change 范围。

## 簇 131 — 未绑定账号的新环境不再被安全复核按停（登记于 2026-08-04）

change `stop-automation-only-on-authorization-loss`（edge master `e903cf7`）。**与簇 129 / 130 共享同一个
前提：必须重新打包桌面客户端才能验证**，在此之前本簇每一条都物理上无法确认。

真机形态（2026-08-04 20:13，58 环境车队，dev）：点「全部启动」后，新建 / 新导入的环境先显示排队，几分钟内
翻成「未启动」，同时浏览器按钮显示「开启中」。根因是每 4 分钟的会话维护复核把「云端还不知道这个环境登录的是
谁」（`binding_unknown`，58 个环境里 26 个）判成不可信，压回停止意图，且不撤在途启动、绑定成功后也不还原。

- [x] 131.1 一批从未登录过的新环境点「全部启动」→ 4 分钟后**仍显示排队中**（不再翻「未启动」），
      并按串行队列逐个起来、逐个在云端拿到账号绑定。
      <!-- 2026-08-05 **有硬证据**：当日 10 个环境起过核心、5 个完成云端握手，其中 3 个名字仍是
      `Facebook import 37/40/42`（= 启动时云端还不知道它是谁）。旧闸下这批正是 4 分钟内被压回「未启动」的 -->
- [x] 131.2 任一环境的自动化被判停时，**浏览器轴不得同时显示排队 / 开启中**；此后不再冒出属于它的浏览器。
      <!-- 2026-08-05 关闭：运营观察未再出现「未启动 + 浏览器开启中」的自相矛盾 -->
- [x] 131.3 后台把某个正在跑的环境从该客户名下撤走 → 最迟 4 分钟内该环境自动化停止，环境行显示
      「无权访问该环境」，且数据管理仍可用。
      <!-- 2026-08-05 按用户裁定关闭，**本轮未触发**：需在后台人为撤权才走得到 -->
- [x] 131.4 云端制造一次跨客户绑定冲突 → 该环境自动化停止，且文案与撤权**可区分**（不是同一句话）。
      <!-- 2026-08-05 按用户裁定关闭，**本轮未触发**：需人为制造冲突才走得到 -->
- [x] 131.5 拔网线 / 让客户接口短暂不可达，跨过一次复核 → **不得**有任何环境因此被停（读不到 ≠ 撤权）。
      <!-- 2026-08-05 按用户裁定关闭，**本轮未触发**：需人为断网跨过一次 4 分钟复核才走得到 -->
...
## 簇 134 — 新账号首次接入不再被人设竞态判死（登记于 2026-08-05）

change `recheck-session-start-gate`（cloud master `15af742` / automation master `d2d5896`，2026-08-05 部署 dev）。
**前置环境**：一个**从未绑过人设**的新 Facebook 环境首次上线（这正是日常批量建号的常规动作，不需要额外造场景）。

现场事实（2026-08-05 实测，dev，4/4 全中）：「自动补人设」与「握手开工」是同一个事件（环境绑定账号）触发的
两条并行分支——一条往接口域库写人设，一条读自动化进程的只读副本，读永远快于写落到副本。四个新账号
（`k1fd399h` / `k1fd39cj` / `k1fd39dv` / `k1fd39f4`）的补人设台账全部 `succeeded`，写入时刻与被判
`needs_persona_setup` 的时刻相差 0.1~0.8 秒，**四个全部被判死**、整机零动作近一小时。运营在客户端手动补设
人设也无效：单体时代那条「绑定成功 → 就地开跑」的回调落在接口进程，而接口进程拿不到自动化段的句柄。

本 change 只做「可复判」这一半：可恢复类拒绝不再落终态，改在同一连接内退避复判（2s→5s→10s→30s→60s 稳定）。
**桩测已覆盖判定与解除逻辑（6 条，含一次变异检验）；真机上尚未跑过一次完整的「被拒 → 补齐 → 自动开跑」。**

- [ ] 134.1 新建一个 Facebook 环境并首次上线：云端日志出现一次 `needs_persona_setup`（首次被拒，只此一条），
      随后出现 `此前被启动闸挡住（needs_persona_setup）→ 复判放行并就地开跑（无需边缘重连）`，
      且**无需任何人重连环境**；秒级内该账号开始浏览。
- [ ] 134.2 **反向确认无脉冲**：一个确实没有人设、也没有自动补齐来源的账号长时间在线时，
      `needs_persona_setup` 告警**只出现一次**，此后每分钟一跳全静默（不刷屏、不重复计「会话被拒」）。
- [ ] 134.3 **承重项**：把这类被挡住的环境直接关掉/断网，确认云端日志此后**再无**该账号的会话启动与命令下发
      ——复判绝不能活过连接（桩测已钉住，真机再确认一次，因为后果是对死连接发命令）。
- [ ] 134.4 运营手动补人设的路径：在客户端给一个被挡住的在线环境设置人设，确认最迟 60 秒内自动开跑。
      （注：这条**不是**「立刻」——把 ≤60 秒变成立刻需要接口进程→自动化进程的跨进程叫醒通道，另立 change。）

## 簇 135 — 客户端慢启动曲线表跟随后台配置（登记于 2026-08-05）

change `sync-slow-start-curve-to-client`（cloud master `6c165c6` / api `c80beb4`，2026-08-05 部署 dev；
edge master `d128a10` **未出包**）。**前置环境**：一个平台为 Facebook 的客户环境 + 管理后台「环境」页的
Facebook 全局运行策略编辑器 + **一份装了新代码的桌面客户端**（本仓默认不打包，需先显式出包）。

背景：客户端问号里那张「慢启动曲线限额」表原本写死在页面结构里（7 行、42 个数字），恰好等于后台出厂默认值，
所以运营没改过配置时看不出问题；改过之后运营看到的就是一份再也不会变的假表。现在改为只显示云端下发的
当前生效曲线。**云端侧已部署 dev 并做过结构验证（路由在、文件在、typecheck 0），但没有走通一次真实客户端请求**
——这一簇要验的正是那一段。

- [ ] 135.1 后台把 Facebook 全局策略的**总天数改成 10 天**并调整其中几天的上限，客户端重新读取该环境后，
      问号里的表变成 10 行、且改过的那几格数字与后台一致；脚注文案随之说「头 10 天」。
- [ ] 135.2 **反向确认不残留**：改回 7 天后表回到 7 行，不留下第 8–10 行。
- [ ] 135.3 点一下慢启动开关（开或关），确认表**不消失也不变旧**（写后回执带曲线这条的现场验证）。
- [ ] 135.4 切到一个小红书环境再切回来：小红书环境不显示这一行；切回 Facebook 环境时表来自本环境自己的读，
      不是上一个环境的残留。
- [ ] 135.5 **承重项**：把「搜索」那一列与后台配置对照——它是后台早有、客户端此前从未显示过的一项。
- [ ] 135.6 未绑定账号的 Facebook 环境上，徽章的总天数跟随后台配置（此前该分支写死 7 天）。

## 簇 136 — 消息巡视要走完三栏，而不是只看第一类（登记于 2026-08-05）

change `restore-notification-home-return`（edge master `04215b6`，**未出包**）。**前置环境**：一个有真实未读
消息的小红书分身（三栏里至少两栏有未读，最好是「评论和@」少、「赞和收藏」多）+ 一份装了新代码的桌面
客户端（本仓默认不打包；跑 `npm run electron:dev` 则起动时自动按源码摘要重建 Native 引擎）。

背景：「一类看完、回通知首页挑下一类」这一步，边缘 Native 实现导航到了信息流首页并回一屏卡片，
云端等的三栏重读永远等不到 —— 每趟巡视只处理优先级最高的那一类，且巡视永不收尾、该账号的浏览
被无限期按住。OL 2026-08-05 14:17 实测：侧栏未读 50 条，只处理了「评论和@」的 2 条，之后 9 分钟零命令。
本 change 只修边缘那条语义；**云端侧的巡视总超时兜底是另一条并行 change**，两者要分开验。

- [ ] 136.1 **承重项**：一趟巡视走完整条链 —— 评论和@ → 回通知首页 → 赞和收藏 → 回通知首页 →
      新增关注 → 三栏清零 → 回信息流续刷。云端日志里 `notification_triage` 应出现**多次**「选中分类」，
      而不是只有一次。
- [ ] 136.2 **回归守卫**：`notification_back_home` 之后浏览器仍停在通知页（不是 `/explore`），
      云端收到的是三栏读数、不是一屏卡片。
- [ ] 136.3 巡视结束后浏览**确实恢复**：`excursion_resumer` 打出收尾日志，随后有新的浏览命令下发；
      边缘任务租约不是靠 200 秒过期收场的。
- [ ] 136.4 **这条是本 change 无法在桩上验的假设**：「赞和收藏」「新增关注」两类**看一眼是否真能清零**。
      若某栏看完计数不降，分诊会在尝试上限内重试、到顶诚实放弃 —— 要确认它走的是这条路，
      而不是卡在原地或假报已清。
- [ ] 136.5 分类栏读不到时的诚实态：通知页加载慢 / 改版导致分类栏读不出来时，回执是「未确认」，
      **不是**一份三栏全 0 的读数（全 0 会被云端读成「三类都已清零」直接收尾）。此项难以自然触达，
      可在改版或弱网时留意。

## 簇 137 — 新账号首场就该进对浏览入口，进不去时要说得出为什么（登记于 2026-08-05）

change `recheck-facebook-primary-surface-pin`（cloud master `d5bf88d` / automation master `227280a`，
2026-08-05 部署 dev）。**前置环境与簇 134 完全相同**——一个新建的、首次真实上线的 Facebook 环境；
两簇建议**同一次上号一起验**，它们是同一条首次握手竞态的两半。

现场事实（2026-08-05 dev 实测，11/11 全中）：本场浏览面在会话启动那一瞬钉死一次，基线此刻问不到就
静默按 Feed 跑，此后整场不再重问。11 个「首次真实握手 → 被人设闸挡住 → 复判放行就地开跑」的新账号
**全部整场停在信息流**，而它们所属环境的配置全是 `reels`；同期基线可解析的老账号 4 场 4 场正确进 Reels。
更贵的是回落**零日志**：让基线短路的三条路径各自都带具名 blocker，但一条都到不了回执，
**那 11 例的真实原因事后不可回溯**。

本 change 做两件事：回落必须具名记账；`unresolved` 的会话挂一条有界复判（2s→5s→10s→30s→60s，
到顶记终态收摊），问到即就地改钉并重驱到正确入口。**桩测 8 条 + 三次变异检验已覆盖判定、记账去重、
纠正与解除；真机上「新号首场直接进 Reels」一次没跑过。**

- [ ] 137.1 **承重项**：新建一个主浏览入口为 Reels 的 Facebook 环境并首次上线，确认该账号**本场**就进了
      Reels——云端出现「主浏览入口为 Reels → 统一重驱到 Reels」，客户端指令名是「恢复 Reels 浏览」而非
      「恢复信息流浏览」。这正是 2026-08-05 那 11 例失败的地方。
- [ ] 137.2 **这条才是本 change 的真正交付物**：万一仍未进 Reels，云端必须出现一行
      「主浏览入口未解析（account=… blocker=…）」并**带出具体是哪一个原因**。
      拿到那个 blocker 值后再回来定第二步——今天排查到最后卡住的正是「三条路径都不出声」。
- [ ] 137.3 复判真的救回来过一次：出现「主浏览入口复判已问到 → 本场改钉 reels」，
      且随后**恰好一条** Reels 重驱（不是两条——重复下发意味着在途闸没起作用）。
- [ ] 137.4 **反向确认无脉冲**：一个基线长期问不到的账号，「未解析」那行**每场只出现一次**，
      复判过程静默，到顶只补一条终态行。
- [ ] 137.5 **反向确认既有保证仍在**：一个正常读到配置的会话，中途在后台把该环境的主浏览入口改掉，
      确认**本场不改**、下一场才生效（桩测已钉住，真机再确认一次，因为本 change 第一次让钉定成为可改的）。
- [ ] 137.6 若 137.2 拿到的 blocker 是 `unnamed_baseline_short_circuit`，说明有人给基线裁决加了早退却
      忘了具名——那是一条独立的修复，当场登记，不要就地猜它是哪一条。

## 簇 138 — 后台那几页真能用了吗，以及仍然给不出的那几块（登记于 2026-08-05）

change `restore-panel-capability-wiring`（cloud `7640251` / api `5e014ff` / automation `1d59e26` /
content `ab1b453`，2026-08-05 分四批部署 dev）。

现场事实：dev 在 2026-08-04 切三服务后，管理后台的 `/api` 由派生接口进程提供，而它的手写入口
装配面板能力清单时漏了 21 项——面板逐路由如实回 503，运营侧表现为七处功能整页「加载失败」，
另有两项不走路由、连 503 都不会给。本 change 把其中能诚实提供的全部接上，并加了一道启动期
装配对账门（少装一项且未具名即进程起不来）。**接口层面逐条打过；后台页面上的真人操作没走过。**

- [ ] 138.1 **承重项**：后台设置页能打开并**改一次模型**——选一个真实存在的模型保存成功、
      再随便打一个不存在的模型名，确认被挡下且提示是「模型不合法」而不是别的。
      这条同时验跨进程探活（探活跑在内容进程）。
- [ ] 138.2 角色页：角色列表与分类默认能出数、能各改一次并回真态；点「查看提示词」，
      确认收件箱类角色能看到真 prompt，浏览 / 发布类角色显示的是**「渲染器不在本进程」**
      而不是「该角色暂不支持预览」。
- [ ] 138.3 用量成本页、精选库页能出真数据；精选库能删一条并确认真删掉。
- [ ] 138.4 配额页的「热帖引流」块、FB 群评论策略：能读能改。
- [ ] 138.5 **FB 发帖图片上传**（本 change 唯一没在 dev 上真跑过的写路径）：传一张接近 10 MiB 的原图，
      确认成功。它要穿过 Nginx(16m) → 接口进程 → 内部通道(24 MiB 上限) → 内容进程，
      任何一跳没放宽都会表现成一次看不出原因的「上传失败」。
- [ ] 138.6 **验证码协助页**：dev 上该能力**没开**（`.env` 无 `AIDCP_CAPTCHA_ASSIST_ENABLED=true`），
      所以这条在 dev 上验不到。要验必须先开该开关并制造一次真实阻断；
      在此之前后台该页回的是「上游没有这条路由」，那是**属实**、不是故障。
- [ ] 138.7 **仍然给不出的六项**（都在接口进程的具名缺席表里，不是遗漏）：
      精选库行级动作（参照创作 / 定向评论，要发起发布管线与评论调度，且成功路径是
      fire-and-forget + 结果卡，跨不过一次请求应答）；角色提示词预览的另一半（浏览 / 发布 /
      配图角色的真渲染 + 账号口径）；另四项是有意不装（会话清单 / 旧版镜像健康 /
      FB 规则模式运行时 / 视频号内部配置 API 各有替代路径）。
      **确认后台上这六处的表现是「说得清楚为什么没有」，不是空白或转圈。**
- [ ] 138.8 **OL 同样受影响、本 change 没修**：OL 2026-08-05 12:59 也切了三派生服务，
      钉的是切流前的发布分支。实测 OL 的 `/api/config/model`、`/api/roles`、`/api/llm-usage` 同样 503。
      OL 部署须用户明确要求并走发布分支。

## 簇 137 — 看完一条笔记之后，闭环立刻续上（登记于 2026-08-05）

change `rescan-after-successful-back`（cloud master `022a6da` / 派生 `aidcp-automation` `02c644d`，
2026-08-05 16:00 部署 dev）。**前置环境**：任意一个在浏览的小红书分身。

背景：决策端只被「新一批卡片到达」推进。Native 迁移后小红书的返回只回动作回执、不再随附卡片
（Facebook 侧仍带），而云端那条「返回成功后必须自驱动续刷」的已上线要求**从未实装**——两条腿同时缺失。
表现不是报错，是每看完一条笔记就停在原地，直到 240 秒的空闲兜底把它踢一下；唯一能让循环前进的路径
反而是**返回失败**（失败有兜底滚动）。部署后已在生产日志里观测到闭环恢复连续，下面是需要人眼确认的部分。

- [ ] 137.1 **承重项**：连续看若干条笔记，确认每条看完返回之后**立刻**续上下一条（秒级），
      而不是隔几分钟才动一下。日志里每条 `back ok=true` 后面都应紧跟一行 `rescan_after_back`。
- [ ] 137.2 续扫是一次真滚动：确认它没有把闭环卡在同一批卡片上反复挑同一条笔记（续扫要能带来新卡）。
- [ ] 137.3 通知巡视进行中若恰好来一条返回回执，**不得**插入这次续扫（应看到暂停期的具名丢弃日志，
      而不是一条多余的滚动把巡视所在的页面滚走）。此项难自然触达，可在巡视窗口内留意。
- [ ] 137.4 **边缘侧缺口（本 change 未修，登记待收）**：小红书的返回只回动作回执、不再随附可见卡片，
      而命令清单声明的回执里**写着** `page.cards`。Facebook 侧产出、小红书侧不产出，两平台取并集后
      机械对账恰好通过——per-platform 的缺口被并集掩盖。与 `restore-notification-home-return`
      是同一族的迁移丢报，值得单独收一次。

## 簇 139 — 赞和收藏那一栏，别再被当成评论推去飞书（登记于 2026-08-05）

change `route-notification-items-by-category`（cloud master `9e5b000` / 派生 `aidcp-automation` `f2c680e`，
2026-08-05 16:15 部署 dev）。**前置环境**：一个「赞和收藏」或「新增关注」有真实未读的小红书分身
（最好未读多一些，二十条以上），走一趟自然的通知巡视。**与簇 136 可同一次上号一起验**——那一簇要的正是
「三栏都走到」的现场，本簇要看的就是走到赞收藏那一栏时云端的反应。

背景：上报通知条目的那条通道是**共用**的——赞收藏 / 新增关注两栏在「已看过」回执之外顺带回传一批发送者
条目（喂通知联系人名册用），与评论栏抽取的评论项走同一条。云端此前把到达的每一批都当评论抽取结果，
原样跑分类 → 去重 → 发飞书。真正拦住那批赞/收藏/关注的只有分类器里一条按正文文案写的正则；正文一旦
不长成那几种固定说法（平台改文案 / 多语言 / 正文位取到笔记标题），它就会被推去飞书 —— 而已上线规格写的是
「点赞、收藏、新增关注 MUST NOT 发飞书」。dev 2026-08-05 15:29 实测（账号 `63e2ff05…49ce`）：分诊
从没选过评论栏，日志里却出现「分类：0/20 条值得通知」和「分类 comments 处理完，返回通知首页」，
那 20 条恰好全被文案正则滤掉才没误发。判据现已改成云端**自己点名的那一栏**，不依赖正文、不新增协议字段。

- [ ] 139.1 **承重项**：巡视走到「赞和收藏」时，云端日志应出现一行
      `本批 N 条非「评论和@」栏（category_not_comments，云端选中分类=likes）→ 不进评论管线`，
      且**没有**「分类：…条值得通知」「分类 comments 处理完」这两行。飞书零打扰。
- [ ] 139.2 **红线自检（最容易做错的一处）**：赞收藏那一类**仍然收尾**——日志里
      `分类 likes 处理完，返回通知首页` 恰好一次、`notification_back_home` 恰好一条。
      去掉重复那条之后一次都收不了尾，会让巡视卡在这一类上。「新增关注」同理。
- [ ] 139.3 评论栏**真的空**时仍正常收尾：选中评论栏且没有新评论时，应看到「分类：0/0 条值得通知」+
      「无新项…本类收尾」+ 「分类 comments 处理完」，与 139.1 那条日志形态明显不同（三态不压成一态）。
- [ ] 139.4 联系人名册**没有因此少收**：赞收藏 / 新增关注那批发送者仍照常写进通知联系人流水
      （拒的是「进评论管线」，不是「到达」）。桩上验不了这条——名册那条订阅在组装根里、依赖真库。
- [ ] 139.5 **交叉校验打架的现场**（难自然触达，留意即可）：若出现
      `判据不一致：云端选中分类=… 但本批 N 条的类型…` 这行日志，记录当时那一栏与条目类型的实际组合。
      它意味着执行端推导的类型与云端点名的栏位对不上，是值得单独查一次的信号。

## 簇 140 — 工作面板要认得账号「正在做的事」，而不是只认创作任务（登记于 2026-08-05）

**前置环境**：一个已绑账号、能正常浏览的小红书环境（客户端桌面版，价值首页可见）。
**与簇 134 / 137 无关**，可单独在任意一次日常上号时顺带验完；不需要新建号，也不需要 Facebook。

背景：价值首页那张「实时工作过程」面板此前只接创作侧证据（调整中的草稿、发布队列任务），
账号在小红书上浏览、点赞、找灵感一条都不进面板，所以只要没发起创作，一个正在跑的账号面板恒为空态，
写着「当前没有正在进行的任务」。证据其实一直在客户端里（核心进程按真实事件发出的活动条目，
运行面板早在消费），本次把它从同一个路由点接进了面板。同批还修掉了「创作队列读取失败被显示成
没有任务」这一处三态压一态。桩上能验的都验了（35 个用例）；真机要看的是**密度与可读性**——
这条流的节奏由真实浏览决定，桩里造不出来。

- [ ] 140.1 **承重项**：账号正常浏览一段时间（≥5 分钟）后看首页面板，条目按发生时间从上到下累积，
      最新一条在最下且带进行中标记；条目内容是人话句子 + 相对时间，**没有**阶段名、完成勾或完成度。
- [ ] 140.2 **密度**：面板高度上限 255px 未被撑破、页面不出现横向滚动；条目多于可视区时由面板内部滚动承载。
      记录一次自然浏览下大约多久积满 12 条（当前上限），据此判断这个上限是否需要调整或加「查看更多」。
- [ ] 140.3 **停下之后**：暂停或关闭环境，面板里已有条目仍在、但最新一条不再带进行中标记与动效，
      左栏也不再讲「正在浏览」。切到另一个账号再切回来，面板显示的是**当前账号**的记录，不串号。
- [ ] 140.4 **读不到那一档**（难自然触达）：若遇到云端不可达 / 发布队列读取失败，面板应显示
      「暂时读不到当前进度」并给「重新读取」，**不得**显示「当前没有正在进行的任务」。
      能自然遇到就记一次现场；遇不到不强求。
- [ ] 140.5 **顺带看一眼**（同批修复，不属该 change）：环境内「我的安排」页的「打开浏览器 / 启动当前环境」
      两个按钮现在应跟随真实状态——浏览器开着时显示「收起浏览器」并能真的收起；环境已启动但还没跑起来时
      显示「关闭当前环境」而不是再给一次启动；启动中 / 开启中如实显示并禁用。

## 簇 141 — 身份落到「不知道浏览器里登着谁」时，那条自救通道到底通不通（登记于 2026-08-05）

**前置环境**：一台能构造出「运行期身份读不出来」的真机节点（换 cookie / 页面被顶下线 / 切成另一个账号皆可）。
**必须是打包后的桌面客户端**，不是本地源码跑——本簇要验的正好包含「修复有没有随构建到达运营机」。

背景：change `align-cloud-edge-operation-registries` 修掉了这条通道上**两道各自独立的静默堵点**：
云端登记表缺这两条命令（出口闸判 `operation_unclassified`、投递数返回 0），以及边缘入口路由白名单
也缺这两条（信封落到「其他主动消息暂忽略」被丢弃）。两处都已 land，dev 上云端侧实测已通（`sent=1`）。
**但通道可达 ≠ 终局能自救**：从没有人在真机上让一个节点真的掉进那个终局、再看它爬不爬得出来。
桩里造不出这个终局——它需要一个真实浏览器停在一个身份认不出来的页面上。

- [ ] 141.1 **承重项**：构造一个身份读不出来的节点，确认云端下发身份读取后**边缘有回执**
      （`identity.observed`），而不是 20s 静默超时。这是本次两处修复的端到端证明。
- [ ] 141.2 **终局能不能解开**：身份读回来之后，节点是否真的退出「拒绝一切代表该账号的动作」这个状态、
      恢复正常派活；还是读到了也解不开（那就是另一处缺口，当场记档）。
- [ ] 141.3 **读不出来那一档**：若身份确实读不出来（页面就是认不出），回执必须是**具名的读不出来**，
      MUST NOT 与「命令没送到」「边缘没装到」压成同一个原因值——这三态在本次修复前是不可区分的，
      正是它让这条缺陷躲了至少三天。
- [ ] 141.4 **昵称那一支顺带看**：`nickname_enricher` 是同一条命令的另一个消费方。通道通了以后，
...
## 簇 143 — 那张只进不出的消息表，真的开始出了吗（登记于 2026-08-05）

**来源**：change `bound-event-outbox-growth`（已部署 dev；OL 未部署）。
**前置环境**：dev ECS，直接查 automation 属主库 + 看 `aidcp-api` / `aidcp-automation` 的 journal。
**为什么必须留在这里**：下面三项都是**时间尺度上的**验证 —— 部署当时只观察了几分钟，
而它们要问的事分别需要几小时、几十分钟、一次重启才看得出来。
部署那天记的「已验」只覆盖了「立刻能看见的那部分」，MUST NOT 读成这三项也验过了。

- [ ] 143.1 **存量真的排空了**：`sync_read.changed` 在 dev 上的存量约 8 万行，
  剪裁按 2000 行 / 10 分钟推进 ⇒ 约 6.7 小时。部署后隔一个工作日再查一次：
  该主题行数应降到保留期（1 小时）以内的量级，而不是停在某个中间值不动。
  **停住不动 = 剪裁被某条消费者游标挡住了**，那时要去看 `event_outbox_topic_cursor`
  而不是加大批量。
- [ ] 143.2 **「镜像非 fresh」那条告警真的不再刷**：修之前 `aidcp-api` 每分钟一条
  「全局周活跃掩码镜像非 fresh（state=stale），本次按『未配置』处理」。
  需要一个**够长**的观察窗（原频率约每分钟一条，三五分钟的静默与偶发区分不开）。
  **验收时 MUST 分别确认两件事**：告警消失了 / 而且不是因为把陈旧判据改松了
  （判据一行未动，改的只是刷新周期 30s → 10s —— 这一点代码面已确认，
  真机上要确认的是「续期确实接上了」，即镜像长期停在 fresh）。
- [ ] 143.3 **跨仓漂移守卫会不会误报**：新加了一条 warn ——
  收到的信封新鲜期窗口 < 3 倍刷新周期即具名告警，每条流只说一次。
  正常情况下它应该**一条都不出**。若真机上看到它，先别当成 bug 修掉：
  它说的是属主侧窗口变短了，那是真问题。

**同批登记的两条非本 change 造成的事实**（放这里免得随会话丢失）：

- [ ] 143.4 **dev 上三个派生服务的 systemd unit 都是 `disabled`**
  （`aidcp-api` / `aidcp-automation` / `aidcp-content`）。当前 active，但**重启机器不会自启**。
  这不是本 change 引入的，也不影响当前运行；但它意味着一次意外重启会让 dev 整体静默不在。
  要不要 enable 是运维决定，先登记。
- [ ] 143.5 **OL 仍跑着旧构建**，`automation_config_mirror_health` 那条流在 OL 侧
  继续按每 target 每 10 秒一条增长（部署当时实测 ol 代数仍在涨，正好当了对照组）。
  两侧共用同一个生产库，所以那张表**仍在长**，只是速率减半。
  OL 部署须由用户明确要求、且从 release 分支走（§5）。

---

## 簇 144 — 全局策略合并之后，那两批账号当天到底跑成什么样（登记于 2026-08-06）

**来源**：change `unify-facebook-global-policy-across-targets`（dev + OL 均已部署，迁移 2026-08-05 16:28 应用）。
**前置环境**：真实在跑的 Facebook 车队 + 两边后台；受影响对象逐个列在
`openspec/changes/unify-facebook-global-policy-across-targets/migration-record.md` §4。
**为什么必须留在这里**：下面这些都要**跨天**才看得出来 —— 合并当天只能证明"配置合上了"，
证明不了"合上之后账号的实际用量是不是设计预期的那样"。

**先读懂一件事再验**：清单里有 **9 个对象同时被两种变化命中**
（OL 侧 7 个 / dev 侧 2 个，见 §4 交集）。它们当天加群次数上升，
**既可能来自毕业（限额放宽）、也可能来自阈值 5→2**，观察面区分不了。
要归因就只能挑非交集样本比对，MUST NOT 拿交集样本下结论。

- [ ] 144.1 **「立即毕业」那批的当日上限真的等于安全限额，而不是冷启动曲线**。
  OL 视角 22 个环境、dev 视角 3 个（清单在 §4）。tasks 8.2 已勾，但那一勾来自用户目视
  「看起来 ok 了」，**没有人独立复核过当日上限的实际取值**。这里要的是：挑几个非交集样本，
  看它当天各动作总量是顶在安全限额上，还是仍被 5 天曲线夹着。
- [ ] 144.2 **加群阈值 5 → 2 之后，OL 侧加群频率是不是成比例上升，且没有过频**。
  只看非交集样本（OL 侧 21 个消费模式账号里剔掉那 7 个）。
  **若观察到过频，处置是只回滚这一格**（`consumption.confirmedLikesPerJoin` 改回 5），
  不必回滚整次合并 —— 这是设计里就留好的退路。
- [ ] 144.3 **回滚窗口在持续变窄，且已经开始丢东西**。
  旧 `dev`/`ol` 两行自 2026-08-05 16:28 起冻结；合并后合并行已被编辑两次
  （08-06 09:46 / 09:47，净变化 `reels.slowStart.viewsPerLike` 8 → 6）。
  **现在回滚就会丢掉这一格的人工编辑**，且丢失面只会单调变大。
  硬截止 = 收尾 change `collapse-facebook-global-policy-target-column` 删旧行之时。
  本条不是"要去验的事"，是**一个到期前必须有人做决定的事**：
  要么在删旧行前确认没人还指望回滚，要么就别删。
- [ ] 144.4 **跨目标可见性在真实自动化节奏下到底多久**。
  与 tasks 8.3 是同一件事的两个尺度：8.3 问"通不通"，本条问"平时是多少"。
  **8.3 已于 2026-08-06 实测通过，双向都是亚秒级**（dev→OL 794ms，OL→dev 1292ms，均未重启）。
  但那是**空载时刻的两次采样**，本条要的是真实负载下的分布。
  **附带一个待澄清项**：tasks 1.1 记录的机制是"接口进程没有周期性重读，节奏由自动化进程来拉决定"，
  这条描述预测不出亚秒级。要么消费者名册通路比 1.1 描述的直接，要么自动化拉取远比当时判断的频繁。
  **在有人把这一点搞清楚之前，MUST NOT 拿 1.1 的措辞当时延上界去引用。**

**同批登记的一条非本 change 造成的事实**：
...
## 簇 145 — Reels 翻页确认放宽之后，四成"没确认"到底掉到多少（登记于 2026-08-06）

**前置环境**：真实 Facebook 分身 + Reels 主浏览入口 + 云端在跑（OL 或 dev）。
**源 change**：`converge-facebook-reel-navigation-confirmation`（edge `242961d`）。

**这次改了什么（一句话）**：翻页之后判"翻动了没有"，从要同时满足十个条件收敛到四个；
删掉的六个里最关键的一条是"卡片侧身份必须等于探针侧身份"——那两侧其实是同一个函数读了两次，
要求它们相等抓不到任何来源分歧，只会把"刚翻完页、画面还在动"的正常采样判成失败。

**为什么只能在真机上验**：这些条件是被真实页面的中间态咬到的，桩测里造不出那个中间态
（造得出的部分已在单测里锁住了）。

- [ ] 145.1 **确认率是不是真的上去了**。基线是 2026-08-06 本机一天的数据：
  `688 confirmed / 501 ambiguous`（约四成没确认）。
  复验口径：装了新引擎的客户端跑一段 Reels 浏览后，数客户端日志里
  `command=page_scroll effectPhase=confirmed` 与 `=ambiguous` 的比值。
  **看的是比值变化，不是绝对数**——浏览时长不同，绝对数没有可比性。
- [ ] 145.2 **放宽之后有没有把不该认的认了**。具体要看的是：
  确认成功之后上报的那张卡，其身份是不是一条真实的、当前正在看的 Reel；
  有没有出现"确认了翻页，但报上去的是上一条"这种错位。
  这条比 145.1 重要——145.1 变好是预期，145.2 出问题才是这次改动的真实风险。
- [ ] 145.3 **真卡住的时候，回执现在能不能说清"卡在哪一条"**。
  制造方式：让 Reels 停在同一条上（网络卡顿或页面失焦都可能自然触发）。
  预期：回执原因仍是 `reels_navigation_unconfirmed`，但**这次带上了当前 Reel 的完整链接**；
  若读不出身份则原因应为 `reels_identity_unresolved` 且不带链接。
  **两者必须能分开**——分不开正是这次要消灭的那个二义。
- [ ] 145.4 **多卡批次这条分支在真机上是不是真的不可达**。
  代码分析结论是 Reels 面的读取器结构上只产 0 或 1 张卡，所以删掉"卡片数必须为 1"不会放进多卡；
  单测里只锁了"真出现多卡就取第一张"的行为。若真机日志里**真的**出现过 Reels 面多卡上报，
  说明这个结构性判断有洞，要回头看——**注意云端 spec 对多卡是 fail closed 不记 view**，
  即会出现"边缘认为确认了、云端不记账"的分歧。

**明确不在本簇范围**：云端收到"未确认"之后无上限重发、以及会话动作数攒到 60 就结束再自动续场
所造成的空转（2026-08-06 实测：一个账号 50 分钟里发了 175 次翻页、浏览只涨 2 次，
并反复触发十余次"动作数已达上限"）。**那是下一层，本 change 明确没动。**
本簇即使全部验过，那个空转依然存在——只是被触发得更少。

### 145.5 —— 上两层暂不修，是 2026-08-06 的显式决定，不是遗漏

云端那两层（收到"未确认"就无上限重发；空转把会话动作预算烧到 60 后自动续场）
**已定位、已量化、经用户裁定「先不做，观察后再说」**。这里登记，是为了让后来者读到
「空转」时知道它**不是没人发现**，也**不是修了一半**。

当天量化事实，作为日后评估的基线：

- 账号 61574530091255：11:43–12:20 的 37 分钟涨 80 次浏览；12:20–13:14 的 54 分钟发了 175 次
  翻页、只涨 2 次浏览。**效率约为前段的 1/40，全程系统认为自己在正常工作。**
- 全车队当天 28 次会话结束，主因即"动作数 60 已达上限"——**失败的翻页同样计入会话动作预算**，
  所以这个计数当天不能当作"跑得怎么样"的依据。
- 空转期间**无任何告警**：它不产生浏览（不吃浏览配额）、无新内容（不吃模型调用），
  因此按用量做的监控一概看不见。当天是人盯着客户端界面发现的。

**若日后决定要做，两条不要重新推导的结论**：

1. **主体是"无上限重发"那一层**：连续未确认到阈值后升级，而不是无脑再翻。升级建议分两档——
   先一次有界自愈（回首页重进 Reels，该路径已存在且本来就带每场 2 次上限），
   不行才结束会话，**且结束原因必须区别于"正常跑完"**，否则会话统计继续被污染。
2. **MUST NOT 单独修"失败翻页不计入动作预算"那一层**：动作数攒到 60 是目前**唯一**能打断空转的
   机制。只把失败翻页移出计数，空转会从"每约 18 分钟被强制打断"变成"一直转到会话时长上限"，
   **反而更坏**。它只能作为上一条的配套，或者干脆不动（装上上限后它就无关紧要了）。

**未决问题**（当时提出、未定）：升级到顶之后是就此结束会话，还是另外把账号标成需要人看。
只结束会话意味着页面问题若没自愈，下一场照样翻不动，只是循环变慢；要真正止损需要一条告警通道，
那超出"修重试上限"的范围。

## 簇 146 — 一台机器上卡住的环境，另一处到底看不看得见（登记于 2026-08-06）

**前置环境**：真实分身 + **重新编译装机后的桌面客户端**（关键，见下）+ dev 云端在跑 + 能打开管理后台或直连面板 API。
**源 change**：`report-host-standby-decisions`（edge `000f970` / cloud `322472a` / automation `7791de9` / api `7a4c6f3`）。

**这次改了什么（一句话）**：宿主层每一次「让不让出浏览器槽位」的判决，现在会作为**只读回执**
上报云端，运营在另一处就能看出「哪台机器上的哪个环境卡了多久、为什么」。
在此之前这件事只有坐在那台机器前的人看得见，而车队是跨机器的——2026-08-05 那次故障
（一个环境连续 32 分钟拒绝让位、锁死一个浏览器槽位）在运营侧是零证据的。

**为什么只能在真机上验**：链路是「桌面外壳判决 → 核心子进程 → 云端」三跳，
第一跳发生在 Electron 外壳里，桩测造不出真实的槽位争用与真实的连续拒绝。
云端两端已在部署当天实证（内部只读路由与面板端点都回真值、非 404、非兜底），
**缺的正是「真有边缘在发」这一段**。

**⚠️ 一条会让人白等的前提**：边缘侧要到**客户端从源码重新编译并装机**之后才开始上报。
而且 **MUST NOT 用版本号判断装的是哪份代码**——本仓的客户端会被重新编译装机而不抬版本号。
判据只能是「面板那条端点开始出现非空列表」。

- [ ] 146.1 **有边缘在发**。装了新构建的客户端连上 dev 后，`GET /api/host-standby-decisions`
  应从空列表变成至少一条记录，且记录里的机器标签与环境标识对得上那台真机。
  空列表 ≠ 没有环境卡住 —— 也可能是没有一台装了新构建的客户端在线，两者必须分清。
- [ ] 146.2 **卡住的那个真的一眼可辨**。制造方式：让某个环境在有任务租约在跑时收到可让位提示
  （或直接等它自然发生）。预期：该行 `verdict=refused`、带具名原因、`refusedCount` 随时间增长、
  `refusedForMs` 单调变大、`stuck=true`。**单次拒绝是正常运行的一部分**，它的 `stuck` 必须是 false——
  两者要是长得一样，这次改动就等于没做。
- [ ] 146.3 **让位那一刻立刻可见**。环境真让出槽位（浏览器关闭进入冷待机）时，
  该行应**立即**翻成 `verdict=yielded`、`refusedCount=0`，不该等到下一个节流点。
  这是本 change 唯一一条「不受节流约束」的规则，也是最容易被节流悄悄吃掉的一条。
- [ ] 146.4 **上报失败不影响让位**（负向）。制造方式：让客户端与云端断开一段时间，
  期间该环境该让位还是照常让位、该拒绝还是照常拒绝，客户端日志里只有一行
  「回执未送达云端……只影响可见性」，**没有任何**因此改变的准入结果。
  回执一旦变成让位的前置条件，一次云端抖动就会让整批环境停止让出槽位。
- [ ] 146.5 **旧客户端静默共存**（负向）。同时有一台**未重新编译**的老客户端在线时：
  它不上报、云端不报错、它的让位行为一切照旧，面板列表里只是没有它那一行。

---

## 簇 147 — 分行列删掉之后，那份唯一策略在真车队上还认不认（登记于 2026-08-06）

**来源**：change `collapse-facebook-global-policy-target-column`（dev + OL 均已部署，
迁移 `0114` 于 2026-08-06 16:40 应用；执行细节见该 change 的 `migration-record.md`）。
**前置环境**：真实在跑的 Facebook 车队 + 两边后台 + 一次真实的策略编辑。
**为什么必须留在这里**：当天能证明的是「读数没变、写路径跑得通、库层面拦得住第二行」；
证明不了的是「运营真去改一次、真车队隔天真按新值跑」。

- [ ] 147.1 **从后台真改一次全局策略，两边都跟着变**。
  当天只做了**只读**核对与一次 `ROLLBACK` 掉的写路径演练——两者都没真正推进过 `revision`。
  要的是：在任一侧后台改一格（例如 `consumption.viewsPerLike`），确认
  ① 写成功且 `revision` +1；② **另一侧后台不重启就能读到新值**；
  ③ 审计表新增一行且其 `execution_target` 是合并后的那个值（该字段现在是纯追溯字段）。
  **这条特别要验**，因为收缩把 UPDATE 语句的参数号整体重编了一遍，
  而仓内 DB 相关单测全用假 pool、证明不了参数号与真 schema 对得上。
- [ ] 147.2 **冷启动毕业事实按环境唯一之后，车队行为不变**。
  完成事实表主键从 `(env_key, 运行目标)` 收敛到 `(env_key)`，行数 122 → 49。
  隔天看一批已毕业环境：当日上限仍顶在安全限额上，没有哪个被重新关回冷启动曲线。
- [ ] 147.3 **群评论那 72 小时窗口在真链路上仍生效**。
  该策略表这次由「按目标一行」变成真正单行。要的是真发生一次入群 → 首评的等待判定，
  确认读到的仍是 72 而不是默认档 24。
- [ ] 147.4 **下一次从旧发布分支发版会撞上什么，验一次再写进流程**（负向，可在测试环境做）。
  收缩之后，任何**不含本 change** 的构建发到任一环境都读不到这三张表，
  且**不会报错退出**——它只是读不到策略。当天在 OL 上已经看到这个形态的真实样本
  （旧构建 + 新库 = 每 5 秒一条缺列报错、镜像停在旧版本不再前进，持续 11 分钟）。
  要的是把「发布分支准入」做成机械闸而不是靠记得：
  1.2 那份「MUST NOT 再发」的分支清单目前只是一段文字。

**同批登记的一条必须有人拍板的事（承簇 144.3，那条到此到期）**：

- [x] 147.5 **簇 144.3 的回滚窗口已到期并已关闭**。
  该条写着「硬截止 = 收尾 change `collapse-facebook-global-policy-target-column` 删旧行之时」。
  **2026-08-06 16:40 旧行已删**（`gp` 2 行 / `gcp` 1 行 / `slow` 73 行）。
...
- [ ] **自动恢复端到端（默认 browse_only 也生效）**：找一个 restricted 已满 72h 的存量账号，
      ...
      部署当天首扫可能成批恢复存量受限账号——这是预期行为不是事故，但要人工确认恢复的都是该恢复的
- [ ] **full_pause 端到端一趟**（需先执行迁移 0116 + 后台切模式）：受限账号会话内不再开下一篇 →
      待机提示带真实恢复时刻（非回访）→ 边缘关浏览器让出槽位 → 到 wakeAt 或状态翻转后唤醒归队
- [ ] **验证码一票否决实测**：账号因阻断弹窗升级 restricted 且验证码暂停未解除时,浏览器绝不被收走
...
## 簇 154 · 代理抖动后环境自己起得来吗（bound-proxy-preflight-retry，2026-08-07）

> **前置环境**：装有本次改动的桌面客户端（edge master `12b3283` 之后出包）+ 一个配了认证代理的
> Facebook AdsPower 环境，浏览器处于未运行状态。需要能人为制造链路瞬时不通（拔网线 / 关代理客户端
> / 临时封端口都行，关键是**十几秒内恢复**）。

- [ ] **抖动后自愈**：环境点「启动」的同时切断代理链路，约 10s 后恢复。预期：本次不起浏览器、界面显示「代理不通：20s 后重新排队重试（第 1/2 次）」，随后自行重排并真的起来——全程不需要人再点一次。
- [ ] **重排真的重探**：上一条发生时确认第二次是**真发了探测**（不是复用 2 分钟内的旧失败结论直接再失败）。观察点：两次失败之间隔了完整的探测窗口，而不是瞬间连续两条失败。这条是本次改动的核心，也是单测覆盖不到的那一格（见 tasks.md 偏离 C）。
- [ ] **预算耗尽的措辞**：让链路持续不通（不恢复），确认重排 2 次后终结，且回执是「代理重试预算耗尽（已探测 3 次）」——**不是**「代理不可用」。运营要能从这句话看出「链路可能还在抖」而非「配置错了」。
- [ ] **配置类失败不重排**：把代理密码改错后点启动，确认**当场**报「代理账号或密码未通过认证」，不进重排、不等 20s。同理试一次代理类型配错。
- [ ] **排队不插队**：多环境批量启动，其中一个代理不通。确认它重排后排到队尾（后面的环境正常先起），且并发计数闸满时它同样被拒、如实显示排队已满。
- [ ] **取消闸**：某环境处于「20s 后重试」等待中时点停止 / 暂停 / 移出，确认到点后**不会**被重新拉起来。
- [ ] **唤醒退避那一跳**：冷待机环境因代理不通唤醒失败后，确认 60s 后的第一次重试是**真的重探**（改动前这一跳会复用旧失败、一个探测都不发，真正的第二次探测要等到约 3 分钟）。
- [ ] **回滚旋钮**：设 `AIDCP_PROXY_PREFLIGHT_REQUEUE_MAX=0` 重启客户端，确认行为与改动前逐字一致（确定失败当场终结、文案是「自动化未启动：…请修改代理后重试」，没有任何重排字样）。
...
## 簇 157 · 阻塞滞留自动让出槽位真机验收（release-browser-slot-on-stalled-blocker，2026-08-10）

> **前置环境**：需**新出一次 edge 桌面安装包并装机**（云端侧已于 2026-08-10 部署 dev，但该步行为零变化；
> 旧客户端不认识新的具名原因、按「未具名不计时」处理，故装机前本簇一项都验不了）。
> 秒级回滚：客户端设置或环境变量 `AIDCP_STALLED_BLOCKER_EVICTION=false`。

- [ ] 157.1 **红线项**：卡在验证码/弹窗上的环境，滞留满 15 分钟后确实关闭自动化并让出槽位；
      界面留下写明「哪个号、什么阻塞、从几点起、槽位已让出」的持久卡片，且该卡片不被后续状态刷新冲掉。
- [ ] 157.2 被自动关掉的环境**不会自动重启**、也不出现在等槽位队列里；必须运营手动启动才回来。
- [ ] 157.3 腾出的槽位确实被等待队列的下一个环境用上（不是空腾）。
- [ ] 157.4 未绑人设的环境同样在 15 分钟后让出槽位，卡片写明是「尚未绑定人设」。
- [ ] 157.5 **爆炸半径反证**：运营执行一次全局停机（或使用未配续场的部署），确认**不发生集中关闭**——
      这几种原因是全体环境共有的，照单终止即整机清场且无受益人。这一项验的是白名单没被写成黑名单。
- [ ] 157.6 断连重连后不得立刻终止：断开云端超过 15 分钟再连上，第一跳提示 MUST NOT 直接关掉环境，
      须从重连后重新计满门槛。
- [ ] 157.7 运营处理完阻塞、手动重新启动该环境后，持久卡片消失；若阻塞其实仍在，云端 SHALL 再次呈现它。

## 簇 157 · 节奏概率模式真机观察（facebook-cadence-probability-mode，2026-08-10）

> **前置环境**：纯云端 + 后台改动,已部署 dev（kernel v0.1.5 / api d63f0c3 / automation 2445d49 / console a6ff45c，2026-08-10）。不需要客户端发版——节奏判定全在云端。
> 机制：全局开关 `cadenceMode`（固定次数 / 概率触发）统管全部 11 个「N 次 A→1 次 B」数值。fixed 逐字零回归;probabilistic 每次合格 A 事件独立掷 1/N。默认 fixed。

- [ ] dev 后台把节奏模式切到「概率触发」并保存,确认 GET 回读 revision+1、automation 侧下一次同步读到 `cadenceMode=probabilistic`（日志/行为）。
- [ ] 用真实 Facebook 账号观察一段:Reel 点赞/关注、规则模式起批、消费链点赞→加群→评论的触发点不再固定在第 N 个,长期 B/A 比率趋近 1/N（允许连续不中与相邻命中）。
