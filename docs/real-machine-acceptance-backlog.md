...
- [ ] 130.1 多环境车队启动过程中，对某个**尚未拉起核心**的环境点关闭 → 此后不再出现属于它的核心进程；
      用 `ps` 对账（进程数 = 界面显示的运行中环境数），且该环境不停在「关闭中」。
- [ ] 130.2 对某个**已经拉起核心**的环境点关闭 → 走的是「有子进程」的关闭路径（发关闭指令），
      核心退出后名额归还、队列里下一个环境接着起。
- [ ] 130.3 关闭 → 立刻再点启动：新的一趟正常起来，没有两个核心并存、没有 edgeId 撞车。
- [ ] 130.4 原始日志里能看到被取代代次的输出（`[superseded-generation]` 前缀）——这条是**可观测性**
      验收：本次事故 13 分钟零日志正是这一层缺失造成的。
- [ ] 130.5 某环境超出 180s 就绪预算时，界面如实显示「已超预算 / 队列已放行下一个 / 仍在继续启动」，
      且**没有**被标成失败、错误或已放弃。
- [ ] 130.6 受限离场清理核心与无浏览器控制面 bootstrap 核心仍能正常起（准入闸没有误杀它们）。
...
- [ ] 131.1 一批从未登录过的新环境点「全部启动」→ 4 分钟后**仍显示排队中**（不再翻「未启动」），
      并按串行队列逐个起来、逐个在云端拿到账号绑定。
- [ ] 131.2 任一环境的自动化被判停时，**浏览器轴不得同时显示排队 / 开启中**；此后不再冒出属于它的浏览器。
- [ ] 131.3 后台把某个正在跑的环境从该客户名下撤走 → 最迟 4 分钟内该环境自动化停止，环境行显示
      「无权访问该环境」，且数据管理仍可用。
- [ ] 131.4 云端制造一次跨客户绑定冲突 → 该环境自动化停止，且文案与撤权**可区分**（不是同一句话）。
- [ ] 131.5 拔网线 / 让客户接口短暂不可达，跨过一次复核 → **不得**有任何环境因此被停（读不到 ≠ 撤权）。
