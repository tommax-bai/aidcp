...
## 簇 106b — 让位真的发生，且每一次拒绝都留痕

### change `admit-browser-standby-on-live-facts` 真机验收（Edge `00fda89` 源码已验证、**未出安装包**；登记于 2026-08-05）
...
> **本地已坐实**：Edge acceptance 39/39、全量 3167（3166 pass / 1 gated skip）、typecheck 通过；含一条定向回归（两个推断轴均为陈旧值时仍让位）与一次变异自查（把该判据加回去，恰好由该定向回归 + 结构断言两条抓住）。**触发本次登记的真机现象**：2026-08-05 dev 车队，账号 Lê Thảo Hân（`ads-k1fd38qw`）小时浏览配额跑满后连续 32 分钟拒绝让位、锁死两个槽位之一，界面与日志零痕迹。**当前 `/Applications/AIDCP.app` 仍是 0.3.26 旧包**，以下各项必须在含本 change 的新包上验收。

- [ ] 106b.1 **（本簇最高价值 · 配额跑满后真的让位）** 让一个 Facebook 账号跑到小时浏览配额上限、云端待机提示转为 `eligible=true` 且剩余等待远超 5 分钟门槛：该环境应在 **5 分钟内**关闭浏览器进入冷待机，槽位被等槽位队列中的账号取走。这正是本次故障的正面复现。
- [ ] 106b.2 **（拒绝可见）** 在开发者详情里确认「浏览器让位」与「让位诊断」两行：能读到当前模式、具名拒绝原因、连续拒绝次数与已持续时长；单次拒绝与连续拒绝在界面上可区分。
- [ ] 106b.3 **（任务在跑时拒绝让位 ≠ 变砖）** 让某环境正在执行发布或评论任务（持有任务租约）时收到待机提示：核心应回 `task_lease_active`，该环境**保持运行态、浏览器保持开启、自动化意图不变**，不得出现暂停/停止；下一条待机提示到达时能重新判定并正常让位。
- [ ] 106b.4 **（真失败仍如实呈现）** 制造一次「已 detach 但杀浏览器未确认」（`browser_close_unconfirmed`）：环境应显示浏览器真态未确认并挂持久失败卡片，但**同样不得**改运营的自动化意图。
- [ ] 106b.5 **（日志可回溯）** 在原始日志里确认 `[cold-standby] 拒绝让出浏览器槽位 reason=… streak=… for=…s` 按首次 + 每 5 次的节流出现；一个卡住半小时的环境必须能只凭日志被认出来。
