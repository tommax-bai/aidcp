...
| # | 现象 | 为什么本批不修 |
| --- | --- | --- |
| 1 | 面板「环境」页答 500（`client_env_automation_read_port_not_configured`） | 不在「切得过去」的最小集内；要先判那个取用口该由谁提供。**优先级最高**，console 主力页面 |
| 2 | 跨进程失败在面板层全兜成 500 `internal_error` | 「对面没起 / 路由没注册 / 自己炸了」从 console 侧同形，原因只在进程日志 |
| 3 | 配置镜像失效信号只有落地端 | 生产方（自动化侧中继 + 四个限频配置存储的推进器）未接线；后台改配额 / 节奏等不会通知对面 |
