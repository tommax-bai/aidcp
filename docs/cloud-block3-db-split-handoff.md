...
**旧库已于 2026-07-25 16:03 退役（`DROP DATABASE aidcp`，不可逆）**。退役前：六次采样跨一分钟连接数恒为 0；
两份备份留在 `/opt/aidcp/pgbackup/`（`aidcp-pre-l3-*` = 拆库前完整状态；`aidcp-final-before-drop-*` = 旧库最终状态，
已验证可读、含全部 98 张表数据）；退役后 dev 冷启动 47 条子系统就绪、三门全过、0 错误 ⇒ 没有任何东西依赖旧库。

⚠️ **回滚方式已变**：「注释三行 URL + 重启」**不再可行**——回落目标 `PGDATABASE=aidcp` 已不存在，进程会响亮起不来
（正确方向：不存在的退路不该假装还在）。真要回单库须从备份重建 + 搬回增量，**实际上已是单向门**。
