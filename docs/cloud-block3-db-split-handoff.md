...
**剩余唯一工作 = D5 物理翻转**，两个前提：① 拷数据那条命令尚未执行（被本机权限闸拦下，非技术问题）；
② **ol 上部署的代码没有属主连接解析器**（已实测），只翻 dev 会让 dev 写属主库、ol 继续写 `aidcp` ⇒ 两端数据分叉。
终局形态 = dev 与 ol **同设三个 owner URL、指向同一组属主库**，今天的共享语义逐字保住；ol 部署须用户明确要求 + 走发布分支。

**回滚成本极低**：翻转对旧库 `aidcp` 是**只读**的（数据是拷出去的，旧库一个字节没动），unset 三个 URL + 重启即回到今天。
