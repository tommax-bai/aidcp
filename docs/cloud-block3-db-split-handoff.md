...
- **⛔ 本批新发现的翻转前置（byte-equivalent、优先级高）**：**automation 属主表的属主 store 自己没接 automation 池**——`InteractionStore` / `ReplyConfigStore` / `ReplyConfigScopeStore` 在组合根构造时不传 pool，各自回落**共享库**配置。翻 `AIDCP_PG_AUTOMATION_URL` 时属主读写旧库、已解耦的读端口读新库 = split brain。今天三 URL 全未设 ⇒ 零影响；**翻 URL 前必须先修**。见 cutover-plan §1 callout c。
- **未做（须监督，同样一律走接口）**：`client-user-store.ts` 余下 **8 处跨库行锁 / 事务内读**（逐处清单见 cutover-plan §2.1；它们的失效**是无声的**——跨库行锁与已被淘汰的库级 advisory lock 同形，两侧各自加锁都会成功、互斥消失且不报错）、automation→api 的 accounts 守卫读（去规范化 / 移守卫）、**9 处跨库事务 + 1 处跨库写**（架构级最终一致）、DDL FK 降级、任何实际翻转（含建库+拷数据+翻 URL）——用户不在时不对生产数据路径盲改。详见 `docs/cloud-block3-l3-cutover-plan.md`。
