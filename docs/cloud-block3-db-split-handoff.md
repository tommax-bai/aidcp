...
- **未做（须监督，同样一律走接口）**：api `client-user-store.ts` 14 读（含 FOR UPDATE 跨库锁 + api+automation 同查询 JOIN 须拆两查询）、automation→api 的 accounts 守卫读（去规范化 / 移守卫）、**9 处跨库事务 + 1 处跨库写**（架构级最终一致）、DDL FK 降级、任何实际翻转（含建库+拷数据+翻 URL）——用户不在时不对生产数据路径盲改。详见 `docs/cloud-block3-l3-cutover-plan.md`。
