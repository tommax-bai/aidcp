...
- **⛔ 翻转前置的剩余部分（一次全量审计后的完整名单，完整表 + 修法要点见 cutover-plan §1 callout c）**：`PgAlertStore` ×2、`PgRiskStore`、`PgRiskCounterOutboxStore`、**`AutomationWriterLock`**、`runSchemaContractGate` 的账本 Client。三条必须知道的性质：
