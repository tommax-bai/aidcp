...
## 1. ⚠️ 先看这条：风控状态机在 dev 和 ol 上都写不进库

**这不是拆仓 Phase 的产物，是物理拆库的遗留后果。严重度高于本文余下任何一条。未修，等用户拍板。**
...
- **`applied` 那条成功路径没在真机上跑通过**——被 §1 的缺陷挡住了。dev 上实测通过的是
  「提交 202 → 回读 failed 带真实原因」与「不存在的单号 → unknown」两条。缺陷修好后必须补验。
