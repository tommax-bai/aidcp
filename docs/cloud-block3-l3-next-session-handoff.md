...
### 收尾项 · 已裁定**保持现状**（2026-07-25 用户定，勿再当作待办）

> 下面四条都属于「保证不够完整」，不是缺陷。**用户已明确裁定不处理**。
> 后续 session 看到它们时**不要再去修**；要改须用户重新拍板。

- **账本「空洞」仍不致命**：判定只看最高版本，中间缺条目会被算出来并打印，但不影响通过与否。
  三个库现在实测零空洞，所以今天没差别；要让它成为硬保证，得再把 `holes` 升级为致命。**保持现状。**
- **13 条残留迁移**（头声明为空、计入全部属主）仍在。它们不声明任何对象，所以不会造成缺失判定，
  但它们让三个属主的范围各自多算了 13 条。补齐这些头声明能让范围更精确。**保持现状。**
- **`scripts/` 不进 typecheck**（tsconfig `include` 只有 `src/**` + `test/**`）。迁移执行器的类型错误
  `npm run typecheck` 抓不到。**保持现状。**
- **`run-migration.ts` 从不写账本**（既存缺口）。用它跑迁移会让账本与库分叉。**保持现状**，理由是
  **enforce 开启后这个坑的性质变了**：以前是账本与库静默分叉、零告警；现在账本对不上，
  下次启动契约门直接拒绝启动 —— 从「静默」变成了「响亮」。对应的 openspec 任务
  `cloud-schema-migration-executor` §5.11（「删除 `run-migration.ts`」）据此标为**有意保留**、不再执行。
...
- ~~**`scripts/` 不进 typecheck**~~ / ~~**`run-migration.ts` 从不写账本**~~ —— 已移入 §2「已裁定保持现状」，**不是待办**。
- ~~**enforce 模式仍未开**~~ —— **这条已经过期，勿信。** `AIDCP_SCHEMA_GATE=enforce` 已于 2026-07-25 16:2x
  在 **dev 与 ol 两端**开启并实测通过，见本文 §2「enforce 模式已开（两端）」。本行是写 §2 之前留下的旧清单，
  保留划掉痕迹是为了让下一个读到它的人知道**这里曾经自相矛盾**。
