...
>
> **要新增或改动云端→边缘的命令，先看 [`edge-command-grammar.md`](edge-command-grammar.md)。**
> 那份文件定义了命令词汇的六条设计规则（拆不拆、什么进名字、归哪类、执行层不补空）
> 与存量 46 条的目标词汇蓝图（分批迁移、直接切换）。新增命令 MUST 逐条过语法；
> 逐命令声明的唯一运行时落点仍是操作说明书，蓝图 MUST NOT 当成第二张表。
