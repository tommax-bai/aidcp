...
> ## ⚠️ 接手第一件事：OL 上 `scroll` 长期 100% 失败（与切流无关，但比本文其余全部更要紧）
>
> 2026-08-05 14:59 实测：`action.completed: scroll ok=false` —— **08-04 全天 2868 次、
> 08-05 切流前 1092 次、切流后同样，成功 0 次**。同期 `like` / `follow` 基本成功。
> **切流前后比例一致 ⇒ 不是切流引入的**，是一条**至少两天、无人报障**的既有故障。
> 浏览是整条自动化的地基，它不动等于账号在空转。**先查这个，别先做下面的收尾。**
>
> 复现：`journalctl -u aidcp-automation --since "-2 hours" | grep -oE "action.completed: [a-z_]+ ok=(true|false)" | sort | uniq -c`
>
> ---
>
> **2026-08-05 12:59 起：OL（生产）也切成三个派生服务了**，单体停并 disable、保留作回滚。
> 逐条记录见 tasks **8.3a**。**dev 与 OL 现在是同一形态**，下面凡写「dev」的检查命令，
> 把 key 换成 `~/codes/ol-0722.pem`、host 换成 `123.56.253.183` 即可照跑。
> **切后 2 小时复测：边缘已自行重连（8787 上 10 条、6 个账号在跑）、同步读中继已追平、
> 三服务 NRestarts=0** —— 切流当时唯一没验到的那一环（边缘 ↔ 派生自动化）现已端到端跑通。
...
**清账（2026-08-05，第六次分诊批）**：28 个 ✓Complete 全部归档、零暂缓。
活跃 change **49 → 21**，已合并 spec **191 → 193**，全库 `validate --specs --all --strict` **214/214**。
真机项登记为 **backlog 簇 132**（分 A–D 四组，按前置聚簇）。
⚠ 那 28 个 change 的任务框**全部勾满**，却有 27 个从未登记过真机项，
而它们改的东西几乎全是只有真机才看得见的 —— **勾满 = 代码落地 + 已部署，不等于验过。**
...
### 2.5 反方向那条通道：**是通的**（本节曾写错，2026-08-05 实核更正）

本文件先前写着「automation→api 的失效信号中继没接线」。**那句话是错的。**
中继存在、在跑、且完全跟上：自动化组装根构造并 `start()` 了它，写提交后 `wake()` 唤醒；
dev 上中继游标与该 topic 的 outbox 队首**逐位相等、零积压**。

**错误来源值得记住**：api 组装根里有一段过期注释写着「生产方那一侧今天还没接线」，
我照抄了它、还据此排了一整拍工作。那段注释已就地改掉并**留作反例**。
判据很简单——**转述的「对面拿不到 / 对面还没接」永远不能当结论，先去 grep 事实源在谁那儿。**
...
## 5.5 剩下 21 个活跃 change 里，有 7 个是**一行没动的提案**

清账后的活跃盘子里，这 7 个 `0/N`：写了提案、没有任何实装。列在这里是因为
**挂着不动的提案会随时间腐烂**（它引用的代码行、它坐实的现状，都在往前走），
下一拍要么排期、要么关掉，别让它们继续攒灰：

| change | 它要治什么 | 规模 |
| --- | --- | --- |
| `captcha-assist-base-url-self-proof` | 验证码协助链接的公开基址**无法自证**：域名割接后那条配置腐烂成跨环境指针，潜伏 5 天没人喊。加周期性自证探针，判死就不发按钮 | 0/37 |
| `unify-facebook-comment-budget-source` | 评论提交的时间预算**两个仓各写了一份同样的公式**（四个常量逐位相同），漂开时无人知 | 0/31 |
| `bound-facebook-comment-migration-latch` | 评论迁移闩有三个生命周期缺口，**都会让一条已批准的评论消失**，其中两个还会把消失归因到错的地方 | 0/31 |
| `wechat-store-and-circuit` | 视频号把四处「暂时做不到」实现成了「永远做不到」；写熔断跳闸后**闭环死锁**，界面显示允许写入而实际每次都失败 | 0/23 |
| `facebook-consent-structural-detect` | 同意浮层靠逐语言按钮词表识别，未覆盖语种**连「是不是同意条」都认不出**、且静默当没有浮层继续 | 0/20 |
| `publish-claim-reject-defer-not-fail` | 「资源暂时被占」被判成失败，且是**2 秒烧光预算**那种；本该 defer 的三个分支因判据写错**永不可达** | 0/14 |
| `register-risk-target-follows-active-session` | 账号归属的设计已被一次提交**整个反转**（先到先得 → 跟随活跃连接），而控制仓里**零记录** | 0/11 |

**最后一条性质不同**：它不是「要做的新功能」，是**已经发生的现状没有记录**。
生产行为与规格不一致这件事，越晚补越难还原当时的裁定。

---

## 6. 下一拍的候选

1. **观察期（tasks 7.1–7.4）**：让 dev 三服务跑一段，**如实记录周期任务的真实行为**
   ——醒了几次、每次判定是什么、原因是什么。这是当前 change 里剩下的核心项，不是走过场：
   它是唯一能发现「进程活着但什么都没干」的办法，而那正是拆仓后最可能出现的形态。
   **MUST NOT 用「进程一直活着」给任何一条划勾。**
2. **tasks 6.2**：逐条点名核对跨进程路由 —— 从**调用方那一侧**实际打一遍。
3. **tasks 8.2**：更新 backlog 簇 60，真验到的划掉、没验到的补「为什么没覆盖到」。
