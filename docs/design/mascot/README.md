...
This directory is the product-wide source of truth for optional mascot concept
assets and their usage rules. It is shared by Edge, Cloud, Console, product
documentation, and future AIDCP surfaces rather than owned by one application.

The concepts are not wired into an application runtime. They do not replace the
transparent state and animation library in
[`../mascot-transparent/`](../mascot-transparent/README.md), or packaged copies
such as the current Edge runtime images under
`aidcp-edge/src/electron/renderer/assets/`.
...
existing Edge runtime files `mascot-task-execution-512.png`,
