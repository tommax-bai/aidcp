...
**是，应该分两层，方向对。** 「让精确命令彻底绕开委托层」这一步曾被判有「后台可见性回归」的代价——**2026-07-16 核实后该代价基本不成立**（见 §4 补记：管理后台**没有**委托任务列表页面，运营从来看不到这些任务），只剩「绕开簇 86 来源会话路由需实测卡仍落对」这一条要守。因此推荐**分阶段**：
...
1. **后台可见性回归 —— 2026-07-16 核实：基本不成立**。原判是「精确命令建任务行、后台可见、可暂停取消，绕开后消失」。但核实 console 前端（`aidcp-console/src/pages/`）**没有任何委托任务列表页面**：`GET /api/delegated-tasks`（`panel-server.ts:418-478`）接口在，但前端只在建任务后 invalidate 缓存键、**从不渲染列表**（`CuratedContentPage.tsx:360/372`，无对应 useQuery）。所以运营**本来就看不到**这些任务；「可见性回归」是我此前的误判。唯一真·可控点是飞书 `/task 状态/暂停/取消 <id>`（`parser.ts:110-114`），但对一次性发帖/评论命令**无可暂停/取消之物**，价值近零。**结论：此项代价可忽略，阶段 2 不必再为它设产品门控。**
...
## 8. 决策状态（2026-07-16 更新）

原本此处挂着一个待拍板的产品决定——「运营是否依赖在后台任务列表里看到并管理精确命令」。**核实后该决定基本消解**：管理后台没有委托任务列表页面（见 §4 补记），运营从来看不到这些任务，所以 route-around 没有可见性代价。

于是：
- **阶段 1（change ①②）先落**——立刻修掉运营能直接感受到的两个高危 bug（A、B）+ 收口免审（C），零可见性代价。
- **阶段 2（change ③）** 现在也低代价，唯一要守的门是：走 route-around 时**实测命令触发的审批/失败卡仍落对来源会话**（簇 86，`server.ts:1607` 直连路径的 `manualApprovalChatId=sourceChatId`），并清掉随之变死的 split-brain 代码。2a（极简删两三元）优先；2b（ActionPort）仅当 E 消解/复用来撑才做。
