...
| **耦合处置 Phase 2** 面板契约析出 | ⏳ 调查/沙箱验证中（最重的串行线） |
| **耦合处置 Phase 4** 端口注入 | ⏳ 调查中 |
| **耦合处置 Phase 5** 三个大件 | ⏸ 风控写异步化**用户已拍板走异步**（见耦合清单 P5-1） |
| **`aidcp-transport`** 共享传输包 | ✅ 已建仓、8 个成员、typecheck 0 |
| **三仓 + kernel 的 `test/`** | ✅ 286 个已按派生归属就位（99 个跨属主留守，随耦合消除自动减少） |
| **批次 2/3/4** 三仓各写 `main()` | ⏸ 未开始（主交付物） |
| **批次 5** dev 三服务部署 + soak + ol | ⏸ 未开始 |

**跨边界豁免轨迹：96 → 86 → 81 → 79 → 69 → 68 → 39。** 共享层成员 57 → 72。
...
1. **Phase 4（端口注入，29 条）** —— 调查与沙箱验证已跑完一轮，产物在会话 workflow 里；
   若已落地则跳过。它是**唯一大量触碰组装根**的一批（每处 1–3 行装配），必须串行独占 `src/server.ts`。
2. **Phase 5（三个大件，12 条）** —— 其中「后台写风控改跨进程」**用户已拍板走异步**，
   完成判据三项写在耦合清单 P5-1（关键是**返回形状必须跟着改**，不能保留写后真态那三个字段）。
   另两件：4 个内容角色脱离公共基类、发布出口角色（先花十分钟核归属，可能 3 条零成本消失）。
3. **批次 2/3/4 三个 `main()`** —— 主交付物。**距离已量化**（见上表）：各仓业务代码断裂只剩 14–15 条，
   写完 `main()` 一次消掉 109–143 条。三仓可并行，但组装根改动期间彼此不冲突（各写各的仓）。
4. **批次 5** —— dev 三服务部署 + soak，再按用户明确要求上 ol。
