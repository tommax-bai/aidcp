...
| **`aidcp-transport`** 共享传输包 | ✅ 已建仓、8 个成员、typecheck 0、已纳入对账 |
| **三仓 + kernel 的 `test/`** | ✅ 286 个已按派生归属就位（99 个跨属主留守，随耦合消除自动减少） |
| **批次 2** content `main()` | ◐ 进行中：6 条前置契约已落 2 条（均已部署 dev），执行清单见 §9 |
...
### 0.0.2 ✅ 已修：风控状态机写不进库（dev 已验证，ol 待部署）
...
| # | 契约 | 状态 |
| --- | --- | --- |
| 1 | 候审卡投递判定 | ✅ `aidcp-cloud@f9b1d46`，已部署 dev |
| 2 | 发布台账窄写（四方法） | ✅ `aidcp-cloud@036f5c3`，已部署 dev |
| 3 | 发布管线日志 sink | ⏸ 未开始（属主反转，`publish_pipeline_logs` 属 api、就在内容段里绑 apiPool 建） |
| 4 | 卡片出口（飞书三方法 + 卡片落点解析 + 机器人会话表 + 免审授权写） | ⏸ 未开始（面已由 `PublishExecutorDeps` 定死，见 9.3 C） |
| 5 | 账号显示名 · 本地镜像 | ⏸ 未开始（**同步读**，见 9.4） |
| 6 | 图片模型配置 · 本地镜像 | ⏸ 未开始（**同步读**，见 9.4） |

**已由 1、2 消掉的 content→api 依赖：** `approvalPolicyStore`、`clientUserStore`、`publishLogStore`。
另**顺带消掉一条本来要开的契约**：候审预览读原先无条件发起，而它的唯一消费者是自动化段的界面推送口
——在 content 进程里恒缺席。改成「取用不到就不去读」后，那条跨进程读根本不需要存在。
