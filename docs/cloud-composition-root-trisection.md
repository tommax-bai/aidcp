...
>    所有「已验证」都只是「看着对」——审计里那一大串未验证项从此可真验。
> 2. **迁移进了同步范围**（§0.0.3 那条缺口已补）。同时坐实：**迁移不能单独搬**，
>    它与「收窄后的 schema 契约（两个常量一起收窄）」「`boundaries/table-ownership.json`」
>    是一个耦合单元——只搬迁移会让 automation 从 6 红涨到 14 红。
...
### 0.0.3 ✅ 已补：迁移文件不在同步脚本范围内

> **2026-07-29 已补。** `scripts/sync-split-repos` 现在按属主对账每个仓的 `migrations/`
> （缺哪条、多哪条逐条点名），并把**收窄后的 schema 契约**一并派生改写。
> 搬动本身仍**只报不改**——搬迁移是有后果的操作，「多出来的」也可能是那个仓自己加的。
>
> 补的过程坐实了一条判据：**迁移不能单独搬**。只搬迁移，automation 从 6 红涨到 **14 红**，
> 因为它与另外两样是一个耦合单元：
> - `REQUIRED_SCHEMA_VERSION` 与 `KNOWN_MAX_SCHEMA_VERSION` **必须一起收窄**
>   （收窄函数本就同时返回两者）。只收窄前者，后者停在三家并集的最大版本上，
>   「已知最高版本 ＝ 目录里的最高版本」当场破——而那正是「加了迁移忘抬版本」的守卫。
> - `boundaries/table-ownership.json` 必须跟上，否则新迁移声明的表它不认识。
>
> 顺带修掉工具自身两处「安静地坏」：派生改写把云端**当时**的版本字面量当成必须存在的锚点
> （云端走到 0097 后 RuntimeError，且**崩在字母序第二个仓**、后三个目标一个都没输出，
> 默认跑法看着像跑完了，实覆盖 1/5）；tsx 临时入口路径写死 + `finally` 无条件删，
> 并发跑互删，表现为随机「迁移对账被跳过」而退出码不变。
>
> 下面保留原始记录供追溯。
...
### 0.0.5 ✅ 已补：迁移的「执行范围」与「账本范围」曾是同一个东西

> **2026-08-06 已补**（同一 change）。一条迁移此前只有一个范围：进了属主 O 的范围
> 就意味着**在 O 的库里跑它**。于是 12 条头声明定位不到表、被按「安全方向」计入全部属主的
> 历史迁移，会在另外两个库里对**不存在的表**执行 DDL/DML；
> `0030_panel_hardening_indexes`（在 automation 与 content 的表上各建索引）
> **在任何单一属主库里都跑不通**——空库实跑正是停在它上面。
>
> 那条「安全方向」的理由写在 `migration-owners.ts` 文件头里：**「这类迁移不持有任何存活对象」**。
> 实测 13 条里 **12 条仍在对真实的表执行 DDL/DML**——头声明为空只说明「它建的对象后来被别的
> 文件重新声明了」（生成规则是「每个对象归**最后**创建它的那个文件」），完全不说明它不碰表。
> 该段注释已同批改掉：**留着会继续骗下一个人**。
>
> 现在两个范围分开：
>
> - **账本范围**（谁记一笔「已处置」）＝ 今日口径逐字不变。分发按它——
>   **MUST NOT 因执行范围收窄就从派生仓删迁移文件**，删了账本行就成了 `ledgerOnly` 噪声。
> - **执行范围**（谁真的发 SQL）由显式声明给出，解析顺序唯一：
>   ① 文件内 `-- aidcp:owner=` 头 → ② 封闭名册 `migrations/legacy-owner-overrides.json`
>   → ③ 对象声明能定位到表 → ④ **失败并指名**。第 ④ 步取代了旧的「推断不出就计入全部属主」。
>
> **历史为什么只能走名册**：那 13 条全部已入 dev / ol 的账本，而校验和是**整文件 sha256**，
> 改一个字节就是 `migration_checksum_mismatch` **整批拒绝**，两个环境的迁移命令当场全停。
>
> **名册是 `migrations/` 下的第一个非 `.sql` 判据文件**，因此同步脚本新增了一段
> `MIGRATION_AUX_MEMBERS`（逐字节镜像三仓一份）：`.sql` 的那道对账**只比 `.sql` 集合**，
> 对目录里的 JSON 判据完全无感——缺了既不报缺也不报多，而三个服务的启动契约门读不到它就拒绝启动。
> 「谁都不管、缺了也没人报」正是 §0.0.4 的同一个根因，不能在隔壁目录再来一遍。

`scripts/sync-split-repos` 的 `.sql` 部分仍是「只管 `src/` + 两个共享包 pin + 点名的 `scripts/` 与判据文件」，
...
### 0.0.4 测试归属：import 与「按路径读文件」是两类证据，不能混算

**2026-07-29 加。** 原判据只看 import 边。但测试还会**按路径直接读仓里的文件**——
读某条迁移的 SQL 文本、读组装根做全仓断言、用模板拼路径遍历一批源文件。
**路径读没有 import 边可推**，于是这类测试被派到读不到目标文件的仓里，
搬过去必红，且红在「文件不存在」上——**看着像同步搬坏了，其实是归属本身判错**。
已付代价：api / automation / kernel 三处共 7 个测试常红或假绿。

两类证据的语义**相反**，必须分开算：

| 证据 | 说的是什么 | 怎么合并 |
| --- | --- | --- |
| import | 需要**谁的代码** | 并集须收敛到一个属主 |
| 字面路径读 | 需要**哪些文件在场** | 同时具备全部被读文件的属主，取**交集** |

混成一个并集会错：一条只读 automation 自己两条迁移的测试，会因为其中一条迁移**同时**属于 api
而被判成跨属主留守——白白丢掉一批本来派得下去的测试（第一版就是这么写的，当场丢了一条）。

三条派生规则：

- 字面串可能只是**前缀**（模板拼到 `${` 为止）。此时按目录处理、要求属主同时具备该目录下全部文件。
  `../../` 即仓根，于是「全仓扫描」这类测试自然收敛到空集、如实留守。
- **组装根另有判定**：它按设计从不同步，读它的测试不是「暂时跨属主」而是**结构上不可派生**，单独计数。
- 三家都发的迁移（共享基础设施 + 头声明为空的残留）在每个仓都读得到，**对归属不构成约束**。

**留守 ≠ 丢失**：`aidcp-cloud` 仍是单体、仍跑全部 463 个测试。留守只表示「这条断言只在整棵树上才成立」。
反过来，把这类测试硬派下去的代价有两种，都比留守糟：
一种是常红（读不到别属主的迁移），另一种更坏——**假绿**（`cross-owner-foreign-keys`
在 automation 里只看得到三分之一的 src，照样打出全局结论）。
