...
1. **⚠️ 先看 §0.0.2：风控状态机在 dev 与 ol 上都持久化不了。** 这是 Phase 5 端到端验证时挖出来的
   **既存线上缺陷**，与拆仓 Phase 无关但同属拆库后果，严重度高于下面任何一条。**未修，等用户定方案。**
2. **批次 2/3/4 三个 `main()`** —— 主交付物。**距离已量化**（见上表）：各仓业务代码断裂只剩 14–15 条，
   写完 `main()` 一次消掉 109–143 条。三仓可并行，但组装根改动期间彼此不冲突（各写各的仓）。
   注：上表是 Phase 4 时点实测；Phase 5 之后 14–15 条那部分已归零，重跑一次再动手。
3. **批次 5** —— dev 三服务部署 + soak，再按用户明确要求上 ol。
4. **补上迁移的同步机制**（见 §0.0.3）——现在 `sync-split-repos` 只管 `src/`，迁移文件靠手工放。

### 0.0.2 ⚠️ 已发现未修：风控状态机写不进库（dev + ol 双中）
