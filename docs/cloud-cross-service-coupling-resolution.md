...
## 8. Phase 5 落地记录（2026-07-26，跨边界边 10 → 0）

三件都做了，另加三条原判「维持豁免」的边。落点：`aidcp-cloud@ca0ba9f`（批 A）、`5a8f2ee`（P5-1）、
`7d32913`（P5-2）；console `bee7391`；五个 sub-repo 同批同步（kernel `c49bdb9`）。
dev 已部署验证（迁移 0079 已跑、三个 schema 闸全过、零错误）。

### 8.1 §7 里那两条「未查实」的结论

**① `publish-executor` 归属（P5-3 前置）—— 核了，原判是错的，已改判 content。**
判据用 §4.6.3 **自己写的**三条机械判据实跑：引 `../comm/`（边缘协议与租约）只命中审批卡合同一处、
引 `../llm/` 与图片供应商零命中、`publish_log`/INSERT/UPDATE/`query(` 零命中（落库经注入端口）。
运行事实同向：组装根把它 `registerRole` 进 `publishOrchestrator`（content 的生成编排器），不在下发器里；
它是 `roles/` 目录 31 个角色里唯一被判 automation 的一个。误判根源与 `approval-gatekeeper`（2026-07-25 修正）
同型——生成与下发切开之后它退化成「生成候审段的出口」，而归属表记的仍是切分前的形态。
**代价确实近零，但不是零**：改判会把两条边换成两条新边（审批卡合同、平台档案），
故顺手把两个新落点也解决了（前者整文件抬 kernel，后者只析出无协议依赖的档案段）。

**② P5-1 同步/异步 —— 用户 2026-07-25 拍板异步，已按三条完成判据落地。**
关键确实是**返回形状必须跟着改**：旧路由回 `{state, statusBefore, changed}`，那是「面板与风控同进程」
才知道的写后真态；异步后 api 在受理那一刻**不可能**知道结果，任何 state 字段都只能是编的。
现在提交回 `202 {accepted, commandId}` 且**只有这两个字段**（有测试逐字段断言禁止出现 state/changed/status），
结果由 `GET /api/risk-commands/:id` 单独回读。

**四态里 `unknown` 是硬要求，不是凑数**：「查无此命令」与「还在处理」是两件事，前者意味着提交那步就没落住。
把前者当 processing，界面永远转圈且永远不报错——这是静默假成功换了身衣服。

**消费者不再按 seamMode 分叉**：它原来的条件是「非 monolith 才起」，前提是「monolith 下没人 emit」——
那个前提被本 change 打破了。若照旧跳过，面板提交完命令**没有任何人会应用它**：202、状态永不变、日志零字。
现在一条路径、从今天起就在生产上跑，拆进程那天这里不用改。

### 8.2 P5-2：四个内容角色脱离公共基类

**从不 emit** 是这条缝干净的原因：实测四个角色全仓零 `emit` 命中，它们只订阅 + 写自己域的存储。
故 kernel 的事件源端口**只给 `on`**。给出 `emit` 会让 content 拆进程后能往一条它并不拥有的进程内总线上发事件——
单体里跑得通、拆完静默失效。

基类本身进不了 kernel（持总线句柄与人设快照 = 带状态的行为类），复制第二份更糟。
按判例拆：无状态段提成 kernel 函数，**两侧基类都改成它的薄壳**，实现只有一份。

`RoleFactory` 原本返回 `BaseRole`，等于要求所有角色都是 automation 那个基类的子类。
调度器实测只调 `subscribe` / `unsubscribe`（外加 `roleName` 供后台预览），已收窄到这三样。

**事件载荷漂移守卫第一次跑就抓到一个真错**：那三个键里有两个其实住在 `EventMap` 而非 `RoleEventMap`。
靠肉眼记「哪个事件在哪张表」必然记错——这正是必须有编译期断言的理由。已用注入字段负向验证过它真会红。

### 8.3 §7 那 5 个隐患的处置

① `ConceptPool.source` 跨进程静默变空 —— **删掉了该字段**，不是改成 `Record` 再养着：
   实测全仓只有写入点、零读取点。关键词来源没丢，它落在 `concept_pool.source_note` 列上、
   经 `ConceptRecord.sourceNote` 读出，那条路径有真实消费者。
⑤ 删 `RoleName` 注解丢掉的编译期防线 —— Phase 0 的合同夹具已覆盖全部五个 content 角色，本批直接复用。
②③④ 不在本批范围，仍待处置（④ 需单独立项）。

### 8.4 端到端验证挖出的既存线上缺陷（未修，等用户拍板）

P5-1 上线后在 dev 上真跑了一次面板改配额档，结果是 `state:'failed'`，原因
`relation "accounts" does not exist`。**这不是本 change 引入的**，旧同步路由调同一个方法、
一样炸、只是回一个没有原因的 500。诊断、影响面、两条候选修法、以及为什么没有当场修，
全部写在 `docs/cloud-composition-root-trisection.md` §0.0.2。**dev 与 ol 双中，严重度高于余下任何一条拆仓工作。**
