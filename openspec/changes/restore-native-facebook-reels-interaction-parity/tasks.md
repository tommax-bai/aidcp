...
- [x] 2.1 Port the established Facebook neutral/reacted and follow/following semantic label families into the embedded Native router with count/control exclusions.
  <!-- Native router now recognizes bounded zh/en/vi neutral, reacted, follow, and following controls; numeric reaction summaries remain non-actionable. -->
- [x] 2.2 Resolve like and follow candidates against the uniquely active canonical Reel across nested and sibling action-rail layouts without document-order fallback.
  <!-- Candidate association uses the active Reel identity, video geometry, nearest-video exclusion, and unique-target checks. -->
...
- [x] 3.2 Keep follow as one trusted pointer write and terminate honestly on active-Reel movement, target loss, ambiguity, already-following, or unconfirmed state.
  <!-- Follow verification compares canonical noteId plus videoKey and never dispatches a recovery click. -->
...
- [x] 4.1 Run focused Native router/session tests, legacy Reel oracle tests, and Native Cargo tests.
  <!-- Focused TS: 74 passed. Cargo library: 51 passed. Full Cargo compiles and passes library/main tests, then hits the pre-existing explore_feed fixture null-field mismatch reproduced on the unchanged default branch. -->
- [x] 4.2 Run Edge protocol acceptance, full tests, and typecheck with bounded output.
  <!-- Acceptance: 30 passed; typecheck passed. Full suite: 2313/2316 passed under concurrent heavy validation; the 3 unrelated timing failures each passed immediately when rerun alone. A final integrated full-suite run remains part of 5.1. -->
- [x] 4.3 Run `openspec validate restore-native-facebook-reels-interaction-parity --strict`.
  <!-- strict validation passed on 2026-07-26 -->

## 5. Integration and Runtime Boundary

- [ ] 5.1 Rebase and fast-forward integrate the isolated Edge and control branches, record commit and validation evidence, and push without force.
- [ ] 5.2 Rebuild the local development Native Page Engine artifact without packaging an installer; keep real-account exact-target like/follow acceptance explicitly pending unless separately authorized and observed.
