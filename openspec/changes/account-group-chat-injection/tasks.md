...
- [ ] 7.5 手动/端到端（部署后真机，待运营操作）：后台粘贴含 emoji/多行码 → 保存 → 刷新持久且原样；清空 → 无码；`/comment <昵称> group:on`（有码）→ 人审卡含完整码 → 发出闭环（正文逐字 + 码整段插入）；`group:on`（无码）→ 告警不发；`/comment <昵称>`（无 flag）→ 普通评论零回归；同码配多账号 → 前端告警。 <!-- 前置：用户本地 edge 须先 pull d714b9f + 重启（否则老 edge 忽略 groupChatCode、只发正文丢码，破审=发）。建议先在自己的测试笔记上跑 -->
