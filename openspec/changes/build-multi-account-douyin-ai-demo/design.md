...
The QR challenge expiry comes from the matching first-party `get_qrcode` response. The configured login timeout is only an upper safety bound; the service must never keep displaying or accepting an unscanned QR after Douyin's earlier `expire_time`. At that display boundary the worker may retain the context for at most one fixed two-second response-drain window so a `check_qrconnect` already accepted near expiry can settle; the UI still shows the QR expired and the window never admits a new scan. The same page tracks only sanitized `new`, `scanned`, `confirmed`, `refused`, or `expired` evidence from the exact first-party `check_qrconnect` response whose request token SHA-256 matches the displayed challenge; raw tokens and bodies are neither retained nor projected. The known `data.status` value is the state authority; the top-level `message` is presentation copy and is not a stable protocol field. An explicit nonzero error still fails closed, with the observed secondary-verification code projected separately instead of being mislabeled as generic schema drift. `scanned` and `confirmed` keep that browser context alive beyond the QR display expiry, but only within the original configured authorization bound. Either state may begin the read-only identity probe, but neither is authorization by itself. Final credential creation requires the same Creator context to prove a stable account ID and IM token and to yield a structurally valid, Douyin-scoped cookie set; no fixed frontend cookie name is treated as an authentication contract. A legacy session cookie already issued to the fresh Profile remains compatible scan evidence but never replaces the identity probe.
...
The observed first-party `error_code=2046` is a bounded secondary-verification state, not authorization and not generic schema drift. The official Creator page owns the verification configuration, UI, and replay of the original `check_qrconnect` request, so the service retains the same page, Profile, Cookie context, and token-bound tracker only until the original authorization deadline. Once this state has been observed, only an explicit `confirmed` response bound to that same QR token clears the verification gate; an explicit `expired` response terminates the attempt even when the fresh Profile already contains a legacy session Cookie, and surface appearance or disappearance never advances authorization. Current first-party Creator and Verify Center assets establish two exact official compositions: the masked cross-origin `https://rmc.bytedance.com/verifycenter/captcha/v2` iframe and a same-document `#vc-second-valid > #account-container-mask.vc-second-valid-mask > .account-container.vc-second-valid` dialog whose fixed, high-z-index mask covers the viewport. The adapter admits exactly one visible candidate tree across those two kinds, fully inside the viewport. When and only when one admitted iframe is contained by one admitted dialog, the hierarchy is one composed official tree and the deepest iframe is the interactive image surface while the dialog remains a revalidated ancestry anchor; disjoint, duplicate, or multiply nested candidates remain ambiguous. Unknown, missing, hidden, moved, malformed, or ambiguous surfaces fail closed with no Creator-page screenshot fallback. Current first-party assets also contain a UC branch: the exact Verify Center script loads the exact UC wrapper `@byted/uc-account-second-verification-web/1.0.16`, whose account-specific renderer is supplied dynamically and has no public stable DOM selector. The adapter therefore distinguishes an exact wrapper whose renderer global has not loaded from a loaded renderer whose surface is not covered by the two admitted structures, using fixed sanitized codes only; it does not promote an arbitrary fixed modal or Creator viewport into a control surface. Each element-only JPEG receives the latest random memory-only frame identity with a server-returned absolute expiry and remaining lifetime of at most thirty seconds plus one-use semantics. The UI derives a local deadline from that remaining lifetime, binds the identity and deadline to the snapshot revision used for the GET, decodes the JPEG before enabling controls, recomputes the remaining safety window after decode, keeps the current operable image stable, and hard-disables it before server expiry even when the tab was hidden or a pointer/wheel gesture was unfinished. Pointer and accumulated-wheel gestures remain bound to the frame on which they started but may be canceled at that safety cutoff; one frame accepts at most one input, temporary text clears on invalidation, and a conflict is never auto-replayed. Before input, the server consumes the frame identity and rechecks surface kind, frame and element identity, any composed ancestry and containment, Creator origin and pathname, surface-document digest, and bounding boxes. Coordinates map only into the leaf box; scroll first targets its center. Text and allowlisted keys require both an editable focus and containment by the admitted surface: the iframe itself must own top-level focus for an iframe candidate, while a DOM candidate must contain the active element. Escape is excluded. Top-level navigation is blocked while verification is current; any newly observed popup is immediately closed and is never exposed as a control or image surface. There is no public browser port, CDP endpoint, clipboard, upload, download, DevTools, arbitrary navigation, whole-page control, or script execution surface. Frame identities, screenshots, coordinates, and temporary text are never logged or stored. Only `verificationRequired` and its bounded expiry are encrypted with the existing challenge and projected; raw decision configuration, SMS keys, QR tokens, Cookies, dynamic renderer URLs, and response bodies remain inside the official page. A transiently missing or invalid surface returns a fixed sanitized missing code, multiple candidate trees return a fixed ambiguous code, an exact UC renderer stage returns one of two fixed stage codes, and a moved or replaced admitted surface returns a fixed stale-frame code; none changes the account revision or authorization state. The UI parses only those bounded codes, retries only the frame read for surface and renderer-stage conditions, and refreshes the account snapshot only for an actual stale revision or ended verification state. Creator origin/path drift, closing, timeout, explicit expiry/refusal, QR schema failure, or cleanup failure terminates or quarantines the attempt without a credential. Official verification completion only resumes the existing stable-identity, IM-bootstrap, and structurally valid Douyin-Cookie proofs.
...
QR startup is also projected honestly: before the first-party QR exists, the account view says that Douyin is preparing it and gives the observed 5–20 second range instead of claiming that a scan occurred. An explicit secondary-verification state opens the authenticated one-time viewport and keeps the account authorizing; it never changes the account to authorized by itself.
