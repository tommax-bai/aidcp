...
The observed first-party `error_code=2046` is a bounded secondary-verification state, not authorization and not generic schema drift. The official Creator page owns the verification configuration, UI, and replay of the original `check_qrconnect` request, so the service retains the same page, Profile, Cookie context, and token-bound tracker only until the original authorization deadline. Current first-party Creator and Verify Center assets establish two exact official compositions: the masked cross-origin `https://rmc.bytedance.com/verifycenter/captcha/v2` iframe and a same-document `#vc-second-valid > #account-container-mask.vc-second-valid-mask > .account-container.vc-second-valid` dialog whose fixed, high-z-index mask covers the viewport. The adapter admits exactly one visible candidate across those two kinds, fully inside the viewport; unknown, missing, hidden, moved, malformed, or duplicate surfaces fail closed with no Creator-page screenshot fallback. Each element-only JPEG receives the latest random memory-only frame identity with a server-returned absolute expiry and remaining lifetime of at most thirty seconds plus one-use semantics. The UI derives a local deadline from that remaining lifetime, binds the identity and deadline to the snapshot revision used for the GET, decodes the JPEG before enabling controls, recomputes the remaining safety window after decode, keeps the current operable image stable, and hard-disables it before server expiry even when the tab was hidden or a pointer/wheel gesture was unfinished. Pointer and accumulated-wheel gestures remain bound to the frame on which they started but may be canceled at that safety cutoff; one frame accepts at most one input, temporary text clears on invalidation, and a conflict is never auto-replayed. Before input, the server consumes the frame identity and rechecks surface kind, frame and element identity, Creator origin and pathname, surface-document digest, and bounding box. Coordinates map only into that box; scroll first targets its center. Text and allowlisted keys require both an editable focus and containment by the admitted surface: the iframe itself must own top-level focus for an iframe candidate, while a DOM candidate must contain the active element. Escape is excluded. Top-level navigation is blocked while verification is current; any newly observed popup is immediately closed and is never exposed as a control or image surface. There is no public browser port, CDP endpoint, clipboard, upload, download, DevTools, arbitrary navigation, whole-page control, or script execution surface. Frame identities, screenshots, coordinates, and temporary text are never logged or stored. Only `verificationRequired` and its bounded expiry are encrypted with the existing challenge and projected; raw decision configuration, SMS keys, QR tokens, Cookies, and response bodies remain inside the official page. A transiently unavailable, changed, or ambiguous verification surface yields no image or input but leaves the attempt pending within its original deadline. Creator origin/path drift, closing, timeout, explicit expiry/refusal, QR schema failure, or cleanup failure terminates or quarantines the attempt without a credential. Official verification completion only resumes the existing stable-identity, IM-bootstrap, and structurally valid Douyin-Cookie proofs.
