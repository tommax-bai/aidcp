...
Edge SHALL 在 `hello` 中上报当刻的浏览器状态快照，并 SHALL 在同一 Cloud 连接内于浏览器状态发生变化后上报最新状态。状态上报只陈述 `absent | ready` 事实，重复上报同一状态 MUST 是幂等的。该状态契约只适用于已经启动的核心，MUST NOT 被用来为首次排队环境预启动 browser-absent 核心。
...
#### Scenario: 首次排队环境不建立握手
- **WHEN** an environment is waiting for its first browser slot and its core has never started
- **THEN** edge keeps it only in the local FIFO and does not establish Cloud hello/welcome until the slot is admitted and the full core plus browser startup begins

#### Scenario: 冷待机核心在同一连接内恢复浏览器就绪
- **WHEN** an already-running environment wakes from cold standby, the browser starts, and login plus account identity are verified without replacing the Cloud connection
