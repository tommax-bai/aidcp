...
- [x] 1.4 （据 1.3）整机模板 MUST 显式 pin OS + 限定桌面（Win/Mac，绝不放任随机分到 iPhone/Linux）；webgl 二选一（`webgl='2'`+OS 匹配 renderer / `webgl='3'` 纯委托） <!-- aidcp-edge b92989b DEVICE_TEMPLATES 全 pin uaSystemVersion(win/mac)、assertOsCoherent 拒非桌面、webglMode 逐模板二选一 -->
...
- [x] 4.4 凭据只内存持有、绝不明文落盘 <!-- aidcp-edge beda2e5 main.cjs ads:createEnv 用 resolveAdsOpts 结果建 writeApi(deps 内存)、不写 settings；ads-write-api 错误不含 body + redactSensitive(7227783) -->
...
- [x] 5.1 `preload.cjs` + 主进程新增「创建环境」IPC 通道（`ads:createEnv`/`ads:templates`，调 `ads-create-flow`）；凭据只内存不落 `settings.json` <!-- aidcp-edge beda2e5 main.cjs + preload.cjs + 只读客户端加 listGroups 定位专用分组；进程级单飞互斥 -->
