...
> **接手先读 §0.0（当前状态），再读 §0（核对现状）。三个岔口已全部裁决，见 §2。**
> 本文所有数字与位置都是本机实测，但 fleet 活跃——**一律自己重跑一遍再信**。

## 0.0 当前状态（2026-07-29 第二次交接，**接手第一件事读这里**）

**第 0 段（准入与裁决）已做掉大半；第 1–4 段（写代码）一行未动。** 别把第 0 段的进展
当成「快做完了」——真正的体量在第 3 段（automation 的真 `main()`），那是唯一没有现成模板的一块。

### 已推送的分支与提交

| 仓 | 分支 | HEAD | 内容 |
| --- | --- | --- | --- |
| `aidcp`（控制仓） | `main` | 见 `git log` | change 文档与台账 |
| `aidcp-cloud` | `codex/split-cloud-automation-production-runtime` | `abdadae` | 事实源全部改动 |
| `aidcp-automation` | 同名 | `32f3aa8` | 台账锚 + 欠账补记 + docstring 收敛 |

**尚未合回各仓默认分支**，也**未部署**。回归实测（在 worktree 上）：
cloud typecheck 0 / 验收 169 全过 / 全量 3870 过 0 失败 11 skip；
automation typecheck 0 / 验收 74 全过 / 全量 1845 过 0 失败 3 skip；跨层边 **0**。

### 这一轮实际做了什么（都不是准备工作，是真改动）

1. **归属改判落地**（§7.2.1 + 规则表六条 override）：四个角色 + 基类 + 共鸣预筛模块由 content 改 automation。
   刷新后 automation 225→231、content 83→77、kernel 96→101，**跨层边仍为 0**。
2. **三处 kernel 抬升**：LLM 错误族、文字卡的两个纯函数与三个纯接口、两条 content 端口契约 + 失败信号。
3. **文字卡缺席不再被静默吞**：构造期结算成显式能力状态，缺席具名留痕、全程不抛。
4. **台账**：三份对齐，automation 那份加了机械锚（双向变异实测过）；
   12 条探针改成有判别力的形态（同样变异实测）；补记三条真实欠账；`new`/`call` 探针加了 seam 过滤。

### 接手要知道的三件事

- **第 0 段任务从 8 条涨到 30+ 条，是因为侦察与两轮对抗性复核推翻 / 补正了十几处判断**，
  其中若干处照原样做会白干。涨的不是范围，是**原任务书的错误密度**。
- **本轮所有「已完成」都带机械证据**（变异实测 / 门禁实跑），不是「看着对」。
  凡是没有机械证据的，我都留成了未勾选项并写明原因。
- **下一步的最大单块是第 3 段**（真 `main()`）。第 1 段有现成模板可抄、且其中一条通道已经写好只差接线、
  另一条通道**根本不存在**（省一半）；第 2 段契约已定义，剩注册路由 + 客户端 + 堵五处吞点。

### 立刻能开工的（互不冲突，可并行）

- **0.3f**（已裁定要做）：把 seam 过滤放宽到「automation 模式下不执行的分支」，并让它响亮报出丢弃了哪几条证据。
- **0.6f**：堵五处把抛出压成空数组的降级形状。
- **第 1 段**：四条运营指令通道（照 4a 的 paired command 抄）。
- **第 2 段**：content 侧注册四组写口（模板见 §2.6）+ automation 侧客户端组。
