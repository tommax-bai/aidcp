...
- [x] 3.5f **互动能力二态口接通**（**用户 2026-08-02 拍板：这一批就接通，不走具名缺席**）。
  ...
  <!-- aidcp-cloud c394f36（新窄端口）+ aidcp-automation 173b234（装配）；
       派生：kernel 6101b1e / transport 1444d59 / api 35c63ba / content 46c13f7。
       dev 已部署到 c394f36。落点 `src/automation-interaction.ts`（派生仓私有，已登记两处）。

       上面的计划照做，另有**五件是落地时才定的**：

       ① **⚠️ 一处偏离计划的行为判定：半迁移（`legacy_read_only`）MUST NOT 判 `unavailable`。**
          计划注释把它列为缺席理由之一，**实读单体后不采纳**：半迁移在单体里仍然组装收件箱与
          运行时开关，只把**写**总开关关掉（读已恢复）。判成整体缺席等于连读一起停掉 —— 那是
          行为回归。正解照单体：仍 `wired`，写开关按 schema 模式算。
          连带一条容易漏的：`interactionWritesAllowed` 的**第三个实参是部署目标**，
          半迁移只在 dev 上仍允许写。漏传会让 dev 的写悄悄关掉 ——「看着更安全」的静默行为变更。
       ② **`unavailable` 的具名理由实际只有两条真的**，不是计划里说的五条。逐条核过：
          回复配置解析器 / 撤权 hold 端口在本进程的组装根里**恒存在**（api 客户端由配置构造，
          缺配置在读配置那一层就抛了）⇒ 给它们写 `unavailable` 分支就是造死代码。
          做成必填参数，让编译器保证；真能在运行时缺席的只有 **schema** 与 **回复生成**。
       ③ **新开的 api 窄端口挂进 `offboard-admission-ledger` 既有组，不另开一组**（组数仍 21，
          方法槽 57→58）。它就是同一本台账的读面。属主侧**不写第二份 SQL**：台账类只持 api 池、
          解析不出账号的环境键，故它改成 `Omit` 如实声明只覆盖写面，组装根把读面委托给属主存储
          自己那一份（谓词 `materialized_at IS NULL` 与「失败必须抛」都是有讲究的）。
          **单体消费方一并改指端口** —— 那是 4a 普查「每个方法槽要有真实生产消费点」这条守卫
          的要求，也正是「建好零消费方」的防线。
       ④ **真环用晚绑定薄壳破**：本能力要边缘推送出口，而边缘接入的构造入参里又有本能力。
          薄壳未绑定时**具名抛错**，绝不返回 0 —— 返回 0 会让「推送出口还没接上」与
          「边缘不在线」同形，而这两者的处置完全不同。重复绑定同样抛。
       ⑤ **dev 上这条路径验证不了，如实登记**：dev 的互动 schema 本来就缺
          （`interaction_schema_missing_run_0042`，本次部署前 6 小时内已出现 7 次），
          单体那一侧的互动域整段未启用 ⇒ 改指端口的那处消费方在 dev 上根本不执行。
          本片的证据止于契约测试与变异实测；真机验收按 5.5 登记 backlog。

       变异逐条给出是哪条抓住的：半迁移判成缺席 / 写开关漏传部署目标 / 撤权 hold 失败吞成 false /
       晚绑定薄壳未绑定返回 0 —— 四条各由对应那一条用例抓住。
       typecheck 干净；automation 2138 pass / 0 fail（基线 2132）；cloud 4115 / api 502 /
       content 441 / kernel 70 / transport 36 全 0 fail。 -->
- [x] 3.5g **批 H 主体接线**：12 个工厂接进组装根，写 `main()`。
  <!-- aidcp-automation 0044881 落地：新增 `src/automation-main.ts`（真装配 + `main()`）
       与 `src/automation-persona-view.ts`（人设唯一取用口）。typecheck 干净；
       全量 2148 pass / 0 fail（此前 2138）；`boundaries:refresh` 后 forbidden=0、豁免未增；
       六仓对账零漂移（只剩设计上必然的组装根噪声）。两份新文件已登记进中控
       `scripts/sync-split-repos` 的 `DERIVED_COMPOSITION` 与 automation 侧
       `boundaries/ownership-rules.json`（前者判 composition 并进 compositionWhitelist，
       后者判 automation —— 它是纯派生逻辑、零装配）。

       ## ⚠️ 首版写了 29 处 `as never`，去掉之后编译器点名 5 处**真的接错了**

       这条是本片最值钱的一条，下一批照办：**搬运时属主客户端与目标契约之间一个整体强转都别加**。
       五处逐条：
       ① / ② 评论人审与发布下发的授权口**接成了发布日志客户端** —— 两者答的不是同一个问题
            （发布日志答「稿子怎么样了」，授权答「批没批、推进到哪一步」）。组装根那 17 个
            客户端里**根本没有**授权权威客户端，`main()` 要自建 `PublishApprovalAuthorityHttpClient`，
            并**逐方法转接**（属主收信封对象、下发器要位置参数）。另：属主那条列表口没有
            `envKey` 参数、回来的视图上才有这个字段 ⇒ 过滤在转接层做，不过滤会把别的环境的
            待下发算进本机窗口。
       ③ 回执处理器要的记账口（`{enqueue, record}`）与漏斗交付的 `recordRiskFact` **不是一个形状**
            ⇒ 批 C 工厂补 `edgeHandlerPort()`，漏斗没起来时**如实答 undefined**
            （那正是消费方写明的回落条件：字段省略 ⇒ 处理器保持改动前行为）。
       ④ 推给边缘的信封没收窄（评论调度器那条口刻意声明成 `unknown`）⇒ 加守卫，不写裸强转。
       ⑤ 首作进度 `sourceId` 两侧形状不同（属主 `string | null` vs 下发器 `string | undefined`）。

       ## ⚠️ 顺带修的两处「透传丢掉调用点现推的那一层」（与 08-02 第 4 条同形）

       - **通知巡视投递的 accountId 在进程级端口上丢了**：单体里那个闭包是每连接
         `buildDispatcher` 里现造的、捕获的是该连接账号；批 E-2 把它做成进程级端口直接透传，
         账号就没了 ⇒ **所有账号的入站消息都会落默认群**，运营再也分不出是谁的。
         已把 `AutomationDispatcherDeps.notifyComments` 签名补回 `accountId`，由工厂现推。
       - **发布下发私建了一个会话配置存储且不对外暴露**：它与每连接调度器的 `sessionLimitProvider`、
         业务配置的全局活跃周历、`session_config_global` 那条属主同步读流要的是**同一个实例**。
         已改成必填注入（`sessionConfig`），编译器点名了唯一那处测试夹具。

       ## 三处真环的破法（实际选的与附图不同，理由写下来）

       环二（边缘接入 ↔ 每连接运行时 ↔ 调度器工厂）**晚绑定的是调度器工厂、不是运行时**：
       环上只需破一处，而工厂那一侧是**一个函数**，薄壳就是「调用时才解引用」一行；
       反过来破另一侧得给冷却闸与去重守卫**两个对象**各做一层代理，而代理对象正是
       「看着接上了、其实少实现一个方法」的高发区。⇒ 构造顺序里每连接运行时提到了 6b。

       ## 变异实测：逐条给出是哪条抓住的

       - 关停里调 `facebookGroupTargets.close()`（禁止调那一族）→ 结构断言红；
       - 发布下发里塞回第二份人设绑定判定 → **结构断言红，而同文件 8 条行为用例全绿**
         （「行为测试原理上看不见第二份」再次坐实）；
       - 把 schema 门挪到建池之后 → **第一版断言没抓住**：它把锚点打在整文件上，
         量到的是那条 `import`。改成只看 `runAutomationMain` 函数体后才红。
         ⇒ 「结构断言别按整文件匹配」这条，本片自己又踩了一次。

       ## ⚠️ 本片没能做到、已如实登记的三件

       1. **`runAutomationMain()` 一次都没真跑过。** 它要真属主库（写者锁在构造期就抢），
          本地桩验不了。本片交付的是「编译通过 + 结构上钉住」，**不声称进程能起来**——
          三进程真跑属批次 5，本 change 的分层验收口径本来就不声称它。已登记 backlog 簇 60。
       2. **`configMirrorHealth()` 这条属主流如实报「没有在跑的镜像刷新」**（`enabled:false`、
          `entries:[]`），不是编一份看着健康的条目表。真相是：单体那份 health 来自
          `src/config/mirror-refresher.ts`，而它在归属表里是 **api**、派生器不会拷进本仓；
          它报的四类限频配置（配额 / 节奏 / 单场 / 续场）**本进程就是属主、直读自己的库**，
          不存在「副本落后于属主」。**真正的缺口不在这条流上**，而是「后台改了这四类配置之后
          谁去通知本进程重读」——三进程形态下今天没有这条通道，见 3.5h。
       3. **运营的调度启停通道本片不接**：`isDispatchActive` 按裁定是进程内布尔、`main()` 自持，
          但 `AutomationDispatchCommandReceiver` 的路由没注册 ⇒ 本进程今天恒为「在跑」。
          它卡在接口侧、已是台账条目 `feishu-operator-dispatch-start-stop`，属**已登记缺口**。
          **MUST NOT 为了凑「接好了」的样子塞一个没人能调的接收方。** -->
...
- [x] 3.5i **跨进程「谁在服务哪条路由」的普查（3.5g 之后立刻做的，因为第 5 片刚接了四个内容客户端）。**
  <!-- **结论：派生仓手写入口服务的路由 ≠ 单体服务的路由，而此前没有任何机制会发现这件事。**
       判据是机械的：把三仓 `src/server.ts` 里真被调到的 `register*Routes` 与共享包导出的那批
       逐一对照。实测结果（2026-08-04）：

       | 进程 | 缺口 |
       | --- | --- |
       | content | **七组内容属主路由只注册了两组**，而自动化的真装配已经在调其中四组 |
       | api | 缺 `schedule-feedback` 一组（自动化已消费） |

       **为什么零信号**：客户端建得出来（构造函数只吃基址与令牌）、调用点编译得过
       （类型描述形状、不描述「对面有没有这条路由」）、两仓测试各自全绿 —— 只有真把两个进程
       一起跑起来才 404。而那是**跨进程**的 404：调用方读到的是「对面不支持这个方法」，
       一个本该留给「对面版本落后」的具名原因，被一次纯接线遗漏冒名顶替。
       单体那份代码早就写下过这句预言（用量记账那段注释：「MUST NOT 因为『现在没人调』就不注册：
       那会让第 3 段写 main() 时才发现对面根本没有这条路由」）—— 它成真了，
       **只是发生在派生仓的手写入口上，不是单体上**。

       **content 侧已修**（aidcp-content `f8b714d`）：补注册三组（curated-write /
       facebook-publish-media / llm-usage-recording），属主实例本来就已经在组装根里建好、只差注册。
       形态照单体：各注册各的 + 缺实例即具名 warn，**绝不注册一条属主不在就静默成功的空路由**。
       **回复生成随后也接上了**（aidcp-content `7305b46`）：它只要模型客户端 + 单步超时，
       两样本进程都有；单体早把它从自动化段挪到内容段、理由逐字就是「内容进程因此拿不到它、
       那条路由永远注册不上」，本仓漏了同一步。
       ⚠️ 缺它的后果不是「回复质量差一点」：自动化侧对回复生成是「缺席则整条能力不组装」，
       而**跨进程 404 不等于缺席** —— 客户端在、路由不在 ⇒ 能力照常组装、每一次分类 /
       润色 / 风险复核都拿到失败，表现成「互动一直在跑但什么都没产出」。
       **只剩文字卡转写一条 pending**：它还缺视觉客户端与形态判别器两样料，不是一行注册能补的。
       并加了一条**只许下降的清单闸**（`test/acceptance/content-authority-routes.test.ts`）：
       共享包导出的每个内容属主 registrar MUST 在清单里（**漏登记是静默的，比漏注册更危险**），
       状态与组装根逐条一致，pending 接好后必须移走否则闸自己红。
       变异实测两条都抓得住：删一组注册 → 红；清单漏登记一条 → 红。

       **api 侧只记不修**（见 3.5j）：那一条不是漏一行注册，属主实现也不在。

       **顺带更正一条已过期的记录**：HANDOFF §4.8 still 写着「0.8g —— 活缺口：api 侧手写 main
       既没注册 `provider-secret/get-for-runtime`、也没注册 `role-model-selection/fetch`」。
       **实测两条都已注册**（tasks 0.8g 自己也标着 2026-07-31 已修）。已在 HANDOFF 里改掉。
       ⇒ 又一次印证：登记比代码旧，动手前先读代码验证缺陷还在。 -->
- [ ] 3.5j **api 进程不服务 `schedule-feedback`，而自动化已经在调它**（3.5i 查出，只记不修）。
  <!-- **不是漏一行注册**：单体里注册的是一个**转发器**，真正的落点是内容排期器
       （`ctx.scheduledTaskNotStartedSink`），而三进程形态下排期器本该跑在**接口进程**里 ——
       实测 `ContentScheduler` 类在 aidcp-api 仓里，但 api 的手写 `main()` **没有构造它**。
       ⇒ 补这条要先把排期器接进接口进程，那是另一件事，不在本 change 的任务清单里。

       **当前后果是可接受的、且已如实降级**：自动化侧那个转接层把回程失败吞成一行 warn
       （名额不归还，与「排期器看过了、没接管」同后果），不会拖住调度链。
       但**账面上仍不诚实**：自动化装配把这条能力标成 `wired`，而对面根本没这条路由。

       **顺手更正 HANDOFF §三那句「另两条 api 属主事实（账号暂停 / 排期名额回程）已接（f7e9043）」**：
       只有一半成立 —— 账号暂停**真的通了**（`pauseAccount` 在 api 组装根里有实现、
       且经 `registerAccountRuntimeRoutes` 注册），排期名额回程只有客户端与单体路由，
       接口进程这一侧是空的。

       接的时候 MUST 照单体那个形态：**缺排期器时响亮抛具名错误，绝不回 false** ——
       回 false 读作「排期器看过了、没接管」，与「这个进程里根本没有排期器」完全同形，
       而后者是配置问题、必须有人去修。 -->
- [x] 2.4d-用量 **模型 token 用量记账：三段全接通**（属主方法 + 路由 + 调用方合并缓冲）。
  <!-- **它此前被记成「决策不是落地」，两个决策实际都已经有答案了**：
       ① 属主补方法还是交适配对象 —— 属主**早就补了** `recordUsage`（本轮实读 `token-usage-store.ts`
          确认；tasks 里那句「属主今天没有这个方法」已过期，是本轮第三条陈旧记载）；
       ② 合并缓冲落在哪 —— 模型出口工厂的注释里早写着「合并缓冲的家是自动化进程自己的 `main()`」。

       落地：aidcp-content（路由注册，见 3.5i）+ aidcp-automation `b9a2caf`
       （`src/automation-llm-usage-buffer.ts` + 挂到文本模型出口的 `onCall`）。

       **形状为什么不是照抄属主签名**：属主入口是同步、无返回、纯内存的 `add()`，
       那个签名跨不过进程边界（`void` 无处报错），而一次调用一条 HTTP 会把一条低频旁路
       挂到每一次模型调用的热路径上。所以跨界端口是「提交已合并的增量行」，合并留在调用方。

       **三条 MUST 各有一条用例，且都做过变异实测**（逐条给出是哪条抓住的）：
       - 桶起点由本侧在调用发生那一刻戳 → 改成提交时算，跨桶用例红；
       - 传输失败 **MUST NOT 重投**（属主侧是 `ON CONFLICT … 累加`、不是幂等写，重投即翻倍；
         超时是「结果未知」，用量是可丢的观测数据 ⇒ 丢弃 + 计数留痕）→ 加一次重投，两条用例红；
       - 回执是「真落库了几行」，`applied` 小于提交行数即计数留痕 → 与传输失败**分两个计数器**，
         含义不同不许混成一个。
       另：总 token 取厂商回报值、绝不由输入 + 输出凑 → 改成相加，用例红
       （凑数会把缓存命中 / 思考 token 那个差额抹平）；缺账号就地丢弃、绝不兜默认账号 →
       兜 `'default'`，用例红；`record` 绝不抛（它跑在模型调用完成路径上）。 -->
- [ ] 2.4d-用量-占位 **三个维度占位值是跨仓手抄件，应当抬进 kernel**（本轮留下的已知债）。
  <!-- `untagged` / `unknown` / `unknown` 两侧各持一份：属主那份是 content 用量存储里的模块私有
       常量，自动化那份在 `automation-llm-usage-buffer.ts` 里。**不一致不会报错**，
       只会让同一种「未打标」的调用按写入路径落成两行不同维度的账 —— 表不报错，曲线悄悄裂成两条。
       正解是把这三个值（连同那条「空串/缺省 → 占位」的归一规则）抬进 kernel 的
       `llm-usage-recording-port.ts`，两侧都从那里取；顺带把属主侧 `normalizeDim` 改成调它。
       **没有当场做的理由是成本**：动 kernel 要走完 kernel → cloud（事实源）→ sync → transport pin
       → 三仓 pin → `npm install` 整条链，而本轮已经在做另一件事，半途开这条链的风险高于收益。
       在它落地之前，**改任何一侧的占位值 MUST 同批改另一侧**（两处代码注释里都写了这句）。 -->
- [ ] 3.5h **四类限频配置在三进程形态下「改了没人通知本进程重读」**（3.5g 实读发现，未修）。
  <!-- **不是本片引入的，是拆进程本身带出来的一个还没人处理的缺口。**

       事实：`quota_config` / `pacing_floor_config` / `session_config_global` / `resume_config_global`
       这四类的**属主是 automation**（表在本进程的库里），本进程直读自己的库；而单体里让这四份
       进程内副本保持新鲜的，是 `src/config/mirror-refresher.ts` 那个轮询刷新器 ——
       它在归属表里是 **api**，派生器不会把它拷进本仓，本仓也确实没有任何调用点。

       ⇒ 三进程形态下，本进程的这四份内存副本**在启动时读一次之后就不再刷新**。
       现象是「后台改了单场时长 / 配额 / 节奏地板，自动化进程一直按旧值跑，且不报错」。
       表在自己库里、读得到、只是没人去重读 —— 这正是本 change 反复点名的那种形态。

       **别按「把刷新器搬进来」直接开工**，先裁定三件事：
       ① 刷新器读的是 `config_mirror_version`（**api 属主表**），本进程能不能读、还是要换判据；
       ② 这四类的写路径今天走 `writeWithMirrorBump`（本域 outbox → 中继推给 api），
          那么「本进程自己写完之后刷新自己」与「别的进程改了之后通知本进程」是两条不同的路，
          可能只需要后者；
       ③ `automation_config_mirror_health` 那条属主流的载荷届时要不要跟着变
          （3.5g 现在如实报 `enabled:false / entries:[]`，**MUST NOT** 为了让面板好看而先编条目）。
...
- [x] 4.1 **首批撤条：11 → 6**（automation `5fe3f97`，2026-08-04）。
  <!-- 撤的五条全属**真靠接线消掉**：概念池写 / 精选写 / FB 发帖素材 / 用量记账 / 回复生成。
       判据就是台账文件头那一句（只列「阻止本包交付完整生产进程」的依赖），判例是 4b 那组。

       **逐条的证据是「两端都查过」，不是「接了个客户端就算」**：`main()` 真把客户端喂进了消费点，
       **且 content 进程的手写入口真的注册了那条路由**。后半句是本轮才补的核查（见 3.5i）——
       此前 content 入口只服务七组里的两组，而「客户端在、路由不在」编译得过、两仓测试各自全绿，
       只有真跑两个进程才 404。

       ⚠️ **差点多撤一条，值得记**：起初把 `content-generic-llm-authority` 也算进来了，
       理由是「本包的文本模型客户端取自共享包、不再碰 content」。**判错了** ——
       它锚的不是模型客户端，是**内容生成链**（`PublishGenerationHttpClient`），
       本根至今没构造它，且另有一条断言明令禁止伪装。
       是**编译器**按 id 字面量当场点名的（撤条后联合类型里没有它了），**不是读文档发现的**。
       ⇒ **撤条前先 grep 那个 id 的全部引用，别只看名字像什么。**

       余 6：运营指令 3（要先裁定「那些文案串不再构成证据」，见 1.3b）、
       content 2（文字卡转写：对面至今没构造转写器实例；内容生成链）、组装根 1。

       三处同批改到位：常量 + JSON 投影（`--refresh-ledger`）+ 派生分叉那份测试里写死的计数
       （已写明裁定 —— 计数只许因裁定而下降，无裁定的下降 = 某个探针不再命中 = 回归）。
       **cloud 那把尺不跟着降**（AST 派生、问的是另一个问题），两把尺合法分叉。 -->
- [x] 4.1c **第二批撤条：6 → 5**（automation `30d2cc3`，2026-08-04），撤的是**调度启停**。
  <!-- 第六条真靠接线消掉的。缺的那一半是**接口侧客户端**：api 手写入口对四条运营指令一律
       `throw automation_operator_command_unavailable:*`。本次建 `AutomationDispatchCommandHttpClient`
       并把读写两条一起接上（api `3159e10`）——**只接读会变成「灯亮着、按钮按下去没人接」**。
       语义逐条照单体：三种非成功回执分开说（没送达 / 键撞了）、回执 state 原样回传不重组、
       状态灯原样回三态且读失败**抛**（MUST NOT catch 成 `active:false`）。
       配结构断言 + 变异实测：压成 active:false → 红；两种回执合并成一句失败 → 红。

       **撤条前两端都去对面 `main()` 里读过**：automation 侧路由真注册（`2f5f6a9`）、
       api 侧客户端真构造。**一件必须一起知道的**：接口进程按设计**不起面板监听**
       （`API_SYNC_READ_PUBLIC_SURFACE_LEDGER`：那些 DTO 面仍归 cloud-panel），
       所以三进程形态下运营还点不到那个按钮 —— 但那是「面板整体还没搬」、对每个面板面都成立，
       不是这条通道特有的欠账，也不阻止本包交付完整生产进程。飞书那侧本就没有 dispatch 动作（1.4）。

       ⚠️ **顺带查出一件更要紧的事：`aidcp-api` 的 master 头当时 typecheck 是红的**，
       而且不是本轮改动引入的。上一次同步（api `35c63ba` / kernel `6101b1e`，derive 自 cloud `c394f36`）
       把「撤权保持读」并进了台账端口，端口与 api 属主存储都随同步进了本仓，
       **而手写入口从不同步、没人跟着补** ⇒ `PgOffboardAdmissionLedger` 少一个方法、整仓编译不过。
       已按单体的接法修（读面委托客户花名册属主那一份，绝不在台账类里抄第二条 SQL）并配正向委托断言。
       ⇒ **同步之后 MUST 逐仓 typecheck**（5.1/5.2 本来就这么写着）；只跑对账不跑编译，
       红会静静躺在派生仓的 master 上。实测当时只有 api 红，cloud / automation / content 都绿。 -->
- [x] 4.1d **第三批撤条：5 → 4**（content `e924e2a` + automation `8d67f5a`，2026-08-04），
  撤的是**文字卡转写**——它是 08-04 首批里唯一没跟着撤的那条。
  <!-- 缺的一半在**对面**：本包早就齐了（`main()` 建客户端 + 配旗标取值闭包 + 按 `{state:'wired'}`
       喂角色工厂），而 content 进程从没构造转写器实例，那条路由在 content 的清单闸里一直挂 pending。
       本次在 content 的 `main()` 里建出属主实例并注册路由：
       · **判形那一档另起一个 sensor**（逐条照单体，也是最容易被「简化」掉的一处）——
         旗标不同（封面感知 `AIDCP_COVER_FORM_SENSING` vs 转写准入 `AIDCP_TEXTCARD_OCR`，
         复用一个会把两个能力绑死）、回写不同（封面那个带 `annotate` 回写精选行缓存，这档刻意不带）；
       · 视觉模型也分两档，转写走自己的模型 / 供应商解析。
       · **路由无条件注册**：旗标关时属主答「未启用」并回显取值供客户端对账 —— 那是答案；
         挂在旗标上的话，关旗标时客户端拿到跨进程 404、被译成「对面不支持这个方法」。

       **变异实测暴露了一处闸的盲区，值得记**：把注册塞进 `if (旗标)` 之后，
       content 那条只许下降的清单闸**照样全绿** —— 它只数「注册函数被调了几次」，
       调用在不在条件里它看不见。已补一条按**顶层语句**判的断言（main 体内 2 空格缩进、独占一行）。
       另一条变异（转写器改吃封面那个 sensor）被新写的结构断言当场抓住。
...
- [x] 4.1g **内容生成链那条：整条链已实读测绘，且顺手修掉一处「只有真跑两个进程才现形」的接线错**（2026-08-03）。
  <!-- automation `c2c35c0`。**本条不撤台账**——它记的是「这条为什么比字面大得多」的**具体形状**，
       下一手照着开工，不必再测绘一遍。

       ── 一、先修掉的那处（与本条同批，automation `c2c35c0`） ──────────────

       **发布授权客户端拿错了令牌。** 接口进程给「授权权威」与「授权决定写」那两组路由挂的是
       `AIDCP_PUBLISH_APPROVAL_INTERNAL_TOKEN`，而 automation 的 `main()` 传的是
       `config.apiInternalToken` —— **两个 env、没有互相回落**，单体两侧用的一直是前者。
       后果是本进程**每一次**授权读写都被判未授权。
       **零信号**：编译过、两仓测试各自全绿，只有真把两个进程一起跑起来才现形，
       且现形方式是「授权读不出来」——一个很容易被当成业务原因的说法。
       修法：令牌进根配置、同档必填（缺了启动就停），并给**每一个必填 env** 都配了拦停用例
       （此前只覆盖了 content 那一个，所以新增一个 env 时它会悄悄可缺省）；
       另配结构断言钉住「MUST 用授权专用令牌」，变异实测能红。
       **它是测绘这条链时顺手撞见的，没有任何机械手段会发现它。**

       ── 二、这条链的完整测绘（下一手照这个开工） ─────────────────────────

       **链是这样的**：生成链客户端 → `PublishScheduler`（发帖触发器）→ 某个真触发它的东西。
       前两段今天就能接（见下），**卡点在第三段**：

       · **`PublishGenerationHttpClient`**：内容进程的手写入口**已无条件注册**那条路由
         （`registerPublishGenerationRoutes(httpServer, publishOrchestrator)`，已去对面 `main()` 读过）。
         客户端基址取 `config.contentBaseUrl`；**单次调用超时必须取 `INTERNAL_HTTP_TIMEOUT_CEILING_MS`
         （180s）**，否则每一段 long-poll 都会在服务端回 `{done:false}` 之前被客户端切断
         ⇒ 每次跨进程生成在默认 15s 确定性失败。
         **本进程 MUST NOT 有「本地编排器」回落**：单体那条 local 分支取的是内容段构造的编排器，
         而本进程根本不跑内容段；给它回落只会得到「启动日志说已就绪、每次发帖在调用点炸」，
         而排期发帖的小时格幂等票**在触发前就已认领** —— 失败一次就烧掉那一小时。

       · **`PublishScheduler` 的依赖今天全都在**：概念池 / 精选召回取组装根那两个 content 客户端；
         点赞库与风控解析取 `riskFoundation`（`likedNoteStore` 是**可选**的，缺了 MUST 按单体口径
         具名不建、别硬上）；发布台账与账号平台取 api 客户端；人设三态与取人设取共享那两个函数；
         「唯一真实账号」取花名册端口、**读失败回 null 不抛**（调用方据此 honest-fail）。

       · ⚠️ **卡点：本进程今天没有任何东西能触发它**。三个候选全不可用：
         ① **排期 tick（`ContentScheduler`）属 api**（`boundaries/module-ownership.json` 实查），
            automation 仓里根本没有这个类；**而 api 的手写 `main()` 也没建它**
            ⇒ 三进程形态下今天**没有任何进程在跑排期发帖**。这是本轮测绘出的最要紧的一条。
         ② **手动发布那条运营指令路由刻意不接**（1.7b 裁定：接一条今天不执行的通道就是新增假绿）。
         ③ **委托任务工作器本进程没建** —— 这是唯一可达的那个，见下。
         ⇒ **只加生成链客户端 + 建 scheduler 是「建好零消费方」**（本项目点名的反模式，
            且 typecheck 当场就以「声明了没人读」拦下来了）。本轮据此**主动回退**了那半成品。

       ── 三、下一手最省的一条路：先建委托任务工作器 ───────────────────────

       它同时是 scheduler 的真消费方，且**四个回调所需的东西几乎都已经在**：
       `AutomationPublishDispatch` 已经导出 `preflightApprovePublish` / `notifyPublishRejected` /
       `refreshPublishPreview` / `triggerPublishDispatchOnApprove` / `uiSnapshot`；
       评论那半取 `main()` 已有的评论调度器；候选稿读写取 publishLog 客户端。
       **唯一缺的是 `writeApprovalDecision`** → 建一个 `PublishApprovalDecisionWriterHttpClient`
       （**路由 api 的手写 `main()` 已经注册**、用授权专用令牌 ⇒ 零新增契约，且本批已把令牌修对）。

       ⚠️ **顺带记一条本轮实读发现的现网缺口（不属本条，但必须有人知道）**：
       委托任务在自动化进程里今天**能建、能确认，但没有任何东西会执行它**，
...
- [x] 4.1b 余下 **2** 条的清零路径（各自的前置写在下面，别当成一批）。
  ...
  <!-- · **运营指令 3 条 —— 前提 2026-08-04 被推翻，别照旧读**：原先记的是「接线早已完成，
         ...
         拆开之后才成为 api → automation 的一跳，而**自动化进程的手写入口一条都没注册**
         （组装根只注册三个成对指令接收方），api 的手写入口也没构造任何对应客户端。
         ⇒ 与 3.5i 完全同一形态的第四次撞见，**只是这次撞的是本 change 自己刚写的入口**。
         探针那一关**仍然在**，但它排在「路由真的注册了」之后，不是替代关系。

         **调度启停已清零**（automation `2f5f6a9` 服务端 + api `3159e10` 客户端，撤条见 4.1c）。
         余下两条（委托自由文本 / 委托卡片动作）仍要各自的处理器，不是一行注册。
         ...
         **2026-08-04 实读补：这两段读的不止精选，还有候选稿**——三条候选稿动作
         （approve / reject / modify）要 `publishLogStore.loadForDispatch(recordId)` 校验
         「存不存在 / 归不归这个账号与平台 / 是不是待审 / 版本对不对得上」。那是 **api 属主**的
         publishLog 4a 端口，而自动化根**已经构造着它的客户端**（判例：2.9 撤条时查过同一条链），
         所以这一半零新增。⇒ 这条的新增面只有精选那一读。
         ...
         **⚠️ 2026-08-04 实读补一条决定性证据：A 其实不成立，别再当成「省事的那个选项」。**
         ...
         代码明写着 `MUST NOT 复用 curated_target_unavailable（那句是「这行不存在」= 谎）`，
         靠的是 kernel 那个**按名字判**的守卫（`isCuratedContentUnavailableError`）。
         而裸那条路由的客户端是**光秃秃的 `http.call`**：跨进程后对面的 42P01 到这边只剩一个
         普通传输错误，守卫恒 false ⇒ 「精选库暂时不可用」会被如实地报成「目标不存在或不属于该账号」。
         那正是本 change 的红线形态，且**编译期与测试都看不见**。
         受鉴权那一族（`content-authority-http.ts`）本来就把错误按码还原成 `ContentPortError`，
         而 kernel 的 `curatedContentFailureReason` **两类抛出物都认**（注释明写「只认前一类今天恒 false，
         只认后一类拆完恒 false」）——所以走 B 是顺着既有判例，不是额外讲究。
         另：裸那条路由今天的唯一消费方是 `DataGateway` 的 `gatewayMode === 'http'` 旁路
         （cloud `src/server.ts` 的 `gatewayRemote`），不是主线三进程路径。
         ⇒ **选 B**（或先把裸那族整体升级成信封形态，但那要连它现有消费方一起动，范围更大）。
         选 A 除了「无鉴权、无 target 校验」这两条，还要自己再补一遍错误还原 —— 省不下什么。

         **MUST NOT 省掉这两个钩子把服务先接上**：它们是「目标到底存不存在 / 是不是待审 /
         是不是这个账号的」三问的唯一执行点。省掉之后确认卡照发、任务照建，
         等真去执行时才发现目标不对 —— 那正是本 change 的红线形态。
         `listAccounts` 同理不能凑合：单体那份要账号显示名与平台，本进程得从花名册客户端
         + 账号投影镜像拼，缺了会让委托解析选不中账号。
       · **文字卡转写：已清零**（见 4.1d）。当时记的「MUST NOT 只补一行注册」这条判据成立且被照办 ——
         属主实例先建出来，路由才注册。
       · **内容生成链**：本根要真构造 `PublishGenerationHttpClient` 才谈得上撤，
         **且比字面大得多（2026-08-04 实读）**：本进程连**发帖触发器**（`PublishScheduler`）都还没建，
         而单体那处的依赖表有十几项（概念池 / 点赞库 / 发布台账 / 风控解析 / 人设绑定三态 /
         精选召回 / 取人设 / 三个并发帽 …）。生成链客户端只是它的一个入参 ——
         **别按「构造一个客户端」估这条的工期**。
