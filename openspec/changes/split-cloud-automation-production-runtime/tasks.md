...
- [ ] 3.1c-裁定 **用户 2026-08-01 选定路线 A（改判归属 / 析出纯判断段）。⚠️ 实读后只有「析出纯段」那一支成立，
  ...
  <!-- **整文件改判不成立的证据**：新鲜度查询口在单体里有 **7 个 api 属主消费方**直接 import
       ...
       **落地顺序（动手前照这个走，别跳步）**：
       1. 控制仓事实源 `docs/cloud-service-decomposition-proposal.md` §4.7 先落这条裁定（含 docpatch R1 的回写）；
       2. kernel 加无状态工厂 → 单体那三份改成调它（**行为 MUST 逐位不变**，7 个 api 消费方一行不改）；
       3. 自动化侧按同一个工厂建自己的闸，事实源接本进程自己的镜像刷新（**注意本进程恰恰是
          「配置副本健康」那条属主流的发布方**，两者要对得上）；
       4. 四份 `ownership-rules.json` 手工增量追加（**绝不整体重序列化**），跑一次门禁对账。
