...
- [ ] 3.1e **批 E 后半（每连接角色调度器工厂）撞到一个必须先裁定的归属问题 —— 与 3.1c 同形，
  <!-- **步骤 1 已落**（cloud bc690ce / kernel ac02b74 / transport 8f88509 / api 6b00006 /
       ...
       **⚠️ 批 G 后续片开工前必须知道的一件（G-1 实读得出）**：
       消费协调器要的运营策略决策是 **api 属主的 `resolveForAccount`——异步、且含慢启动解析**，
       与步骤 2 给自动化侧的**同步基线口 `facebookOperationBaseFor` 不是同一个东西**。
       自动化进程今天**拿不到**它。⇒ 协调器那一片要么再补一条跨进程口，
       要么把慢启动那段也析出。**MUST NOT 用同步基线口顶替**——
       那会丢掉慢启动档位，让本该按爬坡节奏跑的账号直接按满档跑。
