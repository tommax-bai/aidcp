...
> **2026-08-01 晚 · 现状（这一段是当前事实，读它就够；逐批沿革移到文末 §10）**
>
> **六仓**：cloud `8d62dca` / kernel `27cbfc5` / transport `51e6351` / api `5d54143` /
> content `1ddb917` / automation `f1b6a22`；控制仓见 `git log`。
> 共享包 pin 已按 kernel → transport → 三个业务仓抬完，六仓 pin 全对齐。
...
>
> ### 第 3 段八批：A / A-1 / B 已落，C 落大半，D 落了前置
>
> | 批 | 状态 | 落点 |
> | --- | --- | --- |
...
> | D | 前置 | 出口闸放行判定析出 kernel（`8d62dca`）；**正文未开工** |
> | E / F / G / H | ⬜ | 未开工 |
>
...
> ### 下一步（按依赖）
>
> 1. **批 D 正文**。实读结论：边-云服务端的构造**深度纠缠批 E/F**（出口闸闭包、`onClose`、
>    `onEdgeRegistered` 三处分别要连接运行时注册表、界面快照、互动存储）。
>    **按 B/C 的办法**：取批 E/F 的东西一律做成**必填注入端口**，让编译器逼后面那两批面对；
>    本批只装配自己那几件（验证码协助与协调、指令定序器、租约客户端、消息处理器、
>    边-云服务端、两个成对指令接收器）。
>    验收：`AC-PROTO-*`，外加 CLAUDE §2 那**第 4 处同步**（主动命令路由白名单，typecheck 抓不到）。
> 2. **批 C 剩的三件**（不阻塞 D）：面板事件投递客户端、配置面审计中继
>    （**写的是接口属主表，跨进程后 MUST 走已有通道、别直连**）、风控指令消费者接线。
> 3. 批 E 与批 F **可并行** —— 整个第 3 段唯一一处能并行的缝，别浪费。
>
> ### ⚠️ 三条现在就成立的状态，别读错
>
...
> - **另一路 session 在同时推主干**：`46a7003`（FB Reel 节奏）已随本轮派生进子仓；
>   `534af19`（FB 主浏览面）**刚推、本轮未派生** —— 对账里剩下的「内容不同」全是它。
>
> ### 本轮结掉的两件裁决（都动了事实源文档，本 change 第一次）
>
...
> ② automation 另有 **6 条 `⊙` 派生仓私有文件**（组装根 + 本轮新加的启动外壳 / 判据清单 /
>    模型出口 / 风控底座 / 记账 / 停手闸）——**新增这类文件 MUST 登记进控制仓
