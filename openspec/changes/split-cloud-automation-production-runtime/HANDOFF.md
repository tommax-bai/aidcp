...
> | 项 | 值 |
> | --- | --- |
> | 六仓 | **cloud `0d507c5`** / **api `f69bfe0`** / **automation `6f1a544`** / **content `039cd26`** / **kernel `e8396ce`** / **transport `346d0c8`** |
> | 工作区 | 六仓全干净、全已推、**对账零漂移**、两个共享包 pin 全对齐（本轮走了一遍完整级联） |
> | 测试 | **cloud 4121**（+11 skip）/ **api 505** / **automation 2167**（+3 skip，与基线同）/ **content 444** / kernel 70 / transport 36，全 0 fail |
> | 门（真交付物） | **2 条**（内容 1 / 组装 1）—— **运营指令那一类已归零**；四批撤条 **11 → 6 → 5 → 4 → 2** |
> | tasks.md | **89/146**（这把尺量的是「查清了多少」，不是交付；分母会随勘察长大） |
...
>
> **别按「构造一个客户端」估内容那条的工期**：单体那处发帖触发器的依赖表有十几项
> （概念池 / 点赞库 / 发布台账 / 风控解析 / 人设绑定三态 / 精选召回 / 取人设 / 三个并发帽 …），
> 生成链客户端只是它的一个入参。而 `test/transport/publish-generation-http.test.ts`
> 那条断言正是防止「伪装成已接线」。
>
> ---
>
> ### 三、本批（委托任务）做完之后仍然成立的几条判据
>
