...
> | 项 | 值 |
> | --- | --- |
> | 六仓 | cloud `c394f36` / api `35c63ba` / **automation `173b234`** / content `46c13f7` / kernel `6101b1e` / transport `1444d59` |
...
> | tasks.md | **81/135**（这把尺量的是「查清了多少」，不是交付） |
> | 边界 | 跨域边 0，豁免 0；4a 方法槽 **58**、组数 21 |
> | dev | 已部署到 cloud `c394f36`（含迁移 0107 / 0108） |
>
> **第 3 段八批：A–G 全落，H 已落 4/5 片，只剩写 `main()` 那一片。**
>
> | 片 | 内容 | 状态 |
> | --- | --- | --- |
...
> | **5** | **12 个工厂接进组装根、写 `main()`** | ⬜ **就剩这个**（3.5g） |
>
> **门为什么一直是 11**：整个第 3 段它都会停在 11。撤条判据写在 automation 台账自己的 docblock 里
> （「阻止本包交付完整生产进程的依赖」），而写 `main()`（3.1）排在清台账（4.1）之前。
> **第 3 段的交付物是「让每条依赖变得可满足」，减门是第 4 段的事** —— 不是漏了。
>
> ---
>
> ### 二、下一步：只剩第 5 片（tasks 3.5g），前四片已经把料备齐
>
> **现成可用的（不必再推一遍）**：
>
> - **`main()` 第一句是 schema 契约门**：`runAutomationStartupSchemaGate()`，建池之前。
>   **这条不靠记** —— 启动外壳的 `schemaGate` 是必填、回执品牌位不导出、外部造不出来，漏了编译期就红。
> - **养号事实第四项**：`AutomationSyncReadMirrors.facebookSlowStartPolicy()`。三态处置写在它的注释里：
>   新鲜用它 / 陈旧**沿用上一份** / 一次都没收到过就**整个不提供** `facebookSlowStartPolicy` 这个方法
>   （契约上只允许「方法不在」这一种缺席表达，返回 `undefined` 会让调用点 `.totalDays` 当场炸）。
> - **边缘接入的 `interaction` 必填口**：`createAutomationInteraction()`。**它是异步工厂**
>   （二态要由真实 schema 探测决定）。返回的 `start()` 要在**就绪闸放行之后**调（保留期清理表），
>   `dispose()` 无条件调一次。
> - **破真环的现成范式**：同文件的 `createLateBoundInteractionEdgeBinding()` —— 先建薄壳、把
>   `binding` 交给互动工厂，边缘服务端造好后 `bind()` 一次。**未绑定即具名抛错、重复绑定也抛，
>   绝不返回 0**（返回 0 会让「出口没接上」与「边缘不在线」同形）。三处真环都照这个形态破。
>
> **14 个空口的当前状态**（逐条判据仍以 tasks 3.5g 为准）：
>
> | 口 | 现状 |
> | --- | --- |
> | 配置副本陈旧判定 / 记账断链判定 | 实现早已有（批 C 两半），**只差喂进批 B 的底座** |
> | `interaction` | ✅ 已有工厂（第 4 片） |
> | 风控 `nurture` 的第四项 | ✅ 已有取用口（第 3 片） |
> | 会话配置的全局活跃周历 | automation 属主、本进程直读。⚠️ 与发布下发工厂**私建**的那个存储必须是同一实例，照现状接会有两份、各持一套缓存、两边都不报错 |
> | 群评论时序策略 | **用户已裁定暂不接**，必须显式传具名缺席 |
> | 组装根的 `syncReadSources` / `facebookScope` | **无任何工厂供给**；前者是构造入参、供给方却住在工厂身上 ⇒ 真环 |
>
> **⚠️ 写 `main()` 之前必读的三条**：
...
> 3. **别照抄单体自动化模式的启动顺序**：单体是先开边缘口、后装同步读，与本 change 要求的顺序
>    正好相反。这一段是**新写**，不是搬运。
>
...
> 台账常量当前 11 条，与 JSON 投影逐位相等。
>
> **⚠️ 部署形态 MUST 是 stop→start，禁止滚动 / 蓝绿**：风控写者锁是会话级 advisory lock、
> 构造期就抢，两个进程重叠期间后起的那个会抢不到锁并拒绝启动。
>
> > 📌 **施工附图：`batch-h-wiring-map.md`（与本文件同目录）。** 逐工厂契约（要什么 / 给什么 /
> > 关停语义 / 构造期起不起表）、14 个空口逐条怎么办、三处真环的破法、**16 步构造顺序**、
> > 关停的「安全调 / 禁止调」两栏、五条 owner 流的供给方 —— 全在那份里。
> > **判据与红线仍以 `tasks.md` 3.5c–3.5g 为准，附图只回答「具体怎么接」。**
