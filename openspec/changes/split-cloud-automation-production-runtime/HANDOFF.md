...
>
> **第 3 段八批：A–H 全落 —— 第 3 段的接线部分做完了。**
>
> | 片 | 内容 | 状态 |
> | --- | --- | --- |
> | 1 | 关停真空（业务入口必填 `dispose()`）+ 属主池透传 | ✅ `fc99d52`（3.5c） |
> | 2 | schema 契约门接线 | ✅ `1e6d3b4`（3.5d） |
> | 3 | Facebook 慢启动曲线进同步读流 + cursor 迁移 `0108` | ✅ cloud `0fac1ce`+`ed78c2c`（3.5e） |
...
> ---
>
> ### ⚠️ 写完 `main()` 之后立刻查出来的一件事（3.5i，已修一半）
>
> **派生仓手写入口服务的路由 ≠ 单体服务的路由，而此前没有任何机制会发现这件事。**
> 第 5 片刚把四个内容客户端接进自动化装配，顺手去对面查了一下：
> content 进程的手写 `main()` **七组内容属主路由只注册了两组**；api 进程缺 `schedule-feedback` 一组。
>
...
>
> **委托任务那条的尺寸已实读定过，别再推一遍**：接收方与服务本身依赖都很薄，
> 要点在目标校验那两个钩子 —— 它们要按「精选条目 + 账号」去读一行精选内容。
> **MUST NOT 省掉这两个钩子先把服务接上**：它们是「目标存不存在 / 是不是待审 /
> 是不是这个账号的」的唯一执行点，省掉 = 确认卡照发、任务照建、执行时才发现目标不对。
>
> ⚠️ **这里我当天写错过一句，更正留在 tasks 4.1b 里（错的那句更有教训）**：
> 先写的是「那个读方法今天没有任何跨进程端口」—— **只 grep 了 kernel 的端口、没 grep 传输目录**。
> 事实是那条路由早就存在、客户端现成、**content 进程也已经在服务它**。
> ⇒ 真正要裁的是**走哪条路**：既有那条是**裸形态**（无鉴权、无信封、**无 target 校验**，
> 而 DEV/OL 长期共库），还是给 kernel 精选端口补一个受鉴权的读方法（口径统一、顺手还债，
> 代价是整条 pin 级联）。**建议后者**，别顺手选前者 —— 那会把一条遗留旁路变成承重依赖。
>
...
>
> **3.5g 得出的、下一批照办的两条**（详情在 tasks 3.5g 注释里，那份是权威）：
>
> - **搬运时属主客户端与目标契约之间一个整体强转都别加。** 首版写了 29 处 `as never`，
>   去掉之后编译器当场点名 **5 处真的接错了**（两处授权口接成了发布日志客户端、
>   记账口形状不对、信封没收窄、首作进度字段形状不同）。24 处纯属多余，5 处是真 bug ——
>   **这个比例本身就是判据**：加强转的时候你以为在省事，实际是在关掉唯一会说话的机制。
> - **「直接透传端口」会丢掉调用点现推的那一层**（08-02 第 4 条，本片又中两处）：
>   通知巡视投递的 `accountId` 在进程级端口上丢了 ⇒ 所有账号的入站消息都会落默认群；
>   发布下发私建的会话配置存储与调度器 / 周历 / 属主同步读流要的是同一个实例。
>   两处都已修，且第二处改成必填注入之后**编译器当场点名了唯一那处夹具**。
>
> **⚠️ 三处真环的实际破法与附图不同，按这个读**：环二（边缘接入 ↔ 每连接运行时 ↔ 调度器工厂）
> 晚绑定的是**调度器工厂**，不是运行时 —— 环上只需破一处，而工厂那侧是一个函数、
> 薄壳就是「调用时才解引用」一行；反过来要给冷却闸与去重守卫**两个对象**各做代理，
> 而代理对象正是「看着接上了、其实少实现一个方法」的高发区。构造顺序里每连接运行时因此提到 6b。
>
> **⚠️ 关停时逐条对着 close 语义表写**，关错打死整个进程：裸 `pool.end()` 那一族
> （锚点缓存 / 群目标 / 群成员 / 加群审计 / 告警 / 点赞 / 有价值评论 / 互动 feed / 群路由 /
> 委托任务存储）**禁止调**；`ownsPool` / `ownedPool?` 守卫那一族安全。
> **同一族存储的 `close()` 语义可以不一样，而调用点看不出来** —— 逐个看它关的是谁的池。
> 这条现在有结构断言钉着（`test/acceptance/automation-main.test.ts`），变异实测抓得住。
>
> **⚠️ 可执行入口 `runAutomationEntry()` 仍然 fail-closed，本片没动它**：
> 切成真启动属第 4 段（要等台账清零，且要有用例证明「台账非空时仍拒绝启动」这条闸没被删）。
> 台账常量当前 11 条，与 JSON 投影逐位相等。已有一条结构断言钉住「入口 MUST NOT 直接改调真装配」。
>
> **⚠️ 部署形态 MUST 是 stop→start，禁止滚动 / 蓝绿**：风控写者锁是会话级 advisory lock、
> 构造期就抢，两个进程重叠期间后起的那个会抢不到锁并拒绝启动。
>
> > 📌 **施工附图 `batch-h-wiring-map.md` 已完成使命**（第 5 片是它的最后一个消费者）。
> > 它的「16 步构造顺序」与真实装配有两处出入（环二的破法、每连接运行时提到 6b），
> > **以 `src/automation-main.ts` 的实际顺序为准**；那份文档只作追溯用。
>
> ---
>
> ### ⚠️ 3.5g 没能做到、已如实登记的三件（别读成「做完了」）
>
> 1. **`runAutomationMain()` 一次都没真跑过。** 它要真属主库（写者锁在构造期就抢），
>    本地桩验不了。本片交付的是「编译通过 + 结构上钉住」，**不声称进程能起来** ——
>    三进程真跑属批次 5，本 change 的分层验收口径本来就不声称它。已按 5.5 登记 backlog 簇 60。
> 2. **新立了 tasks 3.5h：四类限频配置在三进程形态下「改了没人通知本进程重读」。**
>    配额 / 节奏 / 单场 / 续场四张表**属主就是本进程**，但让内存副本保持新鲜的那个轮询刷新器
>    （`src/config/mirror-refresher.ts`）在归属表里是 **api**、不会被派生进本仓 ⇒
>    本进程启动读一次之后**再不刷新**，现象是「后台改了配额，自动化进程按旧值跑且不报错」。
>    不阻塞门清零，但三进程真跑前必须有结论。**别直接搬刷新器**，先裁定 3.5h 里那三件事。
> 3. **运营的调度启停通道本片不接**：`isDispatchActive` 按裁定是进程内布尔、`main()` 自持，
>    但接收方的路由没注册 ⇒ 本进程今天恒为「在跑」。它卡在接口侧、已是台账条目
>    `feishu-operator-dispatch-start-stop`。**MUST NOT 为了凑「接好了」的样子塞一个没人能调的接收方。**
>
> ---
>
> ### 三、又一次坐实：结构断言的锚点打错了，等于没有
>
> 本片给 schema 门写了一条「门 MUST 在建池之前」的结构断言，**第一版把锚点打在整个文件上** ——
> 于是把门挪到建池之后，用例照样全绿（当场变异实测）：它量到的是那条 `import`，不是调用点。
> 改成只看 `runAutomationMain` 函数体后才红。
>
> 另两处变异都一次抓住，其中一处再次坐实那条老结论：**在发布下发里塞回第二份人设绑定判定，
> 同文件 8 条行为用例全绿，只有结构断言红。**
>
> ⇒ 「结构断言别按整文件匹配」这条，**要连 import 区一起排除**，不是只排除注释。
>
> ---
>
> ### 三、第 3 段之外还挂着的
...
>   要接口侧装配 + 四条新窄 route + 两个附带归属动作。
>   **其中「权限总览」那条零成本（零表、零运行时状态），可单独先清，别跟互动那两条捆在一起。**
> - **群评论时序策略仍未接**（用户裁定留到真要切三程序时再做）：接口域配置表，
>   既没有同步读流也没有 HTTP 口，而协调器要的是**同步**取用 ⇒ 只能补一条同步读流，
>   而那正是「名单类常量」那条教训的高发区。**缺席后果**：本进程 FB 覆盖评论一条都不会发。
> - 另两条 api 属主事实（账号暂停 / 排期名额回程）已接（`f7e9043`）——
>   **⚠️ 2026-08-04 实读更正：只有一半成立。** 账号暂停真的通了（属主实现在 api 组装根里、
>   且经账号运行时那组路由注册）；**排期名额回程只有客户端与单体路由，接口进程这一侧是空的**
>   （而且不是漏一行注册：单体注册的是转发器，真落点是内容排期器，而 api 的手写 `main()`
>   根本没构造排期器）。见 tasks 3.5j。
>
> ---
>
> ### 四、本轮（08-03）新得出、对后面每一批都适用的四条
>
> 1. **改任何一条同步读流的载荷形状，必须同时推进那条流的镜像版本 cursor**
>    （加一条 `…_snapshot_revision` 迁移，判例 `0091`，本次 `0108`）。cursor 来自
>    `config_mirror_version`、**只在有人写配置时才动**；已持有 checkpoint 的消费方重启后会在
>    **旧 cursor 上看见不同的整份 payload**，按设计正确拒收（`same_cursor_payload_drift`）——
>    而那条拒收落在单体启动路径上，直接起不来。**本地全量测试与 typecheck 全绿，
>    只有真拿一个带旧 checkpoint 的库重启才现形。** 顺手 grep 一下同流还有没有别人的改动
>    （本次就撞上另一路 change 埋着同一颗雷）。
> 2. **「systemd 说 active」不构成「起来了」。** 上面那次启动失败时 `main()` 抛了，
>    但 config-mirror 的定时器还在跑 ⇒ 事件循环不空 ⇒ 进程不退出 ⇒ `is-active` 显示
>    `active (running)`、重启计数 0，而 8787/8090/8091 一个都没在听、面板 502。
>    **健康检查必须看端口在不在听 + 自重启以来的 error 行。**
> 3. **动 `src/schema/` 或任何被 transport 点名的文件，先把跨仓级联算进成本**：
>    改一处要走完 cloud → transport → 三仓 pin → `npm install` 整条链（5 仓 5 次提交）。
>    `sync-split-repos --apply` 会自动改 pin，**装机要自己跑**，不跑的话 npm 静默装旧 sha。
> 4. **凡文档里写着「今天没有 X」/「本进程拿不到 X」，动手前先 grep 一遍事实源到底在谁那儿。**
>    本 change 已因此栽过三次（协调器那条错判被连写三片；五条 open 的「通道不存在」前提；
>    3.5f 的「半迁移算缺席」）。判断写进文档之后**没有任何机械手段会复核它**。
>
> **第 4 片推翻的那条，单独点名**：3.5f 原写着「schema 半迁移」算 `unavailable` 理由之一 ——
> **不成立**。半迁移在单体里仍然组装收件箱与运行时开关、只关写开关（读是恢复的），
> 判成整体缺席等于连读一起停掉。同处还有一条易漏：写开关判定的**第三个实参是部署目标**，
> 半迁移只在 dev 上仍允许写，漏传会让 dev 的写悄悄关掉。
> 连带更正：`unavailable` 的具名理由**实际只有两条真的**（schema / 回复生成），
> 计划里说的另外三条在本进程的组装根里恒存在 ⇒ 写成分支就是死代码，做成必填参数由编译器保证。
>
> **dev 上验不到的**：dev 的互动 schema 本来就缺（`interaction_schema_missing_run_0042`），
> 单体那一侧互动域整段未启用 ⇒ 第 4 片改指端口的消费方在 dev 上**根本不执行**。
...
> - **dev 停在 cloud `c394f36`**（当前主干头）。健康检查全过：`active`、重启计数 0、
>   8787/8090/8091 三口在听、error 0 行、飞书长连接已建立、面板 API 200、isales 四服务未触碰。
>   **判据始终是「当前 cloud 主干头 vs dev 上跑的那个」**，别拿本文里的 sha 当现状，
>   也别按「本轮谁改了什么」推断 —— 只动派生仓的批次与 dev 无关（dev 上跑的是单体）。
>   逐次部署明细（含失败那一轮）在 §10。
> - **⚠️ 第 4 片那条路径 dev 上验不了**：dev 的互动 schema 本来就缺
>   （`interaction_schema_missing_run_0042`），单体那侧互动域整段未启用 ⇒ 改指端口的消费方
>   **根本不执行**。已按 5.5 登记 backlog 簇 60。
