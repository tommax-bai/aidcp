...
1. **2.5 落地两个岔口的既有裁决**（模型出口 A1 / 四个角色工厂 B1）——**裁决早拍过了，是落地不是决策**。门再掉 2 条。
2. **发帖素材 + 用量记账**：契约已在，缺**注册路由 + automation 侧接线**。
   用量那条注意端口是「提交已合并的增量」而非逐条上报，**属主今天没有这个方法**（接线时补方法或交适配对象）。
3. **2.6**（`ReplyWorkflow` 的 content 属主具体类实参）、**2.7**（传递性检查，特别点名 optional 参数）。
