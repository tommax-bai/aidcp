...
> ### 其余可并行推进的
>
> - **批 C 只剩一件**（2026-08-02 复核，原先记的"三件"已过期）：**配置面审计中继**
>   —— 把本进程 outbox 里的互动配置面审计行排空到接口属主表。
>   **写的是接口属主表，跨进程后 MUST 走已有通道、别直连。** 不需要任何裁定，是纯搬运。
>   面板事件投递客户端与风控指令消费者**都已在本仓**。
>   **配置副本停手闸那条"待拍板"也已作废**：原判据「自动化侧缺新鲜度事实源」是错的 ——
