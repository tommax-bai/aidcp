...
> **门 11 条**（运营指令 3 / 内容 7 / 组装 1）。**tasks.md 77/129。跨域边 0。**
> **dev 已在 cloud `f7e9043`**（2026-08-02 当日第三次部署，健康检查全过）。
>
> **上一手（2026-08-02）做完的**：批 G 第三/第四片（评论与加群调度器 + 联系评论闸 / FB 消费协调器，
> **10 个口至此全满**）、批 H 第一片（四个业务配置取值口）、批 H 第二片（账号暂停 + 排期名额两条通道）。
> 期间推翻并作废了一条连写三片的错误判断（协调器要补跨进程口），另有两条判断由变异实测改过来。
>
...
>
> ### 下一步：批 H 主体（写 `main()`，把工厂接进组装根）
>
> **批 H 已落两片，主体没动**：
> - **第一片**（automation `1de0876`）：四个业务配置取值口的实现，
>   落点 `src/automation-business-config.ts` —— **零解析逻辑**，只做「取快照 → 喂判定」。
>   顺带补掉一处真缺口：那条环境流**此前不带环境键**，而 FB 基线解析第一跳就是「账号 → 环境键」
>   ⇒ 补上之前自动化进程**根本解析不出基线**（下游即「这个账号永远不开始浏览」）。
> - **第二片**（cloud `f7e9043` 起六仓）：接掉「账号暂停」「排期名额回程」两条 api 属主通道。
>
> **主体是这一件**：十二个工厂全部就位、全部**没接进组装根**。骨架与模板见 design.md §4
> （监听先起、业务入口靠标志位 + 去重 promise、定时器 unref）；启动外壳 `automation-service-entry.ts`
> 已经写好且**必填参数无缺省**，缺哪个供给方编译器会当场点名。
>
> **⚠️ 可执行入口 `runAutomationEntry()` 在批 H 里也 MUST 保持 fail-closed** ——
> 切成真启动属第 4 段（要等台账清零，且要有用例证明「台账非空时仍拒绝启动」这条闸没被删）。
>
> **批 H 主体会当场撞上的、今天仍空着的口**（都是**编译期可见**的，不是隐患）：
>
> | 口 | 谁留的 | 缺了会怎样 |
> | --- | --- | --- |
...
> | 会话配置的全局活跃周历 | 批 H 第一片的必填口 | automation 属主、本进程直读（`session-config-store`），不走镜像 |
> | 群评论时序策略 | 批 G-3 / G-4 的二态口 | 见下表，**用户已裁定暂不接** |
>
> **另有一件不在编译面上、要主动去做**：判据清单里 5 条 `open` 逐条裁定（tasks 3.5a）。
> 五条同形——在自动化段装配、本进程一次都不读、只有接口服务段消费，**而通道今天不存在**。
> 两条路都成立、都要拍板：① 开一条端口让接口进程取；② 装配整体移到接口进程。
> **既不能判 construct（构造了没去处），也不能判 skip（那等于这条能力消失）。**
> 上一手的下一步本来就是「先逐条查清每样是什么、读的是谁的数据，再一次性请用户拍板」——**这一步还没做**。
>
