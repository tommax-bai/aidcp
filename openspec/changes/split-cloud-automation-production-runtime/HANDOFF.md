...
> **下一手直接照 tasks 4.1b 里那份逐条清单接线**（⓪ 补账号显示名读 ① 建客户端 ② 装配委托服务与两条路由
> ③ 两个钩子 ④ api 侧两个客户端合成 7+1 ⑤ 撤两条），别再重推裁决。
>
> **⓪ 是 2026-08-04 实读补进去的一步，此前整份计划都漏了它，且它决定这一批不能再拆小**：
> 委托解析按**昵称**选账号，而 `listAccounts` 要的显示名与别名候选**今天没有任何跨进程读**
> （kernel / transport / api 三处都 grep 过，只活在 api 的账号主数据存储里）。
> 缺了会让每一条「给<昵称>…」都回「可用昵称：无可用昵称」——响亮，但这条能力对运营等于不可用。
> 它**不能当独立前置先落**：4a 普查要求每个方法槽都有真实生产消费点
> （`PRODUCTION_CONSUMER_BINDINGS`，光加端口方法过不了），所以**单体那份 `listAccounts` 闭包
> 必须同批改指新端口**，再连带那五处手抄计数。⇒ 这一批是一个耦合单元，别再切。
>
> **裁决存档：走 B，A 不成立。**
