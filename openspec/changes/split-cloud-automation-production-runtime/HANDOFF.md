...
> | E-2 | ⚠️ | **每连接角色调度器工厂** —— 勘察已完成，卡在一条归属裁定（tasks 3.1e） |
> | G / H | ⬜ | 未开工 |
>
...
> ### 下一步（按依赖）
>
> 1. **批 E 后半：每连接角色调度器工厂 —— ⚠️ 撞到一个必须先拍板的归属问题，与 3.1c 同形。**
>    **勘察已做完（2026-08-01），结论全文在 tasks.md 的 3.1e，别重查。** 要点：
>    - 工厂本体约 400 行、传 41 个顶层选项；调度器选项面 70 余个字段**几乎全可选** ——
>      task 2.7 点名的「optional 参数是静默缺席主要来源」的最大一处。
>    - **四个业务配置存储是 api 属主、不在本仓**（排期 / 热帖阈值 / FB 评论配置 / FB 运营策略），
>      自动化段对它们有 **32 处同步热路径读**。前三个有镜像事实源、缺的是「有效值」那一段；
>      **第四个连事实源都没有**（不在同步读消费流清单里），而它决定**整个 Facebook 浏览模式**。
>    - **MUST NOT 给它一个恒 blocked 的实现把编译过掉** —— 不报错，只是 FB 账号永远不浏览。
>    - 三条路（析出纯判定段进 kernel / 本仓再实现一遍 / 补一条同步读流）见 3.1e，**需用户拍板**。
>    - **不被它挡住的那一半可以先做**：工厂本体照批 D/F 的办法写，把四个业务配置与评论域
>      （均属批 G）做成必填注入口或能力二态。裁定只决定「谁来喂这个口」，不阻塞工厂交付。
...
> ② automation 另有 **7 条 `⊙` 派生仓私有文件**（组装根 + 启动外壳 / 判据清单 /
>    模型出口 / 风控底座 / 记账 / 停手闸 / 边缘接入）——**新增这类文件 MUST 登记进控制仓
