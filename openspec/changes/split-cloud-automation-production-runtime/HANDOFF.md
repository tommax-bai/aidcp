...
> | 批 | 状态 | 落点 |
> | --- | --- | --- |
> | A | ✅ | 启动外壳 + 41 个导出面判据清单（`6035fa4`；cloud 侧漂移闸 `f83e266`） |
> | A-1 | ✅ | 首次依赖共享传输包 + 模型出口工厂（`c365b1a`） |
> | B | ✅ | 风控单写者 + 告警底座（`4d3fb89`） |
> | C | ✅ | 停手闸（`089e2cc`）+ 记账漏斗与保留期（`6958e55`）+ 配置面审计中继（`6ff2416`） |
> | D | ✅ | 出口闸放行判定析出 kernel（`8d62dca`）+ 边缘接入正文（`ff44774`） |
> | F | ✅ | 发布下发与陪伴界面（`613338a`） |
> | E-1 | ✅ | 每连接运行时注册表 + 冷却闸 + 互动守卫（`ac09a7f`） |
> | **E-2** | ✅ **三步全落**（1/2 已上 dev，3 只在本仓） | 工厂本体 `automation-connection-dispatcher.ts`；批 G 的开工面就是它的 10 个必填口 |
> | G / H | ⬜ | 未开工 |
>
> **形态在批 B 定下来了，后面每批照做**：写成**可单测的工厂**，不写进 `main()`。
> **可执行入口在批 H 之前一律保持 fail-closed** —— 它是中间态的保护罩。
>
> ### 批 E-2 · 三步全落（2026-08-02），**下一步是批 G**
>
...
>
> **步骤 3**（automation `72373f4`）：每连接角色调度器工厂，落点
> `src/automation-connection-dispatcher.ts`，交付批 E-1 留下的那个必填口。
> **保护线画在本文件的依赖面上，不在调度器的选项面上** —— 那 200 余个字段几乎全可选、
> 漏传不报错。批 G 的 10 个口（评论域 7 + FB 运行时 3）一律必填或能力二态。
>
> ⇒ **批 G 的开工面已经是现成签名**：填 `AutomationCommentPorts` 与
> `AutomationFacebookRuntimePorts`，填不上的必须给 `unavailable` + 具名理由，别塞空实现。
> **工厂还没接进组装根**（那要等 10 个口填上，属批 H 的 `main()`）；今天口是空的、
> 且**编译期可见** —— 与批 B 留给批 C 的办法一致。
>
...
> ### 其余可并行推进的
>
> - ~~**批 C**~~ ✅ **已全落**（2026-08-02）：最后一件配置面审计中继落在
>   `automation-config-audit-relay.ts`（automation `6ff2416`）。
>   面板事件投递客户端与风控指令消费者此前已在本仓；
>   **配置副本停手闸那条"待拍板"是误记**——原判据「自动化侧缺新鲜度事实源」是错的，
>   本仓同步读镜像早就带了一个、只是零消费方，接上即可，不涉及归属改判（`089e2cc`）。
...
> - **dev 停在 `c0de08b`（本轮部署到此为止），健康检查全过**
>   ⚠️ 之后并发 session 又推了 `8773130`（把就绪日志里的 consumer 计数也改成派生），
>   **dev 未含它**。判据始终是「当前 cloud 主干头 vs dev 上跑的那个」，别拿本文里的 sha 当现状。
