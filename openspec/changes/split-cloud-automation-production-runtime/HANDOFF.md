...
| | 卡在哪 |
| --- | --- |
| **调度启停 api 侧**（tasks 1.3a） | **卡的是签名不是接线**：面板的「调度是否在跑」是**同步布尔**，远端读是**异步三态**（读不到 ≠ 停着）。要正确接必须改面板签名。**硬接的后果有先例**——派生仓那句 `dispatchActive: () => false` 就是这么来的 |
| **飞书 `/delegate` 端到端** | 那条链**一次都没真跑过**（要真发一条飞书消息）。验完才能名正言顺清掉运营指令那三条台账；不验也能清，但要带「端到端未验」的限定 |
