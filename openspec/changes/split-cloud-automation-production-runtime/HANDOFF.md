...
> **重整于 2026-08-01 晚**，行号 pin 在 `aidcp-cloud@8d62dca`。
>
> **怎么读这份文档**：
...
>
> **批 H 会当场撞上的、今天仍空着的口**（都是**编译期可见**的，不是隐患）：
>
> | 口 | 谁留的 | 缺了会怎样 |
> | --- | --- | --- |
...
>
> ### ⚠️ 三个 api 属主事实今天没有通道（批 G 留给批 H 的裁决）
>
> 都做成了必填能力二态、各自具名，**缺席一律不报错**——所以必须在批 H 显式裁一次「接还是不接」：
>
> | 事实 | 缺席后果 | 今天缺什么 |
> | --- | --- | --- |
> | 群评论时序策略（预热 / 同群再评冷却） | **本进程 Facebook 覆盖评论一条都不会发**；消费模式评论段也不推进 | 接口域配置表，**没有同步读流、也没有 HTTP 口**。协调器要的是**同步**取用 ⇒ 真要接只能补一条同步读流（那是「名单类常量」那条教训的高发区，见下面五条教训之一） |
> | 账号暂停（加群连续失败到顶时自保） | 加群照常记账本、照常停重试，**但账号不会被真的暂停**，明天再撞一次同样的墙 | 接口域账号状态管理器，无写通道 |
> | 排期名额回程 | 排期评论没接管到边端时，**那一小时的名额不归还**，日志上与「今天没排期」同形 | 三进程形态下排期跑在接口进程，这是它的回程；无通道 |
>
...
>
> ### 2026-08-02 深夜新增的三条（批 G 第三 / 第四片实测）
>
...
> ② automation 另有 **10 条 `⊙` 派生仓私有文件**（组装根 + 启动外壳 / 判据清单 / 模型出口 /
>    风控底座 / 记账 / 停手闸 / 边缘接入 / 发布下发 / 每连接运行时）——**这个数字会随每批增长，
>    以当场跑一次为准**；**新增这类文件 MUST 登记进控制仓
