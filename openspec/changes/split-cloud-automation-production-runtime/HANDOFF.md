...
> **2026-08-02 夜 · 现状（这一段是当前事实，读它就够；逐批沿革移到文末 §10）**
>
> **六仓**：cloud `2875244` / api `8255d52` / automation `6ff2416` / content `493241e` /
> kernel `9cfd1c9` / transport `5ad4b41`；控制仓见 `git log`。
> 六仓全干净、全已推、**对账零漂移**、pin 全对齐。
> **测试**：cloud 4088 / api 502 / automation 2077 / content 441 / kernel 70 / transport 36，全 0 fail。
...
> | F | ✅ | 发布下发与陪伴界面（`613338a`） |
> | **G** | **7/10 口** | FB 两套运行时（`806c0fa`）+ 评论域审批与通知五口（`c8172bf`） |
> | H | ⬜ | 未开工 |
>
...
> ### 下一步：批 G 剩的 3 个口
>
> 签名全在 `src/automation-connection-dispatcher.ts` 的 `AutomationCommentPorts` /
> `AutomationFacebookRuntimePorts` 上，照着填；**填不上的必须给 `unavailable` + 具名理由，
> 别塞空实现**。`test/acceptance/automation-connection-dispatcher.test.ts` 那组用例
> **同时是批 G 的验收夹具**：把 `state` 从 `unavailable` 改成 `wired`，
> 「逐项接线」那条会立刻告诉你有没有真接上。
>
> | 口 | 单体里是谁 | 备注 |
> | --- | --- | --- |
> | `comment.scheduler` | `CommentScheduler`（约 26 个构造参数） | 最大的一块；加群调度器与它同段 |
> | `comment.fireAutoContactComment` | `triggerGatedAutoComment` 那个闭包 | 缠着评论调度器，同片做 |
> | `facebookRuntime.coordinator` | `FacebookConsumptionModeCoordinator` | **先解决下面那条跨进程口** |
>
> ### ⚠️ 消费协调器：它要的决策口今天自动化侧没有（用户 2026-08-02 已同意补跨进程口）
>
> 协调器要的是接口属主的 **`resolveForAccount`——异步、且含慢启动解析**；
> 步骤 2 给自动化侧的是**同步基线口 `facebookOperationBaseFor`**，**两者不是同一个东西**。
> **MUST NOT 拿同步基线口顶替** —— 那会丢掉慢启动档位，让本该按爬坡节奏跑的新账号直接按满档跑，
> 而且不报错。方向已定：**补一条跨进程口**（kernel 契约 + 传输三件套 + api 注册 + automation 客户端）。
> 建议与协调器同片落，避免又造出一个「建好零消费方」。
>
...
>
> ### 2026-08-02 新增的五条教训（与下面那六条并列，后面每批照办）
>
