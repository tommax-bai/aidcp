...
> | 项 | 值 |
> | --- | --- |
> | 六仓 | **cloud `0d507c5`** / **api `f69bfe0`** / **automation `c2c35c0`** / **content `039cd26`** / **kernel `e8396ce`** / **transport `346d0c8`** |
> | 工作区 | 六仓全干净、全已推、**对账零漂移**、两个共享包 pin 全对齐（本轮走了一遍完整级联） |
> | 测试 | **cloud 4121**（+11 skip）/ **api 505** / **automation 2168**（+3 skip，与基线同）/ **content 444** / kernel 70 / transport 36，全 0 fail |
> | 门（真交付物） | **2 条**（内容 1 / 组装 1）—— **运营指令那一类已归零**；四批撤条 **11 → 6 → 5 → 4 → 2** |
> | tasks.md | **90/147**（这把尺量的是「查清了多少」，不是交付；分母会随勘察长大） |
...
> | 条目 | 卡在哪 | 尺寸 |
> | --- | --- | --- |
> | `content-generic-llm-authority` | **整条链已实读测绘完（tasks 4.1g），照那份开工、别再测绘一遍** | 中，但**必须先建委托任务工作器** |
> | `automation-production-runtime-composition-unwired` | 它就是空壳入口本身，等前面清完 | 收尾 |
>
> **顺序是硬的**：内容那条不清完，组装那条写不了（§3）。清完之后是 tasks 4.3
> （入口从 fail-closed 切真启动，切换本身要有用例证明「台账非空时仍然拒绝启动」这条闸没被删）。
>
> **链是这样的**：生成链客户端 → `PublishScheduler` → 某个真触发它的东西。
> 前两段今天就能接（依赖全在，逐条写在 4.1g 里），**卡点在第三段**——本进程今天
> **没有任何东西能触发发帖**，三个候选全不可用：
>
> 1. **排期 tick（`ContentScheduler`）属 api**，automation 仓里根本没有这个类；
>    **而 api 的手写 `main()` 也没建它** ⇒ 三进程形态下今天**没有任何进程在跑排期发帖**。
>    这是本轮测绘出的最要紧的一条，此前没有任何地方记着。
> 2. **手动发布那条运营指令路由刻意不接**（1.7b 裁定）。
> 3. **委托任务工作器本进程没建** —— 唯一可达的那个。
>
> ⇒ **只加生成链客户端 + 建 scheduler 是「建好零消费方」**（本项目点名的反模式；
> typecheck 当场以「声明了没人读」拦下来了）。本轮据此**主动回退**了那半成品，没留在树上。
>
> **下一手最省的一条路：先建委托任务工作器。** 它同时是 scheduler 的真消费方，且四个回调
> 所需的东西**几乎都已经在** `AutomationPublishDispatch` 上（preflight / 驳回通知 / 预览刷新 /
> 批准后触发下发 / 界面快照），唯一缺的是 `writeApprovalDecision`，
> 而**那条路由 api 的手写 `main()` 已经注册**（零新增契约）。逐条在 tasks 4.1g。
>
> ⚠️ **一条现网缺口，建工作器时一并修**：委托任务在自动化进程里今天**能建、能确认，
> 但没有任何东西会执行**，且**连单体那句具名警告都没有**（单体缺 worker 时会 warn
> 「任务可确认但不会执行」）。4.1f 撤的两条锚的是**指令通道**、与执行侧是两件事，
> 但「确认了却永远不跑、且不出声」本身就是红线形态。
>
> ---
>
> ### 三、两批（委托任务 / 发布链测绘）之后仍然成立的几条判据
>
...
| --- | --- | --- |
| tasks.md 条数 | 89/146 | **「查清了多少」，不是「交付了多少」。** 分母会随勘察长大——最近几批往里加了十来条实测发现的新任务 |
| **那张清单（门）** | **2 条** | **真正的交付物。** 14 → 2：**头两条是撤的、不是干活减的**（一条被算了两遍 1.7b，一条记错了属主 2.9）；**其余十条都是真靠接线消掉的** |
...
**已撤的十条**（全属真靠接线消掉）：`content-role-factories`（07-31）、概念池写 / 精选写 /
