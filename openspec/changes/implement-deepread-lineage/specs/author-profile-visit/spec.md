...
系统 SHALL 通过专用指令 `profile.open`（cloud → edge）让边缘进入作者主页，MUST NOT 复用 `note.open` 携带非协议字段（如 `type:'profile'`）。`RoleDispatcher` MUST 将 `profile.entered` 翻译为 `profile.open` 指令（payload 含 `authorId?`、`reason?`、`thinkMs?`），`command-bridge` MUST 提供 `profile_open → profile.open` 映射。边缘 `dispatchCommand` MUST 新增处理：导航进入该作者主页（点击作者头像或跳转主页）并等待主页渲染就绪。

#### Scenario: 评估值得进主页后真正进入
- **WHEN** `AuthorEvaluator` 判定 visit、`ProfileOpener` emit `profile.entered`
- **THEN** 系统下发 `profile.open` 指令，边缘导航到作者主页并等待渲染，而非打开信息流中的某条笔记

#### Scenario: 不再因缺字段开错笔记
- **WHEN** 需要进入作者主页
- **THEN** 系统下发的是 `profile.open` 而非 `note.open`，边缘不会因 `type` 字段被丢弃而按 `index=0` 打开错误笔记
