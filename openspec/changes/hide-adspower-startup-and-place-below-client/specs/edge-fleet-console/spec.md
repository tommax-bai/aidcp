...
主界面与引导流中明确要求人工操作的「打开窗口」SHALL 尽力把目标环境的浏览器窗口抬到前台；环境头像的显示手势 SHALL 先把浏览器移动到可见检查位，再把 AIDCP 客户端恢复到前台，使浏览器处于客户端下方而不遮住控制面。两类入口 SHALL 以窗口错位摆放 + 行↔窗口同名 / 同色让窗口与环境行可对应。请求无法送达、执行失败或有界完成回执超时时 SHALL 诚实告知，MUST NOT 假装窗口已按目标层级放置。

#### Scenario: 环境头像显示时客户端保持在上方
- **WHEN** 运维在已选中环境上触发头像显示手势
- **THEN** 浏览器移动到可见检查位后，AIDCP 客户端恢复到前台并位于浏览器上方
