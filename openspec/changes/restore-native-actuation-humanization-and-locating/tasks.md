...
## 归档前置 — spec delta 与实装的逐条对账（2026-08-01 做的，**未修完**）

> 依据 7.1 的补注做的那一遍「把每条具名偏离对着 delta 读一遍」。**结果比预期严重**：
> delta 是 propose 当时按**意图**写的，此后 2.9 / 3.7 / 5.4 / 5.5 / 定时设置 / 遗留步骤路径
> 陆续被显式弃守或收窄，而 **delta 一次都没跟着改**。
>
> **照原样归档 ＝ 往主 spec 里并进 8 条实装明知违反的 MUST。**
> `openspec validate --strict` 抓不到（它只校验 delta 内部结构、从不比对实现），
> 归档流程也不会问。**本节是归档硬前置，逐条改完之前不得 archive。**
>
> ⚠️ 改法不是「把 MUST 删掉」。弃守的是「本 change 交付这件事」，不是「这件事不该做」——
> 正确形态见迁移主线 3.2 的注释口径：**规格只能写「已实现」那一列**，未交付的部分
> 要么从 delta 里摘出去另立，要么把要求收窄到实际成立的范围，并在 change 里留下承接方。

| # | delta 里的要求 | 与实装矛盾在哪 | 依据 | 建议改法 |
|---|---|---|---|---|
| 1 | `native-actuation-primitives`：**Native command execution MUST consume ... inside it**，且 scenario 要求「在该命令**第一条影响页面的输入**之前」等 | 消费点落在**宿主 TypeScript 层**、等待发生在**整条命令之前** | §4 头部「具名推翻 design D1」；design D1 已于 2026-08-01 更正 | 把「inside it」改成「在边缘侧」，并把 scenario 收窄成「在该命令派发任何页面输入之前」；把「第一条输入之前」的更强口径作为已知近似写进 delta 正文 |
| 2 | 同上：**Forwarding a timing field to Native MUST imply a consumption point ... inside that runtime** | 门禁**在**、判据是行为派生的（真的等了多久），但「inside that runtime」这句不成立 | 1.2 / 4.5 的三腿口径 | 只改措辞：消费点在**边缘侧**，不指定进程 |
| 3 | 同上：**Native text entry MUST be hardware-level ... on every platform**，scenario 点名「comment、publish field、**candidate keyword**、**schedule value**」 | 定时设置**保持 setter**（用户裁定）；候选只做了 `topic`，`mention`/`location`/`collection` 仍是整段赋值；遗留步骤路径的输入步具名收窄不做 | 8.1 的三条裁定 | scenario 的枚举收窄到「comment / publish field / topic candidate」；把定时字段与其余候选写成具名例外 + 承接方 |
| 4 | 同上：**Page scrolling MUST use the shared humanized wheel gesture on every platform**（「Any command that advances a list, a detail body or a comment area」） | 遗留步骤路径的滚动步、通知清零里的第四处页面内滚动**未做**，仍是页面内平滑滚动 | 8.5 范围说明 | 把「any command」收窄到已接线的命令面，未接线的两处具名登记 |
| 5 | 同上：**Injected click helpers MUST NOT move the page or fabricate pointer events**（含静态契约检查） | **3.7 显式弃守**；小红书那一半由 7.11 交接给单写区属主 | 3.7 弃守理由（反检测质量、不消除假成功） | 整条从本 change 的 delta 摘出，随 7.11 的承接方走 |
| 6 | 同上：**Captcha assist actuation MUST run on a dedicated high-scrutiny pacing tier** | **2.9 显式弃守** | 2.9 弃守理由（同上，且是人工介入低频路径） | 整条摘出，留待反检测专项 |
| 7 | 同上：**Assist click receipts MUST report the actual replay mode and MUST NOT silently discard a supplied trajectory** | **2.10 未做**（实现点在他人单写区，未与属主对齐） | 2.10 进度说明 | 2.10 若不做完则整条摘出；做完则保留 |
| 8 | `native-locating-gates`：**Native anchor learning MUST stage before promoting and drop on failure** | **5.4 / 5.5 显式弃守**，且前提不成立 —— 引擎里没有任何非确定性锚点来源，暂存区**恒为空** | 7.16 裁定（用户：暂不做，要重新设计） | 整条摘出；`locating_gates.rs` 里那条「暂存区恒为 0」的守卫用例**必须留着**（它钉的是「别把这层读成已生效」） |

**另需复核、本次未逐条读完的两条**：`native-locating-gates` 的
**Post-action validation criteria MUST meet a minimum strength bar** 与
**Post-action evidence MUST NOT be the command's own input**。前者对应 5.9 / 5.10 / **5.11（未做）**，
后者本 change 已在**引擎特化的新路径上**兑现（话题真 token、评论到达确认），
但注入路由里 `mention` / `location` / `collection` 三支**仍是自证**（§9.5-0）。
两条大概率需要按「已实现那一列」收窄，收口时须逐条读。

**为什么这一节值得单独存在**：这批 change 的 delta 都是 propose 时按意图写的，
而实装期跨了多轮裁定。**「裁定改了任务、没人回头改 delta」是一条系统性的漏法**，
不是本 change 独有 —— 同批其余几个 change 归档前都该做一遍同样的对账。
