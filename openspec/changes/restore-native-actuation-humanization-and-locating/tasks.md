...
| 13 | 「**每一条**写命令都必须做后置校验」 | 共享的三道闸编排**只接了一条真实命令**；而各命令自带的后置判据**从没被逐条盘点过**（5.6 / 5.11 未做） | ✅ 加一段说清：合规与否是**该命令自己的后置判据**的性质、不看它写在哪个模块里；但**逐面盘点不存在**，所以这条保证 MUST 只按「已逐条核过的面」读、**MUST NOT 当成整个运行时的属性**。并明写盘点里「没有校验」也要如实列出 —— **没列出的面会被读成已覆盖** |
