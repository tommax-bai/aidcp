...
Whether a command satisfies this is a property of its own post-condition, not of which module it is written in: a shared resolve–act–validate orchestration exists and is wired to a first real command, but a command with its own honest post-condition is equally compliant. What this requirement does **not** yet come with is the per-surface inventory that would let anyone say which side of the line each command is on. Until that inventory exists, this guarantee MUST be read as holding for the surfaces that have been checked individually, and MUST NOT be cited as a whole-runtime property. Producing it is a named follow-up, and the inventory MUST record "unvalidated" where that is the truth rather than leaving a surface unlisted — an unlisted surface reads as covered.
