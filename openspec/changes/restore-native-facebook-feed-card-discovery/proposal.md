...
| 其中已带可接受 permalink（**冷启动、零指针事件**） | **0 张** |
| 页面滚动 | 正常（高度 2010 → 9407，懒加载在工作） |

> **一处必须写明的自我更正**：首轮测量得到「5 张中 2 张已带 permalink」，随后发现那 2 张正是调查过程中**被我自己悬停过**的卡——测量被自身干扰污染。重载页面、改用纯程序化滚动（`window.scrollBy`，不产生任何指针事件）复测：4 张水合卡，**可上报 0 张**。本 change 的收益必须按 0 计，不得按 2 计。
...
- **时间戳诱饵链接**：实测该版式的时间戳 href 平时指向站点根路径，**鼠标真实悬停后**才换成 `/<主页>/posts/pfbid…` 或 `/permalink.php?story_fbid=…`（视口内 7/7 复现）。这是 Facebook 新加的反抓取、**退役实现同样没有**，属新能力而非回归。
  另测得一条决定实现形态的事实：**页面内合成的 `pointerover` / `mouseover` / `mouseenter` / `mousemove` / `focus` 一律无效**，四张卡的 href 路径在合成事件后仍是 `/`。只有经 CDP 下发的**可信**指针移动才会让 Facebook 换出真链接。这意味着取身份不能留在注入脚本内（注入脚本只有一次求值、发不出可信输入），必须改成「探测 → Rust 侧移动真实指针 → 复探」的多步编排。范围与风险都远超本 change，另起 change。

**因此必须诚实说明本 change 的收益边界**：它把「找不到卡」修成「找得到 5 张真卡且物理卡证据为真」，但在这个版式上**可上报卡仍为 0**——首页依然读不了。它的价值是 ① 让 Reels 兜底以真依据触发而非拿空壳凑数；② 是悬停取身份那一步的前置（找不到卡就无从悬停）；③ 在 permalink 未被诱饵化的版式上直接恢复读卡。**不要把本 change 当作「首页恢复可读」的交付。**
