...
Partial multi-image: each `upload_image` is independent. If image k fails, set `imagesOk=false`, stop emitting/continuing further uploads, and skip `set_cover` (a cover of a never-attached image is logically impossible). For this change images are single (`imageCount` hardcoded 1 cloud-side), so partial-multi is dormant; the loop is count-agnostic for forward compat only.
