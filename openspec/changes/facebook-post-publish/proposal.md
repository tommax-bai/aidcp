...
本 change 建立 Facebook 发帖的最小可验证闭环：账号配置中批量上传发帖图片，发布时从该账号图片素材池锁定对应图片，生成待审草稿，经既有审批/派发/边缘租约机制下发到 Facebook。页面内的正向提交信号对用户表示“已提交”；同页取得帖子 ID 或 permalink 后才记录为“已发布”。
...
- Edge 新增 Facebook 发帖执行器，覆盖宽屏/窄屏 composer 打开、聚焦、正文逐字输入、上传、提交、同页确认；页面已接受提交但未取得链接时显示“已提交，待链接确认”，不刷新页面、不报失败、不自动重试。
