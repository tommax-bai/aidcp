...
无法读取 AdsPower 环境代理配置或检测设施自身异常 SHALL 表示“无法确认”，MUST NOT 冒充代理失效，也 MUST NOT 成为绕开既有启动行为的新单点阻断。与此不同，用户显式开启系统前置代理模式后，系统代理缺失、不受支持、中继无法建立或既有浏览器无法证明已应用中继 SHALL 表示确定的双跳配置不可用，并阻止本次启动，MUST NOT 沿用单跳行为。客户端内修改代理配置或双跳开关后 SHALL 立即作废旧结果和旧链路。
