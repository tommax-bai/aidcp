...
`adspower` 模式下，分身 id **默认不需要手敲**：运维在环境列表中选中某项后，其 `user_id` SHALL 自动带出为将写入的分身 id（只读呈现）。桌面外壳 SHALL 另提供「手动填写」开关，**默认关闭**（此时分身 id 取选中环境的值）；打开后 SHALL 呈现手敲输入框作为兜底（列表拉不到等场景）。无论来自选中还是手敲，保存 / 启动前 SHALL 校验最终分身 id 非空，且写入的 SHALL 始终是环境的 `user_id`（非序号）。API 地址与 API Key 两个选填项 SHALL 默认折叠于「高级」中、点开才呈现。
