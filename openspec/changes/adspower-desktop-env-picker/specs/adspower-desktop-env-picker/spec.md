...
### Requirement: 保存与启动分离、会话由悬浮三态按钮控制

桌面外壳 SHALL 把「保存浏览器设置」与「启动 / 控制会话」分离：
- **保存** SHALL 仅持久化浏览器设置、**不打断**正在运行的核心；写盘失败仍 SHALL 诚实回报（沿用既有红线）。`self`（本机 Chrome）模式无可持久化的设置项 → SHALL 不呈现「保存」。
- **会话控制** SHALL 由窗口内一个显著（悬浮）主按钮承载，随边缘 / 会话状态呈**三态**：停止 / 空闲时为「启动」、运行时为「暂停」、已暂停时为「恢复」，分别触发启动 / 暂停 / 恢复。
- 当保存后核心仍以旧设置运行时，桌面外壳 SHALL 提供**显式**的「按新设置重启」入口，MUST NOT 由「保存」隐式重启打断当前会话。

#### Scenario: 保存只持久化不打断
- **WHEN** `adspower` 模式运维改了环境 / 高级项并点「保存」
- **THEN** 设置被写盘，正在运行的浏览 MUST NOT 被打断；若写盘失败则如实提示「本次生效但未持久化」

#### Scenario: 悬浮按钮随状态三态切换
- **WHEN** 边缘处于停止 / 运行 / 已暂停三种状态之一
- **THEN** 悬浮主按钮相应呈现「启动」/「暂停」/「恢复」并触发对应动作；顶部不再另设重复的暂停 / 恢复控件

#### Scenario: self 模式不呈现保存
- **WHEN** 运维切到 `self`（本机 Chrome）模式
- **THEN** 面板不呈现「保存」按钮（无可持久化设置项），仅由悬浮按钮启动 / 暂停 / 恢复
