...
### D9. 连接项（API 地址 / API Key）默认折叠进「高级」（UX 追加）
**选择**：API 地址、API Key 两个**选填**项默认收起在「高级设置」里，点开才现；主区只留环境列表 + 分身 id。
**理由**：二者多数情况不填（AdsPower 常不开 API 校验、地址用默认），常驻占位徒增噪音。
**取舍**：开了 API 校验的用户需多点一下展开——可接受（少数场景）；折叠不影响 D5 的「刷新取表单当前值」（展开填了即时生效）。
