...
### D11. 「保存」与「启动」分离；会话控制收敛为**悬浮三态按钮**（UX 追加）
**选择**：拆「保存并启动」为两件事：
- **保存**：只把浏览器设置写盘（`settings:save` 去掉其中的重启副作用），**不打断**在跑的浏览；`self` 模式无可存项 → 不显示保存（见 D12）。
- **会话控制**：窗口右下角一个**悬浮主按钮**，随状态三态：`edge 停止/idle → 启动`、`运行中 → 暂停`、`已暂停 → 恢复`；顶部原「暂停/恢复」toggle 收进它，避免两处打架（「重新登录」保留原位）。
- **应用改动到在跑核心**：保存后若核心在跑且设置已变，面板给「按新设置重启生效」的显式入口（走既有 `stopAndRestart`）——不由「保存」隐式重启。
**理由**：用户要「保存只存不打断、启动单列」；把启动/暂停/恢复三态收进一个显眼悬浮按钮，交互更聚焦。
**取舍**：改动到在跑核心需显式点重启（多一步，换来保存不再意外打断会话）；启动时自动化（应用启动即 `startFlow`）保留，悬浮按钮承载后续手动控制。

### D12. `self` 模式不显示保存（无可存项）；悬浮按钮语义跨 provider 统一（UX 追加）
**选择**：`self`（本机 Chrome）模式没有分身 / key 可存 → **不显示「保存」**，只由悬浮按钮启动 / 暂停 / 恢复；`adspower` 模式才显示保存（存分身选择 / 高级项）。悬浮三态按钮的语义两 provider 一致。
**理由**：self 无设置可存，硬留个保存按钮是空操作、徒增困惑。
**取舍**：provider 切到 self 时保存按钮隐藏、切回 adspower 再现——面板按 provider 动态显隐（已有 `applyProviderSelection` 显隐 `ads-config` 的先例，加一处）。
