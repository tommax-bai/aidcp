...
配图生成的时长 SHALL 可经环境变量配置，且默认值足以容纳较慢的文生图。多图下计时 SHALL **下沉到每张图**（env `AIDCP_PUBLISH_PER_IMAGE_TIMEOUT_MS`）：单张超时只丢该张、不砍断整批；角色级总闸 SHALL 设为远大于 `张数 × 单图轮询总预算` 的绝对兜底，且 MUST NOT 因总闸超时把已成功生成的图整体清零（超时须用已累积结果产出）。单图轮询总预算（轮询次数 × 间隔）SHALL 严格小于每图超时，使慢图在被砍断前完成。发布执行角色的超时 SHALL 覆盖"审批等待 + 张数 × 单图上传 + 余量"，且 `submit_publish` 成功后 MUST NOT 因超时把记录翻成 `failed`。

#### Scenario: 每图超时 env 可调且大于单图轮询预算
- **WHEN** 设置文生图轮询次数与每图超时的环境变量
- **THEN** 二者 SHALL 生效，且每图超时（毫秒）SHALL > 轮询次数 × 轮询间隔，使慢图能在被砍断前完成；角色级总闸 SHALL 远大于 张数 × 单图预算
