...
- **选**：① `ImageSetPlanner`（图集选题，新）watch `createdContent` → 读正文决定张数 + 每张主题（业务语言）+ 风格倾向，写新键 `imageSetPlan`；② `ImagePromptComposer`（配图指令，新）watch `imageSetPlan` → 把每个主题翻成一条万相 prompt（统一风格/无文字/无真人/英文），写**现有键** `imagePlan`（升级为 `imagePrompts: string[]`）；③ `ImageGenerator`（现有升级）watch `imagePlan` → 逐张出图。
...
### D2. N 张为各异画面（顺序单图调用），不用万相 `n>1`
- **为何**：`n>1` 出的是同一语义的多采样（画面近重复），做不出图文叙事递进，且一个慢任务卡死即全失败、无法部分成功。N 个不同 prompt + 共享**固定风格基底**（模板常量，不让 LLM 产）= 统一风格、不同主体。
- **去重护栏**：prompt 归一化近似比对，命中即丢该张（不补不复用）——但**永远保住第 0 张**（封面位），由护栏自身保证 `wantImage:true → ≥1 张`，不让护栏吃光导致"本可发图却自杀"。
- **接口零改**：循环调现有单图 `generate(prompt, style)`，不动 `ImageProvider` 接口、不动 `WanxiangClient` 的 `n:1`。

### D3.（关键正确性）超时下沉到每张图，绝不用角色级总闸裹整个循环
- **问题**（已核实）：若生成角色把 N 张循环放进一个 `execute()`、靠 `BasePublishRole` 的 `Promise.race` 总闸（`base-role.ts:74`）计时，则总闸到点 reject → `fallback:'skip'` → `getDefaultOutput()` 返回空 directive（`image-generator.ts:66`）→ **已成功生成的前几张 URL 被整体丢弃** → 误判无图 → 诚实 failed 但白干。这是红线另一面（部分成功被静默清零）。
- **做法**：生成角色循环内对**每张** `Promise.race(generate, perImageTimeout)`，单张超时只丢该张、继续下一张；累积成功 URL 到 `execute()` 可返回的局部；角色级总闸设为**远大于** `N × 单图轮询预算` 的绝对兜底，且即便触发也用"已累积 `imageUrls`"构造 directive 返回、绝不清零已成功。`perImageTimeout` env：`AIDCP_PUBLISH_PER_IMAGE_TIMEOUT_MS`；张数上限 env：`AIDCP_PUBLISH_MAX_IMAGES`。
...
- **[每图超时与角色总闸冲突]** → 总闸设为绝对兜底（≫ N×单图预算），超时也返回已累积；杜绝"封顶 < N×预算"自相矛盾配置（D3）。
...
- **[同账号 N:1 去重窗口被拉长的发布超出]** → N×上传总时长 vs `interaction-guard` in-flight 去重/超时窗口**待坐实**（Open Question 2）。
...
2. **去重窗口**：N×上传总时长是否落在同账号 N:1 多节点 `interaction-guard` 的 in-flight 去重/超时窗口内？若否，需把窗口随发布动作放大。→ 实装前坐实。
3. **默认张数 3** 是否合适？（可经 `AIDCP_PUBLISH_MAX_IMAGES` 与选题角色策略调整。）
