...
- Enrich Facebook account nicknames only from verified startup identity.
  - Rationale: probes show Facebook home/login pages may expose generic titles or many unrelated profile links, while `/me` resolves to `profile.php?id=<numeric-id>` for the logged-in profile. The edge may navigate to `/me` after establishing a stable numeric identity, but it may only accept `h1`/OpenGraph/title text as a display name when the final URL's numeric id matches the established account id. Display names are never identity evidence, and generic titles such as `Facebook` are ignored.
