...
## 7. Native 图片上传后的 composer 换代与输入连续性

- [x] 7.1 记录 Tianxing Bai 真机失败证据并补契约：上传图片会保留旧 composer、生成携带预览的新前台 composer；禁止以任意 dialog/头像图片确认上传，禁止按 DOM 第一个 dialog 选编辑器。 <!-- profile k1ei3dbi 首次修复验证在 5s 内诚实回报 media_preview_unconfirmed；随后 DOM 证据确认新前台 composer 出现精确文件名 blob: 预览，旧 composer 仍在。契约已补同代绑定、精确预览与焦点连续性要求。 -->
- [x] 7.2 将文件输入、文件名匹配的 `blob:` 预览、正文编辑器和提交按钮绑定到同一代前台 composer；上传换代后重新绑定，无法唯一确立时诚实失败。 <!-- aidcp-edge 7c29a24；文件输入、预览、编辑器与提交目标均使用当前 composer 绑定，重叠换代选前台，非重叠多 composer 回报 ambiguous_target；精确预览有界等待 20s 且仍受命令绝对截止时间约束。 -->
- [x] 7.3 逐字派发每个字符前验证绑定编辑器仍持有焦点；漂移即 `composer_focus_lost` 停手，剩余字符不得写向任意焦点，失败结果的外层 `reasonCode` 不得为 `confirmed`。 <!-- aidcp-edge 7c29a24；guarded humanized typing 在每个 Unicode scalar 前复核绑定与 activeElement，目标丢失停止并只清理由同一绑定确认的编辑器；非 confirmed 结构化结果统一映射 probe_failed。 -->
- [x] 7.4 补 router DOM 与 Rust Fake CDP 回归：头像假预览、双 composer 前台选择、上传换代、输入中途焦点漂移、失败清场和外层失败原因码。 <!-- aidcp-edge 7c29a24；router 67/67，Rust publish 23/23；全量 Rust lib 85/85、contract 1/1、feed-like 2/2、Fake CDP 29/29、process protocol 1/1。 -->
- [x] 7.5 完成 Rust/Edge 定向与全量验证、Native 构建，并在 AdsPower profile `k1ei3dbi` 用同一图片和正文执行一次真实发布；记录平台结果、产物哈希、集成与未打安装器边界。 <!-- aidcp-edge 7c29a24 已快进集成并推送 origin/master；clippy -D warnings、Native TypeScript 定向 100/100、Edge acceptance 30 pass / 1 gated suite、全量 2438 pass / 1 skip、typecheck、dist 与 desktop build input 均通过。canonical darwin-arm64 artifact SHA-256 cd07f91cb5c298625b7590796d64c1f6143203fee2ff008198018d68c2ea1186。真实发布 navigate 4596ms、select 1314ms、upload 6218ms、逐字 fill 155669ms、submit 78ms；平台回读 909 code points 正文逐字一致并有 1 张 CDN 图片，permalink=https://www.facebook.com/permalink.php?story_fbid=pfbid0WKSnokrBwBtxhiPmUfcDPRYpb8hBETrPC4jrDbLLz3wK256R86CmuBxQEihbzokZl&id=61591753702668。首次失败草稿已删除，成功帖子保留；未打包安装器、未重启在途 AIDCP。 -->
