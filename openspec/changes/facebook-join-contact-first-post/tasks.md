...
- [x] 7.5 Commit, rebase, fast-forward integrate, and push the scoped Edge/Cloud/control changes; deploy only the eligible Cloud runtime change to DEV, do not package Edge, and do not touch OL
  <!-- 2026-07-27: Edge abde530, Cloud 373bc82, and control c2d8fd5 were rebased onto the latest defaults, revalidated, fast-forward integrated, and pushed. DEV already matched the intervening Facebook rule-mode baseline and migrations, so deployment was limited to the changed Cloud facebook-edge-steps.ts after backups /opt/aidcp/cloud.bak.20260727-225131.tar.gz and /opt/aidcp/cloud/.env.bak.20260727-225131. Local/remote file SHA-256 matched 0a6c96b; content/automation/api pending migrations=0; aidcp-cloud.service active with NRestarts=0; 8787/8090/8091, both health endpoints, PostgreSQL SELECT 1, all three enforce schema gates, automation writer lock, Feishu WS onReady, and isales-api/isales-scheduler were healthy. No Edge installer/package was built; OL was untouched; no Cloud-orchestrated real comment was submitted. -->

## 8. Native Below-Fold First-Post Regression

- [x] 8.1 Record the Gi Vo below-fold DOM evidence and the Native `note.open` selection/container migration gap without broadening first-post semantics
  <!-- 2026-07-27: the live group page had innerHeight=1066, documentHeight=2633, two article cards starting near y=1951/2249, zero visible articles, and no initially hydrated group-post permalink. Two Cloud runs visually opened posts but emitted no canonical note.detail before the 45s wait expired. TypeScript Native mapping and Rust NoteOpenParams omitted selection/container, while the legacy JS handler supported them. -->
- [x] 8.2 Preserve and validate the first-post selection/container in Native command mapping, then implement fixed bounded same-group scroll/probe/open with exact canonical identity and editor readiness
- [x] 8.3 Add focused Edge mapper/Rust tests covering parameter preservation, below-fold hydration, same-group URL validation, no-candidate stop, and no generic-note-open degradation
- [x] 8.4 Preserve Cloud first-post failure reasons in the combined user receipt, distinguishing timeout, no candidate, editor-not-ready, and target-context mismatch, with focused tests
- [x] 8.5 Run focused/full Edge and Cloud validation, both typechecks, and strict OpenSpec validation; record real-account read/write evidence separately
  <!-- 2026-07-28: Edge focused Native/mapper tests passed, serial full Rust passed 132 tests, full TypeScript passed 2480 with 1 gated skip, and typecheck passed. Cloud focused passed 130, full passed 3730 with 11 gated skips, and typecheck passed. Strict OpenSpec validation passed. An earlier parallel Rust run had two unrelated publish deadline timing assertions fail; both isolated reruns and the post-rebase serial full suite passed. This turn used the pre-change Gi Vo read-only DOM/runtime evidence only; no post-change real-account read replay or comment write was performed. -->
- [x] 8.6 Commit, rebase, fast-forward integrate, and push the scoped Edge/Cloud/control changes; refresh only eligible DEV runtime state, do not package Edge, and do not touch OL
  <!-- 2026-07-28: rebased and fast-forward integrated/pushed Edge c433063, Cloud 222d0f4, and control ccdaa02 plus this closing evidence commit. DEV Cloud deployed only comment-scheduler.ts after backups /opt/aidcp/cloud.bak.20260728-003042.tar.gz and /opt/aidcp/cloud/.env.bak.20260728-003042; local/remote SHA-256 matched 59c81fb. Content/automation/api pending migrations=0; aidcp-cloud.service stop→start active with NRestarts=0; 8787/8090/8091, panel/client health, all three PostgreSQL SELECT 1 checks, all three enforce schema gates, automation writer lock, RiskControllerRegistry, Feishu WSClient onReady, zero startup error/fatal/failed lines, and both isales services passed. Edge dist and unsigned darwin-arm64 Native artifact were rebuilt from c433063 and verified at SHA-256 a5637bb; no installer/package was built. The already-running source Electron process predates those artifacts and was intentionally not killed after recent automatic group tasks; operator restart remains required before a post-change real-account read/comment acceptance. OL was untouched and no real comment was submitted. -->
