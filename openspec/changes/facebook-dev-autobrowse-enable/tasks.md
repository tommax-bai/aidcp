...
- [x] 2.6 Audit every cloud-dispatchable action against `FacebookBrowseSession`; canonicalize unsupported completion names, apply `pacing.update`, and restore the remembered feed/search context before scrolling. <!-- 2026-07-13: edge commit `7b9b37e` rebased, merged, and pushed to `master`; full Edge tests 1139/1139, acceptance 18/18 (one gated E2E skip), typecheck PASS, then local `build:dist` refreshed. -->
...
- [ ] 3.1 Update this task record with commits, validation, and the real-machine outcome; validate the OpenSpec change strictly. <!-- 2026-07-13: cloud `25379e6` merged/pushed to `master` and deployed to dev (backup `cloud.bak.20260713-183857.tar.gz`; service, 8787, panel, PostgreSQL, Feishu, and isales health checks pass). Edge `7b9b37e` merged/pushed to `master` and local `build:dist` refreshed. The Tianxing Bai debug endpoint was no longer listening, so the post-fix rendered observation remains pending under 2.3 and is intentionally not claimed here. -->
