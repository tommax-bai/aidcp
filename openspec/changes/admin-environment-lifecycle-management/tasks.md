...
<!-- aidcp-cloud d893245; storage/API focused tests 115 passed, full 2660 passed + 8 gated skips; live additive schema verification is task 5.4. -->
...
<!-- aidcp-edge 1198b26; maintenance/registry focused tests 9 passed and full 1969/1969 passed; no installer built and no real AdsPower profile deleted. -->
...
<!-- aidcp-console b66579d; focused UI tests 20 passed, full 217 passed + 1 skipped, typecheck and production build passed. -->

## 5. 验证、集成与 dev 发布

- [x] 5.1 运行 Cloud/Edge/Console 聚焦测试、适用 acceptance、全量测试、typecheck/build，并记录真实通过范围。
<!-- Cloud 2660 pass/8 gated skip; Edge 1969 pass; Console 217 pass/1 skip; all typechecks passed and Console build produced assets with the existing large-chunk warning. -->
...
- [ ] 5.3 各仓提交后 fetch/rebase 最新默认分支，复验并 fast-forward 推送 Edge/Cloud/Console/control 默认分支，绝不 force。
- [ ] 5.4 按部署规范从干净默认分支部署 Cloud 与 Console 到 `dev`，验证服务、监听、健康、数据库 additive schema、Panel 环境 DTO 与静态页面；不构建 Edge 安装包。
- [ ] 5.5 记录非破坏性验证边界：自动测试不得删除真实 AdsPower profile，真实删除只在后续具备明确一次性测试环境和操作者授权时执行。
