...
- [x] 7.2 各域各出一段窄口、接口侧拼装。
  <!-- 2026-08-05 **结案 = 改判取代，不是做完**。本条按原文**没有做**，也不该做：
       7.3 已经裁定改成「本进程只提供它能诚实提供的那一半」（18 个就地渲染，
       其余回 available:false + 说清渲染器在哪个进程），并已实装。
       原方案要开两条窄口、其中一条还要跨进程搬「渲染时的调度器上下文」，
       成本与一个只读预览页不成比例——判断依据逐条写在 7.3 的批注里。
       剩下那一半（浏览 / 发布 / 配图角色的真预览 + 账号口径）已按 7.3 的结论登记 backlog。
       ⚠️ MUST NOT 把本条读成「窄口做好了」：接口侧至今没有拼装，也不打算有。 -->
...
- [x] 8.4 精选库跨进程后账号隔离仍由属主侧保证的用例。
  <!-- aidcp-content 61bd075 test/acceptance/curated-cross-process-account-isolation.test.ts
       （标 `aidcp:test-owner=derived`：它读共享包的路由表，而 cloud 不依赖那个包，
       不标就会被跨仓同步当成「多出的文件」，`--prune` 一跑就没了）。

       **守的形态**：拆仓前 `accountId` 是本地调用传下来的，越权只可能来自本仓代码；
       拆完之后它是**远端进程经内部 HTTP 递进来的参数**，于是「谁保证只碰这个账号的行」
       第一次有了错误答案——「调用方会传对的」。属主一旦不约束，任何能打到内部口的进程
       换个 accountId、或直接猜一个全局自增 id，就能读到 / 删掉别人的行。
       **这类回归不报错**：SQL 跑通、路由 200、两仓测试各自全绿，只表现为数据串账，
       而串账在观察面上与「这个账号本来就有这些内容」完全同形。

       **两条判据，缺一条闸就是空的**：
       ① 面**穷举**——跨进程能碰到精选库的方法全从路由表现算（`CURATED_CONTENT_ROUTES` /
          三张 authority 表的键就是属主方法名），面板那五条路由名≠方法名、单列一张对照表
          且该表本身被「路由表里多一条 curated* 而对照表没有 ⇒ 红」钉住。
       ② 约束**真到达数据库**——属主发出的每条打到精选表的语句，SQL 里要有对 `account_id`
          的等值约束（先抹掉 SELECT 投影再判，否则面板列表把它 SELECT 出来就能冒充约束），
          且调用方给的那个值要绑在**对应的**占位符上。只查 SQL 文本＝绑错账号照样过；
          只查参数＝删掉 WHERE 照样过；只查「参数里有这个值」＝约束读错位照样过。

       **口径刻意不写死成「任何时候都必须过滤」**：面板两条聚合读的 accountId 是可选的
       （不给＝全账号合并视图，运营控制台的正当用法）。本闸判的是「给了就必须绑」。
       写成前者会逼下一个人去改一个正确的行为，那种闸迟早被整条注释掉。

       **四轮变异，并记了是哪条抓住的**（`git checkout --` 还原、`git status` 核过干净）：
       · 去掉 `getOneForAccount` 的 `AND account_id = $2` ⇒ 只有行为那条红
       · `listForPanel` 的账号条件换成恒真 ⇒ 只有行为那条红
       · 约束在、但绑成写死账号 ⇒ 只有行为那条红
       · 从覆盖表里删掉一个跨进程方法（漏登记，最静默的那种）⇒ **只有清单那条红，行为那条全绿**
         ⇒ 清单闸独立承重，不是行为闸的重复。
       content：acceptance 33/33、全量 472/472、typecheck 0；`scripts/sync-split-repos` 对账无新差异。 -->
