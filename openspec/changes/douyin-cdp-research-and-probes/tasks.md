...
- [x] 5.3 Implement a single-conversation direct-message reply probe gated by `AIDCP_DOUYIN_PROBE_DM_REPLY=1`, exact profile confirmation, proven inbound direction, and the exact allowlist `好的`/`ok`, without reporting identities or message bodies.
...
- [x] 7.4 With the user's explicit authorization, run at most one follow, one collection, one DM reply `好的`, one live ordinary chat `1111`, and one comment-targeted live reply `666`; preserve honest UI evidence and report unavailable/ambiguous actions without substitution.
- [x] 7.5 Keep real like, ordinary work-comment submission, file selection, and final publishing unrun unless separately authorized; record these as intentionally not run without weakening automated coverage.
...
<!-- Implementation evidence (2026-07-22): aidcp-edge commit 11022ec on codex/douyin-cdp-research-and-probes; 14/14 focused tests passed; npm run typecheck passed. The worktree uses a physical npm ci --prefer-offline dependency tree. No production router, deployment, package, merge, or archive was performed. -->
<!-- Live evidence boundary: exact profile k1evgky5 and its start-page marker were confirmed. One follow and one collection reached same-modal positive UI states; one allowlisted DM reply of length 2 was submitted and its editor cleared; one live chat 1111 was submitted, its editor cleared, and one exact message node was read back. The page exposed no unique comment-bound live reply control, so 666 was not sent and ordinary chat was not used as a substitute. -->
<!-- Intentional exclusions: real like, ordinary work-comment submission, file selection, upload, draft creation, and final publishing were not run. Initial pointer attempts were intercepted by a first-use overlay and produced no state change; after the user identified it, the exact visible 我知道了 button was dismissed and target hit-testing was added. Unchecked tasks remain open for the broader browse, like-shadow, comment-fill, publishing-surface, and automatic DevToolsActivePort research scope. -->
