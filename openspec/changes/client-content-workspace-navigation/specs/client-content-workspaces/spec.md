...
客户端 SHALL 在标题栏右侧控制区提供一个紧凑的灵感库入口：账号名与平台标识留在左侧，弹性留白把灵感入口推到右侧，灵感入口紧邻环境名左侧。入口展示当前账号的精选池总数、真实参照成稿总数和非百分比的储备丰富度条。运行首页 MUST NOT 同时保留大卡片入口。储备条 SHALL 只由精选池总数驱动，不得由成稿数计算使用率或剩余量；低储备不得使用风险告警色，达到展示饱和阈值后才可使用轻微蓝绿强调。
