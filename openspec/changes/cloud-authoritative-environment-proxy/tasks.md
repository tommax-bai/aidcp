...
- [ ] 4.6 Perform a real Edge smoke test covering create/edit, direct mode, GOST mode, close restoration, and cross-`AIDCP_USER_DATA_DIR` authority consistency
  <!-- Not run in this delivery: no Edge installer was requested or built, and a real AdsPower/customer session is required. The new DEV authority table intentionally starts empty; existing environments must migrate from the original safeStorage record or explicitly re-save their proxy, never bootstrap from the mutable AdsPower execution copy. -->
