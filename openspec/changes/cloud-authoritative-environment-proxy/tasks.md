...
- [x] 3.9 Retain the frozen effective proxy's observed egress and accept a configured Active browser only when its one-time browser egress matches exactly
  <!-- aidcp-edge@f7c6382: preflight retains only the normalized controlled-endpoint egress in its private revision-bound result; the child pipe carries that evidence, and the core compares it exactly with the attached Active browser before takeover. No profile field or generation marker is polled after takeover. -->
- [x] 3.10 Make Active-browser egress mismatch/unavailable terminal for the current launch, preserve the pre-existing browser, and keep explicit no-proxy behavior unchanged
  <!-- aidcp-edge@f7c6382: stable adspower_active_proxy_takeover_rejected classification stops respawn with streak 0 and never invokes browser stop; no_proxy produces no authority pipe, no expected-egress probe, and no Active takeover marker. -->
...
- [x] 4.10 Run focused provider, preflight, lifecycle, and no-proxy regression tests, Edge typecheck, and strict OpenSpec validation for Active-browser takeover
  <!-- validation: 95 focused provider/preflight/runtime/fleet/lifecycle tests pass; npm run typecheck and npm run build:dist pass. The post-fix full parallel suite had one unrelated interaction-workspace timing assertion advance from "waiting" to "completed"; its isolated rerun passed. No Edge installer was built or installed, and task 4.6 remains the live AdsPower acceptance boundary. -->
