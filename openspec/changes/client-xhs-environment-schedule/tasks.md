...
- [x] 4.3 Rebase/integrate clean feature branches into the latest default branches, rerun required validation, push Cloud/Edge/control defaults and deploy eligible Cloud runtime changes to DEV with backup and post-deploy checks.
  <!-- Integration: Cloud feature 6c9ab00 is contained in clean/pushed master c0de08b; Edge feature ac79920 is contained in pushed master c0bdc34; control feature 40cc848 is contained in main. Current-default validation on 2026-08-01: Cloud schedule/auth focused 87/87, full suite exit 0, typecheck passed; Edge schedule/security focused 25/25, full suite 2964/2964, typecheck passed. DEV was already redeployed from the repaired Cloud master after backups under /opt/aidcp; a checksum dry-run from git archive c0de08b found no content differences against /opt/aidcp/cloud. Post-deploy checks passed: aidcp-cloud active, 8787 and 8090 listening, external WebSocket handshake, customer-auth 8091, all three enforce schema gates, automation writer lock, Feishu onReady, and all four isales services remained active. The optional .deployed-commit marker is currently missing, so deployed-source equality is evidenced by the full committed-file checksum comparison rather than that marker. -->
- [x] 4.4 Verify the DEV customer-auth schedule endpoint and served/runtime source boundaries, and record that installed Edge clients remain unchanged until a separately authorized package/release.
  <!-- DEV authenticated read-only probe: GET /environments/:envKey/schedule returned 200 for an owned/bound XHS environment with data+meta, seven days, timezone, weekStartsOn=monday and no accountId/raw masks/source/updatedBy fields; the external /capi/health and 8787 WebSocket probes also passed. Cloud served source matches c0de08b. Edge source contains ac79920, but source integration alone does not update installed clients and this closeout did not build or install a package. Current workstation evidence shows the separately signed 2026-07-31 dev package is installed at /Applications/AIDCP.app (0.3.25, dev /capi target), its app.asar matches the corresponding signed build and contains the schedule feature; other installations remain unchanged unless separately packaged and installed. -->
