...
>
> 用户在通盘复核任务量时裁定：**Facebook 群组帖身份泛化** 这条不继续做。
> 下面**每一条未勾选项都已就地标注「不是待办」**——`- [ ]` 的方框在这里不表示待办，只表示「没做，也不打算做」。
>
> **进度快照（划掉时）：已做 38/56，剩余 18 项作废。**
>
> ## 两条接手时必须知道的
>
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 6.1 基线：冷启动 + 纯程序化滚动，记录可上报卡数（当前实测 **0**）
- [ ] **【已划出计划 2026-07-30，不是待办】** 6.2 改后：可上报卡数应 > 0；逐张核对作者与正文正确
- [ ] **【已划出计划 2026-07-30，不是待办】** 6.3 就地点赞：能命中、能按按钮状态确认；**绝不能**点到别的卡上
- [ ] **【已划出计划 2026-07-30，不是待办】** 6.4 确认云端从未对会话内引用下发导航类命令
- [ ] **【已划出计划 2026-07-30，不是待办】** 6.5 采样引用碰撞率（招聘 / 租房群是最坏场景）；碰撞只应导致"少读一条"，绝不导致"操作错帖子"
- [ ] **【已划出计划 2026-07-30，不是待办】** 6.6 虚拟化复用场景：滚走再滚回，引用不得漂到别的帖子上

## 7. 部署

- [ ] **【已划出计划 2026-07-30，不是待办】** 7.1 边缘侧改的是编入 Rust 二进制的引擎与注入脚本，**仅 push 不生效**，须重打客户端包 <!-- 未做：打包属用户显式触发（CLAUDE.md §6），已登记 backlog 簇 121 前言 -->
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 9.1 `facebook-feed-browse` 现有正文隐含「可上报 = 持有平台永久链接」，与本 change 的第二类身份口径不一致。本 change 刻意不叠 delta，**待门槛齐备后补一条只改措辞的 change 收口**——两份规格同时有效期间，后来人按哪一份实现都说得通，这是真实的歧义风险

  > **2026-07-29 复核，本条的范围与门槛都被改写过，见 §12。**下面是收口时必须覆盖的清单
  > （原文只点了 `facebook-feed-browse` 一份能力，实测**划窄了**）：

- [ ] **【已划出计划 2026-07-30，不是待办】** 9.2 **收口范围（三份能力、至少七条要求）**。逐条核对过，勿再按「就那三条」估工：
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 9.3 **`facebook-feed-browse:63` 不是歧义、是直接互斥，优先级高于其余各条。**
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 11.5 **`6.x` 真机验证全部未做**（已解耦到 backlog 簇 121）。可上报卡数是否真的从 0 变正、
  引用会不会漂到别的卡上、碰撞率如何，一概未测。**前置是 11.6 的边缘出包**。
- [ ] **【已划出计划 2026-07-30，不是待办】** 11.6 **边缘未出包**：cloud 侧 `424834d` 已部署 dev；edge 侧改的是编入 Rust 二进制的引擎与注入脚本，
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 12.2 **但门槛仍在，只是换了三条真理由**（对抗性复核，三路全部推翻「现在就能收口」）：
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 12.3 **此前不归档本 change**——一旦归档，§9.1 就只活在归档目录的 tasks.md 里，
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 12.2 归档时顺带确认：本 change 的 spec delta 只新增 `facebook-content-derived-post-identity`
  一份能力，不碰 `facebook-feed-browse`（刻意为之，见提案）。

## 13. 实装期发现（不属于本 change，但别忘了）

- [ ] **【已划出计划 2026-07-30，不是待办】** 13.1 **两份 `protocol.ts` 已有既存漂移（非本 change 造成）**：`origin/master` 上 18 行不一致
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 13.2 **`contentRefNoteIds`（云端调度器侧，前一批 `3e4203b` 引入）没有清理点**：
...
- [ ] **【已划出计划 2026-07-30，不是待办】** 13.3 **`fake_cdp` 的 reel 滚动用例在并行满载下偶发超时**（`deadline_unix_ms = now + 8s` 是墙钟预算，
