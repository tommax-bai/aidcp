...
A ManagedPlan SHALL carry an account-level operating-window calendar (day-of-week × hour) that applies across plans, cycles, and trigger sources. Outside the window Automation MUST NOT start or resume a ManagedCycle, Task, or TaskRun, and work already running SHALL terminate at its next declared safe point rather than merely being barred from restarting. This prohibition governs autonomous business work only: recovery-class work (captcha, risk-control, and human-assist work that itself needs the browser) and the narrowly enumerated clearing path and session-end commands required to lift an existing suppression SHALL remain admissible outside the window, because an account blocked at night must not stay blocked until the window reopens; such admission MUST NOT be used to advance any business action. A write/content action window SHALL be a subset of the account's operating window, and an action-level window MAY only narrow it. A missing or invalid window definition MUST be treated as "no autonomous work" and MUST NOT inherit a fail-open default from another mask. The window SHALL be enforced server-side independently of any client or editor guarantee, and when the window reopens Automation SHALL proactively re-admit eligible work rather than wait for an external event.
...
The Automation component that owns account risk state and quota admission SHALL run as exactly one instance **per automation owner database**, covering every execution target that shares it, enforced by acquiring a database-level session-scoped mutual-exclusion handle keyed by that owner scope before enabling any risk or quota write path. Target-keyed single-instance applies only to workers whose state is target-partitioned (claims, scans, recovery, lifecycle). Keying the risk/quota handle by target would let each target acquire its own non-excluding handle and write the same unpartitioned account consumption ledger concurrently, defeating both the single-writer invariant and the quota gate. Failure to acquire within a bounded wait MUST alert and exit non-zero; loss of the holding connection MUST stop dispatching new interaction commands and MUST NOT perform further authority-granting writes (risk-state transitions, quota grants or reservations, new Attempt dispatch); recording already-actuated facts and reconciling already-dispatched Attempts to their real results SHALL remain permitted, MUST NOT be discarded, and MUST be idempotent. Deployment of this component MUST be stop-then-start; rolling or blue-green rollout MUST NOT be used.
