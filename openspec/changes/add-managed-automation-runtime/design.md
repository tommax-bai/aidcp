...
如果直接在 Agent 中写循环，Agent 会同时承担业务规划、持久状态、账号资源争用和平台副作用；如果直接在 Edge 中写流程，客户端会拥有本应在 Cloud 的策略与权限；如果新建一个通用 Workflow 服务，则会在 Automation 之外再造一套运行、租约、重试、审计和状态真相。
...
| Agent Service | 对话理解、目标分解、提出 `PlanProposal` / `PlanPatch`，不执行平台动作 |
| `aidcp-content` | 内容理解、评估、创作作业、候选版本、媒体资产 |
| `aidcp-automation` | 触发、周期、工作流、账号仲裁、安全准入、执行账本、Edge Gateway、对账 |
...
flowchart TB
  ...
  subgraph L2["第二层：决定怎样推进、何时可执行"]
    WF["workflow-runtime<br/>推进类型化步骤"]
...
| 模块 | 拥有 | 不拥有 |
| --- | --- | --- |
| `trigger-registry` | 已登记事件/排期/人工触发与 Definition 的绑定、去重、并发策略、因果深度 | 工作流步骤、Edge 命令、业务授权 |
| `managed-cycle-runtime` | 一天或一轮活动的目标、预算、子运行引用、周期总结 | 稿件、审批、平台尝试细节 |
| `workflow-runtime` | Definition 解释、Run/Step 状态、等待点、恢复位置 | 最终风险判断、平台结果证据 |
...
| `execution-ledger` | 不可变意图、幂等键、Attempt、派发、回执、证据、取消请求 | 工作流选路、自由文本解释 |
...
- Worker 进程承载 Trigger、Cycle、Workflow、Arbiter、Policy、Ledger 和 Trace；
...
- 新建 Workflow 服务：会重复 Automation 已必须拥有的任务、租约、策略、账本和恢复机制。

### 2. 运行对象分五层，长期目标不等于一个永不结束的 Run
...
classDiagram
  ...
  ManagedCycle "1" --> "*" AutomationRun
  AutomationRun "1" --> "*" WorkflowStep
  WorkflowStep "1" --> "0..*" ExecutionAttempt
```

- `ManagedPlan`：客户可见的长期运营目标和授权边界。Agent 可以提出，但由 API 鉴权、持久化和激活。
- `ManagedCycle`：Automation 拥有的有界执行周期，例如一个自然日、一次活动或一次人设刷新后的研究周期。
- `AutomationRun`：一个 Definition 的一次业务运行，例如“研究 30 条内容”“发布候选 v8”“回复消息 m1”。
- `WorkflowStep`：可恢复的类型化步骤。
- `ExecutionAttempt`：一次真实平台动作尝试。

Run 和 Step 不使用大量互斥顶层状态，而使用正交字段：
...
**未选择一个长期 Run 的原因：** 长期运行会累积无限历史、无法定义重试边界和预算，也使版本更新、日总结、归档和故障恢复难以验收。

### 3. ManagedPlan 由 API 授权，Automation 只拥有运行副本

对象所有权固定如下：

| 对象 | 单一写入者 | 其他服务如何使用 |
| --- | --- | --- |
| `PlanProposal` / `PlanPatch` | Agent Service | 提交给 API；仅为建议 |
| `ManagedPlan`、客户可见动作授权 | API | 通过持久 `ManagedPlanActivated/Updated/Paused` 事件进入 Automation |
| Automation Definition / Trigger Binding runtime | Automation | API 只引用已发布版本 |
| `ManagedCycle`、Run、Step、Attempt、DecisionTrace | Automation | 以结果事件回流 API 投影 |
...
Automation 中的 Plan runtime 行保存 `planId + planVersion + authorizationRevision`，不复制客户可编辑全文成为第二事实源。

### 4. Definition 是受管、类型化、版本化模板，不是任意 Workflow 脚本

首版 Definition 由代码和受审配置发布，至少包含：
...
每种步骤都有固定输入/输出 schema、允许产生的外部动作、超时和终止语义。Definition 不允许：
...
Agent 只能返回符合 schema 的 `PlanProposal`、`PlanPatch` 或步骤输入候选。Automation 必须再次校验 Definition、动作范围和能力，不能把 Agent 输出当作已授权命令。

### 5. Trigger Registry 只决定“是否创建 Run”，不执行流程
...
`latest_wins` 只可替换尚未进入外部 `dispatched` 的旧 Run。旧 Run 已派发写动作时，新版本必须另建 Run，并等待旧结果归并。
...
人设或内容更新不会原地修改运行中的步骤输入，而是按 Binding 的 supersession policy 创建新 Run。已经 `dispatched` 的动作永不被静默替换。
...
优先级只在安全点生效。安全点位于步骤边界或 Edge 已确认的原子命令边界；不得在填充一半、提交在途或平台结果未知时抢占。暂停浏览时，Workflow 保存当前位置和已验证内容 ID；恢复后重新验证页面身份，不能假设浏览器仍停在原页面。
...
1. **计划准入**：创建 Run 时确认计划引用了有效动作域、预算和 Definition。
...
每个 Plan/Cycle/Run 可以分配三类独立预算：

1. **平台/风控预算**：每日动作数、会话动作数、冷却、重复目标限制。
2. **执行预算**：浏览器分钟数、唤醒次数、步骤数、等待上限、尝试次数。
3. **AI/内容预算**：模型 token、图片/视频生成次数、创作尝试和成本上限。

预算不能互相替代。例如“仍有 AI token”不能允许超过评论配额，“还有平台动作数”不能允许无限占用浏览器。Cycle 给子 Run 分配预算，子 Run 只消费自己的额度；实际平台动作只在平台确认或现有风控合同指定的保守事实点记账。

### 11. Execution Ledger 是平台动作真相，Workflow 只引用它

每次准备真实动作时，Workflow 先向 Ledger 建立不可变 `ExecutionIntent`：
...
- `prepared` 阶段尚未派发可按 Definition 的有界策略重试；
...
当 Core 未连接时，Run 进入持久 `waiting_for_edge`。Classic 或未来 Agent Client 启动/恢复 Host，Core 完成权威握手后 Gateway 才唤醒 Run；客户端不能自行把它标成已执行。
...
### 15. 创作是外部子作业，不把 Content 变成 Workflow 的内部步骤实现
...
2. Trigger Registry 命中 Plan 的绑定，以 `accountId + personaVersion + definitionVersion` 去重。
3. 创建 Run，冻结 persona v7、Definition v3、账号绑定、30 条阅读预算和截止时间。
...
7. `browse` 只统计“拿到稳定内容 ID 且完成规定阅读证据”的唯一内容；页面卡片出现、重复卡片或连接前已看内容不计数。进度持久写入 Step。
8. 满 10 条后执行第二次搜索，再累计 20 个新的唯一内容。
9. Edge 重连时从 Ledger/Step 已确认的 ID 集合恢复，不从 0 开始，也不把旧卡重复计数。
10. 在最大页面数、时间或内容供给耗尽后仍不足 30 条，Run 以 `partially_succeeded` 结束并给出实际 `10 + 13`，不得报告 30。

这类规则沉淀在 Automation Definition 和 Workflow，而不是 Agent prompt 或 Edge 的环境变量阈值。
...
一个 Cycle 可以包含研究、互动、创作、发布多个子 Run，但只保存引用、预算和结果摘要。研究成功而创作失败时，Cycle 是部分完成；发布 `submitted_unknown` 时，Cycle 不能显示“已发布”，而是等待 Reconciler 或结束为需关注。

### 18. 典型场景三：入站回复和无需浏览器的执行

入站消息先由 Automation 拥有的 inbox/connector 形成 `InboundMessageReceived`。Trigger Registry 创建 reply Run：
...
planVersion / definitionVersion / personaVersion
...
Trace 不能反向覆盖 Run/Ledger 状态；删除 Trace 也不能让平台结果消失。敏感原文尽量保存引用、哈希和必要摘要，避免把第三方内容、私信和完整 prompt 无限保留。
...
| 来源 → 目标 | 合同 |
| --- | --- |
| API → Automation | `ManagedPlanActivated`、`ManagedPlanUpdated`、`ManagedPlanPaused`、`ApprovalRecorded`、`RuntimeControlChanged` |
...
| Automation → API | `ManagedCycleChanged`、`AutomationRunChanged`、`PlatformActionConfirmed`、`AutomationAttentionRequired` |
| Automation ↔ Edge | protocol v2 versioned commands、capabilities、receipts |

跨服务命令/事件使用 Outbox/Inbox；会话内逐卡浏览事件继续留在 Automation 进程内 EventBus，不把高频热路径全部写入消息总线。只有 Run/Step checkpoint、实际动作和恢复所需事实持久化。

所有可认领/扫描记录带服务端注入的 `execution_target`。缺失或非法 `AIDCP_DEPLOY_ENV` 时相关 worker 禁用；客户端、Agent、自然语言和 `envKey` 都不能指定 target。

### 22. 安全、隐私和提示注入边界

- 平台内容是非可信输入；不得把帖子、评论、私信中的文本解释成 Agent tool call 或 Automation Definition。
...
| 失败点 | Run/Step 结果 | 禁止行为 |
| --- | --- | --- |
| Trigger 重复 | 不创建重复 Run，记 `duplicate_trigger` | 重复执行 |
| Definition/能力未知 | `skipped(unsupported)` 或 `failed(contract_invalid)` | 近似命令回退 |
| Agent 输出非法 | `failed(invalid_plan_patch)` | 当自然语言继续执行 |
...
| 计划新版本到达 | 未派发可 supersede；已派发独立归并 | 原地改写旧意图 |

## Risks / Trade-offs

- **[状态对象变多，理解成本上升]** → API 对客户只投影“周期、当前工作、结果”三层；内部 Run/Step/Attempt 主要用于恢复和审计，不全部暴露。
- **[通用 Workflow 逐渐变成可编程平台]** → 步骤类型、触发类型和 Definition 发布均走 allowlist 与代码评审；拒绝任意脚本和无限循环。
...
- **[版本冻结让新配置生效不够“即时”]** → 安全控制即时生效；普通业务更新通过显式 supersede 创建新 Run，保留可解释性。
...
- 冻结 Definition/Run/Step/Attempt、事件信封、reason code 和 capability schema。
...
**回滚：** 按 Definition version 关闭新入口并恢复 legacy trigger；已 dispatch 的动作只能继续归并，不能回滚数据库状态伪装未发生。

### Phase 6：Managed Cycle 与全托管

- API 提供分动作授权、预算和紧急停止。
- 启用每日/活动 Cycle，先 read-only，再逐个开放互动、创作和发布动作域。
- 客户端展示 Cycle/Run/平台结果三层真相及 Decision Trace 摘要。

**回滚：** 暂停 Plan/Binding，阻止新 Cycle；当前未派发 Run 取消，已派发 Attempt 继续回执/对账。
...
1. 首版 Definition 是纯代码发布，还是允许受审 JSON 配置；无论哪种都不开放客户任意脚本。
...
5. Publish/Comment 各平台的 `latestStartAt` 默认值和 `missPolicy` 由产品配置还是 Definition 固定。
