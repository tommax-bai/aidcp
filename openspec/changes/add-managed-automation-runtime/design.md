...
**裁决状态（2026-07-25 更新）。** 下表的「裁决建议」列是盘点当时的建议，其中多数**已经落进规格正文**，不必重新裁决；只有三条真正悬空。接手前先看这一段，不要照着「裁决建议」重做已完成的事：

- **已落规格**：C3（安全点按有无平台副作用定义）、C4（recovery 优先级类可抢占）、C6（显式 unsupported 与解析失败分极性）、C8（审批保留 + 窗口内等 Edge）、C9（窗口过期只撤授权、内容可重批）、C11（逻辑 claim 与资源占用解耦）、C12（记账极性按账本类别声明）、C13（lane 只覆盖有平台副作用的工作）、C15（派发前知会属授权链、派发后通知 best-effort）、C16（消耗账本不按 target 分裂）、C17（lane 身份＝平台账号）。
- **已裁决但方向与下表建议相反**：C1、C2 的人工半条、C10 —— 用户裁定采本方案原样（无 override 档、无能力级审批下限），下表建议的「两者都要 / 补下限」**已作废**，两条被放弃的保证见 §24.4。
- **仍须裁决（仅此三条）**：**C5** 一条 `submitted_unknown` 可否长期占住账号 lane、以及占用上限；**C7** 协议字段滚动兼容 vs 能力版本锁步的分面边界（落 Phase 2 cutover 说明）；**C14** 运营手动指令是否属于「全托管」从而适用两段占用（落仲裁规格的 in-session 审批那条）。**C2 的自动化半条**（确定性命中可否跳过会话软预算与冷却）随 C5 一并判定——它越的是软闸，不随 C1 的硬闸裁决自动落定。
