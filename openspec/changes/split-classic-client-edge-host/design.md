...
**A 类 —— 真正命令运行中执行侧的非 lifecycle 操作，只有两个。** `browser:showDriven`（把被驱动的
浏览器显示到客户端下方，**请求-应答**，带 requestId 与 3 秒超时）与 `browser:resetParking`
（把浏览器还原到停放位，**单向无应答**）。另有一个内部触发的人设提示通知，同样单向。
...
**D 类 —— 完全绕过执行侧，客户端直连云端 HTTPS。** 删除环境、人设读取/生成/落库、稿件审批与
配图删除、互动收件箱全家族、内容工作区全家族、环境风险与慢启动读写。⇒ **归 Classic**，不进 Host
公开合同。
...
4. **Windows 代码签名证书买不买**？不买则接受「未知发布者」提示，并在下载页写明放行步骤；
   该选择只影响 Windows 安装体验，不影响本 change 的边界与验收口径。
