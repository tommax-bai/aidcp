...
## 2. aidcp-edge — 让位探针不许撒谎（抢占的前提）

- [ ] 2.1 Facebook 会话：让位返回值改为基于**在飞写者计数**（与浏览会话的引用计数同形），不能来自命令链的 promise（`facebook-session.ts:335-337`）
- [ ] 2.2 命令超时放行串行链时，把该执行体登记为**孤儿写者**；孤儿存在期间让位一律回「未静默、不可接管」（`:490-510`）

## 3. aidcp-edge — 清场协议（抢占的硬前置，不做就是把偶发雷改成常态雷）

- [ ] 3.1 **小红书填字段**：填写前先清空并回读确认为空（清不掉即诚实失败），填写后改为**全文比对 + 少量多余字符容差**，**废掉 8 字探针**（`publish-command-handlers.ts:559-607`）。移植 FB 侧已走通的做法
- [ ] 3.2 **小红书评论**：输入前清空编辑器；提交回执补**三态**——未派发提交 / 已派发未确认 / 已确认（`browse-session.ts:2284-2400`）
- [ ] 3.3 **FB 发帖**：清场路径已现成（`publish-executor.ts:343-357` 三态诚实回报），新增**关闭发帖弹层**（今天全仓没有）+ 回读确认真的关掉了
- [ ] 3.4 **离开确认框接管**：订阅页面对话框事件并接受（放弃编辑＝接受离开）；清场顺序改成**先清空编辑器 → 再导航**，让确认框根本没有触发条件（**真机项 B** 先证）
...
- [ ] 9.1 **今天的档位是按触发路径分的，不是按任务性质**：同一篇人审通过的稿，走人工入口＝人工档，走 60s 兜底扫描补投＝自动档 → **可抢/不可抢取决于它怎么被触发，且重投时档位反而降级**（教科书式的反 aging）
  - **推荐**：「人工批准的发布」归为**自动档**（它是异步队列作业，批准完人就走了）；真正的人工档只留「**运营在线等回执**」的动作（手动评论、手动加群、客户端内审批的即时动作）
  - **副作用**：手动评论会抢占发布——**这正是设计意图**，但发布被抢占＝整条序列重跑
  - 这一条改口径会顺带消灭「同一篇稿因触发路径不同而档位不同」
