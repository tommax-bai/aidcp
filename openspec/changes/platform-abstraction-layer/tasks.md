...
- [x] 2.1 Create edge `src/platform/driver.ts` with `PlatformDriver`, capability types, `PlatformId`, and honest unsupported-capability errors.
  <!-- aidcp-edge 0c93a83: added platform driver contract, capability guard, platform normalization, and recognized-but-unimplemented facebook failure. -->
- [x] 2.2 Extract existing xhs browse/comment/publish/interact/patrol page-specific logic into `src/xhs/*` while keeping shared CDP, locating, humanize, anti-detection, and browser-provider modules outside the xhs tree.
  <!-- aidcp-edge 0c93a83: xhs driver wraps existing identity and overlay implementations; shared CDP/browse modules remain in existing shared folders. -->
- [x] 2.3 Wire `AIDCP_PLATFORM` or equivalent startup config so unset/default selects `xiaohongshu` and unsupported platform values fail honestly.
  <!-- aidcp-edge 0c93a83: main.ts selects platform driver from AIDCP_PLATFORM; default/xhs aliases resolve to xiaohongshu; facebook is recognized but rejected until a driver exists. -->
- [x] 2.4 Add edge hello/platform metadata and startup logs without changing xhs behavior.
  <!-- aidcp-edge 0c93a83: hello payload includes platform; startup logs selected platform/app/capabilities; xhs app/capability values remain unchanged. -->
- [x] 2.5 Add focused edge tests for driver selection, xhs default behavior, unsupported capability failure, and shared-core non-duplication.
  <!-- aidcp-edge 0c93a83: test/platform/driver.test.ts plus hello/startUrl coverage in existing client/provider tests. -->
...
- [x] 3.1 Add account-store APIs for `getPlatform(accountId)` and `listByPlatform(platform)` with cache behavior aligned to existing account accessors.
  <!-- aidcp-cloud 8496379: accounts.platform DDL default xiaohongshu; init prewarms platform cache; getPlatform/listByPlatform added and tested. -->
- [x] 3.2 Upgrade comment platform profile lookup into a keyed registry with xhs defaults matching current prompts and limits.
  <!-- aidcp-cloud 8496379: XHS_COMMENT_PROFILE keeps current site/content labels, 50 char limit, search defaults, and targeted search limits. -->
- [x] 3.3 Add cloud `PLATFORM_REGISTRY` skeleton with xhs entry and capability/scheduler metadata, without changing current xhs orchestration.
  <!-- aidcp-cloud 8496379: registry contains xiaohongshu entry only; recognized facebook has no registry entry and fails honestly. -->
- [x] 3.4 Inject platform profile into command-style comment roles and scheduler paths while preserving current xhs output.
  <!-- aidcp-cloud 8496379: generator/picker/composer/scheduler accept platform profile; default xhs prompt still contains 50 字以内 and 最近一天·最多收藏. -->
- [x] 3.5 Validate edge platform metadata against `accounts.platform` before routing account work to an edge.
  <!-- aidcp-cloud 8496379: handshake rejects unsupported platform and platform_mismatch before creating/replacing runtime; mismatch does not evict an existing healthy same-edge connection. -->
...
- [x] 4.1 Run edge acceptance tests relevant to browse/comment/publish/protocol, then edge `npm test` and `npm run typecheck`.
  <!-- aidcp-edge: after rebase onto master, npm run typecheck passed; targeted tsx tests passed 50/50; npm run test:acceptance passed 13/13 with gated E2E skipped; npm test passed 641/641. -->
- [x] 4.2 Run cloud acceptance tests relevant to protocol/risk/comment/account routing, then cloud `npm test` and `npm run typecheck`.
  <!-- aidcp-cloud: after rebase onto master, npm run typecheck passed; targeted tsx tests passed 75/75; npm run test:acceptance passed 44/44 with gated E2E skipped; npm test passed 1391/1391. -->
