...
- **下发安全阀先行（Phase 1，独立上线）**：同账号两次真实发布之间强制最小间隔（锚定「上次下发尝试」时刻，等待期不占边缘不让位）；边缘离线导致的下发失败回待审 + 作废授权信号 + 通知重批（不再烧成 failed 终态）；同账号连续 2 次序列失败触发熔断停链 + 飞书告警，熔断清除接人工重批确认路径（含 already-decided 分支，防死锁）；兜底扫描改 fire-and-forget（一账号等待不拖死跨账号扫描）。**BREAKING**：`publish-pipeline` 的「审批通过即下发」语义改为「通过即进入该账号发布队列、按最小间隔出队」，相关 AC-PUB 断言按新契约重述（授权仍是发布的必要条件，绝不删除）。
...
- `publish-dispatch-pacing`: 下发段节奏与熔断——同账号最小发布间隔（锚定下发尝试时刻）、等待期诚实呈现（earliestAt）与醒后重核、离线失败回待审、连续失败熔断与人工清除、已知缺口登记（重启在途下发的 at-least-once 重复发帖窗口）。

### Modified Capabilities
- `publish-pipeline`: 「审批通过即下发」→「通过即进入账号发布队列、按最小间隔出队」；生成候审段从全局单跑改为按键并发（生成段仍不让位浏览不变）；下发段按账号单飞保持、失败路径分界（离线回待审 vs 序列失败 failed+熔断）。
