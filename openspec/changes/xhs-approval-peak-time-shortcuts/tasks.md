...
- [x] 5.3 Rebase/integrate through clean default checkouts, push current default branches, and deploy the runtime Cloud change to `dev` after target precheck and documented health checks; do not package an Edge installer.
  <!-- Integrated/pushed: aidcp main aa2ca73, aidcp-cloud master 1347891, aidcp-edge master 93c3017. Deployed Cloud master 1347891 to dev after `deploy-target dev --check`; backup `/opt/aidcp/cloud.bak.20260720-100047Z.tar.gz` + `.env.bak.20260720-100047Z`. Remote source SHA-256 matched local. Health: service active/NRestarts=0, 8787/8090/8091 listening, panel version + customer-auth direct/nginx health OK, PostgreSQL `select 1`, Feishu WS onReady, isales-api/isales-scheduler still active. Edge installer not built or installed. -->
