...
- [x] 1.1 Record the current `split-cloud-automation-production-runtime` dependency, Automation composition-root hotspots, branch/worktree baselines, and the exact gate that must close before production wiring. <!-- done: repo=aidcp sha=b30b63f6 evidence=references/current-state-inventory.md#1 validation="openspec strict valid; live split status 76/129; readiness ledger 11 blockers" deployment="n/a docs only" -->
- [x] 1.2 Inventory every managed-automation file and commit on the old `aidcp-cloud` feature branch; classify each as port unchanged, adapt to the independent process, replace, or drop, with a named reason. <!-- done: repo=aidcp sha=b30b63f6 evidence=references/current-state-inventory.md#2 validation="11 commits and all 63 diff files classified; automated missing-file comparison empty" deployment="n/a docs only" -->
- [x] 1.3 Inventory legacy per-account scheduling/dispatch entrypoints and in-flight evidence in `aidcp-automation`; define the complete lane-read and legacy-in-flight adapter surface before changing behavior. <!-- done: repo=aidcp sha=b30b63f6 evidence=references/current-state-inventory.md#3 validation="producer/dispatch census plus fail-safe LegacyAccountWorkPort frozen" deployment="n/a docs only" deviation="batch-G content scheduler is not yet wired; absence is explicitly unknown/deny" -->
- [x] 1.4 Freeze the phase-one owner map, current Automation migration ledger maximum, table manifest changes, cross-process DTO locations, and default-disabled rollout flags. <!-- done: repo=aidcp sha=b30b63f6 evidence=references/current-state-inventory.md#4 validation="current shared max 0105; proposed 0106-0109 allocation and four exact-false-default flags recorded" deployment="n/a docs only" -->
