...
## 8. Cross-platform desktop artifact injection regression

- [x] 8.1 Reproduce the Facebook development-client startup failure and add a shell-level regression test proving every browser platform that requires Native receives the verified architecture-matched artifact path before the core is spawned. <!-- observed npm run electron:dev Facebook core exit before handshake: AIDCP_NATIVE_PAGE_ENGINE_BINARY required; new lifecycle contract failed 9/10 against the Xiaohongshu-only shell condition, then passed in the 15/15 focused Native/Electron suite -->
- [x] 8.2 Replace the Xiaohongshu-only desktop injection condition with the platform runtime contract, preserving fail-closed manifest/digest/architecture verification and the WeChat API-only no-browser path. <!-- Edge shell now verifies/injects the artifact for XHS/Facebook before spawn and injects it for WeChat whenever packaged or staged; direct artifact-less WeChat development may still run API-only without starting Native -->
- [x] 8.3 Run focused Electron/runtime tests, typecheck, desktop build-input verification, and a bounded development-client startup probe; record source-only versus installer/real-account validation honestly, then integrate Edge/control through clean default branches. <!-- edge=9c4e609, fast-forwarded and pushed master; focused Electron/Native 21/21, full Edge 2285/2285, typecheck/build:dist/verify:desktop-build-input pass. Browser-absent Facebook core probe reached control identity with the verified arm64 artifact and did not emit the missing-binary failure; it then failed only against the intentionally unavailable loopback Cloud. No installer/signing/notarization or real-account post-fix run was performed; the already-running development shell must restart before an operator retry can consume the new host code. -->
