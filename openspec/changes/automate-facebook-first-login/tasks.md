...
- [x] 3.5 Keep the retained `manual_login_required` session eligible to serially re-enter the existing Facebook auth coordinator before subsequent identity polls, without adding a parallel consumer or transferring fresh-start proof.
  <!-- Edge bdd93fa adds one serialized pre-identity coordination hook. The retained session reuses the existing Native runtime, TOTP broker, lifecycle interrupt tracker, and original fresh-start proof; it creates no timer or parallel watcher. -->
- [x] 3.6 Add lifecycle tests for login-page manual waiting followed by 2FA consumption, unchanged-login no-action cadence, cancellation, stable-identity completion, and unproven-policy refusal.
  <!-- Edge bdd93fa proves manual-to-2FA re-entry reaches authenticated with one entry and one submit, unchanged manual pages dispatch zero actions, interruption wins before coordination, stable identity completes after serial preflight, and unproven mutation fails before identity read. -->
...
- [x] 4.6 Run focused coordinator/startup/manual-session tests, Edge typecheck and relevant acceptance/Native gates, strict OpenSpec validation, then integrate and push the source-only follow-up without mutating live profiles or packaging.
  <!-- Edge bdd93fa is pushed to master. Validation: focused coordinator/startup/manual-session tests 46/46, typecheck, acceptance 31/31, Native staged-artifact verification, and the full Native cargo gate exit 0 (167 unit tests plus all integrations). Strict OpenSpec validation passed. No live profile was probed or mutated, and no desktop package was built, installed, or released. -->
