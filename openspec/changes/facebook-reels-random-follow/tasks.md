...
- [x] 5.3 Fetch/rebase onto latest defaults, rerun required gates, fast-forward push Cloud/Edge/control default branches, deploy Cloud from a clean eligible default checkout to `dev`, and verify service/listeners/health/Feishu/PostgreSQL without touching `isales`.
  <!-- All feature branches rebased cleanly, acceptance/typecheck/strict gates reran green, and default branches were fast-forward pushed: cloud master=b0135d3, edge master=ca6df3b, control main=191fb22. Cloud was deployed from clean dev-eligible master to target dev on 2026-07-22; rollback artifacts are /opt/aidcp/cloud.bak.20260722-114010.tar.gz and /opt/aidcp/cloud/.env.bak.20260722-114010. Six deployed source hashes matched local master before restart. aidcp-cloud.service restarted at 11:41:15 CST with NRestarts=0; 8787/8090/8088/5432 listened, /api/health returned ok, PostgreSQL SELECT 1 passed, Feishu WSClient reached onReady, and no post-start error entries appeared. Four pre-existing isales services remained active and were not restarted or modified. Edge source is landed but no installer was built or installed, so existing installed clients remain version-gated until updated. -->
