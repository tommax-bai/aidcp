...
- **依赖 / 跨 change 协调（非本 change 范围，须转达 `account-identity-from-login` 负责人）**：一台机器开多个 app 实例的**多开隔离**（每实例独立账号/端口/profile + 去静默接管）整条归 `account-identity-from-login`（它已在重定义节点槽位分配=每节点独立端口 + 用户数据目录 `node-<n>`、身份从登录态读出）。**须特别纳入其范围的一条缝**：Electron 自带的独立启动器 `src/electron/chrome-launcher.cjs` 端口写死 `9222`（:6）、profile 共用 `userData/chrome-profile`（:31-32）、且「端口上已有 Chrome 就盲目复用」（:58-60）→ 多开第二个 app 会**静默接管第一个账号的浏览器**（撞「静默接管」红线）；该 `.cjs` 走 Electron 路径，可能不在 `account-identity-from-login` 的 `main.ts`/`launch-multinode.ts`/`src/cdp` 射程内，是一条可能掉在两个 change 中间的缝。
