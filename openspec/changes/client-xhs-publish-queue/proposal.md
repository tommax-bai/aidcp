...
- 参照完整发布队列页面重构首页待发布卡的信息层级：去除无真实内容的装饰封面，改用队列式状态徽标、标题、阶段状态与原生主次操作；外层摘要、内层任务卡和左右切换形成清晰层级，并在窄窗口保持可读、无横向溢出。
- 调整运行首页信息优先级：将“今日进展”完整卡片放在内容发布卡片之前，使用户先看到当天运营成果与运行控制，再查看具体发布任务；只改变页面顺序，不改变两张卡的数据来源、状态语义或交互。
