...
“发布确认”避免“你来确认”在已完成后仍像一句当前指令，也避免“发布审批”带来的内部流程和审核权力感。该阶段在 `waiting_human` 时显示“待你确认”，在 `completed` 时显示“已确认”；发布结果尚未开始时显示“等待发布”。这些只是对 Cloud 明确状态的客户化措辞，不改变完成判定。
...
四阶段步骤条在宽屏采用圆点居中、文案置于圆点下方的横向轨道；每段连接线从当前圆点外缘结束于下一圆点外缘，由节点遮罩保证不穿圆点，第一项之前和最后一项之后没有线。窄屏改为左侧圆点、右侧文案的纵向轨道，线段同样只存在于相邻节点之间。步骤项是只读状态，不增加 hover 或点击暗示；圆点、文字与状态色不是唯一的信息来源。

### 7. 首页发布卡使用本地只读切换，不改写队列

展开态首页卡从同一份当前环境 Cloud 队列构造只读切换序列：先放全部待确认 active，再放其它 active，最后放尚未开跑的 tasks；只有没有进行中内容时才展示 recent。每项以 `active / task / recent + id` 形成稳定身份，刷新后只要该身份仍存在就保持当前项；该项消失时回到新的首项。切换环境、平台或回到非队列形态时立即清除选择，绝不跨环境复用索引。

序列超过一项时，在卡片最左和最右各显示一个原生 `button`。按钮使用循环切换，避免边界上突然失效；默认颜色和背景保持弱化，hover 与 `focus-visible` 时只做轻度加深，并提供“上一条/下一条 + 目标标题”的可访问名称。卡片右上角显示 `当前位置 / 总数`，标题变化通过 polite live region 被读出。单项、加载、错误或收起态隐藏并禁用按钮，隐藏中的按钮不得继续占据键盘焦点。按钮只切换本地投影，不发 HTTP 写、不改变任务优先级，也不把列表位置描述成精确队列名次。
