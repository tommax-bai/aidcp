...
### 8. 首页待发布卡复用队列页的信息层级，而不是放大旧单稿卡

XHS 队列摘要使用“外层摘要 + 内层当前任务卡”两层结构。外层左侧固定显示“发布进度”与当前项所属分区（需要确认、系统处理中、等待开始或最近结果），右侧用小型数量徽标和位置提示表达全局待处理数与当前页；数量徽标不冒充精确队列名次。内层任务卡复用完整队列的白底、细边框、状态徽标、标题和紧凑阶段轨道语言，不再展示没有真实封面的蓝色图片占位。

当前任务卡 SHALL 展示当前项的客户状态；active/recent 有阶段证据时同时展示四个阶段的标签与状态文字，task 没有阶段证据时统一显示“未开始”，不得用装饰性完成态填充。完整队列入口使用次级原生按钮；稿件审核使用主按钮，且只在当前轮播项本身为 `waiting_approval` 且审核入口可用时出现。按钮由独立操作组承载，说明文字不再与链接挤在同一基线上。

宽屏阶段轨道保持横向四列并置于轻量内嵌面板；窄窗口恢复队列页同款纵向轨道，状态文字允许换行，主次操作并排或等宽排列，卡片和文案不得横向溢出。左右切换继续占据外层卡片边缘，不覆盖任务标题、阶段文字或操作按钮。加载、错误、空态和非 XHS 旧发布卡保留诚实语义；切出 XHS 队列摘要时必须移除该视觉模式，避免样式串到其它平台。

### 9. 今日进展在 DOM 与视觉顺序上均先于内容发布

运行首页在运行引导、内核准备与异常提示之后，先渲染完整 `daily-summary`，再渲染 `pub-card`，最后进入“今天做了这些”活动流。顺序通过移动完整语义区块实现，不使用 CSS `order` 只改变视觉位置；因此屏幕阅读器阅读顺序、键盘焦点顺序与用户看到的上下顺序一致。两张卡的 id、内部静态节点、事件绑定、折叠状态和环境级 HTTP 真态均保持不变。
