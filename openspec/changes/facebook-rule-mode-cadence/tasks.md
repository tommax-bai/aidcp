...
- [ ] 8.5 Fetch and rebase each implementation worktree onto the latest default, resolve single-writer hotspots serially, rerun required validation, and fast-forward integrate with explicit path scopes.
- [ ] 8.6 Commit and push the validated default branches, run deployment preflight, deploy runtime Cloud changes to `dev`, and verify service, listener, health, database, and behavior evidence with rollback on failure.
...
<!-- 8.4: OpenSpec strict validation PASS in aidcp control worktree. DEV deployment remains pending 8.6, so its result will be appended after integration. -->
