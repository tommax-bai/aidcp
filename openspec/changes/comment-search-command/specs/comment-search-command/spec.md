...
系统 SHALL 新增一个独立角色，从账号**人设兴趣** + **精选集**（`curated_content` 高收藏笔记标题/主题，按账号）生成一小批搜索词。精选集稀疏/为空时 MUST 退回人设种子词（`seed_keywords`），MUST NOT 编造与账号领域无关或凭空臆造的词。LLM 降级/解析失败时 MUST 诚实少出或退回种子词，MUST NOT 阻塞任务。
...
### Requirement: 候选先去重（未评论过）再人设相关性择优一篇

拿到候选卡片后，系统 MUST **先**滤掉本账号**已评论过**的笔记（复用每笔记已交互去重，`action='comment'`，按账号），**再**由新增的甄选角色在剩余候选里判定**人设相关性**并挑出**最相关且收藏最高的一篇**。去重 MUST 在甄选之前。无合格候选（全被去重或全不相关）时 MUST 诚实结束任务，MUST NOT 强行评一篇不相关或已评过的笔记。每次任务只评一篇。

#### Scenario: 去重在择优之前
- **WHEN** 候选含本账号此前已评论过的笔记
- **THEN** 这些笔记 MUST 在交甄选角色之前被滤掉，不参与择优、不耗模型判定

#### Scenario: 在相关候选里挑收藏最高的一篇
- **WHEN** 去重后仍有候选
- **THEN** 甄选角色 MUST 判人设相关性，并在相关候选里挑收藏最高的一篇作为评论目标

#### Scenario: 无合格候选 → 诚实结束
- **WHEN** 候选全被去重或甄选判定全不相关
- **THEN** MUST 诚实结束任务并回执，MUST NOT 强评不相关/已评过的笔记

#### Scenario: 红线反例——评已评过或不相关的笔记（禁止）
- **WHEN** 有实现跳过去重、或在无相关候选时仍随便挑一篇评论
- **THEN** MUST 视为违规、不予合入；MUST 先去重、且只评人设相关候选
