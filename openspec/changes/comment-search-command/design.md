...
- **角色②·搜索笔记甄选**：输入「去重后的候选卡片（带收藏数）+ 人设」；输出**相关性判定 + 在相关候选里收藏最高的一篇**。现有 `ContentEvaluator` 为自治会话设计、按标题匹配、且 `collectCount` 现恒 0，不复用。
...
- **搜索笔记甄选**：给「账号领域 + 候选卡片列表（标题/作者/收藏数，已去重）」→ 严格 JSON `{pickIndex: number|null, relevantIndexes: number[], reason}`；只在相关候选里挑收藏最高者；无相关候选 → `pickIndex=null`（诚实结束，不强评不相关笔记）。
...
- 搜索词条数 / 每词取多少结果 / 候选池大小的缺省值 → 实现期定，可配。
- 角色②甄选时相关性卡多严（宁缺毋滥 vs 尽量有一篇）→ 真机标定。
