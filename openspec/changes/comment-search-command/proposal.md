...
- **【新角色 ②·搜索笔记甄选】** 在「去重后」的候选里判断**人设相关性**并挑出**最相关且收藏最高的一篇**。
- **【搜索结果原生筛选】** 边端搜索后**驱动小红书搜索页原生「最多收藏」排序 +「一天内」时间筛选**再采卡片（现状只输入关键词+回车、不点排序/时间控件），并**回传每卡真实收藏数**（现状卡片收藏数恒为 0）。协议按 v2 四处同步扩展（搜索指令加排序/时间参数、结果卡片加收藏数）。
...
- **【门槛取舍】** 命令路径**跳过**自动 `CommentAppraiser` 的硬数值阈值（用户已手动指定意图，改由角色②相关性把关），但**保留**飞书人工审核闸、**仍计入**按账号按天评论配额与风控 `canDo('comment')`。
...
### New Capabilities
- `comment-search-command`: 飞书 `/comment` 命令触发的按需、搜索驱动评论任务——含命令与受控编排、两个新角色（搜索词生成 / 搜索笔记甄选）、十步流程（搜索词 → 原生「最近一天+最多收藏」筛选 → 去重 → 人设相关性择优 → 读正文与现场评论 → 撰写 → 人工审核 → 发布 → 记账去重 → 恢复浏览）、搜索结果原生筛选与收藏数采集的协议增量、以及门槛/审核/配额/账号隔离/诚实红线。
...
- **aidcp-edge（新活，主要不确定性）**
  ...
  - `src/browse/feed-scroller.ts` 卡片扫描 + `browse-session.ts` `reportVisibleCards`：**采每卡收藏数**（现状硬编码 0）。
