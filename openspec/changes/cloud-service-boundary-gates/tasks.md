...
<!-- aidcp-cloud 071252c 花名册 = §4.7 的 4 文件，protocol.ts 已删除并进 boundaries/kernel-non-members.json 拒入清单（AC-BOUND-03 断言拒入清单内文件不得标 kernel）。两处待裁决项一次判定：event-bus/types.ts 与 platform/registry.ts **均不析出、整体维持 automation**，理由三条写在 kernel-non-members.json。§4.7 回写属控制仓文档改动，已写成 docpatch P1 交主控套用 -->
...
<!-- BLOCKED：本条要求「写进控制仓」，属控制仓法条改动，本 session 不直接改控制仓 docs。已按首批实测（257 条 import + 10 条表写入）拟出具体节奏并写成 docpatch P15 交主控套用：每批清账 frozenTotal 至少降 12 条（≈seed 的 5%），role-dispatcher 那一簇（重算后 4 条）与表写入侧 10 条各自单列不计入常规配额。 -->
...
<!-- BLOCKED: 控制仓文档改动，编辑已备好（docpatch P8），待主控套用 -->
- [ ] 6.2 改写 §12 阶段 1 的条目顺序：把「模块导入和数据所有权检查」提到**首位**，并加一句「本项 MUST 先于任何边界重构落地」。
<!-- BLOCKED: 定稿现状已满足本条（§12 阶段 1 开头已写「本阶段任务 MUST 按以下顺序执行，门禁先行：1. 先落地 AC-BOUND-* 与 AC-OWN-* 两族门禁…MUST NOT 排在本阶段末尾」），**不产生编辑**；登记为「已满足、无需改动」见 docpatch P9，由主控确认后勾选 -->
...
<!-- BLOCKED: 控制仓文档改动，编辑已备好（docpatch P10），待主控套用。阈值已按实测定出：involvingContent==0、import frozenTotal<=145（=257-112）、table frozenTotal<=10 且不含 content 侧条目。取值来源是门禁每次运行打印的 AC-BOUND metrics 行 -->
...
<!-- BLOCKED: 控制仓文档改动，编辑已备好（docpatch P11：尾部追加红线 31 / AC-DECOMP-31，遵守「1–18 冻结、新增只在尾部追加、AC-DECOMP-* 一旦分配不复用」的编号纪律；另给红线 6 加旁注），待主控套用 -->
...
<!-- BLOCKED: 依赖未满足——§17 第 1 项那个控制仓 change 尚未立项、更未合入，而本 change 的门禁代码已就绪。CLAUDE.md §7 未被本 session 触碰（符合 MUST NOT）。另有一处口径需改：该 change 列的 `aidcp-cloud/src/kernel/**` 路径**不存在**，本 change 按 §4.7 判定不建 src/kernel/ 目录（见 2.6）；应改为列 kernel 花名册的四个现有路径。已写成 docpatch P14 -->
- [ ] 6.6 在 `docs/cloud-service-decomposition-review.md` 的「如果只做三件事」第三件旁标注本 change 名，便于后续对账。
<!-- BLOCKED: 控制仓文档改动，编辑已备好（docpatch P12：标注承接 change 名，并把评审期的「215 条 / 79 里 69 归零 / 43 条集中在调度器」三个已作废数字改为实测的 257 + 10），待主控套用 -->
