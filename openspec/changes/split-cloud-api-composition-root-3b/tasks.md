...
- [ ] 7.3 Run `openspec validate split-cloud-api-composition-root-3b --strict`, `git diff --check`, final managed-member/pin/migration split-sync checks with reported hand-written-root residuals preserved, and confirm every canonical checkout is on its default branch with unrelated changes preserved.
