...
- [ ] 8.4 两侧并发写触发 409 而非互相覆盖
  <!-- 2026-08-06 **两条都已具备执行条件、方案已就绪，只差一次工具层放行**（写操作被 Claude Code
       权限分类器连续拦下，读操作放行）。**这不是「验不了」，别读成技术阻塞。**
       就绪的做法（下一个人照着跑即可，无需重新设计）：
       两台机各自本地登录面板 API，**同时**各 PUT 一格、且各自用自己缓存里的 revision（当时都是 7）——
       这就是真并发的形态，不需要制造毫秒级同时。
       建议 dev 改 `reels.slowStart.viewsPerFollow`、OL 改 `reels.persona.viewsPerFollow`，
       **两侧刻意改不同的格**：这样一胜一负之后，从「输的那一格有没有落地」就能直接判定
       是不是互相覆盖，而不只是看返回码。
       预期：一侧 200 + revision 递增，另一侧 **409 `revision_conflict` 并回带最新投影**，
       且输的那一格在库里**一字未动**（4.4 的单测已断言这一点，本条要的是真机上同样成立）。
       然后轮询另一侧 GET 直到看到新 revision，记下耗时 = 8.3 的读数；
       最后把改动的格改回原值（会再消耗一个 revision，正常）。
       **注意：这次写会同时改变 dev 与 OL 的真实运营配置**——合并之后本来就没有"只影响一侧"的写法，
       这正是本 change 的目的。用户已在本轮口头授权执行。
       脚本形态见本轮 scratchpad `panel-op.js`（登录 / GET / PUT / 轮询四合一，参数走环境变量）。 -->
