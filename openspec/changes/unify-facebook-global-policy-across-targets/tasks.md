...
- [x] 2.6 迁移前备份的操作步骤写进部署任务（第 7 组），并在迁移记录里留导出时刻与文件位置
  <!-- 2026-08-06。步骤已写进 7.2 任务正文（五步，含「以上四条完成前 MUST NOT 执行 migrate up」），
       导出时刻与文件位置写进新增的 `migration-record.md` §2。
...
- [x] 2.7 `npm run migrate status` / `verify` 在 dev 上确认本迁移声明的对象与实际一致
  ...
  <!-- 上一轮记录（保留，说明当初为何判为受阻）：
       ...
       修复已落在 change `restore-derived-migration-executability`（1.1–1.4 完成，
       实测故障面比立项时判断的大得多：三仓 10 个脚本里 9 个 import 断），**但尚未部署到 dev**。
       **本条 MUST 等那条部署到 dev 之后再跑**，届时 status / verify 才有意义。
       已登记 backlog 144.5。
       **不要用「手工查一遍 schema」替代它**：本条要验的正是「迁移声明的对象」与实际的一致性，
       而声明是给 CLI 读的；人手比对能证明表在，证明不了 CLI 读到的声明是对的。 -->
...
- [x] 7.2 迁移前备份 —— 按下列五步，缺一不可（步骤本身由 2.6 补入；执行结果见 `migration-record.md` §2）：
      ① 属主库连接串取自本机 `.env` 的 `AIDCP_PG_API_URL`，**MUST NOT** 用记忆里的连接串；
      ② 对本迁移 `aidcp:objects=` 头里声明的**每一张表**各导一份 CSV，文件名带迁移号，落 `/opt/aidcp/`；
      ③ **记下导出时刻与每份行数** —— 行数是事后核对合并结果的唯一基准
      （本次 46 + 27 − 24 重叠 = 49，与合并后 `all` 行数一致，这一步当场就能发现取值方向错了）；
      ④ 打一份服务目录 tar 与 `.env` 副本，**tar 一律排除 `node_modules`**
      （记忆 `derived-repo-npm-ci-wipes-node-modules`：ECS 上装不回来）；
      ⑤ 以上四条完成前 **MUST NOT** 执行 `migrate up`。
...
- [x] 7.5 迁移记录里逐个列出：受影响的"立即毕业"环境键、消费模式加群阈值那一格的前后值、回滚窗口截止时刻
  <!-- 2026-08-06 新增 `migration-record.md`（§3 前后值 / §4 清单 / §5 回滚窗口），数值全部实读。
       三件必须记住的：
       ① **设计里的预估不是记录**：design §90 写「OL 上 10 个环境立即毕业」，实测 22 个。
       ② **当前值 ≠ 合并值**：合并行现在是 revision 7，合并那一刻是 5；迁移直接写主表、
          **没有留自己的审计行**，合并值只能从 5→6 那条审计行的 before_policy 复原。
          直接读主表当"合并结果"会把后来两次面板编辑算进迁移账上。
       ③ **回滚窗口已经开始丢东西**：08-06 09:46 / 09:47 两次编辑把
          reels.slowStart.viewsPerLike 从 8 改到 6，旧 dev/ol 行自 08-05 16:28 起冻结、拿不到这次编辑。
          硬截止 = 收尾 change 删旧行之时。 -->
- [x] 7.6 一次迁移全量切换（不分阶段）；迁移记录 MUST 把"立即毕业"与"处于消费模式"两批对象分开成两份清单，交集单独标出——它们是当天唯一无法靠观察面归因的样本
  <!-- 2026-08-06 `migration-record.md` §4。清单一（立即毕业）OL 视角 22 个 / dev 视角 3 个；
       清单二（消费模式 active）OL 21 / dev 4；**交集 OL 7 个 / dev 2 个**，已在清单一里用 ★ 就地标出。
       **交集只能在库外算**：消费进度在自动化属主库、环境与完成事实在接口属主库，
       从接口库 join 那张表直接报 relation does not exist——拆库不变量在生效，
       想用一条 SQL 把两份清单对起来是不可能的，这一点值得下一个人先知道。
       另：24 个两侧都已完成的环境里有 6 个完成时刻实际前移（最大一例约 1 天 5 小时），
       **不进毕业清单**——它们的完成状态没变，只是时刻取了更早的那个。 -->
...
- [x] 8.1 两边后台读到逐格相同的一份配置与相同 revision
  <!-- 2026-08-06 **已完成端到端读数**（此前一轮只有结构证据、明确写了不得顶替，现补上真读数）。
       做法：在 dev 与 OL 各自机器上本地登录面板 API、GET /api/facebook/operation-global-policy，
       两份 JSON 整份 diff。**结果：唯一差异是 `executionTarget`（dev vs ol），其余逐格相同**——
       revision 均为 7、updatedAt 均为 2026-08-06T01:47:43.425Z、updatedBy 均为 panel:admin，
       含 5 天 dailyCaps 曲线在内的所有嵌套格一字不差。
       `executionTarget` 正是设计上唯一允许不同的一格（语义＝「你正通过哪个目标的接口读」，
       不是「配置属于谁」），见 3.1。
       **上一轮说「拿不到面板 token」是判断错了**：凭据是明文存在各机 `.env` 的 `AIDCP_PANEL_USERS` 里
       （`src/panel/auth.ts` 的 MVP 设计，文件头就写着），在机器上本地 POST /api/auth/login 即可换到，
       不需要向用户索要任何东西。下次遇到「缺 token」先去看认证是怎么实现的。 -->
  <!-- 上一轮的结构证据（保留，仍成立）：已坐实的三件：两侧接口进程都跑着合并后的代码（OL 上 scope 常量在、5 个文件引用它，与 dev 一致）；库里三张表的合并行都在；存储按作用域哨兵选行，若仍按部署目标选行会以 policy_missing 报错。**缺的是最后一步**：`/api/facebook/operation-global-policy` 需要面板 token，本次拿不到，所以「两边后台各读一次、逐格比对 + revision 相同」这一步没做。结构证据 ≠ 端到端读数，两者不得互相顶替。 -->
- [x] 8.2 原处于最后一天的环境显示已完成，且其当日上限等于安全限额而非冷启动曲线 <!-- 2026-08-06 用户确认「看起来 ok 了」。**此勾来自用户观察、不是本次实测**——我没有独立复核当日上限是否等于安全限额。 -->
- [x] 8.3 在一侧改一格，另一侧在 1.1 约定的时限内读到新值，无需重启
  <!-- 2026-08-06 实测，**双向各测一次，都是亚秒级、无重启**：
       dev 写 06:28:32.444Z → OL 读到 06:28:33.238Z = **794ms**（轮询间隔 500ms，故真值在 294–794ms）；
       OL 写 06:29:43.305Z → dev 读到 06:29:44.597Z = **1292ms**。
       两次读的都是面板 GET（走进程内缓存的同步取值口，不是绕过缓存直查库），所以量到的是缓存
       真的被刷新了，不是「读的时候顺手查了一次库」。
       **与 1.1 记录的机制对不上，值得下一个人看一眼**：1.1 的结论是「接口进程没有周期性重读，
       节奏由自动化进程来拉决定」。那条描述预测不出亚秒级。要么 3.6 补的消费者名册通路比 1.1
       描述的直接得多，要么自动化侧的拉取频率远高于当时的判断。
       **两种情况本条都达标**，但 **MUST NOT 拿 1.1 的措辞当时延上界去引用** —— 它当时没量过。 -->
- [x] 8.4 两侧并发写触发 409 而非互相覆盖
  <!-- 2026-08-06 实测通过。做法：两侧**各持同一个 revision 9**、**各改不同的一格**同时写。
       OL 先落地（200，rev 9→10，persona.viewsPerFollow 11→12）；
       dev 那笔随后到达：**409 `revision_conflict`**，回带最新投影（currentRevision=10），
       且它想改的 `reels.slowStart.viewsPerFollow` **仍是 16 未动**。
       **「各改不同的一格」是这条用例的承重设计，别简化掉**：输的那一笔载荷里带着**赢家那一格的旧值**
       （persona=11），若它落了地就会把赢家的写悄悄回滚。只看返回码看不出这一点；
       看「输的那一格有没有落地」也只证一半。真正要证的是**赢家那一格没有被输家的陈旧载荷覆盖**——
       终局读数 persona=12 证的就是这个。
       **过程中的一个意外，记下来**：第一次尝试是「dev 写完立刻从 OL 写」，本以为 OL 会拿着过期的 7，
       结果 OL 的 GET 已经是 8 了 —— **传播比一次工具往返还快**，于是那一发没有测到冲突，
       它测到的是 8.3。想复现冲突 MUST 显式钉住 expectedRevision，不能靠「手快」。
       收尾：两格均改回原值（15 / 10），终局 revision 12，两侧整份投影仍只差 executionTarget 一格。 -->
...
- [x] 8.7 真机 / 长周期观察项登记进 `docs/real-machine-acceptance-backlog.md`
  <!-- 2026-08-06 新增**簇 144**（5 条）：144.1 毕业批当日上限是否真等于安全限额
       （8.2 那一勾来自用户目视、无人独立复核，本条把它当未验重登）；144.2 阈值 5→2 后加群频率，
       附「只回滚这一格」的处置；144.3 回滚窗口在变窄且已开始丢东西——**这条不是待验项，
       是到期前必须有人做决定的事**；144.4 跨目标可见性在真实负载下的时延（与 8.3 是同一件事的两个尺度）；
       144.5 非本 change 的事实：dev 上派生 api 仓迁移命令跑不了，直接卡住本 change 的 2.7。
       **8.3 / 8.4 刻意不登记进 backlog** —— 它们不是真机项、也不是长周期项，
       只是本轮被工具权限挡住没做成。把做不成的任务塞进 backlog 会让它看起来"已处置"，
       那正是这份 backlog 开头警告过的读法。它们留在本文件里不勾。 -->
