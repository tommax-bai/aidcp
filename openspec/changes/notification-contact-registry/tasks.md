...
- [ ] 8.1 ECS 跑迁移 0016；cloud 按 §5 安全序列部署（备份→rsync→restart→healthcheck，grep 新码确认生效）
- [ ] 8.2 console 构建 + 部署到 8088（与 isales 隔离）
- [ ] 8.3 真机校准：点赞/关注两栏 DOM 行结构 + 主页ID 解析校准（评论栏已校准、这两栏 best-effort 待真机 dump 收口）；验「同人同篇两评论 = 两行事件」
