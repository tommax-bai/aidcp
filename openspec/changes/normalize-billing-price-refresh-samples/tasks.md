...
- [x] 1.2 Add deterministic provider-specific billing model matching for Volcengine runtime ids versus billing labels, while preserving exact matching and honest `no_billing_sample` behavior. <!-- aidcp-cloud worktree: provider-specific Volcengine alias matching added; exact matching preserved -->
- [x] 1.3 Add focused cloud tests covering Volcengine `doubao-seed-2-0-pro-260215` / `doubao-seed-character-260628` billing rows and absent DashScope samples. <!-- aidcp-cloud worktree: npx tsx --test test/billing-price-refresh.test.ts passed (6 tests) -->
...
- [x] 3.1 Run relevant `aidcp-cloud` tests/build for billing refresh changes. <!-- aidcp-cloud worktree: npx tsx --test test/billing-price-refresh.test.ts passed (6); npm test passed (1346); npm run build passed -->
...
- [x] 4.1 Commit and push `aidcp-cloud` changes to the default branch after fast-forward integration. <!-- aidcp-cloud a6c0695 pushed to master; validation: npx tsx --test test/billing-price-refresh.test.ts, npm test, npm run build -->
...
- [ ] 4.3 Commit and push control-repo OpenSpec artifacts. <!-- fill with commit SHA / validation note -->
- [ ] 4.4 Deploy cloud and publish console, then verify the production refresh outcome no longer hides skipped reasons and Volcengine samples can write when billing rows are present. <!-- fill with deployment / verification note -->
