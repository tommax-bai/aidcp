...
- 最终正文回读保留中文、英文等 Unicode 字母和数字，忽略 URL、emoji、标点、空白/换行与 DOM 结构；允许不影响阅读的个别文字偏差达到 90% 相似度后放行。
- 用可复现 selection 回退的 fake 覆盖多段、连续空行与尾字倒序积累回归。

**Non-Goals:**

- 不修改 Cloud 指令编排、协议、审批内容、标题/话题输入或小红书发布元数据。
- 不以固定延迟替代状态确认，也不构建桌面安装包。
- 不把 90% 保底用于标题、Enter 后段落/光标稳定确认、话题附件或提交成功确认。
...
### 5. 最终正文按语义文字做 90% 保底验收

最终回读仍使用 `innerText`，天然不比较 HTML/DOM 标签。输入值与回读值先做 Unicode NFKC 归一，再移除 `http(s)://` / `www.` URL，最后只保留 Unicode 字母（`\p{L}`）与数字（`\p{N}`）；因此中文、英文模型名和数字参数参与验收，空白/换行、标点、emoji、Markdown 定界符和 URL 不参与。

相似度定义为 `1 - levenshteinDistance(expected, actual) / max(expected.length, actual.length)`，按 Unicode code point 计算，具名阈值固定为 `0.90`，`>= 0.90` 放行、低于阈值失败。对称分母同时惩罚缺字、错字和多字；若期望语义文字为空，则回退既有归一化精确子串校验，避免“空投影恒成功”。该保底只用于整篇正文完成后的最终回读，不替代每次 Enter 后的段落数、已写前缀与 caret 稳定检查。
...
- [Enter 后过程前缀仍沿用 Hanzi 优先口径] → 该闸只负责确认已写段落没有回退或丢失，最终整篇验收会用包含英文和数字的语义文字相似度兜底；两层职责不混用。
- [90% 阈值允许最多约一成语义字符偏差] → 阈值是用户明确选择的阅读容差；算法保留字符顺序并对缺失、替换和额外文字对称计分，低于阈值仍清场失败，标题与输入过程结构闸不放宽。
