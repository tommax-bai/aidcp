...
- [x] 7.4 真机 E2E：飞书 `/publish [accountId]` 触发，验多图真出图（并行、wall-clock≈最慢单张）、M/K 记账、封面=成功序列首张、真帖多图上账号；edge"一命令一图"多图上传通道复用正确（edge 零改动） <!-- 2026-07-01 真机通过（record id=25，account 66cd1d4f）：选题→ImagePromptComposer 3 次并行→ImageGenerator 30.6s 并行出 3 张→3 张真上传小红书 status=published、n_images=3、**images_attached_count=3（真实 K）**、封面=首张、edge=edge-selftest（self provider 复用本机 9222 Chrome、edge 零改动）。幂等验证：审批事件+兜底扫描双触发→「已在下发途中忽略」不重复发。附带确认两条非多图既有 edge 瑕疵（不阻断、另开 change）：capture_postId no_target（已提交后抓 id 失败→非致命，不误判 failed）、话题/选项 guard_persist（best-effort 跳过）。 -->


## 8. 归档

- [x] 8.1 全 task 绿 + `validate --strict` 通过 → `openspec archive publish-multi-image`（delta 合并进 `openspec/specs/`）
