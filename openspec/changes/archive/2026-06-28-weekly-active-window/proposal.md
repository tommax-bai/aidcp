...
- **窗口唤醒**：被上述任一闸拦下后，为该连接排一个到**下一个活跃整点**的唤醒计时器，到点主动续场（过续场各闸 + 重驱边端），使可活跃时段一开就续上、不再干等边端重连。叠至多 1min 抖动防整点齐发；与自动续场同一特性闸（续场关则不唤醒，零回归）；掩码缺失/非法、当前已活跃、整周全休眠均不安排。
...
- **已知边界**：时区为服务器本地（单地域、无时区参数）。窗口唤醒按**周历掩码**的边界算；若同时另配了更窄的**单段日活跃窗口**（resume-config）且其在周历活跃整点仍关闭，则该次唤醒会被日窗口拦下且不再重排（回落到边端重连）——日窗口默认全天不限，故此交叉为边角情形。
