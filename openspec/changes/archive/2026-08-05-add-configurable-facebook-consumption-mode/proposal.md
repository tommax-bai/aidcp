...
- **Console**: Adds a target-global rule/consumption/cold-start numeric editor, environment operation-policy editing with explicit inherit/independent cadence source, and group join-to-first-comment timing configuration inside the `/environments` Facebook global editor, using its independent revision, server-provided bounds, write-after-read truth, and explicit error states.
