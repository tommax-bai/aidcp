...
精选语料保存图文笔记参考图时，默认 SHALL 保留最多 30 张有序图片引用，作为洗稿创作的视觉参照池。该上限与发布侧配图张数（小红书图文帖硬上限 9）**解耦**——参照池刻意存更多、发布生成仍只取其中 ≤9 张。系统仍 MUST 保持硬上限 30、URL 去重、无效 URL 过滤、状态诚实和账号隔离；不得因提高默认数量而允许无界存储。
...
#### Scenario: 超过硬上限仍被截断
- **WHEN** 上游异常上报超过 30 张图片
- **THEN** 精选库最多保存 30 张，且保留前 30 张规范化后的有序图片
