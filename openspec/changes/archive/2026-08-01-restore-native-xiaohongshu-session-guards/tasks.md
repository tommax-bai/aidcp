...
- [x] 1.7 高危动作提交前补一道即席新鲜复检：小红书的点赞 / 收藏 / 关注 / 评论提交在派发前重探一次页面阻断态，命中验证码即**零派发**、回诚实原因 `blocked_by_captcha`（沿用 Facebook 同名原因码，不新造）；探测**本身失败**按「有挑战」保守拒绝。只读周期观测缓存不够——缓存可能过期约一个节拍，闸门放行到真正点击之间的拟人停顿里弹出的验证码必漏。桶的取舍（只认验证码 / 登录两桶，页面类型未识别 MUST NOT 拒绝）见 design.md D7。验收标准：搬旧测试「点赞命中验证码 → 放弃点击并诚实回执」（参照 native/page-engine/src/facebook/shared.rs:376-412 的动作前闸与 :404 的 fresh 探测；小红书执行入口 engine.rs:598 与 xhs-command-router.js:229/235 的互动分支目前无任何提交前闸 — 见 oracle.md 覆盖漏洞 1） <!-- aidcp-edge 7b8d556 Rust 侧新增 ensure_xhs_action_gate：四条动作在 evaluate_router 之前重探 probe_page；Captcha→blocked_by_captcha、Login→login_required、Unknown 绝不拒绝、探测失败保守拒绝，一律 EffectPhase::NotStarted 零派发。xhs_session_guard_write_protection.rs 四条用例均断言页面规则表达式一次都没被 evaluate -->
  ...
  - ~~**残留缺口（2026-07-29）**：`interaction.like_comment`（**评论点赞**）同属高危写动作，但本条正文只点名了点赞 / 收藏 / 关注 / 评论提交四条，本轮按范围只接了这四条 —— 小红书的评论点赞目前仍无提交前阻断复检。~~ **【已补齐 2026-08-01，收口时做掉】** <!-- aidcp-edge bc8aca7 `xhs_gated_action_name` 补 `InteractionLikeComment → "comment_like"`；新增用例 `a_captcha_page_refuses_the_comment_like_before_any_dispatch`（该文件 8/8 全绿）。归因实测：删掉新增那一臂 ⇒ 恰好新用例转红，且运行输出里有 `Compiling`（确认不是拿突变前的二进制在测）。动作名取云端关联键 `comment_like`，与页面规则内那条归一同源 -->
    - **为什么收口时把它做掉而不是留给下一个 change**：本节的规格 delta 写的是「**every** high-risk Xiaohongshu action」，
      归档就等于把这条 MUST 并进主 spec。留着这个洞归档，主 spec 上会出现一条实现并不满足的 MUST，
      而它恰恰是「静默假成功」那一族——评论点赞会一路在验证码墙上按下去，回执还是成功。
      同批把规格正文的判据从**枚举**改成**行为判据**（扣配额 / 写风控事实 / 在平台上留下该账号名下的新痕迹），
...
- [x] 3.3 宿主把取证逐字段透传进 `captcha.assist.click_result` 的既有可选字段；`inputMode` 只在确有字符派发时为 `click_type`（参照 src/main.ts:1007-1015；`replayMode` 亦写死 synthetic，但轨迹回放不在本 change 范围 — 见 oracle.md 覆盖漏洞 4） <!-- aidcp-edge 3236d23 取证按六格白名单逐格抄进回执（答案本身进不来是结构保证，不是靠约束）；焦点没落定改报 no_target 而不是压进兜底的 failed。**`inputMode` 判据本波被订正，见下条具名偏离** -->
  - **⚠ 规格 delta 同步补做（2026-08-01，归档前查出）**：下面那条具名偏离**只改了代码与台账、没改规格 delta**——
    `specs/captcha-incident-handling/spec.md` 里仍写着「`inputMode:'click_type'` MUST 只在**确有字符被派发**时出现」，
    正是被推翻的那句。**照原样归档就会把一条实现有意违反的 MUST 并进主 spec**，而且没有任何自动闸会报
    （`validate --strict` 只校验 delta 内部结构，不比对实现）。已按裁定后的语义改写该段
    （判据＝「哪条执行路径驱动了这次协助」，由取证支撑；「有没有真派发」由 `typed` 单独承载），
    并补一条 scenario 钉住「零派发 + 有取证 ⇒ 仍是 `click_type`、版本偏斜探测 MUST NOT 触发」。
    **教训**：具名偏离若只落在 tasks.md，规格与实现会在归档那一刻悄悄分叉——**改判据的偏离 MUST 同批改 delta**。
...
- [x] 5.6 **【已裁定 2026-08-01 —— 走法乙：正式声明不再需要】** 身份翻转 / 重连后「重注入连接级节奏快照」这一步在 Native 形态下的归属未定：Native 会话的同名接收点是空实现且注释写明节奏归云端（src/native-page-engine/browse-session.ts:213-218），会话对节奏更新命令直接返回（:121）。未裁定前任务 5.5 的该步既无法实现也无法声明不做。裁定项与两种走法各自的后果见 design.md「待裁定」§1
  ...
  - ### 裁定（2026-08-01，用户）：**重连后沿用边缘手上已有的那份；只在环境首次启动时向云端取一次。**

    **不实现重注入。** 身份重立链里那个具名占位就此关闭，**不补实现、也不补静默兜底**。

    **为什么重立后的会话可以不带那份快照**（第三条事实点名要求的说明）：

    1. **真正决定每一步快慢的不是这份快照。** 内容与状态相关的时长由云端一处算出中心值、
       **随每一条决策指令下发**；边缘拿到的这份只是**断连兜底**（整体快慢档位 + 最短停留），
       在云端联系得上的时候根本不参与决策。
    2. **它不随会话变化。** 同一个环境重连回来，账号没变、风控档位没变 ——
       重新取一份得到的是同一个值。为一个恒等的结果建一条显式重注入链路，
       是拿真实的复杂度换零收益。
    3. **它也不会过期成危险值。** 兜底方向是**变慢**（降速档），沿用旧值最坏是慢一点，
       不会把节奏放快到反检测阈值之外。

    **一处如实标出的边界（不影响本裁定，但别当它不存在）**：**运行期换号**时，
    新账号的风控档位可能与旧账号不同，而边缘手上那份兜底仍是旧账号的。
    影响面被限制在「换号之后、下一次云端指令到达之前」这段断连兜底窗口内 ——
    云端一旦接得上，逐条指令带的中心值立刻覆盖它。**判定为可接受，不为此建重注入链路。**
    若将来观测到换号后兜底窗口内的节奏明显不匹配，回到本条重议。

    **本裁定同时解除**：5.5 的该步（此前「既无法实现也无法声明不做」）现在是**明确不做**；
    第 6 节 6.1 对账表第 14 行登记的那处「连接级」缺口按本结论结案。
