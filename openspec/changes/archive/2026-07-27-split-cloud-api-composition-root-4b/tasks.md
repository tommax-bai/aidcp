...
- [x] 8.6 Update §10 with post-4a inventory, implementation SHAs and source/monolith/split/UI/installer boundaries; strict-validate and archive only after every required item is complete.
  <!-- §10.11 records exact implementation SHAs and separates source, DEV monolith, split not_started, UI, installer and live-account evidence. `openspec validate split-cloud-api-composition-root-4b --strict` passed after merging the latest control main; every required checklist item is now complete and the change is ready to archive. -->
