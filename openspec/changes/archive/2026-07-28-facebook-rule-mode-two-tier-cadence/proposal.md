...
- **存量配置行随定义身份一起迁移**：账号开关（`enabled`）表达的是「这个账号要不要跑规则模式」，与节奏版本正交，MUST 跟到新定义；回填 MUST NOT 改动开关本身，也 MUST NOT 改写 `updated_by` / `updated_at`（迁移不是运营动作）。**进度、去重事实与批次三张运行时表 MUST NOT 回填**——它们答的是「在这套节奏下做到哪了」，从零重收才是正确语义。
