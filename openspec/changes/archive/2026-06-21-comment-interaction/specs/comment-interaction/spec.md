...
浏览闭环的「发评论」互动能力：接在"互动完成"与"是否进个人主页评估"之间的评估→撰写→去AI味→循环内人审→下发链路；
精品门槛 + 每账号每日上限（后台可配、与风控安全配额取小）；循环内审批暂停态 + 短超时 + 未授权不发；
执行端拟人输入 + 发布后校验如实回报、绝不假成功；风控按账号配额单写、计数挂真实回执；协议新增 `interaction.comment`。
...
系统 SHALL 让 `CommentAppraiser` 仅在笔记过**高热度 / 高价值门槛**时才可能评论；并新增**按账号、可持久化的每日评论上限配置**，
运营在 console 后台读写、经面板 `/api` 下发。**实际生效每日上限 = min(运营配置上限, 风控安全配额)**。"今日已评数"MUST 复用风控按账号按天计数。
数量与门槛 MUST 在评估阶段就判定：超上限 / 不达门槛 MUST 直接走"不评论 → 进主页评估"分支，MUST NOT 进入撰写 / 去AI味 / 审批。
...
#### Scenario: 低热度笔记不评
- **WHEN** 笔记热度 / 价值未达门槛（按详情页真实点赞 / 收藏量等信号）
- **THEN** `CommentAppraiser` MUST emit `comment.skipped`，不进入撰写
