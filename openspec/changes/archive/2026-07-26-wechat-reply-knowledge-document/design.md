...
模型可能出现“理由明确说普通咨询无风险，但 level/tag 仍给 unknown”的自相矛盾。Cloud 仅在一组可机械证明的窄条件下消除这种 model-only unknown：知识文档非空、有效 classifier intent 属于普通问答，或入站命中明确的适龄/年级询问句式；polisher/reviewer 均调用成功；候选通过确定性门禁并至少记录一项文档事实；汇总标签除了 unknown/meaning_changed/introduced_claim 外没有任何内容风险。明确句式允许在 classifier JSON/schema fallback 时替代其分类，但不掩盖 polisher/reviewer fallback。满足这些条件时，内容风险显示 low、移除 unknown，并且 classifier fallback 不再单独把最终等级抬回 unknown；人工审核仍由 AI 实际运行和流程标签强制保留。任一条件不满足就保留 unknown，不用自然语言理由做脆弱推断。
