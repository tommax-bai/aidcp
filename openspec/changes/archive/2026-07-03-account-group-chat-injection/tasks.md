...
- [x] 7.5 手动/端到端：后台粘贴含 emoji/多行码 → 保存 → 刷新持久且原样；清空 → 无码；`/comment <昵称> group:on`（有码）→ 人审卡含完整码 → 发出闭环（正文逐字 + 码整段插入）；`group:on`（无码）→ 告警不发；`/comment <昵称>`（无 flag）→ 普通评论零回归；同码配多账号 → 前端告警。 <!-- **未真机验证**：用户 2026-07-03 决定归档不阻塞、当**部署后冒烟**自行验（[x] = 关闭为「延后冒烟」，非「已验证」）。前置：本地 edge 先 pull d714b9f + 重启（否则老 edge 忽略 groupChatCode、只发正文丢码、破审=发）。建议先在自己的测试笔记上跑 -->
- [x] 7.6 提交 + 部署：全部提交推送并**已部署**（2026-07-03）。cloud（a2c8f09 存储/面板/命令/注入 + 77faef0 分层送达）从**干净 HEAD 导出**部署（排除并发方未提交 WIP）——备份 cloud.bak.20260703-000118 + .env.bak → rsync（不 --delete）→ restart → healthcheck 全绿（active、8787/8090 监听、CommentScheduler 就绪、飞书长连接已建、**group_chat_info 列已自愈**）；干净 HEAD 全量 1089/1089、acceptance 27/27。console（c81bb32 + HEAD 52e1b2b）从干净 HEAD 构建 → 备份 console.bak.20260703-000243 → rsync dist（不 --delete）→ 200 可达、新构建生效。edge（1385fef 探针 + d714b9f 分层输入）已推 master，**用户本地 pull + 重启后方可用 group:on**。**未碰 isales**。 <!-- aidcp-cloud 77faef0 / aidcp-console 52e1b2b / aidcp-edge d714b9f <!-- 2026-07-03 deployed（cloud+console；edge 待用户本地拉取）--> ；注：部署一并带上了并发方已提交到 master 的 feishu/publish（cloud）+ role-config/edit-draft（console）工作——均在 master 发布线内、干净 HEAD 全量测试通过 -->
