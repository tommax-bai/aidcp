...
- 将 macOS 嵌套产物的发行信任门禁与安装后运行可用性检查分离：构建/签名阶段继续严格验证 Developer ID、Team ID、Identifier、架构和版本；客户端运行时只接受固定包内资源、兼容清单、可执行文件和目标架构，并以真实进程启动/就绪结果判断可用性，不因客户机器对外层 App 做 ad-hoc 重签而拒绝 GOST 或 Native Page Engine。
...
- 点对点交付场景下不把安装后自校验作为防篡改边界；发行包可信性由构建期及最终产物门禁保证，安装态仍保留固定资源位置、兼容清单、可执行性、架构和真实启动失败的诚实反馈。
