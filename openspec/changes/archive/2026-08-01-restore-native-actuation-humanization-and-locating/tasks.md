...
- [x] 1.6 为「单次尝试即报升级」写一条仓内合约测试（测试落本 change 的测试目录、不改被测文件）：断言生产步骤执行路径不存在「升级结论 + 尝试次数为 1」的组合；当前实现下必须失败（现状在 `native/page-engine/src/xhs-command-router.js:242`） （参照 src/locating/engine.ts:247-264：escalated 的前提是连续 maxAttempts=3 次校验失败 — 见 oracle.md ⑮） <!-- aidcp-edge d7dc24b 新增 test/native-page-engine/xhs-compat-escalation-attempts.test.ts（9 例）；被测文件 xhs-command-router.js 一字未改 -->
  - **立论被具名改写（2026-08-01）**：任务字面的「不存在『升级 + 次数为 1』的组合」**不可照做** —— 滚动步（`page.scroll`）就是这个组合，且属主已具名裁定「有意未动」（滚不动是结构性到底，那一支根本没有校验重试循环，1 就是它真实的尝试次数）。照字面写会把一条正确行为判红。改成：**凡是经过校验重试循环得出的升级结论，回报的次数必须是实测出来的**；滚动步作具名例外，并由一条用例正面钉住它的形状。
  - **落点**：四条点击步逐条驱动（赞 / 收藏 / 关注 / 评论输入框，各配一份「永远确认不上」的页面）→ 断言 `escalated` + `attempts == 3`；**输入步**单列一条（回读恒不一致 → 重试到顶 → `escalated` + `input_readback_mismatch`）—— 交接点名的那处零覆盖分支就是它；再加「一次就成回报 1」的反向对照（防把次数硬写成上限，方向相反的同一种病）。**完整性腿**：从被测源码解析出白名单键集合，与本文件驱动的集合逐条对齐 —— 新增一条白名单步骤而不给它写用例时响亮失败。
  - **时间线诚实说明（别写成「先红后绿」）**：实现侧的修正由单写区属主**先落**（`aidcp-edge 52a2110`，见 5.3），本文件是事后补的回归保护，「当前实现下必须失败」这道工序本轮**没有走成**。判别力改由变异实测坐实。
  - **归因验证（5 次变异，逐条实测）**：① 点击支 `attempts` 硬写 1 → 4 条点击用例全红；② 输入支硬写 1 → 输入那条红；③ 两支都硬写成上限 `COMPAT_MAX_ATTEMPTS` → 「一次就成回报 1」那条红（反方向同样被杀）；④ 白名单加一条 `note.share_button` → 完整性腿红；⑤ 白名单声明改名（正则失配）→ 完整性腿**响亮失败**而不是退化成「空集 == 空集」恒真（这正是 9.2 记的那种静默恒真陷阱，所以单独验了一次）。
...
- [x] 2.8 让「按下之后才置位的取消」回报为「已派发、结果待定」而非「未开始」，并加断言防止它被当成可安全重放的失败 （参照 cdp-util.ts:236 的「按下即将派发」诚实置真钩子，防云端按提交前失败重投致双发 — 见 oracle.md ②） <!-- aidcp-edge d7dc24b -->
  ...
  - ~~未落的部分：**命令层仍把任何带错误的写一律记成「未开始」**（该判定在引擎主干里，本轮未动那个文件）~~ **【前提有误，2026-08-01 实读代码订正】**：命令层从 `aidcp-edge 746fdcd`（2026-07-26，**早于**上面那条登记）起就是「写命令 + 错误码不在『未开始』小名单里 ⇒ 不确定」。也就是说 2026-07-29 记下这条时它已经不成立了，「云端读到的仍是未开始」这句从来没有对过。**教训与 9.5-2 同族：登记「未落」之前也要实读一遍，不能只凭上一波的印象。**
  - **已补上的断言（任务字面的后半句）**：`actuation_pointer.rs` 那条用例此前只断言错误**文案**；现在同批断言**回执的相位字段**是 `Ambiguous`，并断言错误码不落在「等同于未开始」的那张表里。变异实测：把命令层的相位映射改成恒 `NotStarted` → 该用例当场红。
  - **顺着这条线查出并修掉的真缺口（这才是本条实际的价值）**：命令层把一小撮错误码当成「未开始 ＝ 上游可安全重投」（取消 / 提交窗口不可用），而指针出口在转译「按下已派发」的失败时**原样继承底层 CDP 错误码**。两者一撞，一次已经派发出去的提交就可能戴上「可安全重投」的帽子 —— 文案写得再明确也没用，云端读的是相位。修法：把那张码表提成**单一事实源** `effect.rs::error_code_means_not_started`，命令层的相位映射与指针出口的转译**都按引用取用**（同一判断落在两处实现里迟早分叉，见 memory 里那条「第二份实现须按引用断」）；已派发的失败若碰上表内的码，改带一个可诊断的 CDP 码，不在表内的原样保留。两条单测正反各钉一次，变异实测：去掉那道守卫 → 对应单测红。
  - 另注：本原语把取消检查全部前置到按下之前，因此「按下之后才置位的取消」在结构上不会发生，本条的现实形态就是「按下 / 抬起阶段的 CDP 失败」。
  - **有意保留的两处「已派发 ⇒ 回未开始」**（都不是缺口）：评论编辑器与发布字段的**落焦点击**（`engine.rs` 两处）。它们点的是编辑器本体、不是提交控件，按下派发出去也没有任何不可逆写入发生，「这条评论 / 这次填写没开始」属实。代码里已就地写明。
...
- [x] 2.10 为验证码协助回执的诚实性加一条仓内合约测试（测试落本 change 的测试目录、不改被测文件）：断言宿主向引擎投影验证码点击参数时不得静默丢弃云端携带的轨迹字段（丢弃必须留下可观测记录），且回执里的回放模式不得写成常量。实现点在 `src/main.ts:992-1001`（手工枚举转发、`payload.trajectory` 被丢且无日志）与 `:1014`（回放模式硬编码为合成），该文件归 `restore-native-xiaohongshu-session-guards`：**本 change 不改该文件**，只与其属主对齐落地方式，并把结论与对应 sha 写回本清单 （参照 captcha-assist.ts:300-318「轨迹无效即如实标注回落、绝不谎称用了轨迹」 — 见 oracle.md ④）
  - ~~进度说明（2026-07-29）：**未做**，无结论也无 sha。~~ **【已落，2026-08-01 第六波，`aidcp-edge f5d7b52`】**
  - **「与属主对齐」这个前置已经消失**：`src/main.ts` 归属的 `restore-native-xiaohongshu-session-guards` **已于 2026-08-01 归档**，单写区主张随之失效、**没有属主可对齐**。据此本 change 直接改该文件。
  - **两处缺陷都坐实并修掉**：① 投影处手工枚举字段、云端带下来的轨迹**被静默丢弃**（无日志无记录）—— 现在丢弃处留下点名 incident 与原因的可观测记录；② 回放模式在**两条返回路径上都是硬编码常量** —— 现在由「本次实际走的是哪条路」派生（新增 `CaptchaActuationPath` 入参）。
  - **不转发轨迹本身是对的、没改**：引擎的验证码点击参数结构体带「拒绝未知字段」，多塞一个字段会让整条命令变成非法请求。**轨迹回放通道是另一条 change 的活**（7.10 已具名交接）。本条只做「丢弃可观测 + 回放模式不谎报」这一半 —— 与任务字面一致。
  - **为什么常量比看上去更坏**：那一格是运营判断「我画的轨迹到底用上没有」的**唯一依据**。写死之后它永远说同一句话，而那句话恰好会在轨迹通道接通的那一天开始变成谎话，且没有任何东西会响。
  - **落点**：`test/native-page-engine/captcha-assist-receipt-honesty.test.ts`（2 例）。源码合约那一条**两件事一起断言**（读了这个字段 + 丢弃留了痕），少任何一条都能被一句无关代码喂绿；并顺带钉住「投影仍是手工枚举」，哪天改成整包透传就会响亮失败、逼人重写立论而不是静默转绿。
  - **归因验证**：删掉留痕 + 把回放模式写回常量 → 两条用例同时转红。
...
- [x] 5.5 **【显式弃守 2026-07-30，用户裁定「暂不做」；不是待办】** 实现反污染丢弃：任一次后置校验失败即把相关暂存锚点丢弃，且不得被后续解析当作已确认复用 （参照 cache.ts:119-121 dropStaged + engine.ts:238-243「校验失败即丢弃且强制换路径」 — 见 oracle.md ⑯）
  ...
  - ✅ **【显式弃守，2026-07-30，用户裁定】不做。** 7.16 已结案为「暂不做，待重新设计」，本条随之弃守。**这是决定，不是遗漏，也不是没做完** —— 前提（存在非确定性的锚点来源）在当前引擎里根本不成立，没有不确定的来源就没有可暂存 / 可晋升 / 可丢弃的东西。已有实现与钉住「恒空转」的守卫用例保留原状，**但 MUST NOT 因为它存在就把第三道闸读成已生效**。要做须先重新设计、另起 change（见 7.16）。
- [x] 5.6 逐条盘点当前 Native 命令面，标出哪些已具备后置校验、哪些暂不具备并注明原因；不具备的不得默认返回成功 （参照 engine.ts:39-41 校验器是接口式强制、缺判据即编译不过；判据强度反例见 oracle.md ⑬⑭）
...
- [x] 5.11 按 5.9 / 5.10 逐条盘点现存后置判据（小红书注入路由 8 处、Facebook 加群的有界轮询复查、其余靠一次 sleep 后复读的分支），标出达标 / 不达标；不达标且实现点落在他人单写区（`xhs-command-router.js` 归 `restore-native-xiaohongshu-action-honesty`）的，只与属主对齐落地方式并回写 sha，本 change 不代改 （参照 engine.ts:39-41「校验器是接口式强制、缺判据即编译不过」 — 见 oracle.md ⑫）
  <!-- aidcp-edge fcb1fd2 native/page-engine/command-postconditions.json（42 条写命令）+ test/native-page-engine/command-postconditions.test.ts（5 例门禁）-->
  - **5.6 与 5.11 一起收口，做成登记表 + 常驻门禁而不是散文清单。** 理由：四十二条命令的手工清单必然漂，**而漂掉的那条恰好是最该被看见的那条**——「没列出的命令会被读成已覆盖」。
  - **判据机械派生**：写命令集合从引擎自己的命令类型推出来（全部命令 − `may_write` 里点名的只读名单），与登记表做**恰好相等**断言。新增写命令而不分类，门禁当场失败。
  - **四种状态，其中三种都**不是**「有校验」**：`confirmed` 达标 / `below_bar` 有判据但不达 5.9-5.10 的强度条（**必须写清差在哪**）/ `not_applicable` 结构上没有可读回的业务结果（**必须写明不得因此默认成功**）/ `unread` 没人逐条读过。
  - **实测分布：22 达标 / 3 不达标 / 1 不适用 / 16 未读。** 三条不达标点名：`publish_add_with_candidate` 只有 topic 那一支达标（其余三支仍自证）、`publish_upload_image` 的判据是「该序号位置存在预览图」而**不能**把预览与本次上传绑定、`search_execute` 的输入判据把「读不到」折成「读到了一个否」。
  - **`unread` 是单调棘轮**：预算必须**等于**实际未读数，削减之后不留空位——留空位就等于允许新的未读悄悄补回来（棘轮的经典失效方式）。新增写命令的默认归宿**不是** unread。
  - **门禁首跑就抓到本表自己的一处缺陷**：一条 `below_bar` 的理由只描述了现状、没说清差在哪 —— 那读起来和 `confirmed` 没区别。
  - **归因验证（3 次变异）**：① 删掉一条登记 → 覆盖面断言红；② 上调预算留出空位 → 棘轮断言红；③ **整表降级成 unread**（结构完整但什么都不保证）→ 「本 change 亲手落的那几条必须仍是达标」那条红。第三条是最要紧的：没有它，一张结构完美却什么也不保证的表照样全绿。
  - **这份盘点同时解掉规格里的一处限缩**：`native-locating-gates` 的「每条写命令都必须校验其效果」此前只能按「已逐条核过的面」读、不得当成整个运行时的属性——因为那份逐面盘点不存在。现在它存在了，规格已按实际改口。
...
## 6. aidcp-edge — 仓内验证
  ...
  - ✅ **【改回「要做」，2026-07-30 同日用户再次裁定】** 先前当日曾裁定「不管了、后面看 case」，随后用户明确改口径：**边缘侧「诚实回执 / 后置校验」类的缺口要记录下来并做**。以后者为准。本条属该类，**恢复为待做**（不再是按 case 处理）。触发条件那句作废。另注：5.10 点名的自证循环实现点在他人单写区，本 change 仍不能代改，需与属主对齐。**注意这不是弃守，是没排期**：与 5.4 / 5.5 那种「前提不成立所以永远不做」不同，本条的问题是真的（后置判据里确有过宽的文字匹配，以及读回自己刚写进去的内容那种自证循环），只是暂不投入。**触发条件**：真机上一旦出现「回报成功但实际没做成」的实例，回到本条。另注：5.10 点名的自证循环实现点在他人单写区，本 change 本就不能代改。
- [x] 6.1 运行 Rust 单测与假 CDP 测试（`cargo test --locked`），记录通过数 <!-- aidcp-edge ab31c57 23 个测试二进制 / 349 条全绿（其中库内单测 179 条）-->
  - **收口实测（2026-08-01，合并后的树 `aidcp-edge ab31c57` 上重跑，满足 6.7 的「必须在合并后的树上重跑」）**：`cargo test --locked` **23 个测试二进制 / 349 条通过 / 0 失败**（库内单测 179 条）。前几波只记了门禁总体通过、没有精确计数，这里补上。
...
- [x] 6.2 运行 `cargo fmt --check` 与 `cargo clippy -- -D warnings` <!-- aidcp-edge ab31c57 两者均单独实跑通过 -->
  - **收口实测（2026-08-01，合并后的树 `aidcp-edge ab31c57` 上重跑，满足 6.7 的「必须在合并后的树上重跑」）**：`cargo fmt --check` 与 `cargo clippy --all-targets -- -D warnings` **都单独实跑、都通过**（此前只经 `gate:native` 间接校验）。
...
- [x] 6.3 运行 `npm run test:acceptance`（安全红线 `AC-PROTO-*` / `AC-PUB-*` / `AC-RISK-*` 必须全过） <!-- aidcp-edge ab31c57 31/31 全过、0 跳过 -->
  - **收口实测（2026-08-01，合并后的树 `aidcp-edge ab31c57` 上重跑，满足 6.7 的「必须在合并后的树上重跑」）**：**31 / 31 全过、0 失败、0 跳过**。注意与前几波的 30/30 有一处口径变化：那条 gated 真机 E2E 此前计为「跳过」，本次实测 `skipped 0` —— 数字变化不是本波引入。
...
- [x] 6.4 运行 `npm test` 全量与 `npm run typecheck` <!-- aidcp-edge ab31c57 2913 例 / 2912 绿 / 0 红 / 1 跳过；typecheck 通过 -->
  - **收口实测（2026-08-01，合并后的树 `aidcp-edge ab31c57` 上重跑，满足 6.7 的「必须在合并后的树上重跑」）**：`npm test` **2913 例 / 2912 绿 / 0 红 / 1 跳过**（跳过的是 gated 真机 E2E）；`npm run typecheck` 通过。**另记一次同日观测**：同一批改动在开发树上跑全量时命中过一次既有 flake（会话恢复「传输已死走重建」在满负载下 EPIPE），干净主干基线对照与单跑均绿，按 §5 的指示**没有动它** —— 给它加重试会让唯一的现场证据消失。
...
- [x] 6.5 运行 `npm run build:dist` 并确认生产产物剪枝检查通过、禁入表未放宽 <!-- aidcp-edge ab31c57 reachable=83 removed=68 legacy_page_rules=absent page_rule_fragments_guarded=18 source_maps=absent -->
  - **收口实测（2026-08-01，合并后的树 `aidcp-edge ab31c57` 上重跑，满足 6.7 的「必须在合并后的树上重跑」）**：实跑通过，输出 `reachable=83 removed=68 legacy_page_rules=absent page_rule_fragments_guarded=18 source_maps=absent`。**禁入表未放宽**：分片守卫计数由目录派生，第三波 17 → 现 18（本波新增的话题判据落在既有分片里、不新增文件，那 1 条来自并行流），方向仍是只增不减；`legacy_page_rules=absent` 与 `source_maps=absent` 两条禁入判据均成立。
...
- [x] 6.6 记录 Edge 与控制仓的提交 sha、偏离说明、与并行 change 的重叠文件（sha 必须取自已推送的提交） <!-- aidcp-edge d7dc24b / ab31c57（第四 / 五波）；重叠文件见 §9.2 -->
  - **收口实测（2026-08-01，合并后的树 `aidcp-edge ab31c57` 上重跑，满足 6.7 的「必须在合并后的树上重跑」）**：第四 / 五波 Edge sha **`d7dc24b`**（1.6 / 2.8 / H.1）与 **`ab31c57`**（8.1 话题），两者均已推送到 `master`、且都用 `git merge-base --is-ancestor <sha> master` 判定过（不是「能不能 show 出来」—— 见上面那条 sha 订正的教训）。重叠文件与跨属主越界见 **§9.2** 的第四 / 五波补登表。控制仓 sha 逐条落在各任务行尾。
...
- [x] 6.7 集成前按 design 的重叠文件表逐行核对：`input.rs` / `browse-session.ts` / `command.rs` / `runtime.rs` / `engine.rs` 五处属多流共写，先 fetch + rebase 到各属主最新提交再跑 6.1–6.5；push 遇 non-ff 一律 rebase 重来，绝不 force <!-- aidcp-edge ab31c57 集成经 scripts/land-change（fetch + rebase + 全套闸 + ff-only push），并在合并后的树上重跑 6.1–6.5 -->
  - **收口实测（2026-08-01，合并后的树 `aidcp-edge ab31c57` 上重跑，满足 6.7 的「必须在合并后的树上重跑」）**：**「仍缺的一半」已补上。** 第四 / 五波的集成一律经 `scripts/land-change`：它先 `fetch` + rebase 到 `origin/master`，跑完 `test:acceptance` / `test` / `typecheck` / `gate:native`，再 **ff-only** 推送（撞 non-ff 即中止，全程未 force）。收口时另开了一个**分离头指针的只读校验 worktree**（`ab31c57`，独立 `npm ci`），在**合并后的树**上重跑 6.1–6.5 全套并把数字记回各条 —— 此前各波门禁都只在该波自己的分支树上跑过，那正是本条点名缺证据的那一半。五处多流共写文件本波碰了 `engine.rs`（新增分支 + 相位映射改引用）与 `input.rs`（未动，只被引用）。
...
- [x] 7.1 运行 `openspec validate restore-native-actuation-humanization-and-locating --strict` <!-- aidcp 2026-08-01 实跑：valid -->
  - **⚠️ 归档前还要做一件 `validate` 抓不到的事**（教训来自会话守卫流的收口，见控制仓 `04295ae6` 那一串）：`--strict` **只校验 delta 的内部结构、从不比对实现**。所以每一条**改判据的具名偏离**都必须对着 `specs/` 里的 delta 逐条读一遍 —— 只改代码与台账、没改 delta，照原样归档就是在主 spec 上留一条实现有意违反的 MUST，而没有任何自动闸会拦。本 change 的具名偏离清单见 §9.1 与 §4 头部。
...
- [x] 7.8 记录本 change 明确未做的事：未打安装包、未部署 dev/ol、未改云端节奏中心值、未代改迁移主 change 的 tasks.md <!-- aidcp-edge d7dc24b / ab31c57 -->
  ...
  - **第四 / 五波补充（2026-08-01，据此勾选）**：同样**未打安装包、未部署（dev / ol 都没碰）、未替换运行中的桌面客户端、未做任何真机动作、未改云端节奏中心值（未碰 `aidcp-cloud` 仓 —— 本波只**读**过它来核实「加话题这条指令是不是活的」，一个字没改）、未代改任何他人 change 的 tasks.md**。三点与前波不同：① 两个提交 `d7dc24b` / `ab31c57` 已经 `land-change` 合入并推送 `master`；② **`design.md` 的 D1 债已还**（见下条），不再是「一字未改」；③ 本波**没有**碰 `openspec/specs/` 的 delta —— 而归档前必须对着它逐条读一遍偏离，见 7.1 的补注。
  - **design.md D1 的未收口债已清（2026-08-01）**：第三波带着一条「具名推翻 design D1」的裁定（§4 落点层级：字面写 Rust 命令执行路径，实际落在宿主层），当时按「由主控另改」挂账。本波把结论写回了 `design.md` 的 D1 正文，**不再是台账与设计文档互相矛盾的状态**。
...
> **7.10–7.15 的收口依据（2026-08-01）**：这六条要的是「登记为范围外项 + 在 design.md Non-Goals 具名交接」。
> 实读 `design.md` 的**「范围外项的具名交接表」**，六项**逐条在表内**，每行都写明了「为什么不在本 change 做 /
> 建议承接方 / 不做的后果」三栏 —— 登记这件事本身已经成立，故勾选。
> **勾选的含义只到「已登记 + 已具名交接」，不含「承接方已开工」**：其中四项的承接方仍是「需新立 change」。
> **一处按实际落地订正**：7.12 的话题判据在**引擎特化这条新路径上**已由本 change 兑现，理由见该条。

- [x] 7.10 登记 **运营真机鼠标轨迹回放通道** 为本 change 范围外项，已在 design.md Non-Goals 具名交接给 **需新立 change**（本 change 只做 2.10 的「丢弃可观测 + 回放模式不得硬编码」那一半）
- [x] 7.11 登记 **小红书注入路由的通用点击助手（瞬移滚动 + 伪造指针移动）** 为本 change 范围外项，已在 design.md Non-Goals 具名交接给 **restore-native-xiaohongshu-action-honesty**（该文件的单写区属主；本 change 只在规格层立跨平台要求并改 Facebook 那一半）
- [x] 7.12 登记 **话题标记后置判据（真标记 + 精确相等 + 剔隐藏后缀）** 为本 change 范围外项，已在 design.md Non-Goals 具名交接给 **restore-native-xiaohongshu-action-honesty**（本 change 只在 5.10 立「禁自证循环」的跨平台判据要求）
  - **交接口径已按实际落地订正（2026-08-01，`aidcp-edge ab31c57`）**：本 change 的 8.1 把话题填写整条搬进了引擎特化分支，而**搬进来就必须同时给出「话题真的贴上了没有」的判据** —— 不给就是明知故犯地把一条自证循环搬进新代码。所以这条交接**在引擎特化这条新路径上由本 change 自己兑现了**（判据落在本 change 自建的分片 `xhs-input-targets.js`，整套移植退役实现的实机校准结论）。
  - **仍归属主的那一半**：注入路由 `xhs-command-router.js` 里那条旧分支的判据。它已被引擎截走、不可达，**删除仍归单写区属主**（见 8.7 的待办清单，本波已补登该条）。**没有出现两份实现并存**：新路径的判据在本 change 的文件里，旧路径整段等待删除。
- [x] 7.13 登记 **动作前守卫层（干扰扫描 + 多轮清障 + 停手终局）** 为本 change 范围外项，已在 design.md Non-Goals 具名交接给 **需新立 change**
- [x] 7.14 登记 **语义类名白名单的词边界匹配与匹配唯一性闸** 为本 change 范围外项，已在 design.md Non-Goals 具名交接给 **需新立 change**（与匹配唯一性闸同批）
- [x] 7.15 登记 **按边缘标识派生的每机节奏偏置** 为本 change 范围外项，已在 design.md Non-Goals 具名交接给 **harden-native-engine-runtime-contracts**（会话身份入参的属主）
...
- [x] 8.1 小红书全线文本输入改走硬件级逐字输入原语：评论提交、发布填写、话题 / 提及候选、定时设置、遗留步骤路径的输入步，全部不再用「属性描述符 setter 一次性写 value / 对可编辑元素整段赋 textContent + 手动派发合成事件」 （参照 humanize/keyboard-rhythm.ts:22-81 + cdp-util.ts:310-340；现状 `xhs-command-router.js:34-44` 的输入助手及其 5 个消费点 — 见 oracle.md ⑤）
  ...
  - ~~未落的三个消费点：**话题 / 提及候选**（还需一套「候选项定位 + 点击 + 回读」的分片能力）~~ **【话题已落，2026-08-01 第五波，`aidcp-edge ab31c57`】**、**遗留步骤路径的输入步**（结构上必须把整条步骤循环搬进引擎，不能只截一个步骤）、~~**定时设置**（见下条裁定请求）~~ **定时设置已裁定划出，见下条**。
  - **话题（第五波，`aidcp-edge ab31c57`）—— 这一条不只是打字方式，它修的是一条活的假成功**：
    - **改前**：注入路由把整段正文 `before + ' #话题'` 一次性重新赋值，点完候选后拿「正文里搜得到这几个字」当确认 —— 而它读回的正是自己刚写进去的那串。**自证循环。** 后果不是「没贴上就报错」，而是**没贴上也报成功**：稿子发出去时话题只是一串普通文字，拿不到话题带来的分发，回执上一切正常。**这条路径是活的**：云端 `platform-profile.ts` 对每条稿子的每个 tag 都下发一条 `add_with_candidate{candidateKind:'topic'}`。
    - **改后**：引擎特化分支 —— 落焦正文 → 光标归尾 → **硬件级逐字**打 ` #关键词` → 有界等候选下拉（4s）→ **拟人指针**点候选 → 有界等正文里出现**真话题 token**（4s）。选择器、两个窗口、「只认精确候选或『新建话题』、都没有就不点」、真 token 判据（`a.tiptap-topic[data-topic]` + 剔隐藏后缀 + **精确相等**）整套移植自退役实现的 `runAddTopic` / `committedTopicPill`，**那是实机校准过的，不自创**。
    - **只截 `topic` 一种**：`mention` / `location` / `collection` 仍走注入路由。它们**没有任何校准过的 token 判据**，在桩上现编一个选择器就是拿运营机赌运气；而只把打字搬进引擎、判据仍留自证，等于把同一个谎言搬进新代码。**这条是有意的收窄，不是遗漏**，真机标定项已登记。
    - **🔴 打字之后没有安全取消点，这是有意的**：话题词一进正文就撤不回来 —— 本路径唯一的清场原语是**整字段清空**，而此刻正文里躺着整篇稿子，清它等于毁稿。取消只在打字**之前**生效；之后把两个有界窗口跑完再让位（合计 ≤ 8s），且**每一个出口都回「不确定」、绝不回「未开始」**。
    - **认不出目标候选时不点**：随便点一个会给稿子贴上一个**撤不回来**的无关话题。此时正文里留着一串裸 `#关键词`（与退役实现同行为），回执如实回 `publish_candidate_not_found`。云端把这条指令算作尽力而为，**稿子照发、不丢稿**。
    - **归因验证（4 次变异）**：① 点完候选即判确认 → 「没变成真 token」那条红；② 真 token 判据退回正文子串 → 分片用例红；③ 精确相等放宽成子串 → 「#考研数学 不得让 考研 判成已贴上」红；④ 候选放宽成「取下拉第一项」→ 「全是无关词时不许点」红。Rust 两次的运行输出里都有 `Compiling`。
  - ~~**待主控裁定：定时设置不该逐字打。**~~ **【已裁定 2026-08-01，用户拍板：保持 setter，具名收窄。】** 定时字段是**分段日期时间控件**（年 / 月 / 日 / 时 / 分各一段），退役实现对它用的就是原生 value setter、从未逐字打；照 8.1 字面灌整串不等价，大概率把定时发布功能打坏 —— 那是一次功能回归。
    - **判据与 2.9 / 3.7 两条弃守同源**：那条路径缺的是**反检测质量**，不产生任何假成功（setter 写进去的值是真的，回读也是真的）。本 change 的范围收敛在「会说谎的那些」，故**定时设置从 8.1 的覆盖面里显式划出**，不计入未落项。
    - **不是无限期弃守**：真要兑现，正解是**键盘分段输入**（逐段定位、硬件级按键、段间切换），但分段控件的键位行为（是否接受数字键、是否要补零、切段顺序）在桩上验不出来，须先真机标定 —— 已登记进 §9.4 补登（与 9.4 B-12「定时设置的定论」合并，那条原本就是为这件事留的）。**下次做反检测专项时，连同 2.9 / 3.7 一并做。**
  - **遗留步骤路径的输入步 —— 具名收窄，不做（2026-08-01 第五波裁定；⚠️ 这是一次范围判断，要推翻只改本条即可）**：
    - **它属于哪一类，先坐实**：该支已由单写区属主加了「真可编辑判据」闸（非可编辑元素直接判 `no_target`，堵掉「写进普通 div 再回读」那种自证），随后的判据是「写进去的文本回读得到」—— 而**输入步的业务结果本来就是「字在框里」**，这与话题那条「字在正文里 ≠ 平台把它变成了话题」不是一回事。**所以这一支缺的是反检测质量（合成赋值 vs 硬件级按键），不是一条会说谎的判据。**
    - **判据与 2.9 / 3.7 / 定时设置三条完全同源**，本轮范围收敛在「会说谎的那些」。
    - **另有一条独立的、更硬的理由**：v1 有序步骤是**已弃用的兼容路径**（控制仓 CLAUDE.md §2 明写「勿在遗留路径上改代码」）。而 §8 节首约定的接线方式是「引擎侧新增命令特化分支」，这一支**做不到只截一个步骤**——结构上必须把整条步骤循环搬进引擎，等于在一条正在退场的路径上新建一套实现。
    - **可达性实测**：云端 `SimplePlanner` 确实在生产里接线（`server.ts`），规则支会产出一条 `search.input` 的输入步，但那个 actionId **不在边缘的白名单里**、必然落 `no_target`；能落到白名单的只有模型兜底自由产出 `note.comment_input` + `op:'input'` 的情形，而 `findByWords` 按**可见文本**认元素、空评论框没有可见文本，实践中同样落 `no_target`。**不是零风险，是低到不值得为它在退场路径上新建实现。**
    - **不掩盖那条残留**：contenteditable 目标上「写完立刻回读、命中即收工」有一个**回滚窗口**（富文本编辑器可能下一拍把内容退掉），登记见 §9.5-9。它是真的，只是与本条的取舍无关。
...
- [x] 8.7 记录本节与 `restore-native-xiaohongshu-action-honesty` 的分工结论与对应 sha：本节只接线原语，其回执口径、后置判据、去重键、遗留分支删除均归该 change；两侧对 `engine.rs` 小红书分发的改动按 6.7 的串行集成纪律处理 <!-- aidcp-edge 1b890ea / 25fa1c3（+ 92f7882 / f652786 / 17892c2 / 044f103 / 4593922 / c7c9ea8）；xhs-command-router.js 全程零改动，已按 git diff 文件清单核对 -->
  ...
  - **补登（2026-08-01 第五波，`aidcp-edge ab31c57`）**：`publish_add_with_candidate` 的 **`topic` 那一支**也已被引擎特化截走，同样是死分支、待属主删除。**⚠️ 只删 topic 那一支** —— 同一个 `kind` 里的 `mention` / `location` / `collection` 三支**仍然活着**（引擎的特化带 `candidate_kind == "topic"` 守卫），整段删掉会把它们一起删没。它们**没有**校准过的 token 判据，所以那三支目前仍是自证口径（登记见 9.5，别当成已修）。
...
**第四 / 五波补登（2026-08-01，`aidcp-edge d7dc24b` / `ab31c57`）**

| 文件 | 属主 | 本波改动 | 性质 |
|---|---|---|---|
| `native/page-engine/src/effect.rs` | 引擎主干（多流共写） | **新增**「哪些错误码在命令层等同于未开始」的单一事实源 `error_code_means_not_started` | 纯新增导出，无既有行为变化 |
| `native/page-engine/src/engine.rs` | design 重叠表点名的多流共写 | ① 相位映射改为引用上面那张表（行为逐字不变）；② 评论到达确认改有界轮询 + 两条证据 | 须与并行方对齐；②是本 change 的 H.1 |
| `native/page-engine/src/facebook/shared.rs` | Facebook 侧各流 | 「已派发提交失败」的错误码转译加一道守卫：不许戴上「可安全重投」的帽子 | 方向是收紧、Facebook 侧行为在既有码上零变化，仍属越界，须属主追认 |
| `native/page-engine/src/engine.rs`（第五波） | 同上多流共写 | 新增 `publish_add_with_candidate` 的 `topic` 特化分支（8.1） | 本 change 的任务，纯新增分支 |
| **判据归属的一处变化** | `restore-native-xiaohongshu-action-honesty` | 话题真 token 判据由本 change 在**新路径**上兑现（见 7.12 的订正） | 旧路径的删除仍归属主；两份实现**没有**并存 |
...
**D. 第四波补登（2026-08-01，H.1 与 8.1 裁定引入，3 条 —— 待并入 backlog 簇 123）**

23. **[H.1 的解锁前提] 小红书评论提交成功后，平台到底清不清空评论框。** 这是新的**结构必要条件**，两条证据要同时成立才判确认。若真机上并不清空，全部评论会从「已确认」退成「不确定」（方向诚实，但日志上会非常显眼、且云端可能据此重投）。退役实现拿它当必要条件、有真机历史支撑，但迁移后一次都没复核过。**这条不确认，H.1 的收益就只是「不再自证」，代价却是全量回执降级 —— 优先级最高。**
24. **有界轮询窗口 2000ms 够不够 / 新原因码 `submitted_editor_not_cleared` 的真实频次。** 前者定 8 轮 × 250ms 是照退役量级取的；后者是「正文看到了、编辑器没清空」这一态，真机上它到底是「平台慢了一拍」还是「扫描仍读到了自己写的那份」，只有频次能分辨。**云端侧尚未登记消费该原因码**（裸串），先当排障 token 用。
25. **[8.1 裁定的解锁前提] 定时发布分段控件的键位行为。** 是否接受数字键、是否要补零、段间切换用 Tab 还是方向键。确认之后才可能把定时字段改成键盘分段输入。**没有另开条目** —— 已并进 backlog **簇 123.12**（那条本来就是为这件事留的，「需裁定」那一半就此结案，只剩真机标定）。

26. **[话题] 建议下拉与真 token 的选择器在今天的小红书上是否仍然成立。** 整套（`.tippy-box[role="tooltip"]` / `#creator-editor-topic-container .item` / `a.tiptap-topic[data-topic]` / 隐藏后缀 `span.content-hide`）移植自退役实现的实机校准结论，但那次校准是 Native 迁移**之前**做的。选择器若已漂移，行为是**诚实地全部回未确认**（不会误贴），但话题会全线贴不上 —— 云端算尽力而为、稿子照发，所以**不会有任何报警**，只能靠真机跑一条带 tag 的稿子看。
27. **[话题] 「认不出目标候选就不点」在真机上的触发频率。** 触发时正文里会留下一串裸 `#关键词`。确认这个残留在真机上可接受（退役实现同行为），以及触发是否频繁到值得改成「打字前先探一次下拉」。
28. **[话题] `mention` / `location` / `collection` 三支的 token 判据标定。** 这三支仍走注入路由、仍是自证口径（打进去就算成功）。要把它们也搬进引擎，前置是在真机上认出各自的「真的选中了」结构信号 —— 没有这个就只能在桩上瞎猜选择器。

> **已并入 backlog（2026-08-01）**：23 → **簇 123.29**，24 → **簇 123.30**，25 → 并进既有的 **簇 123.12**；26 / 27 / 28 → **簇 123.32 / 123.33 / 123.34**。**验收状态以 backlog 为准**，本节只保留上下文。

### 9.5 残留与新发现（只登记，本波未动手）

0. **`add_with_candidate` 的 `mention` / `location` / `collection` 三支仍是自证口径**（2026-08-01 第五波登记）。`topic` 那一支已被引擎特化截走并换成真 token 判据；这三支仍走注入路由：`location` / `collection` 有一道入口行回显判据（比纯子串强，但仍不是「真的选中了」的结构证据），`mention` 则是彻底的自证 —— 把 `@用户名` 打进正文，再回读正文里有没有这几个字。**没有一起做的理由是可核对的**：退役实现对这三支用的也是通用锚点 + `domHasText`，同样自证，**没有任何实机校准过的 token 判据可移植**；在桩上现编一个选择器就是拿运营机赌运气。前置是真机标定（§9.4 D-28 / backlog 簇 123.34）。

9. **遗留步骤路径输入步的「回滚窗口」**（2026-08-01 第五波登记，未动手）。该支写完**立刻**回读、命中即 break，中间没有落定等待；富文本编辑器完全可能在下一拍把整段赋值退掉，于是「回报成功」与「内容其实没留住」之间有一个窗口。**与 8.1 的收窄取舍无关** —— 那条说的是「合成赋值 vs 硬件级按键」，这条说的是「回读时机」。之所以只登记：实现点在 `xhs-command-router.js`（单写区属主 `restore-native-xiaohongshu-action-honesty`），且整条 v1 兼容路径正在退场。属主若要修，最省的落法是把「命中即 break」改成「连续两轮命中才收工」（与本 change 归尾确认同口径）。
...
## 归档前置 — spec delta 与实装的逐条对账（2026-08-01，**已修完，剩 1 条待 2.10 定夺**）

> 依据 7.1 的补注做的那一遍「把每条具名偏离对着 delta 读一遍」。**结果比预期严重**：
> delta 是 propose 当时按**意图**写的，此后 2.9 / 3.7 / 5.4 / 5.5 / 定时设置 / 遗留步骤路径
> 陆续被显式弃守或收窄，而 **delta 一次都没跟着改**。首轮读出 8 条，逐条修的过程中又读出 4 条，
> **共 12 条**。`openspec validate --strict` 一条都抓不到（它只校验 delta 内部结构、从不比对实现），
> 归档流程也不问。
>
> **改法不是把 MUST 删掉。** 弃守的是「本 change 交付这件事」，不是「这件事不该做」——
> 规格只能写「已实现」那一列：未交付的部分要么从 delta 摘出去另立，要么把要求**收窄到实际成立的
> 范围**并留下承接方与例外清单。**例外必须留在规格里当例外**，不能删干净 —— 删干净等于
> 让下一个人以为它已经被覆盖了。

| # | delta 里的要求 | 与实装矛盾在哪 | 处置 |
|---|---|---|---|
| 1 | 时间指令 **MUST consume ... inside it**，且要求「在**第一条影响页面的输入**之前」等 | 消费点落在**宿主层**、等待在整条命令之前 | ✅ 改成「在边缘侧、在派发任何页面输入之前」，并把「第一条输入之前」写成**具名的可接受近似**；顺带把离页停留的**锚点**语义写进规格（原文没有），并补一条「锚点读不到时不拿命令开始时刻顶替」 |
| 2 | 转发即消费 **... inside that runtime** | 门禁在、判据是行为派生的，但「inside that runtime」不成立 | ✅ 改成「在边缘侧」；另把**判据必须由行为派生**（不许拿表 / 源码扫描充数）与「消费 ∪ 未消费恰好切分声明、未消费必须写理由」两条写进规格 —— 那正是实装的真实口径 |
| 3 | 逐字输入 **on every platform**，scenario 点名「comment / publish field / **candidate keyword** / **schedule value**」 | 定时设置保持 setter；候选只做了 `topic`；遗留步骤路径具名收窄 | ✅ 枚举收窄到「评论正文 / 发布字段 / 话题候选」，**三条具名例外**（分段日期时间控件 / 其余三支候选 / 遗留兼容路径）连理由一起写进规格，并加一条 scenario 钉住「例外必须一直是具名的」 |
| 4 | 滚动 **Any command ... on every platform** | 遗留步骤路径的滚动步、通知清零里的第四处仍是页面内滚动 | ✅ 收窄到「经引擎命令特化的命令面」，两处**具名例外**入规格；另把「位移按**双向**计」写进规格（原文没有，而单向计会把懒渲染换容器 / 刷新回顶谎报成没动） |
| 5 | 注入点击助手不得移动页面 / 不得伪造指针事件（含静态契约检查） | **3.7 显式弃守**；小红书那一半由 7.11 交接 | ✅ **整条摘出**，原地留说明块写清弃守理由与承接时机 |
| 6 | 验证码协助专用高审查节奏档 | **2.9 显式弃守** | ✅ **整条摘出**，同上 |
| 7 | 协助点击回执须报真实回放模式、不得静默丢弃轨迹 | ~~2.10 未做~~ **已落地（`aidcp-edge f5d7b52`）** | ✅ **保留、且现在真的成立** —— 当时判为「不是漂移、是任务没做完」，处置是不动它、由未勾任务挡住归档。2.10 落地后这条要求与实装一致，**归档前置就此解除**。这也验证了当时那条分法：漂移与「没做完」要分开处置，后者会自己解决 |
| 8 | 锚点先暂存、连续确认才晋升、失败即丢弃 | **5.4 / 5.5 显式弃守**，且前提不成立（引擎无非确定性锚点来源，暂存区恒空） | ✅ **整条摘出**，说明块写清「不是做不完、是前提不成立」，并明写守卫用例必须保留 |
| 9 | 判据强度：类名须按**词边界**匹配 | 词边界匹配与匹配唯一性闸由 **7.14 具名交接**给新 change | ✅ 收窄成「不得拿裸子串当证据、不得把**否定形**读成正证据」（这是实装口径），词边界那一档具名标为范围外 |
| 10 | 后置证据不得是命令自己的输入 | 引擎特化面已兑现；注入路由的 `mention` / `location` / `collection` 仍自证 | ✅ 收窄到「经引擎命令特化确认的面」，三支候选写成**具名例外**；另把本轮真正做出来的两条写进规格：**结构必要条件**（平台清空提交框）与**扫描面必须剔除编辑器及其容器**，再加「结构条件读不到 ≠ 读到否」一条 scenario |
| 11 | pacing 快照须留「**每类操作**的 floor 区间」 | 只存了实际有采样器的那一类；其余存下来是死字段（4.4 具名偏离） | ✅ 收窄成「本路径确实会本地采样的那些操作类」，并明写**没有采样器的类 MUST NOT 存**（存了会让按「已存下」计数的检查喂绿）；顺带把「重连可清锚点 / 中途刷新不得碰锚点」写进规格 |
| 12 | 「**任何**生产步骤路径报『升级 + 尝试次数为 1』即判失败」的机械闸 | 与 1.6 已裁定的**具名例外**（滚动步结构性到底、无重试循环）直接冲突 | ✅ 闸的口径改成「**经重试循环**得出的升级」，并加一条 scenario 把结构性终局写成具名例外；同时点明「照原文写会把正确行为判红，而那种闸最后都是被放宽到什么也不保护」 |

**顺带补进 delta 的一条已实装保证**（不是修矛盾，是原本漏写）：`facebook-humanized-scroll` 新增
**「对齐滚动必须能看见接管」** —— 点赞是写命令，宿主**刻意不对写命令做外层取消竞速**，
所以对齐循环是这条路径上**唯一**能看见接管信号的地方。没有它，后果不是让位慢一点，
而是**协调器已经叫停、循环照样滚到目标并把赞点出去**（变异实测坐实：实测值 `Confirmed`）。
这条保证之前只有代码和用例，规格里没有。

| 13 | 「**每一条**写命令都必须做后置校验」 | 共享的三道闸编排**只接了一条真实命令**；而各命令自带的后置判据**从没被逐条盘点过**（5.6 / 5.11 未做） | ✅ **已两轮**：先加一段说清合规与否是**该命令自己的后置判据**的性质、不看它写在哪个模块里，并因盘点不存在而把保证限缩成「只按已逐条核过的面读」；**随后 5.6 / 5.11 把那份盘点真的做出来了**（`aidcp-edge fcb1fd2`，42 条写命令 + 常驻门禁），规格已据此改口：盘点必须**机械地**与引擎的写命令集合恰好相等（**没列出的面会被读成已覆盖**，所以完整性是强制的、不是靠 review），且必须把「不达标 / 不适用 / 没人读过」三种与「达标」分开，未读那一档是**只降不升且不留空位**的棘轮 |

**第二遍通读补的两处**（不是修矛盾，是把已实装的真实约束写清楚，防下一个人踩）：
- **指针起点「会话内最近一次落点」的真实作用域**：实装是**进程级**的，只在「一个引擎进程驱动一个浏览器、命令串行」这条不变量下成立。规格里明写这条不变量必须写在状态所在处 —— 它一旦不成立，A 会话的光标位置会泄漏进 B 会话的轨迹，**而且是静默的**（点击照样落下，只是轨迹不再是轨迹）。
- **原子区那条规则今天的现实形态**：取消检查全部前置到按下之前 ⇒「按下之后才取消」结构上不可达，真实形态是**按下 / 抬起阶段的 CDP 失败**。规格补明：**错误文案写得再诚实也不够**（调用方读的是相位字段）—— 凡命令层把某组错误码当成「未开始 ＝ 可安全重投」，已派发的失败就 MUST NOT 带上其中任何一个；且那组码 MUST 是**单一事实源**、被相位映射与转译两处**引用**，留第二份拷贝正是这条缝静默重开的方式。

**归档前置已全部解除**：第 7 项随 2.10 落地而成立；第 13 项随 5.6 / 5.11 的盘点落地而由「限缩」改成「按盘点强制」。

**「一次通读不够」本身是个教训**：首轮读出 8 条，边修边又读出 4 条（#11 / #12 / #13 与 fb 那条漏写的保证），
**从 8 涨到 13**。原因是修一条时会被迫把相邻条款也读进去，而通读时眼睛是滑过去的。
⇒ **对账至少读两遍，第二遍在修完之后**。

**这一节的普遍教训 —— 值得带去同批其余 change**：
**弃守 / 收窄类裁定是 delta 漂移的主要来源，比「实装偏离」更常见也更成片** —— 一条裁定
往往一次性作废整条 Requirement。凡是实装期出现过这类裁定的 change，归档前**必须整份 delta 通读**，
不能只读「具名偏离」那几条。
...
- [x] H.1 **评论到达确认的判据**（`native/page-engine/src/xhs-input-targets.js:205-209`，该分片由本 change 8.1 新建）：
      ...
      **复验结论要照抄，别写成已坐实的自证循环**：今天页面结构上编辑器不在被扫描子树内，所以现行事实是**偏向错报失败**；
      ...
      <!-- aidcp-edge d7dc24b -->
  - **两半都落了。** ① **结构必要条件补回**：分片新增 `editorCleared`，确认要求「正文出现在评论区」与「编辑器已被平台清空」**两条独立证据同时成立**。② **扫描面剔除污染源**：与编辑器有包含关系的元素（编辑器自身、其子树、以及**把它包在内的祖先容器**）一律不进扫描 —— 祖先才是真正的泄漏口，`text(祖先)` 会把编辑器里那份活的正文一起收进来。细粒度评论条目与编辑器互不包含，证据不受影响。③ **轮询恢复**：固定睡 800ms 后单次采样 → 有界轮询（8 轮 × 250ms = 2000ms，**按迭代次数限界**、不按墙钟裸跑），命中即收工，所以顺利时反而比原来更快。
  - **三态照旧不塌**：编辑器定位不到时 `editorCleared` **键缺席**（不写 false）；命令侧据此分出 `submitted_editor_not_cleared`（读到了、没清空 —— 两条证据互相矛盾）与既有的 `submitted_ack_unreadable`（确认这一层自己瞎了）。**只新增一个原因码**，其余复用。
  - **归因验证（3 次变异）**：① 确认闸退回「只要 appeared 为真」→ 新用例红；② 轮询退成单次采样 → 慢平台那条红；③ 扫描面不再剔除编辑器 → jsdom 自证循环那条红（报「扫到的是自己刚写进去的那份」）。前两条 Rust 变异的运行输出里都有 `Compiling`。
  - **须真机确认（已登记，见 §9.4 补登）**：这条把确认从「一条证据」抬成「两条」，若真机上小红书提交成功后**并不**清空评论框，全部评论会从确认退成不确定 —— 方向诚实但会很显眼。退役实现拿它当结构必要条件，是有真机历史支撑的，但迁移后没复核过。
- [x] H.2 **指针原语的静默降级**（`native/page-engine/src/input.rs:1132`，即既有 P3）：
      剩余预算收紧时无声降级（低于约 480ms 掉出 15 帧拟人下限、约 63ms 塌成单帧瞬移），
      **无降级标记 / 无记账 / 无诊断**，而同族的逐字输入三样齐备。
      **阻塞照旧**：「降级痕迹到得了人眼前」这条通路整体不存在（§9.3 弃守项②），须单独起 change。
  - **【具名交接，2026-08-01 收口；形态与 7.10–7.15 一致，不是弃守】**
  - **勾选的含义只到「已登记 + 已具名交接」**，与 7.10–7.15 同一口径 —— **缺口仍在，本 change 不交付**。
  - **为什么不能在本 change 做**：它要的不是「加一行记账」，而是**那一行到得了人眼前**。链路查到底（§9.3-2）：引擎是宿主起的子进程，其错误输出既不继承也不落盘；宿主只收进一个 2048 字符的滚动尾缓冲；该缓冲**只在构造进程级失败时**才挂到错误对象上；而**降级是一条成功路径** —— 命令正常返回、进程正常退出 ⇒ 那一步永不触发 ⇒ 记账行被后续输出挤掉、随进程退出丢弃。**没有任何人看得到它。** 在这种前提下加记账，写出来的用例是在断言一个运营侧读不到的字符串确实存在，属自我安慰。
  - **承接方：需新立 change（诊断通路本体）**。两条可行改法都不是一行：走协议加一条诊断记录（跨边界四处同步 + 一个消费端 + 牵动就绪握手），或宿主把子进程错误输出转进日志（给纯协议客户端注入日志出口并一路穿下来）。**同一条通路同时卡着逐字输入的降级记账**（§9.3 弃守项②），两者应同批解决 —— 这也是它值得单独立项的理由：它不是一个缺陷，是一类缺陷的共同前置。
  - **规格上无遗留**：本 change 的 delta 里**没有任何一条要求断言降级留痕**（已 grep 核对），所以交接不会在主 spec 上留下一条实装违反的 MUST。
  - **降级判据的可达性那一半仍有覆盖**，缺的只是「记账行到得了人眼前」这一段。
