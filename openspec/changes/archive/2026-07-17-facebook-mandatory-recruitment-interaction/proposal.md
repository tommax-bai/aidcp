...
- 命中 `comment` 后跳过评论热度门槛、评论 LLM、普通会话软预算与冷却；但在付撰写成本、发免审通知前必须经过可解释的 `RiskController.explain('comment')` 硬风控预检，预检拒绝则不撰写、不发授权卡。评论撰写按规则指引生成，`auto_approve` 仅在免审通知成功后直通，通知失败则 fail-closed。
- 保留全局品牌安全、账号风险状态、分钟/小时/自然日硬配额、目标定位、验证码与提交后置校验；任何拦截或执行失败必须如实记录，绝不把“已决定必做”伪报成“真实成功”。
- mandatory `auto_approve` 通知只表达“已预授权、等待平台执行”，不得使用已成功语义；通知之后必须按同一 requestId 回报平台确认成功、待群管理员批准、确定失败或断线/超时未知终态，成功只认 edge 的真实平台回执。
...
- `comment-interaction`: 强制评论命中后绕过普通热度/判定/逐条人审，但撰写前仍过可解释硬风控；显式免审必须先成功发送中性预授权通知，并在其后补齐真实平台终态。
