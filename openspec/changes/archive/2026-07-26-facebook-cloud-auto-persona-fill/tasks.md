...
- [x] 6.3 Replace the Cloud direction-pool/PersonaGenerator path with exact selected-template application; change customer-auth to accept only `platform` + validated `soulYaml` and reject selectors, strategy, or standalone language.
  <!-- Cloud worktree: selected_persona_v1 persists exact soulYaml; legacy runs fail selected_persona_required; customer-auth validates only platform+soulYaml. Focused 46/46 and typecheck pass after rebasing onto the latest master. -->
...
- [x] 6.6 Add focused Cloud/Edge regressions for identical template writes, legacy fail-closed behavior, no generator calls, strict payloads, removed creation hooks, batch popup flow, and no target/secret disclosure.
  <!-- Edge worktree: import auto-fill removed; FB-filter opens persona-pop batch mode; trusted main builds deterministic YAML and submits only after confirmation. Focused Edge 58/58 + typecheck pass after rebasing onto the latest master; generated YAML round-trips through Cloud loadSoulFromYaml. -->
- [x] 6.7 Run focused/full/acceptance/typecheck and strict OpenSpec validation, commit/push/integrate all owning repos serially, deploy committed Cloud source to dev with runtime checks, and do not build an Edge installer or trigger a real-account fill.
  <!-- Validation and delivery: Cloud focused 46/46, acceptance 59/59, full 2600 (2592 pass, 8 gated skips), and typecheck pass; aidcp-cloud/master 785cbe6. Edge focused 58/58, acceptance 25/25, full 1882/1882, and typecheck pass; aidcp-edge/master 87ef5df. Strict OpenSpec validation passes. Feature branches were rebased; default branches were fast-forward integrated and pushed serially. -->
  <!-- dev deployment 2026-07-19: deploy-target preflight resolved 121.89.85.150; backup /opt/aidcp/backups/cloud-before-facebook-selected-persona-20260719-211352.tar.gz; rsync preserved .env/node_modules/.git and package manifests were unchanged, so npm ci was not run. Restarted only aidcp-cloud.service. Verified active service; listeners 8787/8090/8091; internal and public panel/customer-auth health; PostgreSQL readiness; additive persona_soul_yaml column and selected_persona_v1-compatible strategy constraint; Feishu bot Dev.A; four isales services remained active. The shared dev/ol PostgreSQL boundary received additive-only DDL. No Edge installer was built and no real-account persona fill was triggered. -->
