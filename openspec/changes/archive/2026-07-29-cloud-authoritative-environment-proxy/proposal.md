...
- Reuse an already-Active configured AdsPower browser only when its observed real egress exactly matches the egress observed through the frozen effective proxy. This is a one-time startup takeover gate, not a continuous profile-consistency monitor; mismatch or unavailable evidence stops once without closing the pre-existing browser or entering automatic respawn.
