...
- [ ] 6.5 真机校准（本机，进行中）：本机 edge 连 ECS（ws://121.89.85.150:8787）跑真实小红书，校准——通知首页各类未读探测、三类列表选择器、优先级、"看一眼是否真清未读"、评论/@ 发飞书、巡视后浏览恢复、断连后无残留暂停
  ...
  - [x] 6.5.4 **真机校准完成（2026-06-24，活页面 CDP dump 直接坐实选择器）**：用 bot 专用 profile（`~/.aidcp-chrome-profile`）起带调试端口的 Chrome、登录后通过 CDP 对真实通知页跑只读 DOM 探针，逐项校准——
    - **(a) 三类列表选择器**：真实评论行 = `div.tabs-content-container > div.container`（旧 `[class*="item"]` 命中 23 个 `avatar-item` 头像→垃圾行）；昵称 = `.user-info a`（旧 `[class*="user"]` 先命中空文本 `a.user-avatar`→抽空）；正文 = `.interaction-content`（与 `.interaction-time` 分离、不含时间）；itemKey = `note-id` 属性 / 非 profile 链 / 留空（评论行仅 profile 链）；笔记标题元素不存在→去掉。首页 per-tab 真实 tab = `div.reds-tab-item.tab-item`，角标 = `.badge-container` 内数字叶子 div；tab 范围收到 `[class*="tab-item"]`→**彻底消除 NM-2 包裹泄漏**（`reds-tabs-list`/`sticky-tab` 不含 `tab-item`）。
    - **(b) 优先级**：comments→likes→follows 由云端硬编码、非 DOM 项，无需校准。
    - **(c) "看一眼是否真消红点"→ 确认「消」**：点击前「赞和收藏」tab 角标 `<div>1</div>`；点进去看一眼后角标 div 消失变空槽 `<!---->`。`viewNotificationCategory` 设计行为成立。
    - **(d) 评论@发飞书质量**：真正的生产 `buildNotificationItemsJs` 在活页面抽出 8 条评论**全部干净**（昵称准·含 emoji 完整 / 正文纯无 blob无时间 / itemKey 正确留空）；真机观察到「该评论已删除」占位 → 补云端 classifier 过滤。
    - **黄金验证**：生产 builder 直接跑活页面——有真实未读→`likes:1`、看一眼清除后→`0`、清空→三类全 0（无 phantom），抽取 8 条干净。jsdom 夹具按真机结构重建。edge typecheck✓ / acceptance 11/11 / 全量 314/314；cloud typecheck✓ / acceptance 26/26 / 全量 329/329。<!-- aidcp-edge 6637c3b / aidcp-cloud 61af1b1 (本地已提交，待网络恢复推送) -->
    - **cloud 已部署**（06-24 核实）：ECS 现役 = cloud master HEAD `c7abc67`——并发会话 11:53 部署 account-persona-config 时已 co-ship 本通知修复（f912f75+61af1b1）+ persona + 迁移 0011。本会话备份后正要 rsync 时发现 ECS hash 已==c7abc67、服务 11:53 启动于三 commit 之后 → 停手不重复部署（错峰避免无谓生产重启）；healthcheck 全绿（active / 8787 ws / 飞书长连接 / PG 通 / persona_config 表存在 / isales-api·engine·scheduler 未碰）。
    - **剩余真机尾巴**（非 DOM 校准）：① 一条真实未读→完整巡视→飞书消息肉眼终验（**cloud 已就绪**，只需巡视期有真实未读跑一轮）；② CDP 断连「页面关闭」受控复现（kit 步骤 15-17，本次只做了 DOM 探针未做断连测试）。校准包 P-1~P-5 + 17 步清单见 `calibration-kit-6.5.4.md`。
