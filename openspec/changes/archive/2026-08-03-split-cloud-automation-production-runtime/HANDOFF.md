...
> **重整于 2026-08-01 晚，现状段刷新于 2026-08-03（收尾归档那一批）。**
>
> **一句话现状：门已清零（0 条），第 4 段与第 5 段全部完成，本 change 已收尾归档。**
> 细节见下面那段「现状」。**它不声称三进程能跑起来**——那属批次 5，判据见 §2 末与 tasks 5.3。
>
> **怎么读这份文档**：
...
# ① 六个 canonical checkout 必须都停在默认分支（控制仓 main，其余 master；CLAUDE.md §7 铁律）
...
> **2026-08-03 · 现状（这一段是当前事实，读它就够；逐批沿革在文末 §10）**
>
> ### 一、状态一览（数字以 §1 自检跑出来的为准，本表是快照）
>
> | 项 | 值 |
> | --- | --- |
> | 六仓 | **cloud `0d507c5`** / **api `f69bfe0`** / **automation `dc829c8`** / **content `039cd26`** / **kernel `e8396ce`** / **transport `346d0c8`** |
> | 工作区 | 六仓全干净、全已推、**对账零漂移**、两个共享包 pin 全对齐（本轮走了一遍完整级联） |
> | 测试 | **cloud 4121**（+11 skip）/ **api 505** / **automation 2170**（+3 skip，与基线同）/ **content 444** / kernel 70 / transport 36，全 0 fail |
> | 门（真交付物） | **0 条 —— 台账已清零**（`claimBlocked=false`）。六批撤条 **14 → 6 → 5 → 4 → 2 → 1 → 0** |
> | tasks.md | **94/148**（这把尺量的是「查清了多少」，不是交付；分母会随勘察长大） |
> | 边界 | 跨域边 0，豁免 0；**4a 方法槽 59**、组数 21 |
> | dev | **已跟到 cloud `0d507c5`**（2026-08-03 00:47 部署，tasks 5.4 有逐条记录）。本批现网真正变的只有一处：单体委托候选清单改走新的目录读，已在机器上真验过那条读的六列与数据 |
>
> **第 3 段（八批 A–H）与第 4 段全部落地，剩第 5 段收尾。** 四件里程碑：
>
> - **`main()` 写完了**（automation `0044881` 起）：schema 契约门 → 属主池 → 同步读镜像 →
>   组装根 → 十二个工厂 → 业务入口 → 启动外壳，三处真环用晚绑定薄壳破。
> - **门 14 → 0**：撤的**十二条**全属**真靠接线消掉**（另两条是撤的：一条重复计数、一条记错属主）。
> - **委托任务全链两端接通**：指令通道（tasks 4.1f）+ 执行器（tasks 4.1h）。
>   自动化侧建存储 / 服务 / 幂等台账 / 执行泵并注册两条路由，接口侧两个客户端合成 7+1、
>   `delegate` 真调它、卡片动作入口也拿到端口。
> - **可执行入口已切成真启动**（tasks 4.3，automation `dc829c8`）：读配置 → 过就绪闸 → 真装配；
>   **闸没删**，析出成 `assertAutomationRootReady` 纯函数，由用例喂非空台账独立验。
>
> ---
>
> ### 二、下一步：门清零之后剩什么 —— **已全部做完（2026-08-03 收尾）**
>
> **第 4 段与第 5 段都完成，change 已归档。** 收尾那一批实测：
> 六仓 typecheck 全干净、全量零 fail（cloud 4126 / api 507 / automation 2171 / content 444 /
> kernel 70 / transport 36）；acceptance 185/185；跨域边 / 跨层读 / 跨层写 / 豁免全 0；
> 六仓对账零漂移、两个共享包 pin 全对齐。
>
> **收尾时踩到的一件事值得记**：接手时主干已被两路**并发 change** 推进两个提交，派生仓因此漂了
> 3 个 src 文件 —— **派生仓的漂移不会自己冒头，只有跑一次对账才看得见**，而漂着的时候
> 六仓测试各自照样全绿。收尾前 MUST 跑一次 `sync-split-repos`，别默认「上一批同步过就还齐着」。
>
> **归档前补登了两条只活在 tasks.md 注释里的债**（排期发帖没进程在跑 4.1g + `schedule-feedback`
> 没人服务 3.5j，两条是同一件事的两半）。**归档会把 tasks.md 挪进 archive 目录，
> 注释里的债从此没有任何机制会提醒人** ⇒ 归档前把未了的债搬去 backlog，是收尾的必要动作、不是走过场。
>
> **第 4 段完成。** 台账 0 条、入口已从 fail-closed 切成真启动、闸仍在（析出成纯函数，
> 由用例喂非空台账独立验）。**收工判据（门清零）已满足。**
>
> **⚠️ 交付口径，MUST NOT 读大**：清零的意思是「本包不再缺任何跨属主依赖，且入口能真调起装配」。
> **不等于三进程跑通** —— `runAutomationMain()` 要真属主库（风控写者锁构造期就抢），
> 本地桩验不了，**一次都没真跑过**。分层验收口径见 §2 末与 tasks 5.3，本 change 从头到尾不声称它。
>
> 余下的是第 5 段（派生对账、验收、收尾）：
>
> | 项 | 状态 |
> | --- | --- |
> | 5.1 / 5.2 六仓同步 + 逐仓 typecheck + 全量测试 | **完成**（同步 api `81dbc30` / automation `9371d59`；六仓全绿零 fail、零漂移） |
> | 5.3 分层验收如实分开记 | **完成**，三层口径写死在 tasks 5.3 的注释里 |
> | 5.4 dev 部署 | **完成**：本 change 足迹停在 cloud `0d507c5`（已部署并验过）；上机实测 dev 现已被另一路 session 部到更新批次，服务 active、三口在听、error 0 |
> | 5.5 真机项登记 backlog | **完成**：簇 60 原有四条 + 本轮补登 4.1g / 3.5j 两条 |
>
> **三进程真跑要先解决的两件已知事**（都已登记、都不阻塞门清零）：
> ① **排期发帖今天没有任何进程在跑**——排期 tick 那个类属 api，而 api 的手写入口没建它（tasks 4.1g）；
> ② **四类限频配置在三进程形态下不热加载**（tasks 3.5h）：启动读一次之后再不刷新，
>    现象是「后台改了配额，自动化进程按旧值跑且不报错」。**别直接搬单体那个刷新器**，先裁 3.5h 那三件。
>
> ### 三、第 4 段做完之后仍然成立的几条判据
>
> - **两条台账条目必须一起撤**：它们指向同一个 7+1 端口，注入那一个端口就同时点亮自由文本入口
>   与卡片按钮，分开撤只会留下一条假欠账。
> - **两个目标校验钩子是实质、不是附赠**：省掉它们路由照通、卡片照发，等真去执行时才发现目标不对。
> - **精选目标校验必须走受鉴权那条读**：裸那条的客户端不做按码还原，跨进程后对面的缺表错误
>   只剩一个普通传输错误，于是「库暂时不可用」被如实报成「目标不存在」——编译期与测试都看不见。
> - **探针这次自熄**：api 手写入口里那句 `automation_operator_command_unavailable:delegate`
>   整句消失了（改成真调客户端），所以不必先裁定「探针分不出没通道 / 没配通道」。
>   **能这么办的就这么办**，别把台账挂在一句仍然合法存在的字符串上。
> - **诚实的坎照旧在**：飞书 `/delegate` 那条链**一次都没真跑过**（要真发一条飞书消息才触发），
>   已按 5.5 登记 backlog 簇 60。
> - **「闸恒真通过」＝「闸消失了」**（台账清零那一批的判例）：判断一旦在生产路径上不可达，
>   它还在不在就**没有任何机械手段能证明**。⇒ 这类闸 MUST 析出成能被单独喂输入的纯函数，
>   并配一条直接喂它的用例；**MUST NOT 因为「现在恒真」就把用例删掉**。
> - **空数组 + `as const` 会把元素类型收成 `never`**，于是每个读字段的地方当场编译不过。
>   名单类常量清零时改成显式注解 —— `satisfies` 本来也只校验写下的那些条目，注解一样管得住。
> - **令牌也会「只有真跑两个进程才现形」**（2026-08-03 实撞并已修）：接口进程给发布授权那两组路由
>   挂的是 `AIDCP_PUBLISH_APPROVAL_INTERNAL_TOKEN`，与通用 api 令牌**是两个 env、不互相回落**。
>   拿错了每一次授权读写都被判未授权，而编译过、两仓测试各自全绿。
>   ⇒ **新接一个跨进程客户端时，除了「对面注册了这条路由吗」，还要问「对面给它挂的是哪个令牌」。**
> - **新增一个必填 env，要同时给它配一条「缺了就拦停」的用例**：本轮之前只有 content 那一个有，
>   于是新加的 env 会悄悄变成可缺省，等真跑起来才以别的形态现形。
>
> ---
>
> ### 四、跨进程接线的头号坑：**派生仓入口服务的路由 ≠ 单体服务的路由**
>
...
> | automation | 四条运营指令路由**一条都没注册** | 调度启停 `2f5f6a9` + 委托两条 `6f1a544` 已接；手动发布 / 手动评论按 1.7b 裁定**刻意不接** |
> | api | 四条运营指令**一个客户端都没建**（一律 `throw …unavailable:*`） | 调度启停 `3159e10` + 委托两条 `f69bfe0` 已接；余两条同上 |
> | automation | **令牌**拿错（不是路由缺）：发布授权那一族用了通用 api 令牌 | 已修（`c2c35c0`）+ 结构断言；见「现状」三最后两条 |
>
...
> - **dev 与 cloud 主干头一致（`0d507c5`）**。判据始终是「当前 cloud 主干头 vs dev 上跑的那个」，
>   别拿本文里的 sha 当现状，也别按「本轮谁改了什么」推断 —— 只动派生仓的批次与 dev 无关。
>   **查现状的办法是去机器上 grep 一个本批特有的符号**（本轮用的是 `listAccountDirectory`），
>   机器上没有 git 工作树、也没有版本标记文件，`systemctl` 说 active 更不构成「跑的是哪一版」。
...
>
> ### ⚠️ 手抄件：动 4a 清单要改**六处**，同步脚本一处都不管（2026-08-02 各咬一口）
>
> 加一条 api 属主口（组数 / 方法槽变化）时，机械普查那套计数散在六处，**逐处手改**
（2026-08-03 委托那批又走了一遍，六处全中）：
...
>
> ⑥ 中控 `test/transport/api-direct-inventory.test.ts`（**两个测试名 + 三处写死的槽数**）。
>
> **同步脚本只管 `src/`**，④ ⑤ 两处它一个字都不碰。
> **⑥ 是 2026-08-03 补进来的，此前这份清单只列五处**：前五处逐处改完、逐个文件单测全绿，
> **只有跑全量才红** —— 也就是说，按老清单办事的人会以为已经改完了。
---
...
| --- | --- | --- |
| tasks.md 条数 | 94/148 | **「查清了多少」，不是「交付了多少」。** 分母会随勘察长大——最近几批往里加了十来条实测发现的新任务 |
| **那张清单（门）** | **0 条（已清零）** | **真正的交付物。** 14 → 0：**头两条是撤的、不是干活减的**（一条被算了两遍 1.7b，一条记错了属主 2.9）；**其余十二条都是真靠接线消掉的** |

**收工的判据是门清零，不是 tasks.md 打完勾。** 两者不是同一把尺，别拿后者当进度终点。
**2026-08-03：门已清零**，tasks.md 仍有 54 条未勾 —— 那正是这两把尺不同的证明，别据此以为还没做完。
...
## 3. 门：**0 条，已清零**（当前值以 §1 第 ④ 条跑出来的为准）

`claimBlocked=false`。最后两条于 2026-08-03 清掉：`content-generic-llm-authority`
（生成链客户端 → 发帖触发器 → 委托执行器，automation `d26205d`）与
`automation-production-runtime-composition-unwired`（入口切真启动，automation `dc829c8`）。
**清零 ≠ 三进程跑通**，口径见文首「现状」二。

**已撤的十二条**（全属真靠接线消掉）：`content-role-factories`（07-31）、概念池写 / 精选写 /
FB 发帖素材 / 用量记账 / 回复生成（08-04 `5fe3f97`）、调度启停（08-04 `30d2cc3`）、
文字卡转写（08-04 `8d67f5a`）、**委托自由文本 + 委托卡片动作**（08-03 automation `6f1a544`
+ api `f69bfe0`，两条必须一起撤：同一个 7+1 端口）。
**逐条的理由写在 `aidcp-automation/src/automation-composition-root.ts` 的原地注释里**——
那份台账是 Cloud 普查的永久手写分叉、拿不到任何机械信号，所以理由不放在这里。

**顺序是硬的**：内容那条不清完，组装那条写不了。

> ### ⚠️ 指令那两条为什么「接完了也不一定能清」（tasks 1.3b）——**已过期，保留供追溯**
>
> **实际结局是探针自熄**：接线时 api 手写入口里那句 `automation_operator_command_unavailable:delegate`
> 整句消失了（改成真调客户端），与调度启停那条同形，所以**不必先裁定**。下面这段留作方法论。
...
- **幂等键里塞了 `randomUUID()` —— 仍是活缺口**（2026-08-03 复核）。当前 api 手写入口里有两处随机：
  - `src/server.ts` 的 `api-feishu-resume:<account>:<randomUUID()>` —— **这处是那个缺陷本身**：
    每次重试新随机一个，幂等键形同虚设。**未修。**
  - 同文件 `dispatch` 那处的 `commandId: randomUUID()` —— **这处是对的**，别顺手一起改：
    一次运营动作 = 一把新键，传输层重试沿用同一把（幂等由接收方按键判）。
  - **委托那条不在此列**：它的键由飞书消息 id 算出（`operatorCommandId`），拿不到就拒发、
    不用随机兜底，且有结构断言钉着（tasks 4.1f）。
...
- **`4.1g`（2026-08-03 新增）—— 排期发帖今天没有任何进程在跑**：排期 tick（`ContentScheduler`）
  在归属表里是 **api**，automation 仓里根本没有这个类，**而 api 的手写入口也没建它**。
  不阻塞门清零（门问的是「本包缺不缺跨属主依赖」），但**三进程真跑之前必须有结论**。
- **`4.1h`（2026-08-03 新增）—— 委托任务的执行侧此前是静默缺席**：能建、能确认、没有任何东西会执行，
  且连单体那句具名警告都没有。**已修**（执行泵 + 两种缺席各一句具名警告）。
  留在这里是因为那个形态值得记：**「队列里暂时没任务」与「永远不会有人来处理」在外部完全同形。**
...
> 第 3 段与第 4 段也都做完了（门已清零）。当前的下一步在文首「现状」二。**
