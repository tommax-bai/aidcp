...
- Native `note.open` 保留首帖选择参数，并在同一群讨论流内执行有界视口下滚与探测；Cloud 区分未找到、编辑器未就绪和上下文读取超时，不再把所有失败统一显示为“未找到帖子”。
