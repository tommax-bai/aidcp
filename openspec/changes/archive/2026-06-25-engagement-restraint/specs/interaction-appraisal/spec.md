...
**Reason**: 收紧自动互动以降低每日配额消耗、更拟人——点赞由「低门槛高频」翻转为「选择性」。
**Migration**: 由新需求「点赞是选择性互动、收藏是更稀有的选择性互动」替代（保留「collect 不比 like 易命中」「倾向 both」「与 soul 注入标准一致」三项约束，仅把 like 的门槛由低改高）。「收藏即点赞（配额允许时收藏同时点赞）」需求不受影响、保持不变。
