...
- [x] 13.2 真机项归并入 backlog：12.1–12.8 已登记 `docs/real-machine-acceptance-backlog.md` **簇 85**（A/B-F 合验/C/D-E/提交后被抢不重复/G-FB评论/H逃生梯）
...
- [x] 13.4 archive（用户 2026-07-15 拍板「归档吧」）：代码两半均在各自 master、cloud 已部署 dev、edge 主目录已更新。**归档不等于真机签收**——dev 客户端重启加载批 B 后的真机验收（簇 85）与延后清单（7.7/7.9/7.10b/7.11d/5.6/windowRemainingMs）仍在 backlog 追踪，落地时另开 change 或直接补。spec delta（RENAMED + MODIFIED×2 + ADDED×5）随归档并入主 spec。
