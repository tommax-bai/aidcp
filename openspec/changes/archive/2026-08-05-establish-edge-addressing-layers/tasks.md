...
- [x] 6.1 `openspec validate establish-edge-addressing-layers --strict` 通过
  <!-- 通过 -->
- [x] 6.2 自检：把四条判据套到一个文中未列出的边缘职责上，确认能得出明确结论（验证判据可用，不只是可读）
  <!-- 量了两个文中未写的职责：① 拟人节奏抖动层——四条全不中本地 ⇒ 账号层，与现网「云端算中心值、边缘只叠抖动」一致；
       ② 执行权协调器——①②④中、③不中 ⇒ 按 §3.1 拆开：本机仲裁归本地、跨机器写者协调归账号层，与现网「边缘协调器 + 云端租约客户端」一致。
       判据独立复现了两个早已独立做出的设计决定 ⇒ 可用，不只是可读 -->
- [x] 6.3 提交并推送控制仓（本 change 无代码改动、无部署、无打包）
  <!-- aidcp 9b3c7698 已推送 origin/main。落盘仅两处：新建 docs/edge-addressing-layers.md、docs/architecture.md 加 5 行指针 -->
