...
- [x] 3.3 **现存 5 条区域配置仍是 13 段碎片**（全国 / 北宁 / 北宁北江 / 北江 / 河南，内容相同），不合并仍随机发单行。
      两条路径任选：① 运营在后台把整段广告重新粘一次保存（新分隔符下即为一条）；② 直接改库合并
      （本次尝试被安全策略拦下，需用户明确授权）。已登记为真机 backlog **簇 117.1**，是该簇其余条目的前置。 <!-- backlog 簇 117 -->

## 4. 验收（已解耦到真机 backlog）

- [x] 4.1 回填不拆行、发出整段、换行进评论、带电话模板不再被拒、生成式那条路没被带松、长度余量，
      共 6 条已登记为 `docs/real-machine-acceptance-backlog.md` **簇 117**。归档不 gate 在真机上。 <!-- backlog 簇 117 -->
