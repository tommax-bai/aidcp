...
- 自动入册后，添加环境页面从主进程已落盘花名册同步本地视图，立即并持续显示“已加入”；不得只更新左侧环境栏而让添加页保持旧状态。
- 对由本创建意图产生、尚未首次登录因而没有视频号互动绑定的环境，Cloud 允许在验证创建来源与当前归属后直接完成权威解绑；普通管理员分配环境缺绑定仍 fail closed。
...
- `aidcp-cloud`: customer-auth 新增创建意图/完成端点；`ClientUserStore` 新增幂等、事务化的“登记 + 唯一 owner”单写方法、审计与过期意图存储，并为可证明从未绑定的创建环境提供终态解绑；测试覆盖双客户冲突、重放、未知/过期意图、未绑定删除和旧自绑定路由拒绝。
- `aidcp-edge`: `ads:createEnv` 在主进程内协调创建意图、本地建号、Cloud 完成归属、权威范围刷新和花名册落盘；renderer 仅消费结果，不得提交或改写待归属的 `envKey`，并在主进程入册后同步本地花名册以刷新“已加入”标记。
