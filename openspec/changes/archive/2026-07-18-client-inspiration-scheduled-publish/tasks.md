...
- [x] 2.1 Extend `PublishApprovalActionPayload` with optional publish mode/time while preserving old-client current-plan behavior and rejection semantics.
  <!-- aidcp-cloud 7088b96 + aidcp-edge 99821e6; protocol blocks compared identical and AC-PROTO-19 passed in both repos. -->
...
- [x] 3.1 Add narrow preload/main IPC methods for pending-draft list/detail reads with fixed customer-auth paths, pagination/id validation, main-owned envKey/token, and security tests.
  <!-- aidcp-edge 99821e6; content-workspace IPC security tests passed. -->
- [x] 3.2 Upgrade the draft review page to a paginated inspiration-style card list plus detail view, with single-item fast path, honest empty/error/fallback states, navigation restoration, handled-item filtering, and account-switch stale-response invalidation.
  <!-- aidcp-edge 99821e6; companion jsdom coverage passed for multi-item cards, single fallback, continuation, and stale account responses. -->
- [x] 3.3 Add immediate/scheduled controls at the approval position with explicit Asia/Shanghai parsing, local range feedback, cancel independence, busy/error behavior, and multi-item continuation after success.
  <!-- aidcp-edge 99821e6; pure timezone/range tests and renderer interaction tests passed. -->
- [x] 3.4 Extend Edge approval IPC validation/payload forwarding and add renderer tests for multi-draft reachability, schedule submissions, invalid-time blocking, failure retention, and old Cloud/single-preview fallback.
  <!-- aidcp-edge 99821e6; IPC source-contract, companion UI, and legacy fallback tests passed. -->
...
- [x] 4.1 Synchronize Cloud/Edge `PublishApprovalActionPayload` and result version fields, update protocol contract tests and `docs/protocol.md`, and verify the protocol blocks remain identical.
  <!-- aidcp-cloud 7088b96 + aidcp-edge 99821e6 + aidcp ed0e484; Compare-Object returned no protocol diff. -->
- [x] 4.2 Run focused Cloud/Edge tests, acceptance suites, full suites, typechecks, and any relevant syntax checks without packaging Electron.
  <!-- Cloud: 2507 pass/8 gated skip; Edge after rebase: 1799 pass; both acceptance and typecheck pass; node --check pass. Windows deviation: quoted Cloud npm test discovers 0, so full suite ran via npx tsx --test test/**/*.test.ts. No Electron package built. -->
- [x] 4.3 Run `openspec validate client-inspiration-scheduled-publish --strict`, record commit SHAs/validation evidence, integrate and push with repository helpers, deploy Cloud only from the eligible canonical default checkout, and do not build an Electron installer without explicit user request.
  <!-- Strict validation passed. scripts/land-change integrated and pushed aidcp-cloud 7088b96 and aidcp-edge 99821e6 to their default branches. Clean canonical aidcp-cloud master 7088b96 deployed to dev 121.89.85.150 after exact nine-file checksum scope review; backups: /opt/aidcp/cloud.bak.20260718-202538.tar.gz and /opt/aidcp/cloud/.env.bak.20260718-202538. Health: service active, NRestarts=0, ports 8787/8090 ready, PostgreSQL ready, client-auth unauthorized probe returned 401, Feishu ready, deployed SHA-256 manifest verified. No Electron installer built. -->
