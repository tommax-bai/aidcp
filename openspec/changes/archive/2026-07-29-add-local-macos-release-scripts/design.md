...
6. 仅公证模式要求 `APPLE_API_KEY`、`APPLE_API_KEY_ID`、`APPLE_API_ISSUER`，并严格按“签名 App → 公证/staple App → 从该 App 生成 DMG → 公证/staple DMG”串行执行；仅签名模式跳过公证凭据时必须成功返回并继续构建。
...
9. checkout 门禁拒绝所有已跟踪改动，并拒绝 `src/`、`native/`、`scripts/` 下可能进入构建图的未跟踪源码；根目录临时探针、历史 `dist-electron.backup-*` 等不在打包输入中的未跟踪文件只提示、不阻塞。这样既保证产物来源可审计，也不违反保留无关本地文件的工作区约束。
...
- [无关未跟踪文件可能长期积累] → 脚本打印其数量并继续；是否整理由文件所有者决定，脚本不删除、不移动这些文件。
