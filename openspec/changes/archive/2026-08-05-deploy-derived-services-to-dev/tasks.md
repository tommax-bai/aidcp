...
- [x] 2.5 三仓本地各起一次（连 dev 的属主库、周期任务全关），确认起得来、健康口答得上。
  **本地只做这一步验证，正式运行仍只在 ECS。**
  ...
  <!-- 2026-08-05 **结案 = 那半边被更强的事实取代，不是补做了**。
       「连 dev 属主库在本机起一遍」这半边**至今没做、也不该做**（理由同上：抢按 target 单实例的
       自动化写者锁），而它想证明的事——三个入口起得来、健康口答得上——现在有一份严格更强的证据：
       **dev 与 OL 两台各跑着这三个服务，最短的也已连续 active、NRestarts=0 超过一天**，
       期间还各扛过一次真部署与重启（能力名修复、面板能力）。
       ⚠️ MUST NOT 把本条读成「本机跑通过」：本机从来没连过 dev 的属主库。
       它现在的含义是「这一步的目的已由部署位上的长期运行达成，本机复现没有额外信息量」。 -->
...
- [x] 5.3 顺序实测一次反例（在**非生产**位置）：迁移未应用就重启 ⇒ 确认契约门使进程启动失败，
  ...
  <!-- 2026-08-05 **反例这次真跑了，第二半不成立 —— 是缺陷，不是没验。本条继续不勾。**
       ...
       **修复已落地**（见 5.3a），本条随之勾上。**证据链是两段分别实测、拼接处是机械的**，
       这一点必须写清楚，别读成「整条链在一台机器上从头跑通了」：
       · 段一「门拒启 ⇒ 退出码 1」：真账本超前条件下的真拒启，OL journal 逐次 `status=1/FAILURE`
         （另有 08-04 本机指向不可达库那次）。
       · 段二「退出码 1 ⇒ 有界次数内进 failed、不无声循环」：dev 一次性探针，用的就是**修好之后
         的那份配置**（2min / 3），计数停在 3、状态 failed。
       · 拼接为什么成立：systemd 的重启限速**不看进程为什么退出**，只看退出码与频次；
         且窗口从 10s 抬到 2min 之后，**任何崩溃周期短于约 60s 的服务都必然在窗口内攒够 3 次**，
         于是结论对「起崩耗时多久」不敏感——这正是原配置失效的那个变量被消掉了。
       **没有为了凑这条链去把哪台真服务弄崩**：真造一次账本超前要么改共库、要么停一个在跑的接口面，
       代价与收益不成比例。 -->
- [x] 5.3a **把 `StartLimitIntervalSec` / `StartLimitBurst` 从 `[Service]` 挪进 `[Unit]`**
  ...
  <!-- 2026-08-05 21:51 两台都已落地（用户当场授权改 OL）。改法是幂等的：`[Unit]` 段里已有就跳过。
       备份 dev `*.service.bak.20260805-215145`、OL `*.service.bak.20260805-215204`（各三个）。
       落地后逐台实测：六个 unit 全部 `StartLimitIntervalUSec=2min` / `StartLimitBurst=3`，
       `systemd-analyze verify` 不再报 `Unknown key name`，三服务 active / NRestarts=0 不变；
       OL 上单体仍 inactive/disabled、六个口照常在听。
       **dev 与 OL 的 isales 一个 unit 都没碰**（改后逐条列过：四个服务全 active running）。 -->


## 6. 起三个服务，逐条核对跨进程

- [x] 6.1 三个服务起来，各自健康口答得上。记录：**只声称"三个进程起来了"**，
  ...
  <!-- 2026-08-05 勾选：上面那次演练留下的两处「没验到」**都已被真切流补上**，两个环境各一次
       （dev 08-04 12:07、OL 08-05 12:47），且本 session 当天各复测一次：
       三服务 active / NRestarts=0、六端口全在（8787 边-云 / 8090 面板 / 8091 客户鉴权 /
       8092 内容 / 8093 接口 / 8094 自动化）、就绪口答 `state=ready` `blockers=[]`、
       单体 inactive/disabled。演练当天缺的两条现在都成立：api 的面板与客户鉴权对外口已答
       （见 6.1b），automation 的业务入口已放行、8787 真在监听、OL 上 23 条边缘自行重连。
       **本条的口径纪律照旧**：这里只声称「三个进程起来了、各自健康口答得上」；
       逐条链路仍分开在 6.2 / 6.2a / 6.2b / 6.1b 里各自声明，**MUST NOT 读成「三条链路都验过了」**。 -->
...
- [x] 7.1 派生服务保持运行，**随时可读**地记录周期任务的真实行为：醒了几次、每次判定是什么、原因是什么。
  **"一次都没醒"与"醒了但每次都判跳过"是两件事**，要分得开——两者在"进程健康"这个维度上完全同形。
  ...
  <!-- 观察记录第 2 条 / **收官**（2026-08-05 17:00，用户裁定「观察期结束」）。
       做法：拿 §0.3 那份**单体 15 项周期任务基线**当分母，逐项去 dev 的三个派生服务里找它的落点，
       **0 命中的一律往下挖到「谁接了 / 起没起 / 有没有在做事」**——这正是本组要的那件事。

       **落点已确认在跑的 11 项**（近 6 小时日志实测命中数）：
       · 配置镜像刷新器（5000ms）→ **api**，`[config-mirror] 刷新器已启动 T_poll=5000ms target=dev`
       · 配置镜像失效信号中继（2000ms 兜底）→ **automation**，`topic=config_mirror.bump target=dev`
         （两者合计 312 条）
       · 内容排期心跳（每分钟）→ **api**，`[content-scheduler] 已启动（每分钟心跳）`，92 条
       · `llm_token_usage` 保留清理（24h/45d）→ **content**
       · `interaction_feed`（24h/30d）/ `risk_counters`（24h/7d）保留清理 → **automation**（三者合计 26 条）
       · 风控记账 outbox（启动回收在途行=0）/ 风控计数对账 → **automation**，各 8 条，
         且对账已按 `scope-risk-reconcile-to-owned-accounts` 收窄成「范围=归属为 dev 的账号」
       · 互动配置面审计中继 / 角色模型镜像 / 各 Store 就绪 → automation & content 自报齐全
       · `PublishScheduler` → 2 条

       **挖出来两件，都是「日志上看不出来」那一类，正是本组存在的理由**：

       ① **委托任务执行泵在派生里起了、但一句话都不说。** 单体启动打
          `DelegatedTaskWorker 已启动（automatic priority；并发=N）`；派生**只在被 env 显式关掉时**才打一句
          `已按配置禁用`。dev 上两句都没有 ⇒ 只能反推「它起了」。
          **用事实源正过来了**：`delegated_tasks` 表近 24h 有 completed 2 / failed 4 ⇒ 确实在认领并执行。
          但「起了」与「没起」在日志上同形，**下次真没起时没人会发现** —— 已在 8.1 登记补一条启动自述。

       ② **`event_outbox` 有一个主题只进不出。** dev 上该表 **139,052 行**、最老 2026-07-26。
          按主题拆开：`sync_read.changed` **139,049 行**（占 99.998%），`config_mirror.bump` 17，`risk.command` 7。
          剪裁器本身**是接了的**（在 `automation-risk-accounting.ts` 里随风控记账一起构造并 `start()`，
          只是同样不打启动日志），但它的主题名单
          （`automationOutboxRetentionTopics`）**只有 `panel.event` 与 `risk.command` 两条，
          没有 `sync_read.changed`** ⇒ 该主题在**共用的生产库**上无界增长，约 14k 行/天。
          ⚠️ **有意不在本 change 顺手修**：这个主题的消费方是带游标的同步读中继，
          按年龄裸剪会删掉尚未投递的行，而本仓已因「同游标不同载荷」栽过
          （拒收永久 + 启动期 fail-closed → 整机起不来）。要按「等消费者游标推进」的形态做，属独立 change。已在 8.1 登记。

       **本条到此收口，口径照旧**：以上是**观察到的事实**，
       **MUST NOT 据此给 7.2 的任何一条真机验收项划勾**，也 MUST NOT 概括成「三进程已验证」。 -->
- [x] 7.2 逐条走 `docs/real-machine-acceptance-backlog.md` 簇 60 的验收项，每条记三态之一：
  **已验（附证据）/ 未验（附为什么没观察到）/ 不适用（附理由）**。
  重点是上一个 change 登记的 5 条 + 既有的 `runAutomationMain` 从未真跑、飞书 `/delegate` 从未真跑。
  <!-- 2026-08-05 结案，**依据是用户声明「我在持续验证」**（用户当日原话），本项不再阻塞本 change。
       ⚠️ **证据源必须写清楚：这是用户自验，不是本 session 观察到的。**
       本 session 对这些验收项**没有**任何直接观察，MUST NOT 把本条读成「已有证据在案」——
       它的准确含义是「验收责任已由用户接手并在进行中」。
       7.3 / 7.4 那两条纪律**依然全额有效**：谁要引用本条，只能引到「用户在验」这一步，
       MUST NOT 据此推导出任何一条具体功能「已验证通过」。
       backlog 簇 60（5 条）与簇 132（78 条）原样保留在册、一条都没删——
       真机清单是**登记表**不是任务表，删掉就再也没人知道当初要验什么。已在两簇顶部就地注明持续验证中。 -->
- [x] 7.3 **MUST NOT 用"进程一直活着"给任何一条验收项划勾**。没观察到就是未验，
  它照旧留在 backlog 里等下一次机会，MUST NOT 因为运行时长够久就默认它通过了。
- [x] 7.4 **禁止出现"三进程已验证"这类概括**。概括性的验收结论会以最快速度过期——
  本 change 立项时就当场推翻了一条这样的记载。
- [x] 7.5 **6.2b（能力名漏 `_v1`）的行为验证仍未拿到，本条是它的承接人。**
  验收判据：**有账号带着未用满的浏览配额真跑到 Reel 上**之后，① 日志里不再出现
  `facebook_reel_follow_edge_capability_missing`，且 ② 至少观察到一次 Reel 关注**真被下发**。
  两条都要，只有 ① 不够——「不再报缺失」可能只是因为评估器压根没跑。
  <!-- 2026-08-05 6.2b 的修复已部署 dev 与 OL，但复查那 5 分钟里 `interaction_appraiser` 一次都没跑
       （17 个账号今日 view 配额已用满、在睡），所以「0 条能力缺失日志」**分母是 0、不构成证据**。
       6.2b 本身记的是「代码已改 + 已部署」，它已勾选；**行为验证不能挂在一条已勾选任务的注释里**，
       故在此单列未勾任务，并同步登记 backlog 簇 60（2026-08-05 增补）。 -->
  <!-- 2026-08-05 21:5x **按 2026-07-30 裁定清账勾上——这不是「已验证」，是「不再阻塞归档」。**
       真正的承接人是 **backlog 簇 60 的 2026-08-05 增补那一条**，它不随本 change 归档而消失；
       本条今天单列出来的唯一目的就是「别让这个义务只活在一条已勾任务的注释里」，
       那个目的由 backlog 那条达成，本条勾不勾都不影响。
       ⚠️ **MUST NOT 引用本条推导出「Reel 关注已经在下发了」**：至今没有任何一次观测。
       判据仍是那两条都要——有账号带配额跑到 Reel 上之后不再报能力缺失 **且** 真下发过一次。 -->
...
- [x] 8.1 把 soak 里暴露但本批不修的问题**逐条**登记（backlog 或新 change），
  ...
  ④ **同步读就绪度仍在抖** —— ~~未修~~ **2026-08-05 已由 change bound-event-outbox-growth 收掉**
     （aidcp-api beeeb84）。真因是**消费侧**那一半没跟着改：属主重发周期改成了新鲜期的三分之一，
     但接口进程的周期全量刷新默认值仍**恰好等于**新鲜期窗口（都 30s），于是每个周期末尾照旧留一段
     「刚过期、还没重新取到」的空窗。两条闸要各守一半：属主多久重发一次 / 消费方多久取一次。
     现按同比例压到 10s，并加了跨仓漂移守卫（窗口是另一个仓的常量，本仓的比例断言钉不住它）。
     以下是当时的记载，保留原文：
     把重发周期改成新鲜期的三分之一之后业务入口能开了（一次性闩），
...
- [x] 8.2 更新 backlog 簇 60：本批真验到的划掉并附证据，没验到的留着并补上"为什么没覆盖到"。
  <!-- 2026-08-05 结账写在 `docs/real-machine-acceptance-backlog.md` 簇 60 的「2026-08-05 补」块里。
       划掉三条（边缘真跑浏览 / 周期任务 / 跨进程路由），各自附了证据；
       留着两条并写清为什么（`runAutomationMain` 与飞书 `/delegate` 要真实运营触发；
       契约门那条链是「两段实测 + 一个机械推理」，不是端到端观测）。
       ⚠️ 周期任务那条是**部分**划掉：11/15 确认在跑，余下 4 项是「没观察到它醒」而非「验过是好的」，
       已就地写明别读进「已验」。 -->
