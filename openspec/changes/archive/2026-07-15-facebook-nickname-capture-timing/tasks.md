...
- [x] 3.5 全部完成 → archive（真机验收簇 42 解耦、不阻归档）。<!-- aidcp <sha> -->

> 真机验收（簇 42）为唯一未闭环项，已解耦到 `docs/real-machine-acceptance-backlog.md`——需运营机重建 edge + 真实 FB 号，本机/单测无法确认。code landed + deployed dev + 单测/集成全绿即归档（项目既定节奏）。
