...
- **品类判定（独立 flash 角色）**：新增一个**发布侧品类判定角色**——输入 = 固定品类枚举 + 正文，flash 模型只输出一个 `category`（schema 约束 + 校验重试；判不出回落安全兜底档、绝不 brick）。**一帖判一次**、写进管线状态，配图选题与质量评审复用同一值（守「帖内锁定/帖内一致」）。选它而非让配图选题「搭车分类」，是因为**单一职责的分类比双任务更准**；发布低频，flash 成本/延迟可忽略；且能登记进角色目录供后台单独配模型。**仅发布侧**——浏览侧不给逐条外部笔记加分类角色（高频热路径，门槛用赞藏比例 + 人设即可）。
- **时序**：该角色跑在**正文写完之后**（读正文才准），故只服务配图与质量评审；感叹号分档发生在「写正文之前」、拿不到内容品类，改挂**人设**（见 D7 / 对应任务）。
- **注入时机**：`ImagePromptComposer` 取 `profile.styleBase` 逐字拼进本帖图 1..N、图 0 用 `coverStyleBase`。**不变量从「全局逐字一致」收敛为「帖内逐字一致」**——图集一致完全保留，帖间因品类而异。
- **备选**：① 让 LLM 每张自由产风格（否决：破帖内一致、且 LLM 产风格漂移不可控）；② 保留全局常量但按 persona 换（否决：persona 与内容品类不总一致，正文品类信号更直接）；③ 让配图选题角色搭车分类（否决：双任务互相稀释、分类不稳；独立 flash 角色更准且可后台单配模型）。风格 DNA 仍是模板常量、仍禁止 LLM 产（延续现红线），只是「一个串」变「一张表 + 一次解析」。
