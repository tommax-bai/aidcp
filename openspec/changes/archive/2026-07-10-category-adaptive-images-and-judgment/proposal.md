...
- **配图真人 / 封面文字分级**（保守口径，用户已定）。取代一刀切「无真人 / 无文字」：默认无脸 / 局部 / 背影，需正脸用「明确非写实虚拟人物」、不主动请求写实真人正脸；封面默认留白由后期程序化叠字。已上线范围为 prompt-level 防线与合规元数据标识；真正看生成图的产后视觉校验已拆到 `publish-image-postgen-safety-validation`，避免用占位规则冒充完成。
...
- **aidcp-cloud（`src/publish-agent/`）**：`prompts.ts`（删 `IMAGE_STYLE_BASE`、加 `STYLE_PROFILES` 注册表 + `resolveStyleProfile`、新增 `buildCategoryClassifierPrompt`、`buildImagePromptComposerPrompt` 接品类与中文主体、`buildAssemblerPrompt` 接 soul + 品类维度、`buildCreatorPrompt` 感叹号分档、`BANNED_PHRASES` 校准）、**新增品类判定角色**（发布侧 flash、输入品类枚举 + 正文、输出单个 category、一帖判一次、登记角色目录供后台配模型）+ `role-catalog.ts`/`server.ts`/`roles/index.ts` 装配接线（与并发方 WIP 同文件、须协调排期）、`roles/image-set-planner.ts`（不再分类、专注配图选题）、`roles/image-prompt-composer.ts`（取档逐字注入、图0封面变体）、`roles/quality-scorer.ts`（extractInput 取 soul）、`seedream-client.ts` + `wanxiang-client.ts`（去第二风格源、竖版尺寸）、`post-processor.ts`（感叹号/禁用词检测接品类参数）、`types.ts`（`ImageSetPlan.category`、`StyleProfile`）、`prompts-preview.ts`（改签名处补 `EXAMPLE_SOUL`/style，保与线上同源）。
