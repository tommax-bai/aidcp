...
- 客户端所有“当前环境 / 当前账号”身份锚点统一通过一个主进程与 renderer 共用、返回名字与来源的解析器取显示名；共享文件使用明确的 `.cjs` 边界，保证根目录 `type: module` 下 Electron 主进程仍可同步加载，避免标题栏、内容工作区或浏览器内提示绕过人工优先级或阻断客户端启动。
- 昵称数据修改在当前页面即时乐观显示并标记保存中；本地持久化失败时恢复原昵称与来源并提示真实原因，不把本地设置写入伪装成云端更新。
...
- `aidcp-edge` Electron shared/renderer：明确 CommonJS 边界的环境显示名纯解析器、花名册归一、保存、刷新回填、左栏渲染与交互样式。
- `aidcp-edge` Electron tests：增加使用项目 Electron 内置 Node、且不经过 `tsx` loader 的 CommonJS 启动冒烟。
