...
- FROM: `### Requirement: Slow-render observations SHALL fail the current join attempt without cooldown`
- TO: `### Requirement: Slow-render observations SHALL receive one bounded no-click recovery before terminal failure`
- FROM: `### Requirement: Join execution failures SHALL fail fast while account-level blockers retain pause`
- TO: `### Requirement: Join execution failures SHALL fail after bounded no-click recovery while account-level blockers retain pause`
