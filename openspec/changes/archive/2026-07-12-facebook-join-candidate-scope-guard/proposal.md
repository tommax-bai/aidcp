...
- **corroborating 排除再收窄（非承重）**：在正向包含之上再叠两条排除——E1 候选被导航引用祖先（`a[href]` 或 `[role="link"]`，含**属性值里编码**的 `/groups/<id>`）解析到异于当前页 id 则排除；E2 落在「发现更多小组 / 查看推荐小组」推荐轮播容器内则排除。二者只会让作用域更窄、绝不放宽。
- **（2026-07-12 两轮实装评审订正，共 5+ 确认发现含红线）**：① **异群引用检测收敛为「扫全部元素属性」**——头部块上界与 E1 从「仅 `a[href]`」经两轮收敛到「扫 `[role=main]` 子树全部元素的 href + 所有属性值里的 `/groups/<id>`」（`__hasForeignGroupRef` 用 `querySelectorAll('*')`、`__candForeignRef` 上溯 12 层祖先）：二轮 jsdom 复现证实，推荐位若把异群 id 编码在 `div[role=button]`/裸 div 的 `data-*` 上，「只扫 `a[href],[role=link]`」仍框不住头部块 → 吞进推荐位 → fail-**open**（异群 join 被点、`ok=true`）；② **成员矛盾守卫**——`already_member` 仅当作用域内**无 join 按钮**时成立（有 join CTA 的群不可能已加入，堵推荐位「已加入」伪造 already_member）；③ **fail-closed 回执改可重试 `not_ready`**——不折叠成 `no_button`（后者被云端判永久 `failed`、丢弃可加入群）；④ 残留（foreign id 在轻 DOM 任何属性都不以 `/groups/<id>` 子串出现：JS 闭包 / 裸数字 id / shadow root）靠 0.1 真机坐实，**land 前 0.1 MUST 验证一个非锚点推荐位**、不能只重采锚点形态。
