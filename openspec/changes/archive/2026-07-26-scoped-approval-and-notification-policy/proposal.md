...
- 新增账号级评论审批覆盖策略：默认 `source_rules` 保持各来源既有规则；显式选择 `auto_approve_all` 后，普通浏览、排期、联系评论、强制互动、飞书 `/comment` 与结构化委托评论全部跳过按钮审批并进入既有提交链。免审通知仅作旁路记录，发送失败不阻止提交，也不回退为审批卡。
...
- `comment-interaction`: 普通浏览评论在账号全局免审时跳过按钮人审；免审通知不参与授权。
