...
## 7. Hover-open Click Stability

- [x] 7.1 Specify that clicking a hover-opened group trigger keeps its menu open while pointer leave and navigation still close it.
- [x] 7.2 Separate hover dismissal from click/keyboard opening and add a focused regression test for hover, habitual click, and pointer leave.
- [x] 7.3 Revalidate, commit, fast-forward, and deploy the interaction correction to `dev` with served-asset and health evidence.
  <!-- Console commit `e362a8ad8e1c0480ee5fdf9aa640c6401211a267` rebased onto concurrent `origin/master` commit `3e30747`, then passed AppShell 14/14, typecheck, and production build (3724 modules) before fast-forward push and canonical synchronization. Target `dev` passed `scripts/deploy-target dev --check`; backup `/opt/aidcp/console.bak.20260720-152533.tar.gz` (943148 bytes) preceded deployment of canonical assets `index-DkltVfWQ.js` + `index-BWIuhYCB.css`. The served JS contains `trigger:["hover"]`, so trigger clicks no longer toggle a hover-opened menu closed; `/accounts` and `/api/health` returned HTTP 200, `aidcp-cloud.service` remained active, and 8787/8088/8090 remained listening without a restart. -->
