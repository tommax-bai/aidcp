...
3. 下发段补安全阀：离线失败回待审、连续失败熔断——防批量获批后连环烧稿，且先于并行化独立上线。（最小发布间隔经产品定案本期不做，连发节奏暂由运营错峰批准控制，登记已知缺口。）
...
### D6. 下发韧性与熔断（Phase 1 先行，不依赖并行化）

- **最小发布间隔本期不做**（产品定案 2026-07-09，覆盖初稿与评审的间隔方案）：同窗批准多张 = 账号链尾背靠背连发（每篇 1-3 分钟）的现状保留，节奏由运营错峰批准自行控制；平台侧连发行为指纹登记为已知缺口（D8），后续出现真实风控信号或运营诉求时再按需补间隔机制（当时再议锚点与呈现）。
...
- **熔断**：同账号连续 2 次序列失败 → 停 drain 该账号已批队列（兜底扫描跳过、新 dispatch 拒绝且不烧授权）+ 飞书告警——防一次系统性边缘故障几分钟连环烧掉整批获批草稿（「自愈不自残」在下发段的落点；无间隔节流的现状下，熔断是防连环烧的唯一闸，重要性上升）。**清除接线**（评审修正，防死锁）：熔断中草稿的 approve 信号已在盘上，重批走 first-writer-wins 的 alreadyDecided 分支不产生新事件——故 alreadyDecided 分支在账号处于熔断时视为人工确认、清熔断 + 踢一次兜底扫描；preflight 对新草稿的批准同样清熔断。熔断计数内存态，重启即清（未修复会再烧最多 2 篇后重新熔断，有界代价）。
- `scanAndDispatchApproved` 逐条 await 改 fire-and-forget（多稿同窗获批时一账号多篇背靠背下发不拖死跨账号扫描；幂等由 inFlight+accountTail 已保）。
- 「通过即切」契约保持，增加熔断例外的**重述**（授权仍是发布必要条件、熔断只延后不放行不烧授权），AC-PUB 断言按新契约重述而非删除。
- **不做** console waiting 行状态与 `/api/content/queue` 形状再扩（无间隔等待窗后更无必要，避免又一处形状漂移面）。
...
- 同窗多批背靠背连发（每篇 1-3 分钟、含标签页横跳行为指纹）——最小发布间隔经产品定案本期不做，节奏由运营错峰批准控制；后续按需补间隔机制，运营侧文档必须如实写明「无节流保护」。
...
- [平台侧连发/协同指纹] → 同账号连发无节流（间隔本期不做，D8 缺口）——运营错峰批准是当前唯一节奏控制，熔断只兜连续失败不兜连发；错峰 `offsetMinute` 是平台风控层、与云端并行正交，**绝不可当串行遗产删除**；真实风控信号接线仍缺（D8）。
- [内存态跨重启丢失（claim/全局帽/熔断计数）] → 在途生成轮诚实失败（console 无产出、用户重点即可）；已批未发靠 60s 兜底扫描+信号文件恢复；熔断重清有界代价。
- [排期让位收窄是行为变化] → 已列 Open Questions 待拍板，测试覆盖。
- [同源串行重洗放行 = 同账号可存多份同源草稿] → 账号 pending 帽兜量；挑选场景本就要多版本对比，属需求本身。

## Migration Plan

三期独立上线独立回滚（总量约 6-8 人日）：

- **Phase 1 下发安全阀**（D6，约 1-1.5 人日）：不依赖任何并行化改动，补离线回待审 + 熔断 + 扫描改造——多稿获批世界的连环烧稿防线；建议先于（或同批于）Phase 2 上线。回滚=还原 dispatcher。
...
2. 排期让位收窄（洗稿在途不再让排期槽、自主轮与洗稿轮可重叠受全局帽约束）——确认？
3. LLM/生图成本峰值≈2 倍现状（生成单飞的成本尖峰保护正式放弃）——运营确认？

（最小发布间隔已由用户 2026-07-09 定案本期不做，不再列问。）
