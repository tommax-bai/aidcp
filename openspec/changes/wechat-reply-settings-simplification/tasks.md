...
- [x] 3.2 Rebase and integrate clean default branches, push control and Console changes, deploy Console to `dev`, and verify the documented health/static entry without any real platform write.
  <!-- Integrated by strict fast-forward with no force: aidcp-console master 7981506; aidcp main ba3ead4 before this evidence-only closeout. Built from clean aidcp-console canonical master and deployed only `dist/` to dev `/opt/aidcp/console` after backup `/opt/aidcp/console.bak.20260717-152920.tar.gz` and checksum dry-run. Local/remote SHA-256 matched for index, CSS, and JS; dev console local/public returned 200; Cloud remained active with 8787/8090/8088 listening; PostgreSQL `select 1`, Feishu WS ready, and unrelated isales services remained healthy. No Cloud restart and no real interaction send/preview was performed. -->
