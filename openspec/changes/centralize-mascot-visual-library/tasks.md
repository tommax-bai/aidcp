...
- [x] 2.1 Copy the eight concept PNGs into `aidcp/docs/design/mascot/assets/` without transforming them.
  <!-- repo=aidcp; commit=pending; validation=eight destination PNGs present; deployment=n/a; deviations=none -->
- [x] 2.2 Move the visual action library guidance into `aidcp/docs/design/mascot/` and cross-link it with the existing transparent-state library.
  <!-- repo=aidcp; commit=pending; validation=bidirectional documentation links and design-system reference present; deployment=n/a; deviations=none -->
...
- [x] 3.1 Verify byte-for-byte PNG integrity, destination completeness, source removal, runtime asset presence, and repository references.
  <!-- repos=aidcp,aidcp-edge; commits=pending; validation=eight SHA-256 pairs byte-identical, old worktree path absent, three runtime assets and mappings present with no diff; deployment=n/a; deviations=none -->
...
- [x] 3.3 Run `openspec validate centralize-mascot-visual-library --strict`.
  <!-- repo=aidcp; commit=pending; validation=strict OpenSpec validation passed; deployment=n/a; deviations=none -->
...
- [ ] 4.2 Record repository SHAs and validation evidence, then commit and push the control-repository change to `origin/main`; no deployment or Edge package is required.
