...
配置属于环境这个稳定对象；而「已看过哪些内容」「本轮凑到第几条」是账号自己的行为历史。若把进度也挂到环境，换账号后新账号会继承旧账号的浏览去重集合，导致新账号跳过它其实从未看过的内容——这既不真实，也让「按固定条数确认浏览开一轮」这个规则定义失去意义。所以进度、去重事实与批次终态继续按账号存续，换账号即从零重新收集。

慢启动那次做的是相反选择（起点也随环境走、换账号继承已走天数），因为慢启动约束的是环境与出口链路的成熟度，不是账号的行为历史。两者性质不同，此处不照抄。

## 6.1 基线校正（2026-07-28，实装期间发生）

实装进行中，另一条流 `facebook-rule-mode-two-tier-cadence` 落地到云端默认分支并已部署 dev，
把规则定义换成两级节奏（每 5 条确认浏览开一轮做 1 次点赞，每 2 轮额外 1 次加群联系评论），
并占用了迁移号 `0094` / `0095`。本变更据此校正三处：

- 本变更的迁移改号为 `0096`；schema 契约的所需版本与最高认识版本同步抬到 `0096`（高于 master 的 `0095`）。
- 新建的环境键配置表，其定义身份默认值对齐当前定义、约束同时接受新旧定义号与版本。
  **回填继续原样拷贝旧表的身份值、不改写**——改写会抹掉每一行当初实际配置在哪套节奏下，
  正是下一条要求所禁止的那种顶替。dev 的旧表行已被对方的 `0094` 迁到新定义，
  若约束只钉单值，回填会逐行被拒、整条迁移失败。
- 规格新增「存储的定义身份与编译期常量不符时必须具名报出、不得顶替」一条，实装覆盖存储层与客户投影两级。
