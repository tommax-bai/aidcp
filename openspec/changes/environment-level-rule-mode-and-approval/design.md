...
**这里有一处真实的行为变化必须显式承认**：今天 Facebook 创建无条件开启慢启动，选择普通或规则等于新号从第一天就吃满每日额度，没有 7 天爬坡保护。这是产品取舍不是实现细节，界面 MUST 就地说清楚，创建回执也 MUST 如实反映未配置慢启动，MUST NOT 沿用「已默认开启慢启动」的旧文案。
