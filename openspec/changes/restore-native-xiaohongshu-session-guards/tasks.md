...
> 第一波（2026-07-29）与第二波（2026-07-30）**均为零真机动作**：未打安装包、未部署、未在真机上做任何读写。7.7–7.13 原样保留；7.15–7.22 为第二波新增。
>
> **口径（2026-07-30）**：本节各条**一律不在本文件勾**。**已并入（2026-07-30 收口）**：
> `docs/real-machine-acceptance-backlog.md` **簇 122**（共 17 条 = 第一波 7.7–7.13 + 第二波 7.15–7.24），
> 与拟人化流的**簇 123** 是同一台机器、同一个分身，**一次真机 session 连着验**。
> 并簇后**验收状态以 backlog 为准**，本节只保留上下文。
> **本波全部行为改动只在桩上验过——真机零跑，不得当成已确认事实。**
...
**第二波新增（2026-07-30）——§3 键入取证侧**

- [ ] 7.15 【真机】真实小红书验证码浮层上跑一次带答案的协助，六格取证与页面实际状态**逐格**对账。重点两处：焦点标签在内嵌页 / 画布形态下的真实取值；不透明档下「已尝试清空」是否真清干净。桩只能证结构，证不了真实页面的焦点行为。
- [ ] 7.16 【真机 · 判据由读码推出、未观测过】回读三态里「读不到」那一格（新原因码「回读不可读」）在真实验证码页是否真的出现；出现时是否应改判为焦点丢失。同批另一个新原因码「清空验证不可得」也只经假 CDP 验过——须看一眼云端 / 控制台对未知原因串的展示是否退化成空白（协议上该字段是自由串，静态检查不会拒）。
- [ ] 7.17 【真机】提交后复检的真实行为：回车触发导航时页面探测是否**稳定失败** ⇒ 据此决定是否需要恢复退役实现的 4×500ms 有界复检（现为单次沉降 + 单次探测，读不到即判定不可得）。
- [ ] 7.18 【真机 · 期望值已翻转，别照旧结论看】dev 上人工制造一次「点位没点中输入框（零派发）」，确认控制台**不再**提示「客户端太旧」（3.3 的语义订正后这是期望行为）；另制造一次真正的「老客户端忽略下发文本」，确认该提示仍会出现。
- [ ] 7.19 【真机】搜索框探针抛异常后，该命令改以「探测失败」收尾而不是回一份「找不到搜索框」——须确认云端对这条错误分支的处置不会把调度卡住（不改代码，只核对）。

**第二波新增（2026-07-30）——§5 运行期身份侧（本流全部身份行为真机零跑）**

- [ ] 7.20 【真机】运行中在同一分身里退出登录 ⇒ 周期校验在 ≤2 拍（默认 60s）内判失效，重立链跑完并**停在无身份态、绝不回落默认账号**；以及同一用户数据目录换登另一账号 ⇒ 判换号、按新 id 重连、云端拆旧会话，恢复后可正常浏览。
- [ ] 7.21 【真机 · 两个预算未在真机定档】① 触发失效时页面正停在创作发布页 ⇒ 归位到消费端首页后能读出身份锚点（重立链的归位重读预算是按「整页重载」拍的）；② 正向登出探针的**陈旧预算 = 3 × 观测节拍 ≈ 6s** 是否合适——真实页面上探针的成功率与间隔分布**从未观测过**，太紧会把健康读数当陈旧（漏判真登出）、太松会把陈旧当新鲜（误判登出）；同时确认「连续 3 拍打一次存活告警」在真机上不会每天刷屏。
- [ ] 7.22 【真机】四态运行姿态与外壳呈现逐条对账（桩已覆盖八条路径，真机零跑）：① 终局身份下点「恢复」——界面**一秒都不许闪现运行中**，且给出的处理办法可执行；② 制造一次「重设基线之后收尾失败」——左栏出现「自动化已停摆（身份正常）」而非全绿，且点一次「恢复」真能把浏览与观测拉回来；③ 终局下切云端环境——如实报重绑失败、云端侧**看不到**该账号上线、重启后能连到新选的云端；④ 终局下云端仍派发布 / 评论——边缘回诚实拒绝回执、云端不挂起、平台上无该账号的新痕迹；⑤ 正常换号触发一次重立——「正在重新确认账号身份」在几秒内自动收尾、不产生红角标；⑥ 唤醒被拒（已设账号覆盖值 + 唤醒读不出身份）——留在待机态、可再次唤醒、浏览器被杀回槽位且**槽位没泄漏**。
- [ ] 7.23 【真机 · 把已知缺陷拍成事实，用于 P1 / P2 的代价基线】① 拔掉核心↔外壳 IPC 后触发终局与停摆——确认兜底日志行出现，且界面**只红一行**、随后被下一行普通日志写回运行中（这正是 P2 要修的现象）；② 身份落到终局时云端下发任务认领——用云端日志确认它**空等满认领超时（200s）**才失败（这正是 P1 的代价基线，修完要能对比）；③ 终局 / 停摆各触发一次，确认持久红卡出现且**不被后续普通日志清掉**。
- [ ] 7.24 【发版前必做 · 打包红线】本波改了 `src/electron/main.cjs` 与 `src/electron/fleet.cjs`（外壳侧运行姿态投影与终态白名单）。按 CLAUDE.md §5 打包红线，**出包前须在本机跑一遍编译后的打包产物**（起一次核心、确认能走到云端连接 / 指纹浏览器调用），别把外壳类回归留给运营机。
