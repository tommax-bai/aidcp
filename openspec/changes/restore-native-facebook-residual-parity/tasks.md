...
- [x] 4.1b **必须与 4.1 同改，否则 4.1 是空动作（本次复核发现）**：到底确认的有效性判据 `classify_facebook_bottom_confirmation` 自己也写着 `initial.surface != "home" || current.surface != "home"` 即判无效（`:425-426`），所以只放开 `:203` 的守卫，确认会照样在小组页 / 搜索页返回 `Invalidated` → `facebook_bottom_completion_reason` 回 `None`（`:383-385`）→ 到底状态**依然不可达**。把这两行的面别条件改成「两次探测都在**已声明的列表面**上，且 `initial.surface == current.surface`（确认窗内不许换面）」。验收：单测断言 group / search 面在满足其余全部证据条件时能到 `ExplicitEnd` / `WindowStable`，且确认窗内换面仍判 `Invalidated`（红灯用例现成：facebook-session.test.ts『高度稳定+接近底部+连续无新卡 → feed_exhausted』把桩换成 group/search 即是 — 见 oracle.md ④） <!-- aidcp-edge 9176dcb 有效性判据放开到已声明列表面 + 确认窗内换面仍判无效；用例 every_declared_list_surface_can_reach_a_terminal_bottom_state / a_surface_change_inside_the_confirmation_window_still_invalidates_it -->
...
- [x] 8.3 运行 `openspec validate restore-native-facebook-residual-parity --strict` <!-- aidcp 2026-07-28 回写后实跑通过：Change 'restore-native-facebook-residual-parity' is valid -->
