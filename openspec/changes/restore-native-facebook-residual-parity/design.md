...
到底确认的可达性判据从「是不是首页面」改成「是不是**已声明的列表面**」（首页 / 搜索结果 / 小组，与同文件的活动列表面判据一致）。**这一改必须落在两处**：循环前置守卫（`:203`）与确认有效性判据（`:425-426`）；后者还要额外要求确认窗内**不许换面**（`initial.surface == current.surface`），以免「从小组页滚到别的面」被当成同一批的到底证据。证据链的其余条件（同代、近底、无增高、卡身份集合不变、`explicit_end` 连续两次、确认窗口）逐条不动；首页空态确认（`:492` / `:507` / `:520`）按定义只服务首页，不在放宽范围内。耗尽后的原因分类：在任一列表面上「见过卡但没翻出新的」MUST 回「翻页未确认」这类非终态原因，**MUST NOT** 回「找不到目标」；「找不到目标」保留给真的一张卡都没有的情形。滚动回执 MUST 在既有可选观测里带上所在列表面。云端的 Reels 兜底授权 MUST 限定在首页面到底，非首页面到底走各自的恢复动作。
...
- **边界（本次复核修正）**：`facebook/feed.rs` 的到底证据链归 `repair-facebook-feed-exhaustion-continuation`（已 ✓ Complete）。本 change 动它的**面别谓词两行**（`:203` 与 `:425-426`）——这是让非首页面可达的唯一途径，无法回避；其余证据条件（同代 / 近底 / 无增高 / 卡身份集合不变 / `explicit_end` 连续两次 / 确认窗口 / epoch 状态机 / 首页空态确认）逐条不动。该 change 已 Complete、其判据已在主干，集成期按 fleet 串行 rebase 并在 tasks 里显式登记这两行的改动理由。
...
| `repair-facebook-feed-exhaustion-continuation`（✓ Complete） | `facebook/feed.rs` 首页面 settle / 显式终止证据 / 「翻页未确认」原因，以及云端 Reels 兜底 epoch | **不改** epoch 状态机、首页空态确认，与证据链的其余全部条件（同代 / 近底 / 无增高 / 卡身份集合不变 / `explicit_end` 连续两次 / 确认窗口）。**会改**它的两行面别谓词（`:203` 守卫、`:425-426` 有效性判据）——非首页面可达性无法绕开这两行；另加确认窗内「不许换面」一条。其余为原因分类、回执带列表面、云端 Reels 授权加列表面限定 |
...
判据 `facebook_feed_height_grew`（`facebook/feed.rs:409-413`）由**同一个函数**支撑两处：循环前置守卫（`:202`）与到底确认有效性判据里的「无增高」（`:430`）。4.5 把「无增高」列进不许改的证据链清单，所以不开例外就等于禁止修阈值。例外的边界写死在 tasks 4.6：只动阈值取值这一个常量，语义与证据链其余条件逐条不动。不修的后果是 4.1 / 4.1b 变成空动作——面别守卫放开了，但任何一次重排都算「还在长」，三个列表面都走不到到底确认。
