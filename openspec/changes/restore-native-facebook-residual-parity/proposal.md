...
- 让小组页与搜索结果页也能走到底确认（**两处面别谓词必须同改**：循环前置守卫与到底确认自身的有效性判据，只改前者是空动作），并把「本批看完没翻出新的」按所在列表面如实分类，不再退化成红线词「找不到目标」；滚动回执带上所在列表面。云端同批配上两件事：Reels 兜底授权加列表面限定（否则非首页面到底会把账号从定向面带走），以及非首页列表面每一个终态原因都有有界且可观测的归宿（否则退化成无命令无终态的静默空转）。
