...
- **对应任务**：~~**本 change 无对应任务 —— 见文末覆盖漏洞（这是本参照书发现的最大漏洞）**~~
  **⚠️ 该判断已过时（stale，2026-07-28 实装实测订正）**：tasks.md 已有完整的 §10「Facebook 同意闸的作用域与失败分档」承接这一整条（10.1–10.7），且 10.1–10.6 已随 `aidcp-edge 9176dcb` 落地。文末「覆盖漏洞 A」同此，一并标 stale。
...
> **⚠️ STALE（2026-07-28 实装实测订正）**：本节的「无任何任务」这个前提**已不成立**——tasks.md 现有 §10 完整承接（10.1–10.7，含作用域、present 门合取项、探测失败降级、失败分档、回执可观测性、用例组、歧义处置的阻塞标记），
> 且 10.1–10.6 已随 `aidcp-edge 9176dcb` 落地、10.5 部分完成、10.7 待用户裁定。**下面的正文只保留作为「当初为什么补 §10」的立论记录，不要再据它去判定覆盖缺口。**
