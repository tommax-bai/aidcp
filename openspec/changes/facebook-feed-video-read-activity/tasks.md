...
- [x] 3.3 Rebase onto the latest default branches, rerun required gates after conflict resolution, fast-forward push Edge and control defaults serially, and preserve the concurrent Reel-follow worktree.
  <!-- Edge rebased onto Reel-follow commit ca6df3b, retained both follow/read marker assertions through the only conflict, and fast-forward pushed origin/master to d88e490. Control rebased onto the corresponding Reel-follow/control work plus later main changes and fast-forward pushed origin/main through 3123037 before this closeout marker. The canonical Edge checkout was not rewritten when an independent local-only signing commit 4432206 appeared; that concurrent work remains intact for its owner to reconcile with origin/master. -->
