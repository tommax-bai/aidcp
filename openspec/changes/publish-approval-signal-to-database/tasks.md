...
- [ ] 1.2 在 §6.4 禁止清单（现 7 条）后增第 8 条：「用共享本机文件系统、共享路径或进程内内存充当跨服务的授权凭证、锁或事实源」。
- [ ] 1.3 把 §12 阶段 0 的「盘点进程内状态、EventBus 事件、Store 和每张表」扩为六类，新增：本机文件系统信号与共享路径；本机进程内锁与内存事实表；数据库级 advisory lock（按 key 命名空间登记全部引用点与归属服务）；常驻定时任务（`setInterval` 宿主及其扫描 / 写入的表、`execution_target` 归属）。
- [ ] 1.4 在 §12 阶段 0 补一句盘点行的必填字段：引用点 `文件:行` → 拆分后归属服务 → 是否跨服务 → 替代机制 → 不替代会怎样失效（必须写出失效方向是静默还是报错）。
...
- [ ] 1.7 在仓内产出阶段 0 盘点表初版（六类，至少覆盖本 change 已坐实的：审批信号文件、`interaction-env:<envKey>` advisory lock、`interaction-store.ts:409` / `:989` 两把单服务内锁、`aidcp-cloud/src` 下 24 处 `setInterval` 调用点）。
