...
本节写于定稿整合之前：当时 `docs/cloud-service-decomposition-proposal.md` §12 阶段 0 的清单只有四类（进程内状态、EventBus 事件、Store、表）。**该清单现已扩为六类，下列四项已全部并入**，本节保留只作推导记录；实施时 MUST 以定稿 §12 阶段 0 的六类表为准，MUST NOT 再次扩表。（原文里那个裸行号指针已删除——定稿只冻结 §1–§17 顶层编号、不承诺行号稳定，跨产物引用 MUST 按节号。）
...
4. **常驻定时任务**：`setInterval` 宿主及其扫描 / 写入的表、其 `execution_target` 归属。计数 MUST 同时给出「宿主数」与「调用点数」两个口径并在实施当天重测（定稿 §4.6.5 记 14 个宿主，`grep -rn setInterval src` 2026-07-22 实测 23–24 个调用点）。
