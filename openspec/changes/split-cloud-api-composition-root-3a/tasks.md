...
- [ ] 5.3 Run `openspec validate split-cloud-api-composition-root-3a --strict`, update every completed task with concise `<!-- repo sha validation/deployment/deviation -->` evidence, commit/push the control worktree, fast-forward `main`, and push `origin/main`.
