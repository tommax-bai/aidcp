...
### D9. 键入需 panel 身份；点击不变

含 `text` 且 actor 是 incident 级 scoped token ⇒ 403 `requires_panel_login`。**飞书链接的纯点击流逐字不变、零回归。** 理由入 spec：群路由无内外部标记，**卡的可见范围不是授权范围**；把「点 2 个像素」升级成「往生产浏览器打任意 ASCII + 回车」需要一道显式身份闸。console 侧 JWT 在 localStorage，提交时挂 Bearer 即走通道一；**无 token ⇒ 输入框 disabled + 文案提示**，别让运营打完 24 个字符才吃 403。
