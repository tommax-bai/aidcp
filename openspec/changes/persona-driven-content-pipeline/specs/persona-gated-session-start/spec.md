...
**Reason**: `default` 账号已退役（retire-default-account：ECS 实测 default 已删、0 个未绑人设账号），且本 change 移除了打包默认人设的兜底语义（解析器对无人设返回 null、不再回落 `soul.yaml`）。「default 硬豁免 + 打包默认回落」的前提（存在 default 账号 + 存在默认人设）均不复存在；保留该条会与「人设必填、无豁免」直接矛盾。
