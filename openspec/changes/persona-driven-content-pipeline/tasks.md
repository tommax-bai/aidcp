...
- [ ] 6.5 部署：cloud + console **必须同批**上线（PersonaSource fallback→none 双向不兼容，先 cloud 后 console 连发、窗口仅影响人设页渲染数秒）；⚠️ 运维语义变化周知：PG 故障时人设镜像为空 → 全账号 fail-closed 拒绝运行（此前静默套默认人设照跑）。
