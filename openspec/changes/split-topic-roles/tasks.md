...
- [x] 2.5 **[实机确认通过 + 默认启用]** 真机（工程师大白，登录态创作发布页）驱动**实际** `runAddTopic`（经真 `PublishCommandDispatcher` + `CdpDomProvider`）：dispatch 回 `ok:true`，编辑器真出现 `a.tiptap-topic`（name「大模型踩坑」、text「#大模型踩坑[话题]#」），**未发布**。确认后把 `AIDCP_PUBLISH_TOPIC_CDP` 默认翻为**启用**，保留显式 `0/false/no/off` kill-switch；runAddTopic fail-closed，跨账号 DOM 不符只诚实失败（best-effort 跳过、不误贴），故默认启用安全。<!-- aidcp-edge 06ec7fd 实机确认→默认 ON；需重启 edge 生效 -->
