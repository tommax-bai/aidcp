...
- Require a distinct active-video transition and one matching canonical Reel card before success.
...
Entry retains the existing 15-second canonical-card hydration window. On timeout, Edge performs a fresh active-Reel and next-target readback. Bootstrap is admitted only when both observations bind the same unique `videoKey`, canonical identity is still absent, keyboard input is safe, no blocker or document drift is present, and the command remains active with enough deadline budget.

If canonical identity or a matching card appears before the first write, Edge suppresses navigation and reports the current card. Missing, equally eligible, unsafe, drifted, or already changed observations dispatch no input and terminate with an honest entry-specific receipt.
...
Shared Reels completion accepts success only when a fresh active probe has both `videoKey` and canonical `noteId`, and `page.cards` contains exactly one Reels card whose permalink identity matches that active `noteId`. A `content_ref`, anonymous card, mismatched card, empty batch, or multiple-card batch remains unreportable.
...
When anonymous-entry navigation dispatches input, Edge records a session-local pending Reel observation. It clears the latch only after the moved-to active video is reported as the one matching canonical card, the session ends, or the page leaves the Reels surface.

While the latch remains and another `page.scroll` arrives, Edge performs read-only card and active-video recovery. It MUST NOT dispatch another navigation input. If identity remains absent, it returns `ambiguous/reels_post_transition_identity_pending`; if the matching canonical card appears, it reports that one card and clears the latch.
...
Success remains `confirmed` plus one canonical `page.cards{listKind:'reels'}`. An entry failure after route navigation but before bootstrap input is an entry-specific bounded failure. Any failure after input dispatch remains `ambiguous`; it MUST NOT become `not_started/no_target`, because Cloud may treat `no_target` as safe to re-enter. No protocol field is added.
...
- [Canonical identity appears at the 15-second boundary] → Re-probe the active video and card immediately before input and suppress the write when the current Reel becomes reportable.
