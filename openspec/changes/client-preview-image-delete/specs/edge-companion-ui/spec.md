...
**Reason**: 该条已被现实推翻——**应用内稿件预览 + 发布 / 取消审批早已上线**（协议 `publish.approval_action` / `.result`，72→74；渲染层原来的「打开飞书 ↗」深链已被应用内审批取代），只是那批改动当时未走 openspec change，导致 spec 停留在「端上 MUST NOT 提供任何审批授权控件」的旧口径。本 change 在其上新增「逐张删配图」，必须先把这条与现实冲突的 MUST NOT 收掉，否则新能力与自己所在 capability 的条文直接打架。替代条文见下方 ADDED「发布卡纯展示、审批授权走应用内与飞书双通道」。
