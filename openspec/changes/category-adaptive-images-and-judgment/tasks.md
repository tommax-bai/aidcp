...
> - **2.2/2.4 配图产后校验已拆出（2026-07-15，用户决策「拆出去先解链」）**：「乱码/像不像真人」必须看生成图 → 需新接视觉/多模态模型（新能力+成本/延迟/选型决策），从本 change **拆到独立 change `image-postcheck-vision-model`**（承接产后校验条文 + 视觉模型选型）。本 change 的 publish-multi-image ADDED 需求已改名为「配图真人与封面文字分级」、只声明**已实装**的分级默认（faceless/no-text/no-watermark 风格档 + AI 元数据链路），产后校验条文移出 ⇒ spec 不再声称未实装行为（守无声假成功红线）。**2.3 已完成**（代码核实：`ComplianceDecision.aiEnforced` + `recordMetadata` 链路在；`wanxiang-client watermark:false`；STYLE_PROFILES 全 `no text/no watermark/faceless`；无 prompt 让模型画面内画水印）。本 change 其余全部完成且已上线 ⇒ 可归档。
...
- [x] 2.3 合规 AI 标识确认走既有 `ComplianceDecision.ai/aiEnforced` + 发布声明/元数据，MUST NOT 让模型画面内画水印。<!-- 2026-07-15 代码核实完成：`ComplianceDecision.aiEnforced`(publish-agent/types.ts) + `recordMetadata(id,metadata,aiEnforced)`(server.ts/publish-log-store.ts) 链路在；`wanxiang-client.ts:132 watermark:false`；STYLE_PROFILES(prompts.ts) 全含 `no text/no watermark/faceless/no realistic human face`；全仓 grep 无「让模型画面内画水印」的 prompt。 -->
- [→] 2.2 **已拆到 change `image-postcheck-vision-model`**（2026-07-15，用户决策）：产后校验需新接视觉模型（新能力+成本/延迟决策），从本 change 移出。本 change 只保「已实装的分级默认」；产后校验的 spec 条文 + 视觉模型选型归新 change。
- [→] 2.4 **已拆到 change `image-postcheck-vision-model`**：其回归（高风险图未过校验即重生成）随产后校验条文一并移出，作为新 change 的 scenario「高风险图未过产后校验即重生成」+ task 2.1。本 change 的分级回归（需人物不出写实真脸）由 2.1 的 StyleProfile 三档 + spec scenario「需人物时用非写实虚拟人物、不出写实真脸」覆盖。
