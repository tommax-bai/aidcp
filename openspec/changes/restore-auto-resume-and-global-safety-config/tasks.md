...
- [ ] 1.1 `src/orchestrator/role-dispatcher.ts` `session.should_end` 处理器（约 :1080-1084）：删去其中的 `this.endSession(payload.reason, {autoResumeEligible:true})`（约 :1083），**只保留** `sendCommand({action:'session.end', reason})`；由注入的结束回调（:532 `onSessionEnd`）作**唯一**结束入口
- [ ] 1.2 复核 `src/agents/session-monitor-role.ts` `triggerEnd`（:247-255）顺序保持「先 emit 发结束命令、后走 `onSessionEnd` 回调」不变（顺序正确：结束命令先到边端，再武装续场）
- [ ] 1.3 复核 `endSession`（:729-744）现只被调一次：第一次即过守卫 → 置 `sessionActive=false` → `armRestTimer`（:743）武装续场计时器且不再被同次取消；`cancelRestTimer`（:750）/`armRestTimer`（:757）内部逻辑**不动**

## 2. aidcp-cloud — stream A②：续场后主动重新驱动边端（云端半）

- [ ] 2.1 `src/orchestrator/role-dispatcher.ts` `feed.entered` 翻译处理器（:1028-1038）：为 `payload.trigger === 'session_start'` 增一条分支，下发一次 `this.sendCommand({action:'scroll', reason:'resume_redrive'})`；`back_to_feed` 分支保持不变
- [ ] 2.2 复核 `scroll`→`page.scroll` 映射（`comm/command-bridge.ts:22-23`）与 `server.ts:828-830` 出口不变；**不新增协议消息类型**
- [ ] 2.3 复核 `startSession`/`restartSession`（:635-662 / :693-722）emit `feed.entered{trigger:'session_start'}` 不变（重驱统一由 2.1 翻译分支发，单一发命令出口）

## 3. aidcp-edge — stream A②：边端浏览循环唤醒重启

- [ ] 3.1 `src/browse/browse-session.ts` 云端命令入口（:374-382 `onCloudCommand`）：当 `!this.running` 且收到**浏览类命令**（非 `session.end`）时调 `this.start()`（:276 `if(running)return` 幂等守卫）重启循环；`session.end` 仍只置 `stopRequested`
- [ ] 3.2 处理 `start()` 清空命令队列（:280 `commandQueue=[]`）：靠重启后 `ensureExplore→waitForVisibleCards→reportVisibleCards`（:474-477）重报 `page.cards` 驱动云端；**不依赖排队命令存活、不要先 push 再 start**
- [ ] 3.3 shutdown 守卫：主动关闭/下线流程（`main.ts` shutdown :477 `browse?.stop()`）期间迟到命令 **MUST NOT** 复活循环（加 closing 标志或复用 `stopRequested` 语义）
- [ ] 3.4 回归：保留 AC-PROTO 白名单断言（`page.scroll` 仍在 `edge-client.ts:353`）；新增用例「循环已停 → 收 `page.scroll` → 重启 → 重报 `page.cards`」与「关闭中迟到命令不复活」
- [ ] 3.5 `npm run typecheck` + `npm test` + `npm run test:acceptance` 全绿

## 4. aidcp-cloud — stream A 回归断言

- [ ] 4.1 新增/扩展 `test/`：一次监测体结束（duration / actions / budget / idle 各一）→ `endSession` 恰好一次、结束后 `restTimer`「已武装且未被取消」、`onRestElapsed`/`doAutoResume` 到点触发
- [ ] 4.2 `test/integration/role-dispatcher-resume.test.ts`：续场重开会话后云端对 `session_start` 发出 `scroll` 引导（断言 `sendCommand` 被调、reason=`resume_redrive`）
