...
- [x] 4.3 让 `test/native-page-engine/facebook-router-source.ts:22-24` 的 TypeScript 复刻实现采用与 4.2 完全相同的规则，并加一条断言：两种拼接结果逐字节相等 <!-- aidcp-edge 30f4d9a 两种拼接实现按同一规则、逐字节相等断言已落 -->
...
>
> **⚠️ 2026-07-31 事实订正 —— 这两条的紧迫性此前都被低估了。** 当日一度按「不打客户端安装包」把 5.5 判成弃守，
> 同日经用户澄清：**他一直在手动打包测试其他功能，包在持续更新**。因此：
>
> - **5.5 不是不可达的死代码，它在每一次出包时都会跑。** 而 5.1 出于那条顺序红线只加了字段、没抬版本号 ——
>   于是**下一个动版本号的人会炸打包，且没有任何东西会提前警告他**。5.5 就是拆这颗雷的那一步。
> - **5.4 的解锁条件其实已经具备。** 它卡在「需要一个有已构建产物的工作区」，而持续打包正意味着这样的产物存在。
>   原先记的「本工作区产物目录为空」只是**本仓工作区**的局部事实，被误当成了全局阻塞。
...
- [x] 6.6 记录一次性安装步骤（`rustup toolchain install` 需带 `components`），并检查 `scripts/build-desktop-macos-ol-arm64-common.sh` 里 `--profile minimal` + `export RUSTUP_TOOLCHAIN` 的组合是否会绕开 `rust-toolchain.toml` 的组件声明；若会，一并修正 <!-- aidcp-edge 30f4d9a **实测：会绕开，已修**。两条原因叠加：① 那个判断只看编译器能不能解析，而编译器在最小集里就有——「工具链已装、但缺 clippy / rustfmt」这一档**检测不出来**，直接跳过安装分支；② 随后 export 的工具链环境变量会让 rustup **绕过声明文件**直接选定工具链，于是声明里的组件在这条路径上**永远不生效**。改成无条件补齐组件（幂等）。**后果不是打包失败**（打包只要编译），而是同一台机器之后跑代码门禁时找不到格式化与 lint——现场看着像「门禁坏了」。另登记一处未修：工具链版本号从 `Cargo.toml` 读、而非从工具链声明文件读，两处一致**无任何机械保证**（当前都是 1.97.1，属碰巧） -->
...
- [x] 7.1 把编码密钥收敛到单一定义，`native/page-engine/build.rs:7-9` 与三处运行时副本（`src/xhs.rs:23`、`src/facebook.rs:39`、`src/probe.rs:8`）共用同一来源；纯常量重定向，MUST NOT 改动任何解码语义 <!-- aidcp-edge 30f4d9a 新建 `native/page-engine/src/embedded_asset_key.rs` 为唯一定义，编码端与三个解码端共用。**纯常量重定向，解码语义逐字未动**。两处机械要点写进了文件注释：① 构建脚本是独立编译单元、到不了 crate 内部，只能 `include!` 取用，因此必须同时声明重编触发——少那一句就是「改了密钥不重新编码、运行时拿新密钥解旧资产」；② 被 `include!` 的文件里只能写行注释与外层文档，内层文档展开位置不在文件开头、编译直接报错（实测踩过） -->
...
- [x] 7.2 补一条会真正执行的解码内容断言：对嵌入资产解码后校验其明文特征，密钥不一致时失败（该断言随 6.1 的 Rust 门禁进入自动流程） <!-- aidcp-edge 30f4d9a 新增 `tests/embedded_asset_decoding.rs`：逐个把嵌入资产解码回明文源并校验特征，随 gate:native 进自动流程。**这是 7.1 的护栏**——没有它，密钥收敛万一收错，本地全绿、只有真跑页面命令才拿到乱码，而那时的现场与「平台改版」分不开 -->
  - 【阻塞】依赖 7.1 的单一来源收敛先落地（否则断言只能证明某一份副本自洽）；同样受上述白名单阻塞。6.1 的 Rust 门禁入口已就位，断言一旦落地即自动进流程
- [x] 7.3 在 `aidcp-edge/CLAUDE.md` 新增「Native 页面引擎」一节，写明：这层编码只挡扫读级别、不构成保密，拿到安装包即可还原；MUST NOT 基于「规则已加密」把凭据 / 令牌 / 可访问远端系统的密钥塞进同一通道 <!-- aidcp-edge 30f4d9a 已写入边缘仓 CLAUDE.md 新增的「Native 页面引擎」一节 -->
  - 【阻塞】需改 `aidcp-edge/CLAUDE.md`，该文件在本轨可改文件白名单外。解锁条件：白名单属主放行或由主 session 统一落护栏文档
- [x] 7.4 同节写入本轮四类「本地全绿、只有真跑页面命令才现形」的失效模式法条：① 新增嵌入资产必须同时改构建脚本的读取与 `rerun-if-changed` 声明；② 新增分片必须同时登记进清单；③ 分片命名的词典序即执行结构序，新分片命名须排在依赖它的分片之前；④ 改页面规则后开发态必须由源码摘要强制重编 <!-- aidcp-edge 30f4d9a 四条法条已写入，抬头点明它们同属一个失效族：本地全绿、单测照过，只有真跑一次页面命令才现形，而那时拿到的现场（乱码 / 结果无效 / 空结果）与「平台改版」**完全无法区分**，会把人带往完全错误的方向 -->
...
- [x] 8.5 按同一模式清理 `native/page-engine/tests/fake_cdp.rs` 里的 `unix_time_ms() + N` 绝对墙钟截止期 <!-- 2026-08-01 **维持「以实测结案、不做预防性改写」**（该结论 2026-07-31 首次作出，本次复核后维持）。判据：该文件 38 处绝对墙钟与 8.4 修的那族**性质不同**——8.4 那族是「断言截止期会被跨过」，用 25–250ms 表达，并发下跨期位置会漂到更早的探测、断言直接翻面，方向脆弱；这 38 处绝大多数是 2–60s 的**充裕预算**，语义是「给足时间别超时」，只会因为太短而误报失败、不会误报成功。已有探针实测撑着（8 路并发 32 跑零红）。**改写它们要动一批本来稳定的用例，且没有任何判据能证明改对了。** 新数据点登记：2026-08-01 出现过一次极端负载（三个 Rust 编译并行把机器挤到子代理看门狗超时），该档负载下这些墙钟**未验证**；若今后在那种负载下见红，回到本条。 -->
- [x] 8.6 修 `test/native-page-engine/client.test.ts:13-27` 的子进程握手预算（当前逐条 500 毫秒 / 进程 3 秒）：或抬到能容纳默认并行度下的进程启动，或把该文件显式串行化；两者择一并写清理由 <!-- aidcp-edge 70f6947 **已由他人 change 顺手做掉**（`restore-facebook-post-join-comment-continuity` 的边缘侧时间预算整体放宽那一批），本 change 零开工。走的是「抬预算」那条：默认抬到 10s，理由写在代码注释里——假引擎是真起一个 node 子进程，3s 只够空载启动，满负载并行跑全量时会被吃掉，于是协议校验类用例拿到的是超时而不是它要测的协议错误（单跑必过、全量偶发）。这些用例测的是协议与错误语义、不是启动速度，真要测进程死线的那条自己传更小的值。**⚠️ 订正（2026-08-01 当日）：它只做了一半。** 那批只改了**默认值**，漏掉了本文件里唯一一处**显式覆盖默认值**的调用点——能力清单不符那条用例硬写 500ms，绕过了新默认。它断言的是协议语义（清单不符 ⇒ 协议错误），而要比对清单，引擎必须先真起来握手，所以它**根本不该**opt out。满负载并行下它拿到的是超时而非协议错误，**单跑必过、全量偶发**，本批集成就是被它拦下的。已补修（见后续 sha）。
      **这条订正本身也是个教训**：我核 8.6 时只看了默认值变没变，没查有没有显式覆盖的调用点——与本轮反复踩的「验证只覆盖改动点、不覆盖波及面」是同一个形状。 -->
