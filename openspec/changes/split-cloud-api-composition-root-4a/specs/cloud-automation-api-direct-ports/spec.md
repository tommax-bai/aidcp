...
#### Scenario: automation 启动组合根
- **WHEN** `AIDCP_SERVICE=automation` 使用完整 4a 配置启动
- **THEN** 进程只构造 automation owner pools/stores 和 API HTTP clients，不构造或连接 API owner pool/store
...
### Requirement: 4a 方法账 SHALL 排除 3b 与 4b

4a SHALL 覆盖 design D1 列出的 19 个方法簇、45 个 method slots，并逐方法测试
...
`resumeEdgesForAccount` 与 Facebook scope writes SHALL 作为 4a paired commands，MUST NOT
继续归为镜像或未鉴权的 3a 扩展。
...
4a SHALL 以 direct loopback tests 证明全部 route/client/错误语义，以边界门禁证明 automation 无 API pool；
DEV monolith 验收只证明现网零回归并保持独立 listener 未额外开放。只有 4b 也闭合、独立
api/automation/content units 实际启动后，才可声明 4a 在真实跨进程拓扑可达。

#### Scenario: 只有 loopback 与 DEV monolith 通过
- **WHEN** 4a route/client tests 与 DEV 单体 health 均通过，但独立 units 未启动
- **THEN** 交付记录只声明源码/契约与单体零回归，不声明三进程互通、真实断链恢复或无跨 owner 运行连接
