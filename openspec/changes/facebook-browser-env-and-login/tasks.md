...
- [x] 6.1 Run F1 gated post probe on disposable account/test target and record whether server-confirmed comment verification is possible.
  ...
  <!-- done 2026-07-07: F1 verified live on disposable AdsPower profile k1ebny3j. Operator-owned test post created by the disposable account itself (Friends audience); a comment was posted and, after a full page reload, the comment PERSISTED in the target post's comment region scoped to own identity — i.e. server-confirmed and distinguishable from optimistic local render. No checkpoint/overlay throughout (China-IP no-proxy profile). RESULT: server-confirmed comment verification IS possible. FINDING for task 4.2/4.3: the packaged runFacebookGatedSubmitProbe returned editor_not_found on the permalink.php surface because it did not scroll to / wait for the lazy-rendered comment editor (role=textbox aria-label "写评论…", below the fold); production comment execution must scroll-to + wait for it, and must scope the success check to own-identity + target-post comment region (not full-page text match). -->
  <!-- 2026-07-07: raw probe output was ephemeral (marker hash only, no secrets) and deleted per runbook hygiene; the finding above is the recorded outcome. -->
  <!-- 2026-07-07: F1 real-machine verification performed as part of the fast-push plan (operator deferred F3 multi-day stability). -->
...
- [ ] 6.3 Run F3 low-frequency AdsPower profile stability observation for several days and record result without secrets.
  ...
  <!-- partial 2026-07-06: added phase0-f3-observation-runbook.md to define cadence, counted-sample criteria, failure conditions, and recording template for Day-1+ observations. -->
  <!-- deferred 2026-07-07: per operator decision (skip long stability tests, push features fast, batch stability at the end), F3 multi-day stability observation is moved to the deferred stability completion batch (see facebook-scheduled-comment tasks section 8) and is NOT a gate for landing the edge Facebook driver. Day-0 sample already passed; multi-day observation to be completed before scaling to production. -->
...
- [ ] 7.3 Record repo commit SHAs, probe outcomes, and validation notes in this `tasks.md`.
  ...
  <!-- integrated 2026-07-07: edge Facebook driver + probes landed on aidcp-edge master (rebased onto latest master, clean; typecheck PASS, acceptance 13/13, full 693/693) at master 9cda1d4. The companion edge per-environment platform selection (separate change edge-environment-platform-select) landed at edge master a2fac2a, so an operator can now create a Facebook AdsPower environment and launch it into facebook.com (core selects the facebook driver via AIDCP_PLATFORM). F1 verified (6.1); F2 passed (6.2); F3 deferred to the stability batch (6.3). Change stays ACTIVE (not archived) until F3 multi-day observation + any remaining 4.x wiring for the production comment path. -->
...
- [x] 7.5 Do not start `facebook-scheduled-comment` until F1/F2/F3 are all recorded as passed or the design is revised.
  <!-- satisfied 2026-07-07: F1 verified (6.1), F2 passed (6.2), and the design was explicitly revised — facebook-scheduled-comment defers F3 multi-day stability to its own section 8 "deferred stability completion" per operator decision. Both gates for proceeding (all-pass OR design-revised) are therefore met; facebook-scheduled-comment cloud work has already proceeded and deployed to dev fail-closed. -->
