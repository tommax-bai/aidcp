...
全局免审继续采用“通知先成功、再授权”的现有安全模板：卡片无批准/拒绝按钮，带账号、目标和正文预览；来源为飞书命令时回来源会话，否则按账号团队群解析。通知失败即 fail-closed，不下发评论。最终成功仍只来自边端/平台确认。
...
- [账号全局免审扩大公开写动作权限] → 默认无行=`source_rules`，仅内部后台可写；UI 明示覆盖所有来源并记录操作人/时间。
- [免审通知失败导致评论不发] → 沿用现有通知先行 fail-closed 合同，回执与日志给出 `auto_approve_notice_failed`，不裸发。
