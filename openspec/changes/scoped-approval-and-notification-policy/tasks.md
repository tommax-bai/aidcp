...
- [x] 2.3 Extend auto-approval notification routing and tests so every globally exempt source notifies before authorization and fails closed on notification failure.
<!-- The shared resolver is injected into CommentApprovalGate and CommentScheduler; delegated comment executors already converge on CommentScheduler. Notification remains before authorization and failures remain non-submit terminal skips. -->
...
- [x] 4.1 Add account-level global comment approval controls with explicit all-source warning, server-write truth, audit metadata, and failure handling.
