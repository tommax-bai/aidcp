...
- **每轮 MUST 留一行常态回执**（四个计数 + 偏差数），MUST NOT 只在异常时说话：否则「确实逐项相等」「范围被过滤成空」「周期任务根本没在跑」三者在运维视角下同形，而后两者是故障。
...
- **WHEN** 某一轮对账实际对账了若干账号且逐项相等、零偏差
- **THEN** 系统 MUST 仍留下一行带四个计数与偏差数的回执
- **AND** MUST NOT 以「没有输出」表达「一切正常」
