...
- **红线:采集不等于浏览**。登录引导采集路径 **MUST** 只驱动「访问本人主页」这一个动作(经 `xiaohongshu.profile.open{direct}` + 读 `profile.detail` + 单写),**MUST NOT** 接入浏览反应链;未绑人设的账号采完真名后 **MUST** 闲置,**MUST NOT** 在默认人设上浏览/点赞/关注/评论/搜索。
...
- **有界**:~20s 兜底超时(edge 静默/未登录不困死会话/连接);采空 K 次后退避,不永绕。`xiaohongshu.profile.open` 采集 MUST NOT 触发风控/预算/节奏。
...
该要求 MUST NOT 改变 `account_id` 作为主键,MUST NOT 影响已按账号 keyed 的风控/发布/概念表,MUST NOT 新增协议消息类型(经已有 `xiaohongshu.profile.open` 命令的可选字段 + 已有 `profile.detail` 上报)。

#### Scenario: 真实账号且昵称未知 → 登录后(不经人设闸)驱动一次本人主页采集并持久化

- **WHEN** 某连接账号是真实 userid(非 `default`)且 `accounts.nickname` 为 NULL,该账号登录(edge hello)
- **THEN** 云端角色在**登录引导**(不要求开浏览会话、不要求绑人设)命令 edge 打开本人主页(`xiaohongshu.profile.open{authorId=accountId, direct}`),读上报的主页昵称,经单写接口持久化,且全程不浏览

#### Scenario: 未绑人设账号登录 → 仍采真名但绝不浏览(红线)

- **WHEN** 账号未绑人设、被诚实人设启动闸拦下(不开浏览会话),但库内昵称为 NULL 且全局调度开着,该账号登录
- **THEN** 云端**仅**驱动一次本人主页采集(恰一次 `xiaohongshu.profile.open{direct}`),采到非空昵称即持久化;**MUST NOT** 产生任何浏览指令(open_note/like/collect/follow/comment),采完即闲置
