...
仅对已持久化 `pending_approval`、无来源会话、分组显式 `client_only` 且账号客户审批归属可证明的 `review` 发布稿，发送决策 MAY 在目标解析前抑制飞书按钮卡。该例外 MUST NOT 抑制评论免审通知、发布结果、失败、运维告警、飞书命令回执或任何有来源会话的发布审批卡。策略或归属不可读/不可证 MUST 回退发送并继续上述路由。评论免审通知是旁路消息：目标解析或发送失败只记日志，MUST NOT 反向阻止已获免审授权的评论，也 MUST NOT 生成按钮审批卡作为兜底。
...
#### Scenario: 评论免审通知失败不改变授权
- **WHEN** 评论已由账号或来源规则免审，而通知目标不可解析或发送失败
- **THEN** Cloud 只记录通知失败并继续评论提交链，MUST NOT 阻止提交或回退发送按钮审批卡
