...
#### Scenario: 搜索结果卡片上报等待 noteId 水合
- **WHEN** 搜索结果页已经出现可见卡片但卡片链接或 noteId 仍在水合中
- **THEN** 边端 SHOULD 在单步预算内短暂等待 noteId 可观测后再上报 `page.cards`；若近重复折叠遇到同标题同作者的卡片，SHOULD 优先保留带 noteId 的卡片；若等待后仍无目标 noteId，云端 MAY 按既有有界 fallback 搜索继续
