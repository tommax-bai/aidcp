...
- **WHEN** 某边缘正处于阻断浮层暂停态（验证码或未识别弹窗），滞留时长尚未达到门槛
...
### Requirement: 不让位原因 MUST 具名下发，且终止 SHALL 按白名单而非黑名单

云端待机提示中的不让位原因 MUST NOT 是兜底桶。今天有两个桶：

- **「账号尚未就绪」**装着至少六件处置完全不同的事：人设未绑、云端读不到该账号人设（只读副本
  陈旧）、其他配置副本陈旧、该平台不具备浏览能力、**运营的显式全局停机**、以及**该部署没开
  续场特性**。
- **「需要浏览器才能解除」**装着「该边缘正处于阻断浮层暂停态」，以及「风控拦住且原因不以配额
  开头」这个**取反得来的补集**——补集里将来会落进什么，下发时无人知道。

这些情形里只有极少数满足「人来了就能解决，且此刻确实在白占一格槽」。其余各有各的理由绝不能
终止：副本陈旧会自己追上；平台不具备浏览能力是该平台常态，且该账号很可能正在跑排期发帖或
评论，终止它等于把那些一并停掉；**全局停机与特性未开是全体环境共有的状态，照单终止即全场
清光，且没有任何受益人**——所有环境都停了，腾出的槽位无人可用，运营恢复时反而要逐个手动重启。

因此：

1. 云端 SHALL 把两个桶都拆成**互相可分辨的具名取值**下发。这是「跨层翻译 MUST NOT 有兜底桶」
   这条既有约束在本能力上的落点：把没分类的原因折进一个已有名字，跨层传下去就成了终局判决，
   而这里的「终局」是把整机环境清场。
2. 宿主层 SHALL 持有一份**明确该终止的具名白名单**，清单之外的一切原因 MUST NOT 计时。
   MUST NOT 采用黑名单（排除已知不该关的若干种）：黑名单对**新增原因**的默认后果是终止，
   白名单的默认后果是多占一会儿槽位。上述六选一的分类是逐条读实现才得出的，且首轮分类曾把
   其中三种误判为「该关」——这类判断出错是常态，默认后果因此必须安全。

拆分 SHALL 只扩展原因取值，MUST NOT 改动消息结构，且 SHALL 可独立先行上线——边缘尚不认识新
取值时按「未具名原因不计时」处理，行为零变化。

#### Scenario: 全局性原因 MUST NOT 触发终止
- **WHEN** 某部署未启用续场特性、或运营执行了显式全局停机，全部环境因此持续处于不让位状态
- **THEN** 云端 SHALL 以可分辨的具名原因下发该情形
- **AND** 宿主层 MUST NOT 为任何环境推进滞留计时或终止它们

#### Scenario: 会自愈的原因 MUST NOT 触发终止
- **WHEN** 某环境因云端只读副本陈旧而持续不让位，持续时长超过门槛
- **THEN** 宿主层 MUST NOT 终止它——该状态由副本追平自行解除，无人可"处理"

#### Scenario: 不做浏览的账号 MUST NOT 因此被终止
- **WHEN** 某账号所在平台不具备自动浏览能力，因而持续不产出可让位提示
- **THEN** 宿主层 MUST NOT 终止它——该账号可能正在执行排期发帖或评论

#### Scenario: 人设未绑滞留超限 SHALL 终止
- **WHEN** 某环境因**权威判定的**未绑定人设而持续不让位，且占着一格执行槽，持续时长达到门槛
- **THEN** 宿主层 SHALL 终止其自动化并释放槽位
- **AND** 呈现 SHALL 写明阻塞原因是人设未绑

#### Scenario: 新增原因默认不终止
- **WHEN** 云端新增一种不让位原因并持续下发超过门槛，而宿主层白名单未收录它
- **THEN** 宿主层 MUST NOT 终止该环境
- **AND** 该默认行为 SHALL 由断言钉死，MUST NOT 依赖实现者记得去更新清单
