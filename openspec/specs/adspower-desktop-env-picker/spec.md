...
探测就绪并拉取环境列表后，若恰好存在一个环境、当前分身 ID 为空**且核心未在运行**，桌面外壳 SHALL 自动选中该环境并明示「已自动选中」；存在多个环境、分身 ID 已有值或核心正在运行时 MUST NOT 自动改写用户选择（绝不在运行中偷改配置）。

#### Scenario: 唯一环境自动选中
- **WHEN** 核心未在运行、拉取到恰好 1 个环境且分身 ID 为空
