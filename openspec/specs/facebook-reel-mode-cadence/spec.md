...
Cloud SHALL expose one target-global Reel cadence policy with exactly five `1..100` integers: ordinary persona `viewsPerLike` and `viewsPerFollow`, plus slow-start, rule and consumption `viewsPerFollow`. Defaults SHALL be 4, 10, 15, 15 and 15 respectively. These values MUST be editable only through the internal management backend global policy route; customer, account and environment routes MUST NOT accept overrides.
...
#### Scenario: Other modes do not consume persona like cadence

- **WHEN** the effective mode is slow-start, rule or consumption
- **THEN** the ordinary persona Reel like counter does not advance and no persona-cadence like intent is sent
- **AND** only that mode's own Reel follow counter may advance

#### Scenario: Global write is strict and atomic

- **WHEN** an internal operator writes all five cadence values with the current expected revision
...
- **THEN** the five Reel cadence values remain the target-global values
