...
- **WHEN** 某账号刚成功 `like` 后不到 2 分钟，角色又对另一篇笔记产出 `like` 意图
...
评论（comment）的冷却 SHALL 在**评论评估阶段**（`CommentAppraiser`，与数量闸 / 热度阈值同处的最便宜阶段）就判定：未到 comment 冷却点 MUST emit `comment.skipped{reason:'cooldown'}` 直接进「是否进主页评估」，MUST NOT 进入撰写 / 去 AI 味 / 飞书人审（避免页面久留 + 占用人工却最终被抑制）。

#### Scenario: 评论冷却中在评估阶段即跳过

- **WHEN** 某账号上次成功评论不足 30 分钟，又在一篇笔记上进入评论评估
